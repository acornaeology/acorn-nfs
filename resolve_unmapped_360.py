#!/usr/bin/env python3
"""Resolve UNMAPPED annotations in the NFS 3.60 driver script.

Uses the opcode-level address map from generate_360.py enhanced with:
...
Outputs a report of each UNMAPPED annotation with its resolved 3.60 address
...
DRIVER_FILEPATH = BASE / 'versions' / '3.60' / 'disassemble' / 'disasm_nfs_360.py'
JSON_FILEPATH = BASE / 'versions' / '3.60' / 'output' / 'nfs-3.60.json'
...
def interpolate_address(addr, addr_map, sorted_mapped_addrs):
    """Find the likely 3.60 address by interpolating from nearest mapped neighbours."""
    import bisect
