#!/usr/bin/env python3
"""Generate the initial disasm_anfs_426.py by mapping addresses from
ANFS 4.25.

ANFS 4.26 is the next Master 128 ANFS after 4.25. Both are 65C02 builds
...
remap of the 4.25 driver — no CPU change, no dead runtime ranges, no
move() calls to strip.

Compared with 4.25:
- The ROM title grew from "Acorn ANFS 4.25" to "Acorn ANFS 4.26"; the
...
page and every other fixed (non-ROM) address, and rewrites the 4.25 driver
...
# Both 4.25 and 4.26 are 65C02 (Master 128).
CPU_A = "65c02"
CPU_B = "65c02"

# 4.26 has the same runtime-address model as 4.25: no relocated blocks,
...
def transform_script(script_text, addr_map):
    """Transform the 4.25 script to 4.26 by mapping addresses."""
    ...
    for start_idx, end_idx, group_lines in groups:
        ...
        # move() calls: 4.26 has no relocated blocks (none expected in the
        # 4.25 base, but keep the guard for safety).
...
def main():
    ...
    rom_a_filepath = (
        base / 'versions' / 'anfs-4.25' / 'rom' / 'anfs-4.25.rom'
    )
    rom_b_filepath = base / 'versions' / 'anfs-4.26' / 'rom' / 'anfs-4.26.rom'
    script_filepath = (
        base / 'versions' / 'anfs-4.25' / 'disassemble'
        / 'disasm_anfs_425.py'
    )
    output_filepath = (
        base / 'versions' / 'anfs-4.26' / 'disassemble' / 'disasm_anfs_426.py'
    )
    ...
    def bytes_match(src, dst):
        # Require the instruction/item bytes at the 4.25 source to be
        # identical to those at the 4.26 candidate, so an annotation only
