#!/usr/bin/env python3
"""Generate the initial disasm_anfs_425.py by mapping addresses from
ANFS 4.24.

ANFS 4.25 is the next Master 128 ANFS after 4.24. Both are 65C02 builds
with no relocated blocks (filing-system workspace lives in HAZEL hidden
RAM at &C000-&C2FF), so the transformation is a pure opcode-level address
remap of the 4.24 driver — no CPU change, no dead runtime ranges, no
move() calls to strip.

Compared with 4.24:
- The ROM title grew from "Acorn ANFS 4.24" to "Acorn ANFS 4.25"; the
  copyright ("(C)1986 Acorn") is unchanged.
- The service entry moved from &8A8A to &8A8C.
- 98.2% opcode-similar (88.4% full-instruction). The dominant change is a
  single 2-byte instruction insertion around &85AC, shifting everything
  after it by +2; almost every other apparent difference is a JSR/JMP
  target or dispatch-table byte following that +2 shift.
...
page and every other fixed (non-ROM) address, and rewrites the 4.24 driver
...
# Both 4.24 and 4.25 are 65C02 (Master 128).
CPU_A = "65c02"
CPU_B = "65c02"

# 4.25 has the same runtime-address model as 4.24: no relocated blocks,
# workspace in HAZEL. There are no dead ranges to strip.
DEAD_RANGES = []
...
def transform_script(script_text, addr_map):
    """Transform the 4.24 script to 4.25 by mapping addresses."""
    ...
    for start_idx, end_idx, group_lines in groups:
        ...
        # move() calls: 4.25 has no relocated blocks (none expected in the
        # 4.24 base, but keep the guard for safety).
...
def main():
    ...
    rom_a_filepath = (
        base / 'versions' / 'anfs-4.24' / 'rom' / 'anfs-4.24.rom'
    )
    rom_b_filepath = base / 'versions' / 'anfs-4.25' / 'rom' / 'anfs-4.25.rom'
    script_filepath = (
        base / 'versions' / 'anfs-4.24' / 'disassemble'
        / 'disasm_anfs_424.py'
    )
    output_filepath = (
        base / 'versions' / 'anfs-4.25' / 'disassemble' / 'disasm_anfs_425.py'
    )
    ...
    def bytes_match(src, dst):
        # Require the instruction/item bytes at the 4.24 source to be
        # identical to those at the 4.25 candidate, so an annotation only
