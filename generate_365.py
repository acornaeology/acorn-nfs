#!/usr/bin/env python3
"""Generate the initial disasm_nfs_365.py by mapping addresses from 3.62.

Uses opcode-level sequence matching to build an address map between
the 3.62 and 3.65 ROMs, then transforms the 3.62 disassembly
...
def group_logical_statements(lines):
    ...
    current_lines = []
    paren_depth = 0
    in_s = None  # Track string state across lines
    escaped = False
...
def transform_script(script_text, addr_map):
    """Transform the 3.62 script to 3.65 by mapping addresses."""
    ...
    for start_idx, end_idx, group_lines in groups:
        ...
        if stripped.startswith('#') or stripped == '':
            for gl in group_lines:
                ...
                gl = gl.replace('nfs-3.62', 'nfs-3.65')
                if 'Address map: NFS 3.40' in gl:
                    gl = gl.replace('Address map: NFS 3.40 \u2192 NFS 3.60',
                                    'Address map: NFS 3.62 \u2192 NFS 3.65')
                if 'derived from the NFS 3.60' in gl:
                    gl = gl.replace('derived from the NFS 3.60 disassembly driver',
                                    'derived from the NFS 3.62 disassembly driver')
                if 'NFS 3.62' in gl and 'derived from NFS 3.60' in gl:
                    gl = gl.replace('NFS 3.62 \u2014 derived from NFS 3.60',
                                    'NFS 3.65 \u2014 derived from NFS 3.62')
                if 'copied from the NFS 3.60' in gl:
                    gl = gl.replace('copied from the NFS 3.60 disassembly driver',
                                    'derived from the NFS 3.62 disassembly driver')
                if 'two ROMs are 99.7% identical' in gl:
                    gl = '# The two ROMs are 96.8% identical at the opcode level.'
                output_lines.append(gl)
            continue

        # --- File path replacements ---
        group_lines = [gl.replace('nfs-3.62', 'nfs-3.65') for gl in group_lines]
        first_line = group_lines[0]
        stripped = first_line.strip()
        ...
        if stripped.startswith('move('):
            for gl in group_lines:
                if '0x9321' in gl:
                    gl = gl.replace('0x9321', '0x9324')
                    gl = re.sub(r'#.*', '# &9324 (+3 from 3.62)', gl)
                elif '0x9362' in gl:
                    gl = gl.replace('0x9362', '0x9365')
                    gl = re.sub(r'#.*', '# &9365 (+3 from 3.62)', gl)
                elif '0x9462' in gl:
                    gl = gl.replace('0x9462', '0x9465')
                    gl = re.sub(r'#.*', '# &9465 (+3 from 3.62)', gl)
                elif '0x9562' in gl:
                    gl = gl.replace('0x9562', '0x9565')
                    gl = re.sub(r'#.*', '# &9565 (+3 from 3.62)', gl)
                output_lines.append(gl)
            continue
...
def main():
    ...
    rom_a_filepath = base / 'versions' / '3.62' / 'rom' / 'nfs-3.62.rom'
    rom_b_filepath = base / 'versions' / '3.65' / 'rom' / 'nfs-3.65.rom'
    script_filepath = base / 'versions' / '3.62' / 'disassemble' / 'disasm_nfs_362.py'
    output_filepath = base / 'versions' / '3.65' / 'disassemble' / 'disasm_nfs_365.py'
    ...
    # 3.62 sources → 3.65 sources (all shift +3)
    reloc_blocks = [
        (0x9321, 0x9324, 0x0016, 0x61),    # BRK handler + workspace
        (0x9362, 0x9365, 0x0400, 0x100),    # Page 4
        (0x9462, 0x9465, 0x0500, 0x100),    # Page 5
        (0x9562, 0x9565, 0x0600, 0x100),    # Page 6
    ]
    ...
    # The dispatch tables and ROM header data shift between versions
    # (title "    NET" → "NET" shortens header by 4 bytes).
    # Do NOT add identity mappings for the dispatch area — let those
    # lines be marked UNMAPPED for manual fixup.

    print(f"  Total mapped addresses: {len(addr_map)}", file=sys.stderr)

    key_addrs = [0x80DD, 0x80F3, 0x8100, 0x8140, 0x9321]
