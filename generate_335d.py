...
def transform_script(script_text, addr_map):
    """Transform the 3.34B script to 3.35D by mapping addresses."""
    ...
    for start_idx, end_idx, group_lines in groups:
        ...
        if stripped.startswith('#') or stripped == '':
            for gl in group_lines:
                ...
                gl = gl.replace('nfs-3.34B', 'nfs-3.35D')
                if 'Address map: NFS 3.34 → NFS 3.34B' in gl:
                    gl = gl.replace('Address map: NFS 3.34 → NFS 3.34B',
                                    'Address map: NFS 3.34B → NFS 3.35D')
                if 'derived from the NFS 3.34 disassembly driver' in gl:
                    gl = gl.replace('derived from the NFS 3.34 disassembly driver',
                                    'derived from the NFS 3.34B disassembly driver')
                if 'two ROMs are 98.7% identical' in gl:
                    gl = '# The two ROMs are 87.4% identical at the opcode level.'
                output_lines.append(gl)
            continue

        # --- File path replacements ---
        group_lines = [gl.replace('nfs-3.34B', 'nfs-3.35D') for gl in group_lines]
        first_line = group_lines[0]
        stripped = first_line.strip()
        ...
        if stripped.startswith('move('):
            for gl in group_lines:
                if '0x9308' in gl:
                    gl = gl.replace('0x9308', '0x931A')
                    gl = re.sub(r'#.*', '# &931A (+18 from 3.34B)', gl)
                elif '0x934D' in gl:
                    gl = gl.replace('0x934D', '0x935F')
                    gl = re.sub(r'#.*', '# &935F (+18 from 3.34B)', gl)
                elif '0x944D' in gl:
                    gl = gl.replace('0x944D', '0x945F')
                    gl = re.sub(r'#.*', '# &945F (+18 from 3.34B)', gl)
                elif '0x954D' in gl:
                    gl = gl.replace('0x954D', '0x955F')
                    gl = re.sub(r'#.*', '# &955F (+18 from 3.34B)', gl)
                output_lines.append(gl)
            continue
...
def main():
    ...
    for addr in range(0x8015, 0x806A):
        if addr not in addr_map:
            addr_map[addr] = addr

    print(f"  Total mapped addresses: {len(addr_map)}", file=sys.stderr)

    key_addrs = [0x8099, 0x80AF, 0x80C8, 0x81C0, 0x8486, 0x853C, 0x9660]
