#!/usr/bin/env python3
"""Generate the initial disasm_nfs_360.py by mapping addresses from 3.40.

Uses opcode-level sequence matching to build an address map between
the 3.40 and 3.60 ROMs, then transforms the 3.40 disassembly
...
def transform_script(script_text, addr_map):
    """Transform the 3.40 script to 3.60 by mapping addresses."""
    ...
    for start_idx, end_idx, group_lines in groups:
        ...
        if stripped.startswith('#') or stripped == '':
            for gl in group_lines:
                ...
                gl = gl.replace('nfs-3.40', 'nfs-3.60')
                if 'Address map: NFS 3.35K' in gl:
                    gl = gl.replace('Address map: NFS 3.35K \u2192 NFS 3.40',
                                    'Address map: NFS 3.40 \u2192 NFS 3.60')
                if 'derived from the NFS 3.35K disassembly driver' in gl:
                    gl = gl.replace('derived from the NFS 3.35K disassembly driver',
                                    'derived from the NFS 3.40 disassembly driver')
                if 'two ROMs are 89.4% identical' in gl:
                    gl = '# The two ROMs are 89.9% identical at the opcode level.'
                output_lines.append(gl)
            continue

        # --- File path replacements ---
        group_lines = [gl.replace('nfs-3.40', 'nfs-3.60') for gl in group_lines]
        first_line = group_lines[0]
        stripped = first_line.strip()
        ...
        if stripped.startswith('move('):
            for gl in group_lines:
                if '0x931C' in gl:
                    gl = gl.replace('0x931C', '0x9321')
                    gl = re.sub(r'#.*', '# &9321 (+5 from 3.40)', gl)
                elif '0x935D' in gl:
                    gl = gl.replace('0x935D', '0x9362')
                    gl = re.sub(r'#.*', '# &9362 (+5 from 3.40)', gl)
                elif '0x9456' in gl:
                    gl = gl.replace('0x9456', '0x9462')
                    gl = re.sub(r'#.*', '# &9462 (+12 from 3.40)', gl)
                elif '0x9556' in gl:
                    gl = gl.replace('0x9556', '0x9562')
                    gl = re.sub(r'#.*', '# &9562 (+12 from 3.40)', gl)
                output_lines.append(gl)
            continue
...
def main():
    ...
    rom_a_filepath = base / 'versions' / '3.40' / 'rom' / 'nfs-3.40.rom'
    rom_b_filepath = base / 'versions' / '3.60' / 'rom' / 'nfs-3.60.rom'
    script_filepath = base / 'versions' / '3.40' / 'disassemble' / 'disasm_nfs_340.py'
    output_filepath = base / 'versions' / '3.60' / 'disassemble' / 'disasm_nfs_360.py'
    ...
    # 3.40 sources → 3.60 sources
    reloc_blocks = [
        (0x931C, 0x9321, 0x0016, 0x61),    # BRK handler + workspace
        (0x935D, 0x9362, 0x0400, 0x100),    # Page 4
        (0x9456, 0x9462, 0x0500, 0x100),    # Page 5
        (0x9556, 0x9562, 0x0600, 0x100),    # Page 6
    ]
    ...
    for addr in range(0x8015, 0x8072):
        if addr not in addr_map:
            addr_map[addr] = addr

    print(f"  Total mapped addresses: {len(addr_map)}", file=sys.stderr)

    key_addrs = [0x80E1, 0x80F7, 0x8140, 0x8207, 0x817F, 0x931C]
