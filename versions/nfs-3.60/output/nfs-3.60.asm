...
fs_crc_lo                              = &be
; &be referenced 7 times by &8653, &8705, &87ef, &8b8a, &8c2d, &8c3a, &8d86
fs_crc_hi                              = &bf
...
.pydis_start
; NFS ROM 3.60 disassembly (Acorn Econet filing system)
; ***************************************************************************************
...
;
; X=5: matches "NET" at title+4 — the ROM title suffix — *NET (select NFS) — falls
...
.svc_9_help
    ...
    equb &0d, "NFS 3.60", &0d                                         ; 820b: 0d 4e 46... .NF...   ; Notify current FS of shutdown
...
.issue_vectors_claimed
    ...
    ldx #&92                                                          ; 828b: a2 92       ..       ; X = lo(run_fscv_cmd+5): auto-boot string
; ***************************************************************************************
...
; logon) "EX"     → &8C61 (ex_handler: embedded in table tail) "BYE"\r  → &83C0
...
cmd_match_data = fs_cmd_match_table+1
    ...
    equb &8c                                                          ; 8c57: 8c          .        ; EX handler hi → &8C61 (ex_handler)
    equb &60                                                          ; 8c58: 60          `        ; EX handler lo
...
;
; Sets up a buffer at page &7F offset &25 (length 508 bytes) for the machine type query
; response, then branches to set_tx_reply_flag. Returns system identification data to the
; remote station.
;
...
;
; Mid-instruction label within the INACTIVE polling loop. intoff_operand is referenced as
; a constant for self-modifying code. Disables NMIs twice (belt-and-braces) then tests
...
.add_ascii_base
    ...
    equb &ff, &ff, &ff                                                ; 9ffd: ff ff ff    ...      ; Padding
.pydis_end
