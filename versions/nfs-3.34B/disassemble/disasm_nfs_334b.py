...
# ============================================================
# Address map: NFS 3.34 → NFS 3.34B
# ============================================================
# This driver was derived from the NFS 3.34 disassembly driver.
# The two ROMs are 98.7% identical at the opcode level.
#
# 3.34 Range       3.34B Range      Offset
# $8000-$81C7  →   $8000-$81C7      +0     Identical
#   (n/a)      →   $81C8            n/a    Inserted "B" in version string
# $81C8-$964A  →   $81C9-$964B      +1     Shifted code/data
# $964B        →   (deleted)        n/a    RTS removed (end of page 6 block)
# $964C-$9FFF  →   $964C-$9FFF      +0     Identical
#
# Tube register change: WRCH character I/O moved from R1
# ($FEE0/$FEE1) to R4 ($FEE6/$FEE7), separating character
# transfer from escape/event signalling. Escape forwarding
# and event handling remain on R1.
...
_rom_filepath = os.environ.get(
    "ACORN_NFS_ROM",
    str(_version_dirpath / "rom" / "nfs-3.34B.rom"),
)
...
hook_subroutine(0x853C, "print_inline", stringhi_hook)
...
label(0x8E19, "fs_osword_tbl_lo")      # Low bytes of FS OSWORD handler table
label(0x8E1E, "fs_osword_tbl_hi")      # High bytes of FS OSWORD handler table

# FS OSWORD handler routines
# osword_0f_handler label created by subroutine() call below.
label(0x8E6B, "read_args_size")        # READRB: get args buffer size from RX block offset &7F
# osword_10_handler label created by subroutine() call below.

# Econet TX/RX handler and OSWORD dispatch
label(0x8F69, "store_16bit_at_y")     # Store 16-bit value at (nfs_workspace)+Y
# osword_dispatch label created by subroutine() call below.
label(0x9021, "osword_trampoline")     # PHA/PHA/RTS trampoline
label(0x902C, "osword_tbl_lo")         # Dispatch table low bytes
label(0x9035, "osword_tbl_hi")         # Dispatch table high bytes
...
label(0x90BE, "return_match_osbyte") # Return from match_osbyte_code
label(0x90FC, "return_remote_cmd")   # Return from remote command data handler

# Control block setup
label(0x9167, "ctrl_block_setup_clv") # CLV entry: same setup but clears V flag
...
label(0x92D7, "clear_jsr_protection") # CLRJSR: reset JSR buffer protection bits (4 refs)

# Palette/VDU state save
label(0x92EF, "read_vdu_osbyte_x0")  # Read next VDU OSBYTE with X=0 parameter
label(0x92F1, "read_vdu_osbyte")     # Read next OSBYTE from table, store result in workspace
...
label(0x824E, "fs_vector_addrs")        # FS vector dispatch and handler addresses (34 bytes)
...
#   BPUTV → &83A3    GBPBV → &89EB    FINDV → &894A
#   FSCV  → &808C
# Labels and entry points for FSCV, FILEV, ARGSV, FINDV, GBPBV
# are created by subroutine() calls below in the comment sections.
label(0x8486, "bgetv_handler")          # BGETV entry: SEC then JSR handle_bput_bget
label(0x83A3, "bputv_handler")          # BPUTV entry: CLC then fall into handle_bput_bget
entry(0x8486)
entry(0x83A3)
...
label(0x82E6, "store_rom_ptr_pair")     # Write 2-byte address + ROM bank to ROM pointer table

# --- TX control block and FS command setup ---
label(0x830F, "init_tx_ctrl_data")      # Init TX control block for data port (&90)
label(0x8311, "init_tx_ctrl_port")      # Init TX control block with port in A (2 JSR refs)
label(0x8341, "prepare_cmd_with_flag")  # Prepare FS command with '*' flag and carry set
label(0x8347, "prepare_cmd_clv")        # Prepare FS command with V cleared
# prepare_fs_cmd and build_send_fs_cmd labels created by subroutine() calls below.
label(0x8352, "prepare_fs_cmd_v")       # Prepare FS command, V flag preserved
label(0x8381, "send_fs_reply_cmd")      # Send FS command with reply processing

# --- Byte I/O and escape ---
# handle_bput_bget label created by subroutine() call below.
label(0x83CC, "store_retry_count")      # RAND1: store retry count to &0FDD, initiate TX
label(0x841C, "update_sequence_return") # RAND3: update sequence numbers and pull A/Y/X/return
label(0x842D, "set_listen_offset")      # NLISN2: use reply code as table offset for listen
label(0x8449, "send_to_fs_star")        # Send '*' command to fileserver
label(0x8471, "fs_wait_cleanup")        # WAITEX: tidy stack, restore rx_status_flags

# --- Pointer arithmetic helpers ---
label(0x84A4, "return_bget")             # Return from BGET/BPUT handler
label(0x84A5, "add_5_to_y")             # INY * 5; RTS
label(0x84A6, "add_4_to_y")             # INY * 4; RTS
label(0x84AB, "sub_4_from_y")           # DEY * 4; RTS
label(0x84AC, "sub_3_from_y")           # DEY * 3; RTS
...
label(0x8531, "access_bit_table")       # Lookup table for attribute bit mapping (11 bytes)
...
label(0x85F7, "print_hex_nibble")       # Print low nibble of A as hex digit
...
# --- File info display ---
label(0x8618, "pad_filename_spaces")    # MONL2: pad filename display to 12 chars with spaces
label(0x862B, "print_exec_and_len")     # Print exec address (4 bytes) and length (3 bytes)
label(0x8636, "print_hex_bytes")        # Print X bytes from (fs_options)+Y as hex (high→low)
label(0x8641, "print_space")            # Print a space character via OSASCI

# --- TX control and transmission ---
label(0x864F, "tx_poll_timeout")        # Transmit with Y=&60 (specified timeout)
# tx_poll_core label created by subroutine() call below.
label(0x8685, "delay_1ms")              # MSDELY: 1ms delay loop (nested DEX/DEY)

# ============================================================
# File operations: FILEV, ARGSV, FINDV, GBPBV (&8695-&8B92)
# ============================================================
...
# --- FILEV handler (&8695) and helpers ---

# --- FSCV 1: EOF handler (&8820) ---

# --- FILEV attribute dispatch (&8845) ---
label(0x886F, "get_file_protection")  # CHA1: decode attribute byte for protection status
label(0x8884, "copy_filename_to_cmd") # CHASK2: copy filename string into FS command buffer
label(0x88C6, "copy_fs_reply_to_cb")  # COPYFS: copy FS reply buffer data to control block

# --- Common return point (&892D) ---
label(0x8913, "save_args_handle")      # SETARG: save handle for OSARGS operation

# --- FSCV 0: *OPT handler (&89A2) ---
label(0x8990, "close_single_handle")   # CLOSE1: send close command for specific handle to FS

# --- Address adjustment helpers (&89CB-&89EA) ---
label(0x89CB, "adjust_addrs_9")        # Adjust 4-byte addresses at param block offset 9
label(0x89D0, "adjust_addrs_1")        # Adjust 4-byte addresses at param block offset 1
label(0x89D2, "adjust_addrs_clc")      # CLC entry: clear carry before address adjustment
label(0x8AFC, "copy_reply_to_caller") # Copy FS reply data to caller buffer (direct or via Tube)
label(0x8B8B, "tube_claim_loop")      # TCLAIM: claim Tube with &C3, retry until acquired
...
label(0x821A, "dofsl1")               # NFS03: do FS loop 1
label(0x8300, "fsdiel")               # NFS01: FS die loop
label(0x8320, "fstxl1")               # NFS03: FS TX loop 1
label(0x8330, "fstxl2")               # NFS03: FS TX loop 2
label(0x838B, "dofsl7")               # NFS03: do FS loop 7
label(0x8397, "return_dofsl7")        # NFS03: return from FS loop 7
label(0x8398, "dofsl5")               # NFS03: do FS loop 5
label(0x83DE, "error1")               # NFS03: error handler 1

# --- Net list / pointer arithmetic (&84xx-&85xx) ---
label(0x8429, "nlistn")               # NFS03: net list entry
label(0x842B, "nlisne")               # NFS03: net list next entry
label(0x845B, "incpx")                # NFS04: increment pointer X
label(0x8593, "y2fsl5")               # NFS04: Y-to-FS loop 5
label(0x8599, "y2fsl2")               # NFS04: Y-to-FS loop 2
label(0x85A8, "fs2al1")               # NFS04: FS-to-A loop 1

# --- Number formatting and file info (&86xx) ---
label(0x8638, "num01")                # NFS07: number print entry
label(0x866B, "poll_txcb_status")     # NFS03: net TX polling loop
label(0x869A, "file1")                # NFS05: FILEV entry 1
label(0x86A9, "quote1")               # NFS05: filename quote loop
label(0x86CC, "loadop")               # NFS05: load operation
label(0x86E9, "lodfil")               # NFS05: load file

# --- FILEV, load/save size (&87xx) ---
label(0x870C, "floop")                # NFS01: FS loop
label(0x8736, "lodchk")               # NFS05: load check
label(0x8741, "return_lodchk")        # NFS05: return from load check
label(0x8742, "saveop")               # NFS05: save operation
label(0x874B, "savsiz")               # NFS05: save size handling
label(0x87B0, "lodrl1")               # NFS05: load reply loop 1
label(0x87BE, "lodrl2")               # NFS05: load reply loop 2
label(0x87EC, "savchk")               # NFS05: save check

# --- Channel/attribute handling (&88xx-&89xx) ---
label(0x8862, "chalp1")               # NFS05: channel loop 1
label(0x8879, "chalp2")               # NFS05: channel loop 2
label(0x888B, "cha6")                  # NFS05: channel handler 6
label(0x889A, "cha4")                  # NFS05: channel handler 4
label(0x88A4, "cha5")                  # NFS05: channel handler 5
label(0x88D3, "cha5lp")               # NFS05: channel 5 loop
label(0x893E, "osarg1")               # NFS05: OSARGS handler 1
label(0x89B7, "opter1")               # NFS05: *OPT error 1
label(0x89BC, "optl1")                # NFS05: *OPT loop 1

# --- GBPB handler (&89xx-&8Axx) ---
label(0x89E4, "gbpbx")                # NFS05: GBPB dispatch
label(0x89F6, "gbpbx0")               # NFS05: GBPB dispatch 0
label(0x89F9, "gbpbx1")               # NFS05: GBPB dispatch 1
label(0x8A04, "gbpbe1")               # NFS05: GBPB EOF 1
label(0x8A10, "gbpbf1")               # NFS05: GBPB forward 1
label(0x8A1B, "gbpbf2")               # NFS05: GBPB forward 2
label(0x8A24, "gbpbl1")               # NFS05: GBPB loop 1
label(0x8A46, "gbpbl3")               # NFS05: GBPB loop 3
label(0x8A5D, "gbpbf3")               # NFS05: GBPB forward 3

# --- *INFO and decimal print (&8Axx-&8Bxx) ---
label(0x8ABE, "info2")                # NFS06: *INFO loop 2
label(0x8B23, "tbcop1")               # NFS06: Tube copy loop 1
label(0x8B9A, "decfir")               # NFS07: decimal first digit
label(0x8B9C, "decmor")               # NFS07: decimal more digits
label(0x8BA8, "decmin")               # NFS07: decimal minimum

# --- Logon and *NET (&8Dxx) ---
label(0x8D25, "logon2")               # NFS07: logon handler 2
label(0x8D2B, "logon3")               # NFS07: logon handler 3
label(0x8D7F, "infol2")               # NFS07: info loop 2

# --- File I/O: save, read, open (&8Dxx-&8Fxx) ---
label(0x8DD7, "rxpol2")               # NFS08: RX poll loop 2
label(0x8E0D, "save1")                # NFS08: save handler 1
label(0x8E2F, "copyl3")               # NFS08: copy loop 3
label(0x8E76, "readry")               # NFS08: read ready
label(0x8E9C, "rssl1")                # NFS08: read size/status loop 1
label(0x8EA7, "rssl2")                # NFS08: read size/status loop 2
label(0x8EB8, "rsl1")                 # NFS08: read status loop 1
label(0x8EE4, "readc1")               # NFS08: read char 1
label(0x8F02, "scan0")                # NFS08: scan entry 0
label(0x8F16, "scan1")                # NFS08: scan entry 1
label(0x8F32, "openl6")               # NFS08: open loop 6
label(0x8F3F, "openl7")               # NFS08: open loop 7
label(0x8F44, "openl4")               # NFS08: open loop 4
label(0x8F4F, "rest1")                # NFS08: restore 1
label(0x8F79, "dofs01")               # NFS08: do FS 01
label(0x8FF6, "dofs2")                # NFS08: do FS 2

# --- OSWORD and remote ops (&90xx-&91xx) ---
label(0x901A, "entry1")               # NFS09: OSWORD entry 1
label(0x9086, "nbyte6")               # NFS09: net byte handler 6
label(0x9088, "nbyte1")               # NFS09: net byte handler 1
label(0x90AA, "nbyte4")               # NFS09: net byte handler 4
label(0x90AE, "nbyte5")               # NFS09: net byte handler 5
label(0x90B5, "return_nbyte")         # NFS09: return from net byte handler
label(0x9103, "rchex")                # NFS03: receive hex
label(0x9106, "remot1")               # NFS03: remote handler 1
label(0x9168, "cbset2")               # NFS09: control block set 2
...
label(0x9255, "bsxl1")                # NFS09: broadcast search loop 1
label(0x9272, "bspsx")                # NFS09: broadcast search parse exit
label(0x927A, "bsxl0")                # NFS09: broadcast search loop 0
label(0x9291, "return_bspsx")         # NFS09: return from broadcast search area
...
#   3      &12   &8E7C    FS server station lookup
#   4      &13   &8F73    Econet TX/RX handler
for i in range(5):
    rts_code_ptr(0x8E19 + i, 0x8E1E + i)
...
entry(0x84A5)   # INY*5; RTS (pointer arithmetic helper)
entry(0x84AB)   # DEY*4; RTS (pointer arithmetic helper)
entry(0x87C9)   # PHA; JSR ... (called from &878A and &8A6C)
entry(0x8845)   # STA abs; CMP#; ... (called from &8744)
entry(0x8934)   # TAY; BNE; ... (preceded by RTS, standalone entry)
entry(0x89EB)   # JSR &8509; ... (preceded by RTS, standalone entry)
# entry(0x8E34) created by subroutine() call below
entry(0x9064)   # LDY zp; CMP#; ... (preceded by RTS, standalone entry)
...
entry(0x8F73)   # Econet TX/RX handler (CMP #1; BCS)
# entry(0x9008) and entry(0x903E) created by subroutine() calls below
entry(0x9021)   # Dispatch trampoline (PHA/PHA/RTS into table at &902C/&9035)

# Dispatch table at &902C (low bytes) / &9035 (high bytes)
# 9 entries for function codes 0-8, used via PHA/PHA/RTS at &9021.
# Targets: 0=&8145(RTS), 1-3=&91C8, 4=&903E, 5=&91B6, 6=&8145(RTS), 7=&9064, 8=&90CE
for i in range(9):
    rts_code_ptr(0x902C + i, 0x9035 + i)
...
entry(0x915A)   # ADLC setup: LDX #&0D; LDY #&7C; BIT &833B; BVS c9167
...
entry(0x91B6)   # Function 5 handler
entry(0x91C8)   # Function 1/2/3 handler (shared)
entry(0x90CE)   # Function 8 handler

# --- Code found in third-pass remaining equb regions ---
entry(0x8742)   # BEQ +3; JMP &8845 (called from &8744 region)
entry(0x8E7C)   # CMP #6; BCS ... (after 2-byte inline data table at &8E7A)
entry(0x8F58)   # LDY #&28; ... (preceded by RTS, standalone entry)

# --- Code found in fourth-pass small equb regions ---
entry(0x8D5D)   # JSR ... (preceded by RTS, standalone entry)

# --- Dispatch targets from fs_cmd_match_table (PHA/PHA/RTS) ---
entry(0x8BF3)   # *EX handler (dispatch from command match table)
entry(0x8D07)   # "I AM" handler (dispatch from command match table)
...
# ============================================================
subroutine(0x8509, "save_fscv_args", hook=None,
...
# ============================================================
subroutine(0x8514, "decode_attribs_6bit", hook=None,
...
subroutine(0x851E, "decode_attribs_5bit", hook=None,
...
# Skip spaces (&8556)
# ============================================================
subroutine(0x8556, "skip_spaces", hook=None,
...
# ============================================================
subroutine(0x8561, "parse_decimal",
...
comment(0x8561, "Save A in X for caller", inline=True)
comment(0x8562, "Zero accumulator", inline=True)
comment(0x8566, "Load next char from buffer", inline=True)
comment(0x8568, "Letter or above?", inline=True)
comment(0x856A, "Yes: not a digit, done", inline=True)
comment(0x856C, "Dot separator?", inline=True)
comment(0x856E, "Yes: exit with C=1 (dot found)", inline=True)
comment(0x8570, "Control char or space: done", inline=True)
comment(0x8572, "Mask ASCII digit to 0-9", inline=True)
comment(0x8574, "Save new digit", inline=True)
comment(0x8576, "Running total * 2", inline=True)
comment(0x8578, "A = running total * 2", inline=True)
comment(0x857A, "A = running total * 4", inline=True)
comment(0x857B, "A = running total * 8", inline=True)
comment(0x857C, "+ total*2 = total * 10", inline=True)
comment(0x857E, "+ digit = total*10 + digit", inline=True)
comment(0x8580, "Store new running total", inline=True)
comment(0x8582, "Advance to next char", inline=True)
comment(0x8583, "Loop (always: Y won't wrap to 0)", inline=True)
comment(0x8585, "No dot found: C=0", inline=True)
comment(0x8586, "Return result in A", inline=True)

# ============================================================
# File handle conversion (&8589-&858B)
# ============================================================
subroutine(0x8589, "handle_to_mask_a", hook=None,
...
subroutine(0x858A, "handle_to_mask_clc", hook=None,
...
subroutine(0x858B, "handle_to_mask",
...
comment(0x858B, "Save A (will be restored on exit)", inline=True)
comment(0x858C, "Save X (will be restored on exit)", inline=True)
comment(0x858D, "  (second half of X save)", inline=True)
comment(0x858E, "A = handle from Y", inline=True)
comment(0x858F, "C=0: always convert", inline=True)
comment(0x8591, "C=1 and Y=0: skip (handle 0 = none)", inline=True)
comment(0x8593, "C=1 and Y!=0: convert", inline=True)
comment(0x8594, "A = handle - &1F (1-based bit position)", inline=True)
comment(0x8596, "X = shift count", inline=True)
comment(0x8597, "Start with bit 0 set", inline=True)
comment(0x8599, "Shift bit left", inline=True)
comment(0x859A, "Count down", inline=True)
comment(0x859B, "Loop until correct position", inline=True)
comment(0x859D, "Undo final extra shift", inline=True)
comment(0x859E, "Y = resulting bitmask", inline=True)
comment(0x859F, "Non-zero: valid mask, skip to exit", inline=True)
comment(0x85A1, "Zero: invalid handle, set Y=&FF", inline=True)
comment(0x85A2, "Restore X", inline=True)
comment(0x85A4, "Restore A", inline=True)

# ============================================================
# Mask to handle (&85A6)
# ============================================================
subroutine(0x85A6, "mask_to_handle",
...
receiving handle values from the fileserver in reply packets.""",
    ...
    on_exit={"a": "handle number (&20-&27)",
             "x": "corrupted",
             "y": "preserved"})
comment(0x85A6, "X = 0 (bit position counter)", inline=True)
comment(0x85A8, "Count this bit position", inline=True)
comment(0x85A9, "Shift mask right; C=0 when done", inline=True)
comment(0x85AA, "Loop until all bits shifted out", inline=True)
comment(0x85AC, "A = bit position (1-based)", inline=True)
comment(0x85AD, "Add &1E+C(=0) = &1E; handle=&1F+pos", inline=True)

# ============================================================
# Print decimal number (&85B0)
# ============================================================
subroutine(0x85B0, "print_decimal", hook=None,
...
subroutine(0x85BD, "print_decimal_digit", hook=None,
...
# ============================================================
subroutine(0x85CF, "compare_addresses",
...
# FS flags (&85E0 / &85E5)
# ============================================================
subroutine(0x85E0, "clear_fs_flag", hook=None,
...
# ============================================================
subroutine(0x8601, "print_file_info", hook=None,
...
# ============================================================
subroutine(0x8645, "setup_tx_ptr_c0", hook=None,
...
subroutine(0x864D, "tx_poll_ff", hook=None,
...
subroutine(0x8651, "tx_poll_core",
...
comment(0x853C, "Pop return address (low) — points to last byte of JSR", inline=True)
...
# ============================================================
# Print station identification (&81E7)
# ============================================================
subroutine(0x81E7, "print_station_info", hook=None,
...
# ============================================================
# Initialise filing system vectors (&8218)
# ============================================================
subroutine(0x8218, "init_fs_vectors", hook=None,
...
comment(0x8218, "Copy 14 bytes: FS vector addresses → FILEV-FSCV", inline=True)

# ============================================================
# FS vector dispatch and handler addresses (&824E)
# ============================================================
subroutine(0x824E, "fs_vector_addrs", hook=None,
...
byte(0x824E, 1)
comment(0x824E, "FILEV dispatch lo", inline=True)
byte(0x824F, 1)
comment(0x824F, "FILEV dispatch hi", inline=True)
byte(0x8250, 1)
comment(0x8250, "ARGSV dispatch lo", inline=True)
byte(0x8251, 1)
comment(0x8251, "ARGSV dispatch hi", inline=True)
byte(0x8252, 1)
comment(0x8252, "BGETV dispatch lo", inline=True)
byte(0x8253, 1)
comment(0x8253, "BGETV dispatch hi", inline=True)
byte(0x8254, 1)
comment(0x8254, "BPUTV dispatch lo", inline=True)
byte(0x8255, 1)
comment(0x8255, "BPUTV dispatch hi", inline=True)
byte(0x8256, 1)
comment(0x8256, "GBPBV dispatch lo", inline=True)
byte(0x8257, 1)
comment(0x8257, "GBPBV dispatch hi", inline=True)
byte(0x8258, 1)
comment(0x8258, "FINDV dispatch lo", inline=True)
byte(0x8259, 1)
comment(0x8259, "FINDV dispatch hi", inline=True)
byte(0x825A, 1)
comment(0x825A, "FSCV dispatch lo", inline=True)
byte(0x825B, 1)
comment(0x825B, "FSCV dispatch hi", inline=True)

# Part 2: handler address entries (7 x {lo, hi, pad})
byte(0x825C, 1)
comment(0x825C, "FILEV handler lo (&8695)", inline=True)
byte(0x825D, 1)
comment(0x825D, "FILEV handler hi", inline=True)
byte(0x825E, 1)
comment(0x825E, "(ROM bank — overwritten)", inline=True)
byte(0x825F, 1)
comment(0x825F, "ARGSV handler lo (&88E2)", inline=True)
byte(0x8260, 1)
comment(0x8260, "ARGSV handler hi", inline=True)
byte(0x8261, 1)
comment(0x8261, "(ROM bank — overwritten)", inline=True)
byte(0x8262, 1)
comment(0x8262, "BGETV handler lo (&8486)", inline=True)
byte(0x8263, 1)
comment(0x8263, "BGETV handler hi", inline=True)
byte(0x8264, 1)
comment(0x8264, "(ROM bank — overwritten)", inline=True)
byte(0x8265, 1)
comment(0x8265, "BPUTV handler lo (&83A3)", inline=True)
byte(0x8266, 1)
comment(0x8266, "BPUTV handler hi", inline=True)
byte(0x8267, 1)
comment(0x8267, "(ROM bank — overwritten)", inline=True)
byte(0x8268, 1)
comment(0x8268, "GBPBV handler lo (&89EB)", inline=True)
byte(0x8269, 1)
comment(0x8269, "GBPBV handler hi", inline=True)
byte(0x826A, 1)
comment(0x826A, "(ROM bank — overwritten)", inline=True)
byte(0x826B, 1)
comment(0x826B, "FINDV handler lo (&894A)", inline=True)
byte(0x826C, 1)
comment(0x826C, "FINDV handler hi", inline=True)
byte(0x826D, 1)
comment(0x826D, "(ROM bank — overwritten)", inline=True)
byte(0x826E, 1)
comment(0x826E, "FSCV handler lo (&808C)", inline=True)
byte(0x826F, 1)
comment(0x826F, "FSCV handler hi", inline=True)
...
# ============================================================
subroutine(0x81CD, "call_fscv_shutdown", hook=None,
...
# ============================================================
subroutine(0x822F, "issue_vectors_claimed", hook=None,
...
# ============================================================
# Run FSCV command from ROM (&8241)
# ============================================================
subroutine(0x8241, "run_fscv_cmd", hook=None,
...
# ============================================================
subroutine(0x82D2, "setup_rom_ptrs_netv", hook=None,
...
# ============================================================
subroutine(0x831D, "init_tx_ctrl_block", hook=None,
...
# ============================================================
subroutine(0x8351, "prepare_fs_cmd",
...
# ============================================================
subroutine(0x836B, "build_send_fs_cmd",
...
# ============================================================
subroutine(0x8403, "store_fs_error", hook=None,
...
# ============================================================
subroutine(0x83A4, "handle_bput_bget",
...
# ============================================================
subroutine(0x844B, "send_to_fs", hook=None,
...
# Check escape (&847B)
# ============================================================
subroutine(0x847B, "check_escape", hook=None,
...
subroutine(0x86D1, "send_fs_examine", hook=None,
...
subroutine(0x8717, "send_data_blocks", hook=None,
...
subroutine(0x8747, "filev_save", hook=None,
...
subroutine(0x87AE, "copy_load_addr_from_params", hook=None,
...
subroutine(0x87BB, "copy_reply_to_params", hook=None,
...
subroutine(0x87C9, "transfer_file_blocks", hook=None,
...
subroutine(0x8845, "filev_attrib_dispatch", hook=None,
...
subroutine(0x892D, "restore_args_return", hook=None,
...
subroutine(0x89D3, "adjust_addrs", hook=None,
...
comment(0x89D3, "X=&FC: index into &0E06 area (wraps to 0)", inline=True)
comment(0x89D5, "Load byte from param block", inline=True)
comment(0x89D7, "Test sign of adjustment direction", inline=True)
comment(0x89D9, "Negative: subtract instead", inline=True)
comment(0x89DB, "Add adjustment value", inline=True)
comment(0x89DE, "Skip to store result", inline=True)
comment(0x89E1, "Subtract adjustment value", inline=True)
comment(0x89E4, "Store adjusted byte back", inline=True)
comment(0x89E6, "Next param block byte", inline=True)
comment(0x89E7, "Next adjustment byte (X wraps &FC->&00)", inline=True)
comment(0x89E8, "Loop 4 times (X=&FC,&FD,&FE,&FF,done)", inline=True)

# ============================================================
# ARGSV handler (&88E2)
# ============================================================
subroutine(0x88E2, "argsv_handler",
...
# FINDV handler (&894A)
# ============================================================
subroutine(0x894A, "findv_handler",
...
# CLOSE handler (&8986)
# ============================================================
subroutine(0x8986, "close_handle", hook=None,
...
# GBPBV handler (&89EB)
# ============================================================
subroutine(0x89EB, "gbpbv_handler",
...
# ============================================================
subroutine(0x8AAE, "osgbpb_info", hook=None,
...
# *BYE handler (&834A)
# ============================================================
subroutine(0x834A, "bye_handler", hook=None,
...
subroutine(0x8BD7, "fs_cmd_match_table", hook=None,
...
# ============================================================
# Boot command strings (&8CEB)
# ============================================================
subroutine(0x8CEB, "boot_cmd_strings", hook=None,
...
# ============================================================
# Option name display (&8D3B-&8D4F)
# ============================================================
subroutine(0x8D3B, "option_name_strings", hook=None,
...
# ============================================================
# Reply buffer display helpers (&8D50-&8D73)
# ============================================================
...
# ============================================================
# Filename copy helpers (&8D64-&8D73)
# ============================================================
subroutine(0x8D64, "copy_filename", hook=None,
...
subroutine(0x8D66, "copy_string_to_cmd", hook=None,
...
byte written.""")
comment(0x8D66, "Start copying from offset 0", inline=True)
comment(0x8D68, "Load next byte from source string", inline=True)
comment(0x8D6A, "Store to command buffer", inline=True)
comment(0x8D6D, "Advance write position", inline=True)
comment(0x8D6E, "Advance read position", inline=True)
comment(0x8D6F, "XOR with CR: result=0 if byte was CR", inline=True)
comment(0x8D71, "Loop until CR copied", inline=True)
...
subroutine(0x8DB8, "calc_handle_offset", hook=None,
...
comment(0x8DB8, "A = handle * 2", inline=True)
comment(0x8DB9, "A = handle * 4", inline=True)
comment(0x8DBA, "Push handle*4 onto stack", inline=True)
comment(0x8DBB, "A = handle * 8", inline=True)
comment(0x8DBC, "X = stack pointer", inline=True)
comment(0x8DBD, "A = handle*8 + handle*4 = handle*12", inline=True)
comment(0x8DC0, "Y = offset into handle workspace", inline=True)
comment(0x8DC1, "Clean up stack (discard handle*4)", inline=True)
comment(0x8DC2, "Offset >= &48? (6 handles max)", inline=True)
comment(0x8DC4, "Valid: return with C clear", inline=True)
comment(0x8DC6, "Invalid: Y = 0", inline=True)
comment(0x8DC8, "A = 0, C set (error)", inline=True)
...
comment(0x8DF8, "Command code from &EF", inline=True)
comment(0x8DFA, "Subtract &0F: OSWORD &0F-&13 become indices 0-4", inline=True)

subroutine(0x8E02, "fs_osword_dispatch", hook=None,
...
# ============================================================
subroutine(0x8F73, "econet_tx_rx", hook=None,
...
comment(0x8F73, "A=0: set up and transmit; A>=1: handle result", inline=True)
...
# ============================================================
subroutine(0x9008, "osword_dispatch",
...
# ============================================================
subroutine(0x904C, "setup_tx_and_send", hook=None,
...
# ============================================================
subroutine(0x915A, "ctrl_block_setup_alt", hook=None,
...
subroutine(0x9163, "ctrl_block_setup", hook=None,
...
subroutine(0x918F, "ctrl_block_template", hook=None,
...
comment(0x8E23, "C=0: workspace to param direction", inline=True)
comment(0x8E25, "Load byte from param block", inline=True)
comment(0x8E27, "Store to workspace", inline=True)
comment(0x8E29, "Always taken (C still set)", inline=True)
comment(0x8E2B, "Load byte from workspace", inline=True)
comment(0x8E2D, "Store to param block", inline=True)
comment(0x8E2F, "Advance to next byte", inline=True)
comment(0x8E30, "Decrement byte counter", inline=True)
comment(0x8E31, "Loop while X >= 0", inline=True)

# ============================================================
# OSWORD handler block comments
# ============================================================
label(0x8E33, "return_copy_param")       # Return from copy_param_block

subroutine(0x8E34, "osword_0f_handler",
...
subroutine(0x8E54, "osword_11_handler", hook=None,
...
subroutine(0x8EF1, "osword_10_handler",
...
# ============================================================
subroutine(0x8F58, "setup_rx_buffer_ptrs", hook=None,
...
subroutine(0x9064, "remote_cmd_dispatch", hook=None,
...
subroutine(0x90B6, "match_osbyte_code", hook=None,
...
subroutine(0x90CE, "remote_cmd_data", hook=None,
...
subroutine(0x91C8, "remote_print_handler", hook=None,
...
subroutine(0x91ED, "store_output_byte", hook=None,
...
subroutine(0x9218, "flush_output_block", hook=None,
...
subroutine(0x92DE, "save_vdu_state", hook=None,
...
# ============================================================
subroutine(0x9249, "econet_tx_retry", hook=None,
...
label(0x054A, "tube_poll_r1_wrch")    # Service R1 WRCH requests while waiting for R2
...
comment(0x8095, "Y=&13: base offset for FSCV dispatch (indices 20+)", inline=True)
...
comment(0x809D, "Y=&0E: base offset for language handlers (index 15+)", inline=True)
...
comment(0x8137, "Y=0: base offset for service dispatch", inline=True)
...
comment(0x817D, "X=5: ROM offset for \"NET\" match", inline=True)
...
comment(0x81CC, "Return (service not claimed)", inline=True)
comment(0x81CD, "FSCV reason 6 = FS shutdown", inline=True)
comment(0x81CF, "Tail-call via filing system control vector", inline=True)
comment(0x81D2, "Notify current FS of shutdown", inline=True)
comment(0x81D5, "OSBYTE &7A: scan keyboard", inline=True)
comment(0x81DB, "No key pressed: proceed with auto-boot", inline=True)
subroutine(0x81DD, "check_boot_key", hook=None,
...
comment(0x81DD, "XOR with &55: result=0 if key is 'N'", inline=True)
comment(0x81DF, "Not 'N': return without claiming", inline=True)
comment(0x81E2, "OSBYTE &78: clear key-pressed state", inline=True)
comment(0x81E7, "Print 'Econet Station ' banner", inline=True)
comment(0x8206, "Print ' No Clock' warning", inline=True)
comment(0x8212, "NOP (padding after inline string)", inline=True)
label(0x8213, "skip_no_clock_msg")     # BEQ: clock present, skip "No Clock" message
comment(0x8213, "Print two CRs (blank line)", inline=True)
comment(0x821D, "Write to FILEV-FSCV vector table", inline=True)
comment(0x8220, "Next byte (descending)", inline=True)
comment(0x8221, "Loop until all 14 bytes copied", inline=True)
comment(0x8223, "Read ROM ptr table addr, install NETV", inline=True)
comment(0x8226, "Install 7 handler entries in ROM ptr table", inline=True)
comment(0x8228, "7 FS vectors to install", inline=True)
comment(0x822A, "Install each 3-byte vector entry", inline=True)
comment(0x822D, "X=0 after loop; store as workspace offset", inline=True)
comment(0x8238, "Issue service &0A", inline=True)
comment(0x823B, "Non-zero after hard reset: skip auto-boot", inline=True)
comment(0x823F, "X = lo byte of auto-boot string at &8292", inline=True)
comment(0x824C, "Auto-boot string tail / NETV handler data", inline=True)
comment(0x827E, "A=0 for clearing workspace", inline=True)
comment(0x8280, "Y=4: remote status offset", inline=True)
comment(0x8282, "Clear status byte in net receive buffer", inline=True)
comment(0x8284, "Y=&FF: used for later iteration", inline=True)
comment(0x8286, "Clear RX ptr low byte", inline=True)
comment(0x8288, "Clear workspace ptr low byte", inline=True)
comment(0x828A, "Clear RXCB iteration counter", inline=True)
comment(0x828C, "Clear TX semaphore (no TX in progress)", inline=True)
comment(0x828F, "X=0 for OSBYTE", inline=True)
comment(0x8295, "X = break type from OSBYTE result", inline=True)
comment(0x8298, "Y=&15: printer station offset in RX buffer", inline=True)
comment(0x829A, "&FE = no server selected", inline=True)
comment(0x829F, "Store &FE at printer station offset", inline=True)
label(0x82B5, "init_rxcb_entries")   # Mark all RXCBs as available (&3F) in NFS workspace
comment(0x82B5, "Load RXCB counter", inline=True)
comment(0x82B7, "Convert to workspace byte offset", inline=True)
comment(0x82BA, "C=1: past max handles, done", inline=True)
comment(0x82BC, "Mark RXCB as available", inline=True)
comment(0x82BE, "Write &3F flag to workspace", inline=True)
comment(0x82C0, "Next RXCB number", inline=True)
comment(0x82C2, "Loop for all RXCBs", inline=True)
label(0x82C4, "read_station_id")       # BEQ/BCS: soft break or RXCBs done, read &FE18
comment(0x82CD, "Enable user-level RX (LFLAG=&40)", inline=True)
comment(0x82CF, "Store to rx_flags", inline=True)
comment(0x82D2, "OSBYTE &A8: read ROM pointer table address", inline=True)
comment(0x82D4, "X=0: read low byte", inline=True)
comment(0x82D6, "Y=&FF: read high byte", inline=True)
comment(0x82D8, "Returns table address in X (lo) Y (hi)", inline=True)
comment(0x82DB, "Store table base address low byte", inline=True)
comment(0x82DD, "Store table base address high byte", inline=True)
comment(0x82DF, "NETV extended vector offset in ROM ptr table", inline=True)
comment(0x82E1, "Set NETV low byte = &36 (vector dispatch)", inline=True)
comment(0x82E4, "Install 1 entry (NETV) in ROM ptr table", inline=True)
comment(0x82E6, "Load handler address low byte from table", inline=True)
comment(0x82E9, "Store to ROM pointer table", inline=True)
comment(0x82EB, "Next byte", inline=True)
comment(0x82EC, "Load handler address high byte from table", inline=True)
comment(0x82EF, "Store to ROM pointer table", inline=True)
comment(0x82F1, "Next byte", inline=True)
comment(0x82F2, "Write current ROM bank number", inline=True)
comment(0x82F4, "Store ROM number to ROM pointer table", inline=True)
comment(0x82F6, "Advance to next entry position", inline=True)
comment(0x82F7, "Count down entries", inline=True)
comment(0x82F8, "Loop until all entries installed", inline=True)
comment(0x82FA, "Y = workspace high byte + 1 = next free page", inline=True)
comment(0x82FA, "Y = next workspace page for MOS", inline=True)
comment(0x82FC, "Advance past workspace page", inline=True)
comment(0x82FD, "Return; Y = page after NFS workspace", inline=True)
comment(0x82FE, "Copy 10 bytes: FS state to workspace backup", inline=True)
comment(0x8306, "Offsets &15-&1D: server, handles, OPT, etc.", inline=True)
subroutine(0x830F, "init_tx_reply_port", hook=None,
...
comment(0x830F, "A=&90: FS reply port (PREPLY)", inline=True)
comment(0x8311, "Init TXCB from template", inline=True)
comment(0x8314, "Store port number in TXCB", inline=True)
comment(0x8316, "Control byte: 3 = transmit", inline=True)
comment(0x8318, "Store control byte in TXCB", inline=True)
comment(0x831A, "Decrement TXCB flag to arm TX", inline=True)
comment(0x831D, "Preserve A across call", inline=True)
comment(0x831E, "Copy 12 bytes (Y=11..0)", inline=True)
comment(0x8320, "Load template byte", inline=True)
comment(0x8323, "Store to TX control block at &00C0", inline=True)
comment(0x8326, "Y < 2: also copy FS server station/network", inline=True)
comment(0x8328, "Skip station/network copy for Y >= 2", inline=True)
comment(0x832A, "Load FS server station (Y=0) or network (Y=1)", inline=True)
comment(0x832D, "Store to dest station/network at &00C2", inline=True)
comment(0x8330, "Next byte (descending)", inline=True)
comment(0x8331, "Loop until all 12 bytes copied", inline=True)
comment(0x8333, "Restore A", inline=True)
comment(0x8334, "Return", inline=True)
label(0x833B, "tx_ctrl_upper")       # TX control template upper half (high bytes/masks)
comment(0x8341, "Save flag byte for command", inline=True)
comment(0x8344, "C=1: include flag in FS command", inline=True)
comment(0x8345, "ALWAYS branch to prepare_fs_cmd", inline=True)
comment(0x8347, "V=0: command has no flag byte", inline=True)
comment(0x8348, "ALWAYS branch to prepare_fs_cmd", inline=True)
comment(0x8351, "V=0: standard FS command path", inline=True)
comment(0x8352, "Copy URD handle from workspace to buffer", inline=True)
comment(0x8355, "Store URD at &0F02", inline=True)
comment(0x8360, "Y=1: copy CSD (offset 1) then LIB (offset 0)", inline=True)
label(0x8362, "copy_dir_handles")       # Copy CSD and LIB handles into FS command header
comment(0x8362, "Copy CSD and LIB handles to command buffer", inline=True)
comment(0x8365, "Store at &0F03 (CSD) and &0F04 (LIB)", inline=True)
comment(0x8368, "Y=function code", inline=True)
comment(0x8369, "Loop for both handles", inline=True)
comment(0x836B, "Save carry (FS path vs byte-stream)", inline=True)
comment(0x836C, "Reply port &90 (PREPLY)", inline=True)
comment(0x836E, "Store at &0F00 (HDRREP)", inline=True)
comment(0x8371, "Copy TX template to &00C0", inline=True)
comment(0x8374, "A = X (buffer extent)", inline=True)
comment(0x8375, "HPTR = header (5) + data (X) bytes to send", inline=True)
comment(0x8377, "Store to TXCB end-pointer low", inline=True)
comment(0x8379, "Restore carry flag", inline=True)
comment(0x837A, "C=1: byte-stream path (BSXMIT)", inline=True)
comment(0x837C, "Save flags for send_fs_reply_cmd", inline=True)
comment(0x837D, "Point net_tx_ptr to &00C0; transmit", inline=True)
comment(0x8380, "Restore flags", inline=True)
comment(0x8381, "Save flags (V flag state)", inline=True)
comment(0x8382, "Set up RX wait for FS reply", inline=True)
comment(0x8387, "Transmit and wait (BRIANX)", inline=True)
comment(0x838A, "Restore flags", inline=True)
comment(0x838B, "Y=1: skip past command code byte", inline=True)
comment(0x838C, "Load return code from FS reply", inline=True)
comment(0x838E, "X = return code", inline=True)
comment(0x838F, "Zero: success, return", inline=True)
comment(0x8391, "V=0: standard path, error is fatal", inline=True)
comment(0x8393, "ADC #&2A: test for &D6 (not found)", inline=True)
label(0x8395, "check_fs_error")        # BVC: standard path, BNE to store_fs_error
comment(0x8395, "Non-zero: hard error, go to FSERR", inline=True)
comment(0x8397, "Return (success or soft &D6 error)", inline=True)
comment(0x8398, "Discard saved flags from stack", inline=True)
comment(0x8399, "X=&C0: TXCB address for byte-stream TX", inline=True)
comment(0x839B, "Y++ past command code", inline=True)
comment(0x839C, "Byte-stream transmit with retry", inline=True)
comment(0x839F, "Store result to &B3", inline=True)
comment(0x83A1, "C=0: success, check reply code", inline=True)
comment(0x83A4, "Save A (BPUT byte) on stack", inline=True)
comment(0x83A5, "Also save byte at &0FDF for BSXMIT", inline=True)
comment(0x83A8, "Transfer X for stack save", inline=True)
comment(0x83A9, "Save X on stack", inline=True)
comment(0x83AA, "Transfer Y (handle) for stack save", inline=True)
comment(0x83AB, "Save Y (handle) on stack", inline=True)
comment(0x83AC, "Save P (C = BPUT/BGET selector) on stack", inline=True)
comment(0x83AD, "Convert handle Y to single-bit mask", inline=True)
comment(0x83B0, "Store handle bitmask at &0FDE", inline=True)
comment(0x83B3, "Store handle bitmask for sequence tracking", inline=True)
comment(0x83B5, "&90 = data port (PREPLY)", inline=True)
comment(0x83B7, "Store reply port in command buffer", inline=True)
comment(0x83BA, "Set up 12-byte TXCB from template", inline=True)
comment(0x83BD, "CB reply buffer at &0FDC", inline=True)
comment(0x83BF, "Store reply buffer ptr low in TXCB", inline=True)
comment(0x83C1, "Error buffer at &0FE0", inline=True)
comment(0x83C3, "Store error buffer ptr low in TXCB", inline=True)
comment(0x83C5, "Y=1 (from init_tx_ctrl_block exit)", inline=True)
comment(0x83C6, "X=9: BPUT function code", inline=True)
comment(0x83C8, "Restore C: selects BPUT (0) vs BGET (1)", inline=True)
comment(0x83C9, "C=0 (BPUT): keep X=9", inline=True)
comment(0x83CC, "Store function code at &0FDD", inline=True)
comment(0x83D2, "X=&C0: TXCB address for econet_tx_retry", inline=True)
comment(0x83D4, "Transmit via byte-stream protocol", inline=True)
comment(0x83D7, "Load reply byte from buffer", inline=True)
comment(0x83DA, "Zero reply = success, skip error handling", inline=True)
comment(0x83DC, "Copy 32-byte reply to error buffer at &0FE0", inline=True)
comment(0x83DE, "Load reply byte at offset Y", inline=True)
comment(0x83E1, "Store to error buffer at &0FE0+Y", inline=True)
comment(0x83E4, "Next byte (descending)", inline=True)
comment(0x83E5, "Loop until all 32 bytes copied", inline=True)
comment(0x83E8, "Returns X=EXEC handle, Y=SPOOL handle", inline=True)
comment(0x83F7, "Y=&85: high byte of OSCLI string in ROM", inline=True)
comment(0x83F9, "Close SPOOL/EXEC via \"*SP.\" or \"*E.\"", inline=True)
label(0x83FC, "dispatch_fs_error")     # BNE: no handle match, reset ptr for FSERR
comment(0x83FC, "Reset CB pointer to error buffer at &0FE0", inline=True)
comment(0x83FE, "Reset reply ptr to error buffer", inline=True)
comment(0x8400, "Reload reply byte for error dispatch", inline=True)
comment(0x8403, "Remember raw FS error code", inline=True)
comment(0x8406, "Y=1: point to error number byte in reply", inline=True)
comment(0x8408, "Clamp FS errors below &A8 to standard &A8", inline=True)
comment(0x840A, "Error >= &A8: keep original value", inline=True)
comment(0x840C, "Error < &A8: override with standard &A8", inline=True)
comment(0x840E, "Write clamped error number to reply buffer", inline=True)
comment(0x841C, "Save updated sequence number", inline=True)
comment(0x841F, "Restore Y from stack", inline=True)
comment(0x8421, "Restore X from stack", inline=True)
comment(0x8423, "Restore A from stack", inline=True)
comment(0x8424, "Return to caller", inline=True)
label(0x8425, "error_not_listening")     # Error code 8: "Not listening"
comment(0x8425, "Error code 8: \"Not listening\" error", inline=True)
comment(0x8427, "ALWAYS branch to set_listen_offset", inline=True)
comment(0x8429, "Load TX status byte for error lookup", inline=True)
comment(0x842B, "Mask to 3-bit error code (0-7)", inline=True)
comment(0x842D, "X = error code index", inline=True)
comment(0x842E, "Look up error message offset from table", inline=True)
comment(0x8431, "X=0: start writing at &0101", inline=True)
comment(0x8433, "Store BRK opcode at &0100", inline=True)
label(0x8436, "copy_error_message")     # Copy Econet error message string into BRK block
comment(0x8436, "Load error message byte", inline=True)
comment(0x8439, "Build error message at &0101+", inline=True)
comment(0x843C, "Zero byte = end of message; go execute BRK", inline=True)
comment(0x844B, "Save function code on stack", inline=True)
comment(0x844C, "Load current rx_flags", inline=True)
comment(0x844F, "Save rx_flags on stack for restore", inline=True)
comment(0x8450, "Set bit7: FS transaction in progress", inline=True)
comment(0x8452, "Write back updated rx_flags", inline=True)
label(0x8455, "skip_rx_flag_set")       # BNE: non-zero TX hi byte, skip bit-7 set
comment(0x8455, "Push two zero bytes as timeout counters", inline=True)
comment(0x8457, "First zero for timeout", inline=True)
comment(0x8458, "Second zero for timeout", inline=True)
comment(0x8459, "Y=0: index for flag byte check", inline=True)
comment(0x845A, "TSX: index stack-based timeout via X", inline=True)
comment(0x8462, "Three-stage nested timeout: inner loop", inline=True)
comment(0x8465, "Inner not expired: keep polling", inline=True)
comment(0x8467, "Middle timeout loop", inline=True)
comment(0x846A, "Middle not expired: keep polling", inline=True)
comment(0x846C, "Outer timeout loop (slowest)", inline=True)
comment(0x846F, "Outer not expired: keep polling", inline=True)
comment(0x8471, "Pop first timeout byte", inline=True)
comment(0x8472, "Pop second timeout byte", inline=True)
comment(0x8473, "Pop saved rx_flags into A", inline=True)
comment(0x8474, "Restore saved rx_flags from stack", inline=True)
comment(0x8477, "Pop saved function code", inline=True)
comment(0x8478, "A=saved func code; zero would mean no reply", inline=True)
comment(0x847A, "Return to caller", inline=True)
comment(0x8486, "C=1: flag for BGET mode", inline=True)
comment(0x8487, "Handle BGET via FS command", inline=True)
comment(0x848A, "SEC: set carry for error check", inline=True)
comment(0x848B, "A=&FE: mask for EOF check", inline=True)
comment(0x848D, "BIT l0fdf: test error flags", inline=True)
comment(0x8490, "V=1: error, return early", inline=True)
comment(0x8492, "CLC: no error", inline=True)
label(0x84AF, "return_4")
comment(0x8514, "Y=&0E: attribute byte offset in param block", inline=True)
comment(0x8516, "Load FS attribute byte", inline=True)
comment(0x8518, "Mask to 6 bits (FS → BBC direction)", inline=True)
comment(0x851A, "X=4: skip first 4 table entries (BBC→FS half)", inline=True)
comment(0x851C, "ALWAYS branch to shared bitmask builder", inline=True)
comment(0x851E, "Mask to 5 bits (BBC → FS direction)", inline=True)
comment(0x8520, "X=&FF: INX makes 0; start from table index 0", inline=True)
label(0x8522, "attrib_shift_bits")      # Attribute bitmask conversion (shared tail)
comment(0x8522, "Temp storage for source bitmask to shift out", inline=True)
comment(0x8524, "A=0: accumulate destination bits here", inline=True)
label(0x8526, "map_attrib_bits")         # Map source attribute bits via access_bit_table lookup
comment(0x8526, "Next table entry", inline=True)
comment(0x8527, "Shift out source bits one at a time", inline=True)
comment(0x8529, "Bit was 0: skip this destination bit", inline=True)
comment(0x852B, "OR in destination bit from lookup table", inline=True)
label(0x852E, "skip_set_attrib_bit")   # BCC: source bit was 0, don't OR destination
comment(0x852E, "Loop while source bits remain (A != 0)", inline=True)
comment(0x8530, "Return; A = converted attribute bitmask", inline=True)
label(0x8544, "print_inline_char")     # Print inline string bytes via OSASCI until bit-7 terminator
label(0x854A, "print_next_char")      # INC lo-byte of pointer, load and print next char
label(0x8585, "no_dot_exit")          # BCC: char < '.', CLC then RTS (no dot found)
label(0x8586, "parse_decimal_rts")    # BEQ: char = '.', LDA result/RTS
comment(0x8589, "Handle number to Y for conversion", inline=True)
comment(0x858A, "Force unconditional conversion", inline=True)
label(0x85A2, "handle_mask_exit")     # BEQ/BNE: PLA/TAX/PLA/RTS exit restoring X and A
comment(0x85A6, "X = &1F (handle base - 1)", inline=True)
comment(0x85AC, "A = X = &1F + bit position = handle", inline=True)
comment(0x85CF, "Compare 4 bytes (index 4,3,2,1)", inline=True)
label(0x85D1, "compare_addr_byte")     # EOR bytes of two 4-byte addresses checking for mismatch
comment(0x85D1, "Load byte from first address", inline=True)
comment(0x85D3, "XOR with corresponding byte", inline=True)
comment(0x85D5, "Mismatch: Z=0, return unequal", inline=True)
comment(0x85D8, "Continue comparing", inline=True)
comment(0x85DB, "X=first handle (&20)", inline=True)
comment(0x85DD, "Y=last handle (&27)", inline=True)
comment(0x8698, "Y=1: copy 2 bytes (high then low)", inline=True)
comment(0x869A, "Load filename ptr from control block", inline=True)
comment(0x869C, "Store to MOS text pointer (&F2/&F3)", inline=True)
comment(0x869F, "Next byte (descending)", inline=True)
comment(0x86A0, "Loop for both bytes", inline=True)
comment(0x86A9, "Read next character via GSREAD", inline=True)
comment(0x86AC, "C=1 from GSREAD: end of string reached", inline=True)
comment(0x86AE, "Advance buffer index", inline=True)
comment(0x86AF, "Store parsed character to &0E30+X", inline=True)
comment(0x86B2, "ALWAYS loop (GSREAD clears C on success)", inline=True)
comment(0x86B4, "CR = &0D", inline=True)
comment(0x86B6, "Store CR terminator at end of string", inline=True)
comment(0x86B9, "Point fs_crc_lo/hi at &0E30 parse buffer", inline=True)
comment(0x86BB, "fs_crc_lo = &30", inline=True)
comment(0x86BD, "fs_crc_hi = &0E → buffer at &0E30", inline=True)
comment(0x86BF, "Store high byte", inline=True)
comment(0x86C1, "Recover function code from saved A", inline=True)
comment(0x86C3, "A >= 0: save (&00) or attribs (&01-&06)", inline=True)
comment(0x86C5, "A=&FF? Only &FF is valid for load", inline=True)
comment(0x86C9, "Unknown negative code: no-op return", inline=True)
comment(0x86D1, "Port &92 = PLDATA (data transfer port)", inline=True)
comment(0x86D3, "Overwrite URD field with data port number", inline=True)
comment(0x86D8, "Build FS header (V=1: CLV path)", inline=True)
comment(0x86DB, "Y=6: param block byte 6", inline=True)
comment(0x86DD, "Byte 6: use file's own load address?", inline=True)
comment(0x86DF, "Non-zero: use FS reply address (lodfil)", inline=True)
comment(0x86E1, "Zero: copy caller's load addr first", inline=True)
comment(0x86E4, "Then copy FS reply to param block", inline=True)
comment(0x86E7, "Carry clear from prepare_cmd_clv: skip lodfil", inline=True)
comment(0x86E9, "Copy FS reply addresses to param block", inline=True)
comment(0x86EC, "Then copy load addr from param block", inline=True)
label(0x86EF, "skip_lodfil")           # BCC: skip lodfil block, proceed to compute end addr
comment(0x86EF, "Compute end address = load + file length", inline=True)
label(0x86F1, "copy_load_end_addr")    # Copy 4 load-address bytes and compute end address
comment(0x86F1, "Load address byte", inline=True)
comment(0x86F3, "Store as current transfer position", inline=True)
comment(0x86F5, "Add file length byte", inline=True)
comment(0x86F8, "Store as end position", inline=True)
comment(0x86FA, "Next address byte", inline=True)
comment(0x86FB, "Decrement byte counter", inline=True)
comment(0x86FC, "Loop for all 4 address bytes", inline=True)
comment(0x86FE, "Adjust high byte for 3-byte length overflow", inline=True)
comment(0x86FF, "Subtract 4th length byte from end addr", inline=True)
comment(0x8702, "Store adjusted end address high byte", inline=True)
comment(0x8707, "Transfer file data in &80-byte blocks", inline=True)
comment(0x870A, "Copy 3-byte file length to FS reply cmd buffer", inline=True)
comment(0x870C, "Load file length byte", inline=True)
comment(0x870F, "Store in FS command data buffer", inline=True)
comment(0x8712, "Next byte (count down)", inline=True)
comment(0x8713, "Loop for 3 bytes (X=2,1,0)", inline=True)
comment(0x8715, "ALWAYS branch", inline=True)
comment(0x871A, "Addresses match: transfer complete", inline=True)
comment(0x871C, "Port &92 for data block transfer", inline=True)
comment(0x871E, "Store port to TXCB command byte", inline=True)
label(0x8720, "send_block_loop")       # Outer loop: set up and send each 128-byte block
comment(0x8720, "Set up next &80-byte block for transfer", inline=True)
label(0x8722, "copy_block_addrs")      # Inner loop: swap 4-byte current/end addresses
comment(0x8722, "Swap: current addr -> source, end -> current", inline=True)
comment(0x8724, "Source addr = current position", inline=True)
comment(0x8726, "Load end address byte", inline=True)
comment(0x8728, "Dest = end address (will be clamped)", inline=True)
comment(0x872A, "Next address byte", inline=True)
comment(0x872B, "Loop for all 4 bytes", inline=True)
comment(0x872D, "Command &7F = data block transfer", inline=True)
comment(0x872F, "Store to TXCB control byte", inline=True)
comment(0x8731, "Send this block to the fileserver", inline=True)
comment(0x8734, "Y=3: compare 4 bytes (3..0)", inline=True)
comment(0x8736, "Compare current vs end address (4 bytes)", inline=True)
comment(0x8739, "XOR with end address byte", inline=True)
comment(0x873C, "Not equal: more blocks to send", inline=True)
comment(0x873E, "Next byte", inline=True)
comment(0x873F, "Loop for all 4 address bytes", inline=True)
comment(0x8741, "All equal: transfer complete", inline=True)
comment(0x8742, "A=0: SAVE handler", inline=True)
comment(0x8744, "A!=0: attribute dispatch (A=1-6)", inline=True)
comment(0x8747, "Process 4 address bytes (load/exec/start/end)", inline=True)
comment(0x8749, "Y=&0E: start from end-address in param block", inline=True)
comment(0x874B, "Read end-address byte from param block", inline=True)
comment(0x874D, "Save to port workspace for transfer setup", inline=True)
comment(0x8750, "Y = Y-4: point to start-address byte", inline=True)
comment(0x8753, "end - start = transfer length byte", inline=True)
comment(0x8755, "Store length byte in FS command buffer", inline=True)
comment(0x8758, "Save length byte for param block restore", inline=True)
comment(0x8759, "Read corresponding start-address byte", inline=True)
comment(0x875B, "Save to port workspace", inline=True)
comment(0x875E, "Restore length byte from stack", inline=True)
comment(0x875F, "Replace param block entry with length", inline=True)
comment(0x8761, "Y = Y+5: advance to next address group", inline=True)
comment(0x8764, "Decrement address byte counter", inline=True)
comment(0x8765, "Loop for all 4 address bytes", inline=True)
comment(0x8767, "Copy load/exec addresses to FS command buffer", inline=True)
label(0x8769, "copy_save_params")      # Copy 9 bytes of load/exec address into FS command buffer
comment(0x8769, "Read load/exec address byte from params", inline=True)
comment(0x876B, "Copy to FS command buffer", inline=True)
comment(0x876E, "Next byte (descending)", inline=True)
comment(0x876F, "Loop for bytes 9..1", inline=True)
comment(0x8771, "Port &91 for save command", inline=True)
comment(0x8773, "Overwrite URD field with port number", inline=True)
comment(0x8776, "Save port &91 for flow control ACK", inline=True)
comment(0x8778, "Append filename at offset &0B in cmd buffer", inline=True)
comment(0x877A, "Append filename to cmd buffer at offset X", inline=True)
comment(0x877D, "Y=1: function code for save", inline=True)
label(0x8794, "skip_catalogue_msg")   # BEQ: messages flag=0, skip display
comment(0x8794, "Store reply command for attr decode", inline=True)
comment(0x8797, "Y=&0E: access byte offset in param block", inline=True)
comment(0x8799, "Load access byte from FS reply", inline=True)
comment(0x879C, "Convert FS access to BBC attribute format", inline=True)
comment(0x87A7, "Copied all 4 bytes? (Y=&0E..&11)", inline=True)
comment(0x87A9, "Loop for 4 attribute bytes", inline=True)
comment(0x87AB, "Restore A/X/Y and return to caller", inline=True)
comment(0x87AE, "Start at offset 5 (top of 4-byte addr)", inline=True)
comment(0x87B0, "Read from parameter block", inline=True)
comment(0x87B2, "Store to local workspace", inline=True)
comment(0x87B5, "Next byte (descending)", inline=True)
comment(0x87B6, "Copy offsets 5,4,3,2 (4 bytes)", inline=True)
comment(0x87B8, "Loop while Y >= 2", inline=True)
comment(0x87BA, "Return", inline=True)
comment(0x87BB, "Start at offset &0D (top of range)", inline=True)
comment(0x87BD, "First store uses X (attrib byte)", inline=True)
comment(0x87BE, "Write to parameter block", inline=True)
comment(0x87C0, "Read next byte from reply buffer", inline=True)
comment(0x87C4, "Copy offsets &0D down to 2", inline=True)
comment(0x87C9, "Save FS command byte on stack", inline=True)
comment(0x87CD, "Addresses equal: nothing to transfer", inline=True)
comment(0x87E1, "Store dest address byte", inline=True)
comment(0x87E3, "Advance current position", inline=True)
comment(0x87E5, "Next address byte", inline=True)
comment(0x87E6, "Decrement byte counter", inline=True)
comment(0x87E7, "Loop for all 4 bytes", inline=True)
comment(0x87EB, "SEC for SBC in overshoot check", inline=True)
comment(0x87EC, "Check if new pos overshot end addr", inline=True)
comment(0x87EF, "Subtract end address byte", inline=True)
comment(0x87F2, "Next byte", inline=True)
comment(0x87F3, "Decrement counter", inline=True)
comment(0x87F4, "Loop for 4-byte comparison", inline=True)
comment(0x87F6, "C=0: no overshoot, proceed", inline=True)
comment(0x87F8, "Overshot: clamp dest to end address", inline=True)
label(0x87FA, "clamp_dest_addr")       # Clamp dest addr when block overshoots transfer end
comment(0x87FA, "Load end address byte", inline=True)
comment(0x87FC, "Replace dest with end address", inline=True)
comment(0x87FE, "Next byte", inline=True)
comment(0x87FF, "Loop for all 4 bytes", inline=True)
label(0x8801, "send_block")           # BCC: no overshoot, skip clamp, send block
comment(0x8801, "Recover original FS command byte", inline=True)
comment(0x8802, "Re-push for next iteration", inline=True)
comment(0x8803, "Save processor flags (C from cmp)", inline=True)
comment(0x8804, "Store command byte in TXCB", inline=True)
comment(0x8806, "128-byte block size for data transfer", inline=True)
comment(0x8808, "Store size in TXCB control byte", inline=True)
comment(0x880A, "Point TX ptr to &00C0; transmit", inline=True)
comment(0x880D, "ACK port for flow control", inline=True)
comment(0x880F, "Set reply port for ACK receive", inline=True)
comment(0x8812, "Restore flags (C=overshoot status)", inline=True)
comment(0x8813, "C=1: all data sent (overshot), done", inline=True)
comment(0x8815, "Command &91 = data block transfer", inline=True)
comment(0x8817, "Store command &91 in TXCB", inline=True)
comment(0x881B, "Transmit block and wait (BRIANX)", inline=True)
comment(0x881E, "More blocks? Loop back", inline=True)
comment(0x8820, "Save A (function code)", inline=True)
comment(0x8823, "Convert handle to bitmask in A", inline=True)
comment(0x8826, "Y = handle bitmask from conversion", inline=True)
comment(0x8827, "Local hint: is EOF possible for this handle?", inline=True)
comment(0x882A, "X = result of AND (0 = not at EOF)", inline=True)
comment(0x882B, "Hint clear: definitely not at EOF", inline=True)
comment(0x882D, "Save bitmask for clear_fs_flag", inline=True)
comment(0x882E, "Handle byte in FS command buffer", inline=True)
comment(0x8831, "Y=&11: FS function code FCEOF", inline=True)
comment(0x8838, "Restore bitmask", inline=True)
comment(0x8839, "FS reply: non-zero = at EOF", inline=True)
comment(0x883C, "At EOF: skip flag clear", inline=True)
comment(0x883E, "Not at EOF: clear the hint bit", inline=True)
label(0x8841, "restore_ay_return")      # Restore A and Y registers, return
comment(0x8841, "Restore A", inline=True)
comment(0x8842, "Restore Y", inline=True)
comment(0x8844, "Return; X=0 (not EOF) or X=&FF (EOF)", inline=True)
comment(0x8845, "Store function code in FS cmd buffer", inline=True)
comment(0x8848, "A=6? (delete)", inline=True)
comment(0x884A, "Yes: jump to delete handler", inline=True)
comment(0x884C, "A>=7: unsupported, fall through to return", inline=True)
comment(0x884E, "A=5? (read catalogue info)", inline=True)
comment(0x8850, "Yes: jump to read info handler", inline=True)
comment(0x8852, "A=4? (write attributes only)", inline=True)
comment(0x8854, "Yes: jump to write attrs handler", inline=True)
comment(0x8856, "A=1? (write all catalogue info)", inline=True)
comment(0x8858, "Yes: jump to write-all handler", inline=True)
comment(0x885A, "A=2 or 3: convert to param block offset", inline=True)
comment(0x885B, "A*4: 2->8, 3->12", inline=True)
comment(0x885C, "Y = A*4", inline=True)
comment(0x885D, "Y = A*4 - 3 (load addr offset for A=2)", inline=True)
comment(0x8860, "X=3: copy 4 bytes", inline=True)
comment(0x8862, "Load address byte from param block", inline=True)
comment(0x8864, "Store to FS cmd data area", inline=True)
comment(0x8867, "Next source byte (descending)", inline=True)
comment(0x8868, "Next dest byte", inline=True)
comment(0x8869, "Loop for 4 bytes", inline=True)
comment(0x886B, "X=5: data extent for filename copy", inline=True)
comment(0x886F, "A=1: encode protection from param block", inline=True)
comment(0x8872, "Store encoded attrs at &0F0E", inline=True)
comment(0x8875, "Y=9: source offset in param block", inline=True)
comment(0x8877, "X=8: dest offset in cmd buffer", inline=True)
comment(0x8879, "Load byte from param block", inline=True)
comment(0x887B, "Store to FS cmd buffer", inline=True)
comment(0x887E, "Next source byte (descending)", inline=True)
comment(0x887F, "Next dest byte", inline=True)
comment(0x8880, "Loop until X=0 (8 bytes copied)", inline=True)
comment(0x8882, "X=&0A: data extent past attrs+addrs", inline=True)
comment(0x8884, "Append filename to cmd buffer", inline=True)
comment(0x8887, "Y=&13: fn code for FCSAVE (write attrs)", inline=True)
comment(0x8889, "ALWAYS branch to send command", inline=True)
comment(0x888B, "A=6: copy filename (delete)", inline=True)
comment(0x888E, "Y=&14: fn code for FCDEL (delete)", inline=True)
label(0x8890, "send_fs_cmd_v1")       # BNE: after copy_filename, BIT tx_ctrl_upper
comment(0x8890, "Set V=1 (BIT trick: &B3 has bit 6 set)", inline=True)
comment(0x8893, "Send via prepare_fs_cmd_v (V=1 path)", inline=True)
label(0x8896, "check_attrib_result")  # BCS: A>=7 unsupported, dispatch to error/return
comment(0x8896, "C=1: &D6 not-found, skip to return", inline=True)
comment(0x8898, "C=0: success, copy reply to param block", inline=True)
comment(0x889A, "A=4: encode attrs from param block", inline=True)
comment(0x889D, "Store encoded attrs at &0F06", inline=True)
comment(0x88A0, "X=2: data extent (1 attr byte + fn)", inline=True)
comment(0x88A2, "ALWAYS branch to append filename", inline=True)
comment(0x88A4, "A=5: X=1 (filename only, no data)", inline=True)
comment(0x88A6, "Copy filename to cmd buffer", inline=True)
comment(0x88A9, "Y=&12: fn code for FCEXAM (read info)", inline=True)
comment(0x88AE, "Save object type from FS reply", inline=True)
comment(0x88B1, "Clear reply byte (X=0 on success)", inline=True)
comment(0x88B4, "Clear length high byte in reply", inline=True)
comment(0x88B7, "Decode 5-bit access byte from FS reply", inline=True)
comment(0x88BF, "Y=&0E: attrs offset in param block", inline=True)
comment(0x88C1, "Store decoded attrs at param block +&0E", inline=True)
comment(0x88C3, "Y=&0D: start copy below attrs", inline=True)
comment(0x88C4, "X=&0C: copy from reply offset &0C down", inline=True)
comment(0x88C6, "Load reply byte (load/exec/length)", inline=True)
comment(0x88C9, "Store to param block", inline=True)
comment(0x88CB, "Next dest byte (descending)", inline=True)
comment(0x88CC, "Next source byte", inline=True)
comment(0x88CD, "Loop until X=0 (12 bytes copied)", inline=True)
comment(0x88CF, "X=0 -> X=2 for length high copy", inline=True)
comment(0x88D0, "INX again: X=2", inline=True)
comment(0x88D1, "Y=&11: length high dest in param block", inline=True)
comment(0x88D3, "Load length high byte from reply", inline=True)
comment(0x88D6, "Store to param block", inline=True)
comment(0x88D8, "Next dest byte (descending)", inline=True)
comment(0x88D9, "Next source byte", inline=True)
comment(0x88DA, "Loop for 3 length-high bytes", inline=True)
label(0x88E0, "attrib_error_exit")    # BCS: not-found/error, BPL restore_xy_return
comment(0x88E0, "A>=0: branch to restore_args_return", inline=True)
comment(0x88E2, "Save A/X/Y registers for later restore", inline=True)
comment(0x88E5, "Function >= 3?", inline=True)
comment(0x88E7, "A>=3 (ensure/flush): no-op for NFS", inline=True)
comment(0x88E9, "Test file handle", inline=True)
comment(0x88EB, "Y=0: FS-level query, not per-file", inline=True)
comment(0x88ED, "Convert handle to bitmask", inline=True)
comment(0x88F0, "Store bitmask as first cmd data byte", inline=True)
comment(0x88F3, "LSR splits A: C=1 means write (A=1)", inline=True)
comment(0x88F4, "Store function code to cmd data byte 2", inline=True)
comment(0x88F7, "C=1: write path, copy ptr from caller", inline=True)
comment(0x88F9, "Y=&0C: FCRDSE (read sequential pointer)", inline=True)
comment(0x88FB, "X=2: 3 data bytes in command", inline=True)
comment(0x88FD, "Build and send FS command", inline=True)
comment(0x8900, "Clear last-byte flag on success", inline=True)
comment(0x8902, "X = saved control block ptr low", inline=True)
comment(0x8904, "Y=2: copy 3 bytes of file pointer", inline=True)
comment(0x8906, "Zero high byte of 3-byte pointer", inline=True)
label(0x8908, "copy_fileptr_reply")    # Copy 3-byte file pointer from FS reply data
comment(0x8908, "Read reply byte from FS cmd data", inline=True)
comment(0x890B, "Store to caller's control block", inline=True)
comment(0x890D, "Next byte (descending)", inline=True)
comment(0x890E, "Next source byte", inline=True)
comment(0x890F, "Loop for all 3 bytes", inline=True)
label(0x8911, "argsv_check_return")    # Conditional return after ARGSV file pointer op
comment(0x8911, "C=0 (read): return to caller", inline=True)
comment(0x8913, "Save bitmask for set_fs_flag later", inline=True)
comment(0x8914, "Push bitmask", inline=True)
comment(0x8915, "Y=3: copy 4 bytes of file pointer", inline=True)
label(0x8917, "copy_fileptr_to_cmd")  # Copy 4-byte file pointer into FS command area
comment(0x8917, "Read caller's pointer byte", inline=True)
comment(0x8919, "Store to FS command data area", inline=True)
comment(0x891C, "Next source byte", inline=True)
comment(0x891D, "Next destination byte", inline=True)
comment(0x891E, "Loop for all 4 bytes", inline=True)
comment(0x8920, "Y=&0D: FCWRSE (write sequential pointer)", inline=True)
comment(0x8922, "X=5: 6 data bytes in command", inline=True)
comment(0x8924, "Build and send FS command", inline=True)
comment(0x8927, "Save not-found status from X", inline=True)
comment(0x8929, "Recover bitmask for EOF hint update", inline=True)
comment(0x892A, "Set EOF hint bit for this handle", inline=True)
comment(0x892D, "A = saved function code / command", inline=True)
label(0x892F, "restore_xy_return")     # Restore X and Y from workspace, return
comment(0x892F, "X = saved control block ptr low", inline=True)
comment(0x8931, "Y = saved control block ptr high", inline=True)
comment(0x8933, "Return to MOS with registers restored", inline=True)
label(0x893B, "halve_args_a")         # BEQ: A=2, LSR A then dispatch
comment(0x893B, "Shared: halve A (A=0 or A=2 paths)", inline=True)
comment(0x893B, "Shared: A=0->&05, A=2->&01", inline=True)
comment(0x893C, "Return with A = FS number or 1", inline=True)
comment(0x893E, "Read FS command context byte", inline=True)
comment(0x893E, "Copy command context to caller's block", inline=True)
comment(0x8941, "Store to caller's parameter block", inline=True)
comment(0x8943, "Next byte (descending)", inline=True)
comment(0x8944, "Loop until all bytes copied", inline=True)
comment(0x894A, "Save A/X/Y and set up pointers", inline=True)
comment(0x894D, "SEC distinguishes open (A>0) from close", inline=True)
comment(0x8952, "A=0: close file(s)", inline=True)
comment(0x8954, "Valid open modes: &40, &80, &C0 only", inline=True)
comment(0x8956, "Invalid mode bits: return", inline=True)
comment(0x8958, "A = original mode byte", inline=True)
comment(0x8959, "Convert MOS mode to FS protocol flags", inline=True)
comment(0x895B, "ASL: shift mode bits left", inline=True)
comment(0x895C, "Flag 1: read/write direction", inline=True)
comment(0x895F, "ROL: Flag 2 into bit 0", inline=True)
comment(0x8960, "Flag 2: create vs existing file", inline=True)
comment(0x8963, "X=2: copy after 2-byte flags", inline=True)
comment(0x8965, "Copy filename to FS command buffer", inline=True)
comment(0x8968, "Y=6: FS function code FCOPEN", inline=True)
comment(0x896A, "Set V flag from l83b3 bit 6", inline=True)
comment(0x896D, "Build and send FS open command", inline=True)
comment(0x8970, "Error: restore and return", inline=True)
comment(0x8972, "Load reply handle from FS", inline=True)
comment(0x8975, "X = new file handle", inline=True)
comment(0x8976, "Set EOF hint + sequence bits", inline=True)
comment(0x8984, "ALWAYS branch to restore and return", inline=True)
comment(0x8986, "A = handle (Y preserved in A)", inline=True)
comment(0x8987, "Y>0: close single file", inline=True)
comment(0x8989, "Close SPOOL/EXEC before FS close-all", inline=True)
comment(0x898E, "Y=0: close all handles on server", inline=True)
comment(0x8990, "Handle byte in FS command buffer", inline=True)
comment(0x899A, "Reply handle for flag update", inline=True)
comment(0x899D, "Update EOF/sequence tracking bits", inline=True)
label(0x89A0, "close_opt_return")      # Conditional return after close/opt handler
comment(0x89A0, "C=0: restore A/X/Y and return", inline=True)
comment(0x89A2, "Is it *OPT 4,Y?", inline=True)
comment(0x89A4, "No: check for *OPT 1", inline=True)
comment(0x89A6, "Y must be 0-3 for boot option", inline=True)
comment(0x89A8, "Y < 4: valid boot option", inline=True)
label(0x89B2, "set_messages_flag")     # Set *OPT 1 local messages flag from Y
comment(0x89B2, "Set local messages flag (*OPT 1,Y)", inline=True)
comment(0x89B5, "Return via restore_args_return", inline=True)
comment(0x89B7, "Error index 7 (Bad option)", inline=True)
comment(0x89B9, "Generate BRK error", inline=True)
comment(0x89BC, "Boot option value in FS command", inline=True)
comment(0x89BF, "Y=&16: FS function code FCOPT", inline=True)
comment(0x89C4, "Restore Y from saved value", inline=True)
comment(0x89C6, "Cache boot option locally", inline=True)
label(0x89C9, "opt_return")           # BCC: *OPT 1 done, to close_opt_return
comment(0x89C9, "Return via restore_args_return", inline=True)
comment(0x89CB, "Y=9: adjust 9 address bytes", inline=True)
comment(0x89CD, "Adjust with carry clear", inline=True)
comment(0x89D0, "Y=1: adjust 1 address byte", inline=True)
comment(0x89D2, "C=0 for address adjustment", inline=True)
label(0x89D5, "adjust_addr_byte")      # Add or subtract one adjustment byte per iteration (4 total)
label(0x89E1, "subtract_adjust")      # BMI: fs_load_addr_2 negative, SBC instead of ADC
comment(0x89EB, "Save A/X/Y to FS workspace", inline=True)
comment(0x89EE, "X = call number for range check", inline=True)
comment(0x89EF, "A=0: invalid, restore and return", inline=True)
comment(0x89F1, "Convert to 0-based (A=0..7)", inline=True)
comment(0x89F2, "Range check: must be 0-7", inline=True)
comment(0x89F4, "In range: continue to handler", inline=True)
comment(0x89F6, "Out of range: restore args and return", inline=True)
comment(0x89F9, "Recover 0-based function code", inline=True)
comment(0x89FA, "Y=0: param block byte 0 (file handle)", inline=True)
comment(0x89FC, "Save function code on stack", inline=True)
comment(0x89FD, "A>=4: info queries, dispatch separately", inline=True)
comment(0x89FF, "A<4: file read/write operations", inline=True)
comment(0x8A01, "Dispatch to OSGBPB 5-8 info handler", inline=True)
comment(0x8A04, "Get file handle from param block byte 0", inline=True)
comment(0x8A06, "Convert handle to bitmask for EOF flags", inline=True)
comment(0x8A09, "Store handle in FS command data", inline=True)
comment(0x8A0C, "Y=&0B: start at param block byte 11", inline=True)
comment(0x8A0E, "X=6: copy 6 bytes of transfer params", inline=True)
comment(0x8A10, "Load param block byte", inline=True)
comment(0x8A12, "Store to FS command buffer at &0F06+X", inline=True)
comment(0x8A15, "Previous param block byte", inline=True)
comment(0x8A16, "Skip param block offset 8 (the handle)", inline=True)
comment(0x8A18, "Not at handle offset: continue", inline=True)
comment(0x8A1A, "Extra DEY to skip handle byte", inline=True)
comment(0x8A1B, "Decrement copy counter", inline=True)
comment(0x8A1C, "Loop for all 6 bytes", inline=True)
comment(0x8A1E, "Recover function code from stack", inline=True)
comment(0x8A1F, "LSR: odd=read (C=1), even=write (C=0)", inline=True)
comment(0x8A20, "Save function code again (need C later)", inline=True)
comment(0x8A21, "Even (write): X stays 0", inline=True)
comment(0x8A23, "Odd (read): X=1", inline=True)
comment(0x8A24, "Store FS direction flag", inline=True)
comment(0x8A27, "Y=&0B: command data extent", inline=True)
comment(0x8A29, "Command &91=put, &92=get", inline=True)
comment(0x8A2B, "Recover function code", inline=True)
comment(0x8A2C, "Save again for later direction check", inline=True)
comment(0x8A2D, "Even (write): keep &91 and Y=&0B", inline=True)
comment(0x8A2F, "Odd (read): use &92 (get) instead", inline=True)
comment(0x8A31, "Read: one fewer data byte in command", inline=True)
label(0x8A32, "gbpb_write_path")      # BEQ: A=0 write, keeps X=&91/Y=&0B
comment(0x8A32, "Store port to FS command URD field", inline=True)
comment(0x8A35, "Save port for error recovery", inline=True)
comment(0x8A37, "X=8: command data bytes", inline=True)
comment(0x8A39, "Load handle from FS command data", inline=True)
comment(0x8A3C, "Build FS command with handle+flag", inline=True)
comment(0x8A3F, "Save seq# for byte-stream flow control", inline=True)
comment(0x8A41, "Store to FS sequence number workspace", inline=True)
comment(0x8A44, "X=4: copy 4 address bytes", inline=True)
comment(0x8A46, "Set up source/dest from param block", inline=True)
comment(0x8A48, "Store as source address", inline=True)
comment(0x8A4B, "Store as current transfer position", inline=True)
comment(0x8A4E, "Skip 4 bytes to reach transfer length", inline=True)
comment(0x8A51, "Dest = source + length", inline=True)
comment(0x8A53, "Store as end address", inline=True)
comment(0x8A56, "Back 3 to align for next iteration", inline=True)
comment(0x8A59, "Decrement byte counter", inline=True)
comment(0x8A5A, "Loop for all 4 address bytes", inline=True)
comment(0x8A5C, "X=1 after loop", inline=True)
comment(0x8A5D, "Copy CSD data to command buffer", inline=True)
comment(0x8A60, "Store at &0F06+X", inline=True)
comment(0x8A63, "Decrement counter", inline=True)
comment(0x8A64, "Loop for X=1,0", inline=True)
comment(0x8A66, "Odd (read): send data to FS first", inline=True)
comment(0x8A67, "Non-zero: skip write path", inline=True)
comment(0x8A69, "Load port for transfer setup", inline=True)
comment(0x8A6C, "Transfer data blocks to fileserver", inline=True)
label(0x8A78, "wait_fs_reply")        # BCS/fall: post-transfer, JSR send_fs_reply_cmd
comment(0x8A78, "Wait for FS reply command", inline=True)
comment(0x8A7B, "Load handle mask for EOF flag update", inline=True)
comment(0x8A7D, "Check FS reply: bit 7 = not at EOF", inline=True)
comment(0x8A80, "Bit 7 set: not EOF, skip clear", inline=True)
comment(0x8A82, "At EOF: clear EOF hint for this handle", inline=True)
label(0x8A85, "skip_clear_flag")      # BMI: reply bit 7 set (not EOF), skip clear_fs_flag
comment(0x8A85, "Set EOF hint flag (may be at EOF)", inline=True)
comment(0x8A88, "Direction=0: forward adjustment", inline=True)
comment(0x8A8A, "Adjust param block addrs by +9 bytes", inline=True)
comment(0x8A8D, "Direction=&FF: reverse adjustment", inline=True)
comment(0x8A8F, "SEC for reverse subtraction", inline=True)
comment(0x8A90, "Adjust param block addrs (reverse)", inline=True)
comment(0x8A93, "Shift bit 7 into C for return flag", inline=True)
comment(0x8A96, "Return via restore_args path", inline=True)
label(0x8A99, "get_disc_title")        # Request disc title via FS function code &15
comment(0x8A99, "Y=&15: function code for disc title", inline=True)
comment(0x8A9B, "Build and send FS command", inline=True)
comment(0x8A9E, "Load boot option from FS workspace", inline=True)
comment(0x8AA1, "Store boot option in reply area", inline=True)
comment(0x8AA4, "X=0: reply data start offset", inline=True)
comment(0x8AA6, "Clear reply buffer high byte", inline=True)
comment(0x8AA8, "A=&12: 18 bytes of reply data", inline=True)
comment(0x8AAA, "Store as byte count for copy", inline=True)
comment(0x8AAC, "ALWAYS branch to copy_reply_to_caller", inline=True)
comment(0x8AAE, "Y=4: check param block byte 4", inline=True)
comment(0x8AB0, "Check if destination is in Tube space", inline=True)
comment(0x8AB3, "No Tube: skip Tube address check", inline=True)
comment(0x8AB5, "Compare Tube flag with addr byte 4", inline=True)
comment(0x8AB7, "Mismatch: not Tube space", inline=True)
comment(0x8ABA, "Y=3: subtract addr byte 3 from flag", inline=True)
label(0x8ABC, "store_tube_flag")      # BEQ/BNE: store Tube-transfer flag to l00a9
comment(0x8ABC, "Non-zero = Tube transfer required", inline=True)
comment(0x8ABE, "Copy param block bytes 1-4 to workspace", inline=True)
comment(0x8AC0, "Store to &BD+Y workspace area", inline=True)
comment(0x8AC3, "Previous byte", inline=True)
comment(0x8AC4, "Loop for bytes 3,2,1", inline=True)
comment(0x8AC6, "Sub-function: AND #3 of (original A - 4)", inline=True)
comment(0x8AC7, "Mask to 0-3 (OSGBPB 5-8 → 0-3)", inline=True)
comment(0x8AC9, "A=0 (OSGBPB 5): read disc title", inline=True)
comment(0x8ACB, "LSR: A=0 (OSGBPB 6) or A=1 (OSGBPB 7)", inline=True)
comment(0x8ACC, "A=0 (OSGBPB 6): read CSD/LIB name", inline=True)
comment(0x8ACE, "C=1 (OSGBPB 8): read filenames from dir", inline=True)
label(0x8AD0, "gbpb6_read_name")      # BEQ: A=0 OSGBPB 6, load CSD/LIB/URD handle
comment(0x8AD0, "Y=0 for CSD or carry for fn code select", inline=True)
comment(0x8AD1, "Get CSD/LIB/URD handles for FS command", inline=True)
comment(0x8AD4, "Store CSD handle in command buffer", inline=True)
comment(0x8AD7, "Load LIB handle from workspace", inline=True)
comment(0x8ADA, "Store LIB handle in command buffer", inline=True)
comment(0x8ADD, "Load URD handle from workspace", inline=True)
comment(0x8AE0, "Store URD handle in command buffer", inline=True)
comment(0x8AE3, "X=&12: buffer extent for command data", inline=True)
comment(0x8AE5, "Store X as function code in header", inline=True)
comment(0x8AE8, "&0D = 13 bytes of reply data expected", inline=True)
comment(0x8AEA, "Store reply length in command buffer", inline=True)
comment(0x8AED, "Store as byte count for copy loop", inline=True)
comment(0x8AEF, "LSR: &0D >> 1 = 6", inline=True)
comment(0x8AF0, "Store as command data byte", inline=True)
comment(0x8AF3, "CLC for standard FS path", inline=True)
comment(0x8AF9, "INX: X=1 after build_send_fs_cmd", inline=True)
comment(0x8AFA, "Store X as reply start offset", inline=True)
comment(0x8AFC, "Copy FS reply to caller's buffer", inline=True)
comment(0x8AFE, "Non-zero: use Tube transfer path", inline=True)
comment(0x8B00, "X = reply start offset", inline=True)
comment(0x8B02, "Y = reply buffer high byte", inline=True)
label(0x8B04, "copy_reply_bytes")     # Copy N bytes of FS reply data to caller buffer
comment(0x8B04, "Load reply data byte", inline=True)
comment(0x8B07, "Store to caller's buffer", inline=True)
comment(0x8B09, "Next source byte", inline=True)
comment(0x8B0A, "Next destination byte", inline=True)
comment(0x8B0B, "Decrement remaining bytes", inline=True)
comment(0x8B0D, "Loop until all bytes copied", inline=True)
comment(0x8B0F, "ALWAYS branch to exit", inline=True)
label(0x8B11, "tube_transfer")        # BNE: l00a9 non-zero, claim Tube and send via R3
comment(0x8B11, "Claim Tube transfer channel", inline=True)
comment(0x8B14, "A=1: Tube claim type 1 (write)", inline=True)
comment(0x8B16, "X = param block address low", inline=True)
comment(0x8B18, "Y = param block address high", inline=True)
comment(0x8B1A, "INX: advance past byte 0", inline=True)
comment(0x8B1B, "No page wrap: keep Y", inline=True)
comment(0x8B1D, "Page wrap: increment high byte", inline=True)
label(0x8B1E, "no_page_wrap")         # BNE: no page boundary crossed in copy
comment(0x8B1E, "Claim Tube address for transfer", inline=True)
comment(0x8B21, "X = reply data start offset", inline=True)
comment(0x8B23, "Load reply data byte", inline=True)
comment(0x8B26, "Send byte to Tube via R3", inline=True)
comment(0x8B29, "Next source byte", inline=True)
comment(0x8B2A, "Decrement remaining bytes", inline=True)
comment(0x8B2C, "Loop until all bytes sent to Tube", inline=True)
comment(0x8B2E, "Release Tube after transfer complete", inline=True)
comment(0x8B30, "Release Tube address claim", inline=True)
label(0x8B33, "gbpb_done")            # BEQ: copy/adjust done, JMP return_a_zero
comment(0x8B33, "Return via restore_args path", inline=True)
label(0x8B36, "gbpb8_read_dir")       # BCS: C=1 OSGBPB 8, read dir entries
comment(0x8B36, "OSGBPB 8: read filenames from dir", inline=True)
comment(0x8B38, "Byte 9: number of entries to read", inline=True)
comment(0x8B3A, "Store as reply count in command buffer", inline=True)
comment(0x8B3D, "Y=5: byte 5 = starting entry number", inline=True)
comment(0x8B3F, "Load starting entry number", inline=True)
comment(0x8B41, "Store in command buffer", inline=True)
comment(0x8B44, "X=&0D: command data extent", inline=True)
comment(0x8B46, "Store extent in command buffer", inline=True)
comment(0x8B49, "Y=2: function code for dir read", inline=True)
comment(0x8B4B, "Store 2 as reply data start offset", inline=True)
comment(0x8B4D, "Store 2 as command data byte", inline=True)
comment(0x8B50, "Y=3: function code for header read", inline=True)
comment(0x8B54, "X=0 after FS command completes", inline=True)
comment(0x8B56, "Load reply entry count", inline=True)
comment(0x8B59, "Store at param block byte 0 (X=0)", inline=True)
comment(0x8B5B, "Load entries-read count from reply", inline=True)
comment(0x8B5E, "Y=9: param block byte 9", inline=True)
comment(0x8B60, "Add to starting entry number", inline=True)
comment(0x8B62, "Update param block with new position", inline=True)
comment(0x8B64, "Load total reply length", inline=True)
comment(0x8B66, "Subtract header (7 bytes) from reply len", inline=True)
comment(0x8B68, "Store adjusted length in command buffer", inline=True)
comment(0x8B6B, "Store as byte count for copy loop", inline=True)
comment(0x8B6D, "Zero bytes: skip copy", inline=True)
comment(0x8B6F, "Copy reply data to caller's buffer", inline=True)
label(0x8B72, "skip_copy_reply")      # BEQ: zero byte count, skip copy_reply_to_caller
comment(0x8B72, "X=2: clear 3 bytes", inline=True)
label(0x8B74, "zero_cmd_bytes")       # Zero 3 bytes of &0F07 area before address adjustment
comment(0x8B74, "Zero out &0F07+X area", inline=True)
comment(0x8B77, "Next byte", inline=True)
comment(0x8B78, "Loop for X=2,1,0", inline=True)
comment(0x8B7A, "Adjust pointer by +1 (one filename read)", inline=True)
comment(0x8B7D, "SEC for reverse adjustment", inline=True)
comment(0x8B7E, "Reverse adjustment for updated counter", inline=True)
comment(0x8B80, "Load entries-read count", inline=True)
comment(0x8B83, "Store in command buffer", inline=True)
comment(0x8B86, "Adjust param block addresses", inline=True)
comment(0x8B89, "Z=1: all done, exit", inline=True)
comment(0x8B8B, "A=&C3: Tube claim with retry", inline=True)
comment(0x8B8D, "Request Tube address claim", inline=True)
comment(0x8B90, "C=0: claim failed, retry", inline=True)
comment(0x8B92, "Tube claimed successfully", inline=True)
comment(0x8B93, "Save A/X/Y and set up command ptr", inline=True)
comment(0x8B96, "X=&FF: table index (pre-incremented)", inline=True)
label(0x8B98, "scan_cmd_table")        # Outer command-table search loop for * command match
comment(0x8B98, "Y=&FF: input index (pre-incremented)", inline=True)
comment(0x8B9A, "Advance input pointer", inline=True)
comment(0x8B9B, "Advance table pointer", inline=True)
comment(0x8B9C, "Load table character", inline=True)
comment(0x8B9F, "Bit 7: end of name, dispatch", inline=True)
comment(0x8BA1, "XOR input char with table char", inline=True)
comment(0x8BA3, "Case-insensitive (clear bit 5)", inline=True)
comment(0x8BA5, "Match: continue comparing", inline=True)
comment(0x8BA7, "Mismatch: back up table pointer", inline=True)
comment(0x8BA8, "Skip to end of table entry", inline=True)
comment(0x8BA9, "Load table byte", inline=True)
comment(0x8BAC, "Loop until bit 7 set (end marker)", inline=True)
comment(0x8BAE, "Check input for '.' abbreviation", inline=True)
comment(0x8BB0, "Skip past handler high byte", inline=True)
comment(0x8BB1, "Is input '.' (abbreviation)?", inline=True)
comment(0x8BB3, "No: try next table entry", inline=True)
comment(0x8BB5, "Yes: skip '.' in input", inline=True)
comment(0x8BB6, "Back to handler high byte", inline=True)
comment(0x8BB7, "ALWAYS branch; dispatch via BMI", inline=True)
label(0x8BB9, "dispatch_cmd")         # BMI: end-of-name marker, push handler addr/dispatch
comment(0x8BB9, "Push handler address high byte", inline=True)
comment(0x8BBA, "Load handler address low byte", inline=True)
comment(0x8BBD, "Push handler address low byte", inline=True)
comment(0x8C12, "X=1: copy dir name at cmd offset 1", inline=True)
comment(0x8C14, "Copy directory name to command buffer", inline=True)
comment(0x8C1C, "X=3: start printing from reply offset 3", inline=True)
comment(0x8C1E, "Print directory title (10 chars)", inline=True)
comment(0x8C21, "Print '('", inline=True)
comment(0x8C25, "Load station number from FS reply", inline=True)
comment(0x8C28, "Print station number as decimal", inline=True)
comment(0x8C2B, "Print ')     '", inline=True)
comment(0x8C37, "Non-zero: Public access", inline=True)
comment(0x8C39, "Print 'Owner' + CR", inline=True)
label(0x8C44, "print_public")          # BNE: access non-zero (public), print "Public"+CR
comment(0x8C44, "Print 'Public' + CR", inline=True)
comment(0x8C53, "X=1: past command code byte", inline=True)
comment(0x8C54, "Y=&10: print 16 characters", inline=True)
comment(0x8C56, "Print disc/CSD name from reply", inline=True)
comment(0x8C6B, "X = boot option for name table lookup", inline=True)
comment(0x8C6C, "Print boot option as hex digit", inline=True)
comment(0x8C6F, "Print ' ('", inline=True)
comment(0x8C7F, "Next character", inline=True)
comment(0x8C80, "Continue printing option name", inline=True)
label(0x8C82, "done_option_name")      # BMI: bit 7 set, option name printing complete
comment(0x8C82, "Print ')' + CR + 'Dir. '", inline=True)
comment(0x8CA0, "Print library name", inline=True)
comment(0x8CAB, "Save start offset in zero page for loop", inline=True)
comment(0x8CBD, "Load column count for display format", inline=True)
comment(0x8CBF, "Store column count in command data", inline=True)
comment(0x8CC2, "X=3: copy directory name at offset 3", inline=True)
comment(0x8CC4, "Append directory name to examine command", inline=True)
label(0x8D1D, "jmp_restore_args")      # JMP restore_args_return (common FS reply exit)
comment(0x8D1D, "Restore A/X/Y and return to caller", inline=True)
comment(0x8D20, "Set carry: LOGIN path (copy + boot)", inline=True)
comment(0x8D21, "Copy 4 bytes: boot option + 3 handles", inline=True)
comment(0x8D23, "SDISC: skip boot option, copy handles only", inline=True)
comment(0x8D25, "Load from FS reply (&0F05+X)", inline=True)
comment(0x8D28, "Store to handle workspace (&0E02+X)", inline=True)
comment(0x8D2B, "Next handle (descending)", inline=True)
comment(0x8D2C, "Loop while X >= 0", inline=True)
comment(0x8D2E, "SDISC: done, restore args and return", inline=True)
subroutine(0x8D30, "boot_cmd_execute", hook=None,
...
comment(0x8D30, "Y = boot option from FS workspace", inline=True)
comment(0x8D33, "X = command string offset from table", inline=True)
comment(0x8D36, "Y = &8D (high byte of command address)", inline=True)
comment(0x8D38, "Execute boot command string via OSCLI", inline=True)
comment(0x8DC8, "A=0, C set = error indicator", inline=True)
comment(0x8DCC, "Look up handle &F0 in workspace", inline=True)
comment(0x8DCF, "Invalid handle: return 0", inline=True)
comment(0x8DD1, "Load stored handle value", inline=True)
comment(0x8DD3, "&3F = unused/closed slot marker", inline=True)
comment(0x8DD5, "Slot in use: return actual value", inline=True)
comment(0x8DD7, "Return 0 for closed/invalid handle", inline=True)
label(0x8DD9, "store_handle_return")   # Store handle result to &F0, return
comment(0x8DD9, "Store result back to &F0", inline=True)
comment(0x8DE2, "Look up handle &F0 in workspace", inline=True)
comment(0x8DE5, "Invalid handle: return 0", inline=True)
comment(0x8DFE, "Only OSWORDs &0F-&13 (index 0-4)", inline=True)
comment(0x8E00, "Index >= 5: not ours, return", inline=True)
comment(0x8E03, "Load handler address high byte from table", inline=True)
comment(0x8E06, "Push high byte for RTS dispatch", inline=True)
comment(0x8E07, "Load handler address low byte from table", inline=True)
comment(0x8E0A, "Dispatch table: low bytes for OSWORD &0F-&13 handlers", inline=True)
comment(0x8E0B, "Y=2: save 3 bytes (&AA-&AC)", inline=True)
comment(0x8E0D, "Load param block pointer byte", inline=True)
comment(0x8E10, "Save to NFS workspace via (net_rx_ptr)", inline=True)
comment(0x8E12, "Next byte (descending)", inline=True)
comment(0x8E13, "Loop for all 3 bytes", inline=True)
comment(0x8E15, "Y=0 after BPL exit; INY makes Y=1", inline=True)
comment(0x8E16, "Read sub-function code from (&F0)+1", inline=True)
comment(0x8E39, "User TX CB in workspace page (high byte)", inline=True)
comment(0x8E3B, "Set param block high byte", inline=True)
comment(0x8E3D, "Set LTXCBP high byte for low-level TX", inline=True)
comment(0x8E3F, "&6F: offset into workspace for user TXCB", inline=True)
comment(0x8E41, "Set param block low byte", inline=True)
comment(0x8E43, "Set LTXCBP low byte for low-level TX", inline=True)
comment(0x8E45, "X=15: copy 16 bytes (OSWORD param block)", inline=True)
comment(0x8E47, "Copy param block to user TX control block", inline=True)
comment(0x8E54, "Set source high byte from workspace page", inline=True)
comment(0x8E56, "Store as copy source high byte in &AC", inline=True)
comment(0x8E58, "JSRSIZ at workspace offset &7F", inline=True)
comment(0x8E5A, "Load buffer size from workspace", inline=True)
comment(0x8E5C, "Y=&80: start of JSR argument data", inline=True)
comment(0x8E5D, "Store &80 as copy source low byte", inline=True)
comment(0x8E5F, "X = buffer size (loop counter)", inline=True)
comment(0x8E60, "X = size-1 (0-based count for copy)", inline=True)
comment(0x8E61, "Y=0: start of destination param block", inline=True)
comment(0x8E63, "Copy X+1 bytes from workspace to param", inline=True)
comment(0x8E6B, "Y=&7F: JSRSIZ offset (READRB entry)", inline=True)
comment(0x8E6D, "Load buffer size from workspace", inline=True)
comment(0x8E6F, "Y=1: param block offset for size byte", inline=True)
comment(0x8E71, "Store buffer size to (&F0)+1", inline=True)
comment(0x8E73, "Y=2: param block offset for args size", inline=True)
comment(0x8E74, "A=&80: argument data starts at offset &80", inline=True)
comment(0x8E76, "Store args start offset to (&F0)+2", inline=True)
comment(0x8E82, "Sub-function 4 or 5: read/set station", inline=True)
comment(0x8E84, "LSR: 0->0, 1->0, 2->1, 3->1", inline=True)
comment(0x8E85, "X=&0D: default to static workspace page", inline=True)
comment(0x8E87, "Transfer LSR result to Y for indexing", inline=True)
comment(0x8E88, "Y=0 (sub 0-1): use page &0D", inline=True)
comment(0x8E8A, "Y=1 (sub 2-3): use dynamic workspace", inline=True)
label(0x8E8C, "set_workspace_page")   # Sub-fn 0-1: use static page &0D, store to &AC
comment(0x8E8C, "Store workspace page in &AC (hi byte)", inline=True)
comment(0x8E8E, "Load offset: &FF (sub 0-1) or &01 (sub 2-3)", inline=True)
comment(0x8E91, "Store offset in &AB (lo byte)", inline=True)
comment(0x8E93, "X=1: copy 2 bytes", inline=True)
comment(0x8E95, "Y=1: start at param block offset 1", inline=True)
comment(0x8E9C, "LSR A: test bit 0 of sub-function", inline=True)
comment(0x8E9D, "Y=1: offset for protection byte", inline=True)
comment(0x8E9E, "Load protection byte from param block", inline=True)
comment(0x8EA0, "C=1 (odd sub): set protection", inline=True)
comment(0x8EA2, "C=0 (even sub): read current status", inline=True)
comment(0x8EA5, "Return current value to param block", inline=True)
comment(0x8EA7, "Update protection status", inline=True)
comment(0x8EAA, "Also save as JSR mask backup", inline=True)
comment(0x8EB8, "Sub-function 8: read FS handle", inline=True)
comment(0x8EBA, "Match: read handle from RX buffer", inline=True)
comment(0x8EBC, "Sub-function 9: read args size", inline=True)
comment(0x8EBE, "Match: read ARGS buffer info", inline=True)
comment(0x8EC0, "Sub >= 10 (bit 7 clear): read error", inline=True)
comment(0x8EC2, "Y=3: start from handle 3 (descending)", inline=True)
comment(0x8EC4, "LSR: test read/write bit", inline=True)
comment(0x8EC5, "C=0: read handles from workspace", inline=True)
comment(0x8EC7, "Init loop counter at Y=3", inline=True)
label(0x8EC9, "copy_handles_to_ws")   # Convert handles to bitmasks and store to workspace
comment(0x8EC9, "Reload loop counter", inline=True)
comment(0x8ECB, "Read handle from caller's param block", inline=True)
comment(0x8ECD, "Convert handle number to bitmask", inline=True)
comment(0x8ED0, "TYA: get bitmask result", inline=True)
comment(0x8ED1, "Reload loop counter", inline=True)
comment(0x8ED3, "Store bitmask to FS server table", inline=True)
comment(0x8ED6, "Next handle (descending)", inline=True)
comment(0x8ED8, "Loop for handles 3,2,1", inline=True)
comment(0x8EEC, "Next handle (descending)", inline=True)
comment(0x8EED, "Loop for handles 3,2,1", inline=True)
comment(0x8EF1, "Workspace page high byte", inline=True)
comment(0x8EF3, "Set up pointer high byte in &AC", inline=True)
comment(0x8EF5, "Save param block high byte in &AB", inline=True)
comment(0x8EF7, "Disable user RX during CB operation", inline=True)
comment(0x8EFA, "Read first byte of param block", inline=True)
comment(0x8EFC, "Save: 0=open new, non-zero=read RXCB", inline=True)
comment(0x8EFE, "Non-zero: read specified RXCB", inline=True)
comment(0x8F00, "Start scan from RXCB #3 (0-2 reserved)", inline=True)
comment(0x8F02, "Convert RXCB number to workspace offset", inline=True)
comment(0x8F05, "Invalid RXCB: return zero", inline=True)
comment(0x8F07, "LSR twice: byte offset / 4", inline=True)
comment(0x8F08, "Yields RXCB number from offset", inline=True)
comment(0x8F09, "X = RXCB number for iteration", inline=True)
comment(0x8F0A, "Read flag byte from RXCB workspace", inline=True)
comment(0x8F0C, "Zero = end of CB list", inline=True)
comment(0x8F0E, "&3F = deleted slot, free for reuse", inline=True)
comment(0x8F10, "Found free slot", inline=True)
comment(0x8F12, "Try next RXCB", inline=True)
comment(0x8F13, "A = next RXCB number", inline=True)
comment(0x8F14, "Continue scan (always branches)", inline=True)
comment(0x8F16, "A = free RXCB number", inline=True)
comment(0x8F17, "X=0 for indexed indirect store", inline=True)
comment(0x8F19, "Return RXCB number to caller's byte 0", inline=True)
label(0x8F1B, "read_rxcb")            # BNE: non-zero RXCB param, skip free-slot scan
comment(0x8F1B, "Convert RXCB number to workspace offset", inline=True)
comment(0x8F1E, "Invalid: write zero to param block", inline=True)
comment(0x8F20, "Y = offset-1: points to flag byte", inline=True)
comment(0x8F21, "Set &AB = workspace ptr low byte", inline=True)
comment(0x8F23, "&C0: test mask for flag byte", inline=True)
comment(0x8F25, "Y=1: flag byte offset in RXCB", inline=True)
comment(0x8F27, "X=11: copy 12 bytes from RXCB", inline=True)
comment(0x8F29, "Compare Y(1) with saved byte (open/read)", inline=True)
comment(0x8F2B, "ADC flag: test if slot is in use", inline=True)
comment(0x8F2D, "Zero: slot open, do copy", inline=True)
comment(0x8F2F, "Negative: slot has received data", inline=True)
label(0x8F31, "copy_rxcb_to_param")   # Copy RXCB data from workspace to param block
comment(0x8F31, "C=0: workspace-to-param direction", inline=True)
comment(0x8F32, "Copy RXCB data to param block", inline=True)
comment(0x8F35, "Done: skip deletion on error", inline=True)
comment(0x8F37, "Mark CB as consumed (consume-once)", inline=True)
comment(0x8F39, "Y=1: flag byte offset", inline=True)
comment(0x8F3B, "Write &3F to mark slot deleted", inline=True)
comment(0x8F3D, "Branch to exit (always taken)", inline=True)
comment(0x8F3F, "Advance through multi-byte field", inline=True)
comment(0x8F41, "Loop until all bytes processed", inline=True)
comment(0x8F43, "Y=-1 → Y=0 after STA below", inline=True)
comment(0x8F44, "Return zero (no free RXCB found)", inline=True)
label(0x8F46, "reenable_rx")          # Common exit: ROL rx_flags, RTS
comment(0x8F46, "Re-enable user RX", inline=True)
comment(0x8F4D, "Workspace offset &1C = RX data start", inline=True)
comment(0x8F5A, "A = base address low byte", inline=True)
comment(0x8F5C, "A = base + 1 (skip length byte)", inline=True)
comment(0x8F5E, "Store 16-bit start addr at ws+&1C/&1D", inline=True)
comment(0x8F61, "Read data length from (&F0)+1", inline=True)
comment(0x8F63, "A = data length byte", inline=True)
comment(0x8F65, "Workspace offset &20 = RX data end", inline=True)
comment(0x8F67, "A = base + length = end address low", inline=True)
comment(0x8F69, "Store low byte of 16-bit address", inline=True)
comment(0x8F6B, "Advance to high byte offset", inline=True)
comment(0x8F6C, "A = high byte of base address", inline=True)
comment(0x8F6E, "Add carry for 16-bit addition", inline=True)
comment(0x8F70, "Store high byte", inline=True)
comment(0x8F72, "Return", inline=True)
comment(0x8F75, "A >= 1: handle TX result", inline=True)
comment(0x8F77, "Y=&23: start of template (descending)", inline=True)
comment(0x8F79, "Load ROM template byte", inline=True)
comment(0x8F7C, "Non-zero = use ROM template byte as-is", inline=True)
comment(0x8F7E, "Zero = substitute from NMI workspace", inline=True)
label(0x8F81, "store_txcb_byte")      # BNE: non-zero template byte, skip substitution
comment(0x8F81, "Store to dynamic workspace", inline=True)
comment(0x8F83, "Descend through template", inline=True)
comment(0x8F84, "Stop at offset &17", inline=True)
comment(0x8F86, "Loop until all bytes copied", inline=True)
comment(0x8F88, "Y=&18: TX block starts here", inline=True)
comment(0x8F89, "Point net_tx_ptr at workspace+&18", inline=True)
comment(0x8F8B, "Set up RX buffer start/end pointers", inline=True)
comment(0x8F8E, "Y=2: port byte offset in RXCB", inline=True)
comment(0x8F90, "A=&90: FS reply port", inline=True)
comment(0x8F92, "Store port &90 at (&F0)+2", inline=True)
label(0x8F96, "copy_fs_addr")          # Copy 3-byte FS station address into RX param block
comment(0x8F96, "Copy FS station addr from workspace", inline=True)
comment(0x8F99, "Store to RX param block", inline=True)
comment(0x8F9B, "Next byte", inline=True)
comment(0x8F9C, "Done 3 bytes (Y=4,5,6)?", inline=True)
comment(0x8F9E, "No: continue copying", inline=True)
comment(0x8FA0, "High byte of workspace for TX ptr", inline=True)
comment(0x8FA2, "Store as TX pointer high byte", inline=True)
comment(0x8FA5, "Transmit with full retry", inline=True)
comment(0x8FA8, "Y=&20: RX end address offset", inline=True)
comment(0x8FAA, "Set RX end address to &FFFF (accept any length)", inline=True)
comment(0x8FAC, "Store end address low byte (&FF)", inline=True)
comment(0x8FAF, "Store end address high byte (&FF)", inline=True)
comment(0x8FB1, "Y=&19: port byte in workspace RXCB", inline=True)
comment(0x8FB3, "A=&90: FS reply port", inline=True)
comment(0x8FB5, "Store port to workspace RXCB", inline=True)
comment(0x8FB8, "A=&7F: flag byte = waiting for reply", inline=True)
comment(0x8FBA, "Store flag byte to workspace RXCB", inline=True)
label(0x8FC1, "handle_tx_result")     # BCS: A>=1, save flags and load RX result
comment(0x8FC1, "Save processor flags", inline=True)
comment(0x8FC2, "Y=1: first data byte offset", inline=True)
comment(0x8FC4, "Load first data byte from RX buffer", inline=True)
comment(0x8FC6, "X = first data byte (command code)", inline=True)
comment(0x8FC7, "Advance to next data byte", inline=True)
comment(0x8FC8, "Load station address high byte", inline=True)
comment(0x8FCA, "Advance past station addr", inline=True)
comment(0x8FCB, "Save Y as data index", inline=True)
comment(0x8FCD, "Store station addr hi at (net_rx_ptr)+&72", inline=True)
comment(0x8FCF, "Store to workspace", inline=True)
comment(0x8FD2, "A = command code (from X)", inline=True)
comment(0x8FD3, "Store station addr lo at (net_rx_ptr)+&71", inline=True)
comment(0x8FD5, "Restore flags from earlier PHP", inline=True)
comment(0x8FD6, "First call: adjust data length", inline=True)
label(0x8FD8, "send_data_bytes")       # Outer loop: fetch and TX successive data bytes
comment(0x8FD8, "Reload data index", inline=True)
comment(0x8FDA, "Advance data index for next iteration", inline=True)
comment(0x8FDC, "Load next data byte", inline=True)
comment(0x8FF2, "Not &0D: continue with next byte", inline=True)
comment(0x8FF6, "First-packet: set up control block", inline=True)
comment(0x8FF9, "Y=&7B: data length offset", inline=True)
comment(0x8FFB, "Load current data length", inline=True)
comment(0x8FFD, "Add 3 for header bytes", inline=True)
comment(0x8FFD, "Adjust data length by 3 for header bytes", inline=True)
comment(0x8FFF, "Store adjusted length", inline=True)
comment(0x9001, "Enable interrupts", inline=True)
comment(0x9005, "Transmit via tx_poll_ff", inline=True)
comment(0x9008, "Save processor status", inline=True)
comment(0x9009, "Save A (reason code)", inline=True)
comment(0x900A, "Save X", inline=True)
comment(0x900B, "Push X to stack", inline=True)
comment(0x900C, "Save Y", inline=True)
comment(0x900D, "Push Y to stack", inline=True)
comment(0x900E, "Get stack pointer for indexed access", inline=True)
comment(0x900F, "Retrieve original A (reason code) from stack", inline=True)
comment(0x9012, "Reason codes 0-8 only", inline=True)
comment(0x9014, "Code >= 9: skip dispatch, restore regs", inline=True)
comment(0x9016, "X = reason code for table lookup", inline=True)
comment(0x9017, "Dispatch to handler via trampoline", inline=True)
comment(0x901A, "Restore Y", inline=True)
comment(0x901B, "Transfer to Y register", inline=True)
comment(0x901C, "Restore X", inline=True)
comment(0x901D, "Transfer to X register", inline=True)
comment(0x901E, "Restore A", inline=True)
comment(0x901F, "Restore processor status flags", inline=True)
comment(0x9020, "Return with all registers preserved", inline=True)
comment(0x9024, "Push high byte of handler address", inline=True)
comment(0x9025, "Load handler low byte from table", inline=True)
comment(0x9028, "Push low byte of handler address", inline=True)
comment(0x9029, "Load workspace byte &EF for handler", inline=True)
comment(0x902B, "RTS dispatches to pushed handler", inline=True)
comment(0x9042, "ASL: restore P after ROR zeroed carry", inline=True)
comment(0x9045, "Y = character to write", inline=True)
comment(0x9046, "Store character at workspace offset &DA", inline=True)
comment(0x9048, "Store char at workspace offset &DA", inline=True)
comment(0x904A, "A=0: command type for net write char", inline=True)
comment(0x904C, "Y=&D9: command type offset", inline=True)
comment(0x904E, "Store command type at ws+&D9", inline=True)
comment(0x9050, "Mark TX control block as active (&80)", inline=True)
comment(0x9052, "Y=&0C: TXCB start offset", inline=True)
comment(0x9054, "Set TX active flag at ws+&0C", inline=True)
comment(0x9056, "Redirect net_tx_ptr low to workspace", inline=True)
comment(0x9058, "Load workspace page high byte", inline=True)
comment(0x905A, "Complete ptr redirect", inline=True)
comment(0x905C, "Transmit with full retry", inline=True)
comment(0x905F, "Mark TXCB as deleted (&3F) after transmit", inline=True)
comment(0x9061, "Write &3F to TXCB byte 0", inline=True)
comment(0x9063, "Return", inline=True)
comment(0x9064, "Load original Y (OSBYTE secondary param)", inline=True)
comment(0x9066, "OSBYTE &81 (INKEY): always forward to terminal", inline=True)
comment(0x9068, "Forward &81 to terminal for keyboard read", inline=True)
comment(0x906A, "Y=1: search NCTBPL table (execute on both)", inline=True)
comment(0x906C, "X=9: 10-entry NCTBPL table size", inline=True)
comment(0x906E, "Search for OSBYTE code in NCTBPL table", inline=True)
comment(0x9071, "Match found: dispatch with Y=1 (both)", inline=True)
comment(0x9073, "Y=-1: search NCTBMI table (terminal only)", inline=True)
comment(0x9074, "Second DEY: Y=&FF (from 1 via 0)", inline=True)
comment(0x9075, "X=&0E: 15-entry NCTBMI table size", inline=True)
comment(0x9077, "Search for OSBYTE code in NCTBMI table", inline=True)
comment(0x907A, "Match found: dispatch with Y=&FF (terminal)", inline=True)
comment(0x907C, "Y=0: OSBYTE not recognised, ignore", inline=True)
label(0x907D, "dispatch_remote_osbyte") # Common target for OSBYTE dispatch setup
comment(0x907D, "X=2 bytes to copy (default for RBYTE)", inline=True)
comment(0x907F, "A=Y: check table match result", inline=True)
comment(0x9080, "Y=0: not recognised, return unhandled", inline=True)
comment(0x9082, "Y>0 (NCTBPL): send only, no result expected", inline=True)
comment(0x9083, "Y>0 (NCTBPL): no result expected, skip RX", inline=True)
comment(0x9085, "Y<0 (NCTBMI): X=3 bytes (result + P flags)", inline=True)
comment(0x9086, "Y=&DC: top of 3-byte stack frame region", inline=True)
comment(0x9088, "Copy OSBYTE args from stack frame to workspace", inline=True)
comment(0x908B, "Store to NFS workspace for transmission", inline=True)
comment(0x908D, "Next byte (descending)", inline=True)
comment(0x908E, "Copied all 3 bytes? (&DC, &DB, &DA)", inline=True)
comment(0x9090, "Loop for remaining bytes", inline=True)
comment(0x9092, "A = byte count for setup_tx_and_send", inline=True)
comment(0x9093, "Build TXCB and transmit to terminal", inline=True)
comment(0x9096, "Restore N flag from table match type", inline=True)
comment(0x9097, "Y was positive (NCTBPL): done, no result", inline=True)
comment(0x9099, "Set up RX control block to wait for reply", inline=True)
comment(0x909F, "Bit7 clear: still waiting, poll again", inline=True)
comment(0x90A1, "X = stack pointer for register restoration", inline=True)
comment(0x90A2, "Y=&DD: saved P byte offset in workspace", inline=True)
comment(0x90A4, "Load remote processor status from reply", inline=True)
comment(0x90A6, "Force V=1 (claimed) and I=1 (no IRQ) in saved P", inline=True)
comment(0x90A8, "ALWAYS branch (ORA #&44 never zero)", inline=True)
comment(0x90AA, "Previous workspace offset", inline=True)
comment(0x90AB, "Previous stack register slot", inline=True)
comment(0x90AC, "Load next result byte (X, then Y)", inline=True)
comment(0x90AE, "Write result bytes to stacked registers", inline=True)
comment(0x90B1, "Copied all result bytes? (P at &DA)", inline=True)
comment(0x90B3, "Loop for remaining result bytes", inline=True)
comment(0x90B5, "Return to OSBYTE dispatcher", inline=True)
comment(0x90B6, "Compare OSBYTE code with table entry", inline=True)
comment(0x90B9, "Match found: return with Z=1", inline=True)
comment(0x90BB, "Next table entry (descending)", inline=True)
comment(0x90BC, "Loop for remaining entries", inline=True)
comment(0x90BE, "Return; Z=1 if match, Z=0 if not", inline=True)
label(0x90BF, "remote_osbyte_table") # OSBYTE codes accepted for remote execution
for addr in range(0x90BF, 0x90CE):
    byte(addr)
comment(0x90BF, "OSBYTE &04: cursor key status", inline=True)
comment(0x90C0, "OSBYTE &09: flash duration (1st colour)", inline=True)
comment(0x90C1, "OSBYTE &0A: flash duration (2nd colour)", inline=True)
comment(0x90C2, "OSBYTE &14: explode soft character RAM", inline=True)
comment(0x90C3, "OSBYTE &9A: video ULA control register", inline=True)
comment(0x90C4, "OSBYTE &9B: video ULA palette", inline=True)
comment(0x90C5, "OSBYTE &9C: ACIA control register", inline=True)
comment(0x90C6, "OSBYTE &E2: function key &D0-&DF", inline=True)
comment(0x90C7, "OSBYTE &0B: auto-repeat delay", inline=True)
comment(0x90C8, "OSBYTE &0C: auto-repeat rate", inline=True)
comment(0x90C9, "OSBYTE &0F: flush buffer class", inline=True)
comment(0x90CA, "OSBYTE &79: keyboard scan from X", inline=True)
comment(0x90CB, "OSBYTE &7A: keyboard scan from &10", inline=True)
comment(0x90CC, "OSBYTE &E3: function key &E0-&EF", inline=True)
comment(0x90CD, "OSBYTE &E4: function key &F0-&FF", inline=True)
comment(0x90D2, "OSWORD 7 (sound): handle via common path", inline=True)
comment(0x90D4, "OSWORD 8 = define an envelope", inline=True)
comment(0x90D6, "Not OSWORD 7 or 8: ignore (BNE exits)", inline=True)
label(0x90D8, "copy_params_rword")     # Copy param bytes and tag as RWORD message
comment(0x90D8, "Point workspace to offset &DB for params", inline=True)
comment(0x90DA, "Store workspace ptr offset &DB", inline=True)
label(0x90DC, "copy_osword_params")    # Copy OSWORD parameter bytes from RX buffer to workspace
comment(0x90DC, "Copy parameter bytes from RX buffer", inline=True)
comment(0x90DE, "Write param byte to workspace", inline=True)
comment(0x90E0, "Next byte (descending)", inline=True)
comment(0x90E1, "Loop for all parameter bytes", inline=True)
comment(0x90E3, "Y=0 after loop", inline=True)
comment(0x90E4, "Point workspace to offset &DA", inline=True)
comment(0x90E6, "Store original OSBYTE code at workspace+0", inline=True)
comment(0x90E8, "Store OSBYTE code at ws+0", inline=True)
comment(0x90EA, "Reset workspace ptr to base", inline=True)
comment(0x90EC, "Y=&14: command type offset", inline=True)
comment(0x90EE, "Tag as RWORD (port &E9)", inline=True)
comment(0x90F0, "Store port tag at ws+&14", inline=True)
comment(0x90F2, "A=1: single-byte TX", inline=True)
comment(0x90F4, "Transmit via workspace TXCB", inline=True)
comment(0x90F7, "Restore workspace ptr", inline=True)
comment(0x915A, "X=&0D: template offset for alt entry", inline=True)
comment(0x915C, "Y=&7C: target workspace offset for alt entry", inline=True)
comment(0x915E, "BIT test: V flag = bit 6 of &83B3", inline=True)
comment(0x9161, "V=1: store to (net_rx_ptr) instead", inline=True)
comment(0x9163, "Y=&17: workspace target offset (main entry)", inline=True)
comment(0x9165, "X=&1A: template table index (main entry)", inline=True)
comment(0x9167, "V=0: target is (nfs_workspace)", inline=True)
comment(0x916B, "&FE = stop sentinel", inline=True)
comment(0x916D, "End of template: jump to exit", inline=True)
comment(0x916F, "&FD = skip sentinel", inline=True)
comment(0x9171, "Skip: don't store, just decrement Y", inline=True)
comment(0x9173, "&FC = page byte sentinel", inline=True)
comment(0x9175, "Not sentinel: store template value directly", inline=True)
comment(0x917D, "PAGE byte → Y=&02 / Y=&74", inline=True)
comment(0x917F, "→ Y=&04 / Y=&76", inline=True)
comment(0x9181, "PAGE byte → Y=&06 / Y=&78", inline=True)
comment(0x9183, "→ Y=&08 / Y=&7A", inline=True)
comment(0x9185, "Alt-path only → Y=&70", inline=True)
comment(0x9185, "→ Y=&0A / Y=&7C", inline=True)
label(0x9187, "cb_template_main_start") # Control block template: main-path section
comment(0x9187, "SKIP", inline=True)
comment(0x9187, "→ Y=&0C (main only)", inline=True)
comment(0x9188, "→ Y=&01 / Y=&73", inline=True)
comment(0x9188, "→ Y=&0D (main only)", inline=True)
label(0x918B, "cb_template_tail")    # Control block template: tail section
comment(0x918B, "→ Y=&10 (main only)", inline=True)
comment(0x918E, "→ Y=&07 / Y=&79", inline=True)
comment(0x91B8, "X-1: convert 1-based buffer to 0-based", inline=True)
comment(0x91B9, "Is this the network printer buffer?", inline=True)
comment(0x91BB, "No: skip printer init", inline=True)
comment(0x91BD, "&1F = initial buffer pointer offset", inline=True)
comment(0x91BF, "Reset printer buffer write position", inline=True)
comment(0x91C2, "&41 = initial PFLAGS (bit 6 set, bit 0 set)", inline=True)
comment(0x91C7, "Return", inline=True)
comment(0x91C8, "Only handle buffer 4 (network printer)", inline=True)
comment(0x91CA, "Not buffer 4: ignore", inline=True)
comment(0x91CC, "A = reason code", inline=True)
comment(0x91CD, "Reason 1? (DEX: 1->0)", inline=True)
comment(0x91CE, "Not reason 1: handle Ctrl-B/C", inline=True)
comment(0x91D0, "Get stack pointer for P register", inline=True)
comment(0x91D1, "Force I flag in stacked P to block IRQs", inline=True)
comment(0x91D4, "Write back modified P register", inline=True)
comment(0x91D7, "OSBYTE &91: extract char from MOS buffer", inline=True)
comment(0x91D9, "X=3: printer buffer number", inline=True)
comment(0x91DE, "Buffer empty: return", inline=True)
comment(0x91E0, "Y = extracted character", inline=True)
comment(0x91E1, "Store char in output buffer", inline=True)
comment(0x91E4, "Buffer nearly full? (&6E = threshold)", inline=True)
comment(0x91E6, "Not full: get next char", inline=True)
comment(0x91E8, "Buffer full: flush to network", inline=True)
comment(0x91ED, "Load current buffer offset", inline=True)
comment(0x91F0, "Store byte at current position", inline=True)
comment(0x91F2, "Advance buffer pointer", inline=True)
comment(0x91F5, "Return; Y = buffer offset", inline=True)
label(0x91F6, "toggle_print_flag")    # Toggle print-active flag and update PFLAGS
comment(0x91F6, "Save reason code", inline=True)
comment(0x9217, "Return", inline=True)
comment(0x9218, "Store buffer length at workspace offset &08", inline=True)
comment(0x921A, "Current buffer fill position", inline=True)
comment(0x921D, "Write to workspace offset &08", inline=True)
comment(0x921F, "Store page high byte at offset &09", inline=True)
comment(0x9221, "Y=&09", inline=True)
comment(0x9222, "Write page high byte at offset &09", inline=True)
comment(0x9224, "Also store at offset &05", inline=True)
comment(0x9226, "(end address high byte)", inline=True)
comment(0x9228, "Y=&0B: flag byte offset", inline=True)
comment(0x922A, "X=&26: start from template entry &26", inline=True)
comment(0x922C, "Reuse ctrl_block_setup with CLV entry", inline=True)
comment(0x922F, "Y=&0A: sequence flag byte offset", inline=True)
comment(0x923B, "Old sequence bit into bit 0", inline=True)
comment(0x923C, "Store sequence flag at offset &0A", inline=True)
comment(0x923E, "Y=&1F: buffer start offset", inline=True)
comment(0x9240, "Reset printer buffer to start (&1F)", inline=True)
comment(0x9243, "A=0: printer output flag", inline=True)
comment(0x9245, "X=0: workspace low byte", inline=True)
comment(0x9246, "Y = workspace page high byte", inline=True)
comment(0x9248, "Enable interrupts before TX", inline=True)
comment(0x9249, "Set TX control block ptr low byte", inline=True)
comment(0x924B, "Set TX control block ptr high byte", inline=True)
comment(0x924D, "Save A (handle bitmask) for later", inline=True)
comment(0x924E, "Compute sequence bit from handle", inline=True)
comment(0x9251, "Zero: no sequence bit set", inline=True)
comment(0x9253, "Non-zero: normalise to bit 0", inline=True)
comment(0x9255, "Y=0: flag byte offset in TXCB", inline=True)
comment(0x9257, "Merge sequence into existing flag byte", inline=True)
comment(0x9259, "Save merged flag byte", inline=True)
comment(0x925A, "Write flag+sequence to TXCB byte 0", inline=True)
comment(0x925C, "Transmit with full retry", inline=True)
comment(0x925F, "End address &FFFF = unlimited data length", inline=True)
comment(0x9261, "Y=8: end address low offset in TXCB", inline=True)
comment(0x9263, "Store &FF to end addr low", inline=True)
comment(0x9266, "Store &FF to end addr high (Y=9)", inline=True)
comment(0x9268, "Recover merged flag byte", inline=True)
comment(0x9269, "Save in X for sequence compare", inline=True)
comment(0x926A, "Y=&D1: printer port number", inline=True)
comment(0x926C, "Recover saved handle bitmask", inline=True)
comment(0x926D, "Re-save for later consumption", inline=True)
comment(0x926E, "A=0: port &D1 (print); A!=0: port &90 (FS)", inline=True)
comment(0x9270, "Y=&90: FS data port", inline=True)
comment(0x9272, "A = selected port number", inline=True)
comment(0x9273, "Y=1: port byte offset in TXCB", inline=True)
comment(0x9275, "Write port to TXCB byte 1", inline=True)
comment(0x9277, "A = saved flag byte (expected sequence)", inline=True)
comment(0x9279, "Push expected sequence for retry loop", inline=True)
comment(0x927A, "Flag byte &7F = waiting for reply", inline=True)
comment(0x927C, "Write to TXCB flag byte (Y=0)", inline=True)
comment(0x927E, "Transmit and wait for reply (BRIANX)", inline=True)
comment(0x9281, "Recover expected sequence", inline=True)
comment(0x9282, "Keep on stack for next iteration", inline=True)
comment(0x9283, "Check if TX result matches expected sequence", inline=True)
comment(0x9285, "Bit 0 to carry (sequence mismatch?)", inline=True)
comment(0x9286, "C=1: mismatch, retry transmit", inline=True)
comment(0x9288, "Clean up: discard expected sequence", inline=True)
comment(0x9289, "Discard saved handle bitmask", inline=True)
comment(0x928E, "Toggle sequence bit on success", inline=True)
comment(0x9291, "Return", inline=True)
comment(0x9292, "Save current table index", inline=True)
comment(0x9294, "Push for later restore", inline=True)
comment(0x9295, "Point workspace to palette save area (&E9)", inline=True)
comment(0x9297, "Set workspace low byte", inline=True)
comment(0x9299, "Y=0: first palette entry", inline=True)
comment(0x929B, "Clear table index counter", inline=True)
comment(0x929D, "Save current screen MODE to workspace", inline=True)
comment(0x92A0, "Store MODE at workspace[0]", inline=True)
comment(0x92A2, "Advance workspace pointer past MODE byte", inline=True)
comment(0x92A4, "Read colour count (from &0351)", inline=True)
comment(0x92A7, "Push for iteration count tracking", inline=True)
comment(0x92A8, "A=0: logical colour number for OSWORD", inline=True)
label(0x92A9, "save_palette_entry")    # Per-entry OSWORD &0B palette read and workspace store
comment(0x92A9, "Store logical colour at workspace[0]", inline=True)
comment(0x92AB, "X = workspace ptr low (param block addr)", inline=True)
comment(0x92AD, "Y = workspace ptr high", inline=True)
comment(0x92AF, "OSWORD &0B: read palette for logical colour", inline=True)
comment(0x92B4, "Recover colour count", inline=True)
comment(0x92B5, "Y=0: access workspace[0]", inline=True)
comment(0x92B7, "Write colour count back to workspace[0]", inline=True)
comment(0x92B9, "Y=1: access workspace[1] (palette result)", inline=True)
comment(0x92BA, "Read palette value returned by OSWORD", inline=True)
comment(0x92BC, "Push palette value for next iteration", inline=True)
comment(0x92BD, "X = current workspace ptr low", inline=True)
comment(0x92BF, "Advance workspace pointer", inline=True)
comment(0x92C1, "Increment table index", inline=True)
comment(0x92C3, "Y=0 for next store", inline=True)
comment(0x92C4, "Load table index as logical colour", inline=True)
comment(0x92C6, "Loop until workspace wraps past &F9", inline=True)
comment(0x92C8, "Continue for all 16 palette entries", inline=True)
comment(0x92CA, "Discard last palette value from stack", inline=True)
comment(0x92CB, "Reset table index to 0", inline=True)
comment(0x92CD, "Advance workspace past palette data", inline=True)
comment(0x92CF, "Save cursor pos and OSBYTE state values", inline=True)
comment(0x92D2, "Advance workspace past VDU state data", inline=True)
comment(0x92D4, "Recover saved table index", inline=True)
comment(0x92D5, "Restore table index", inline=True)
comment(0x92D7, "Restore LSTAT from saved OLDJSR value", inline=True)
comment(0x92DA, "Write to protection status", inline=True)
comment(0x92DD, "Return", inline=True)
comment(0x92DE, "Read cursor editing state", inline=True)
comment(0x92E1, "Store to workspace[Y]", inline=True)
comment(0x92E3, "Preserve in X for OSBYTE", inline=True)
comment(0x92E4, "OSBYTE &85: read cursor position", inline=True)
comment(0x92E7, "Advance workspace pointer", inline=True)
comment(0x92E9, "Y result from OSBYTE &85", inline=True)
comment(0x92EA, "Store Y pos to workspace (X=0)", inline=True)
comment(0x92EC, "Self-call trick: executes twice", inline=True)
comment(0x92EF, "X=0 for (zp,X) addressing", inline=True)
comment(0x92F1, "Index into OSBYTE number table", inline=True)
comment(0x92F3, "Next table entry next time", inline=True)
comment(0x92F5, "Advance workspace pointer", inline=True)
comment(0x92F7, "Read OSBYTE number from table", inline=True)
comment(0x92FA, "Y=&FF: read current value", inline=True)
comment(0x92FC, "Call OSBYTE", inline=True)
comment(0x92FF, "Result in X to A", inline=True)
comment(0x9300, "X=0 for indexed indirect store", inline=True)
comment(0x9302, "Store result to workspace", inline=True)
label(0x9308, "reloc_zp_src")        # ROM source of zero-page relocated code
label(0x934D, "reloc_p4_src")        # ROM source of Tube host page 4 code
...
comment(0x970E, "Install next NMI handler at &96DC (RX scout net byte)", inline=True)
...
comment(0x9730, "Install scout data reading loop at &970E", inline=True)
comment(0x973C, "Neither set -- clean end, discard via &99E8", inline=True)
comment(0x974C, "No RDA -- error handler &96FE", inline=True)
...
comment(0x9832, "Install nmi_data_rx at &97E6", inline=True)
...
comment(0x9848, "Install net check handler at &97FA", inline=True)
...
comment(0x9859, "Install skip handler at &9810", inline=True)
comment(0x985B, "High byte of &9810 handler", inline=True)
...
comment(0x9883, "Tube: install Tube RX at &98A0", inline=True)
comment(0x9885, "High byte of &98A0 handler", inline=True)
...
comment(0x98CE, "CR2=&84: disable PSE for individual bit testing", inline=True)
comment(0x98D0, "Write CR2: disable PSE for bit testing", inline=True)
comment(0x98D3, "CR1=&00: disable all interrupts", inline=True)
comment(0x98D5, "Write CR1: disable all interrupts", inline=True)
...
comment(0x9970, "Save &9995 (post-ACK port check) in &0D4B/&0D4C", inline=True)
...
comment(0x9B47, "SR mode 4: shift out under CB1 control", inline=True)
...
comment(0x9CAA, "Install NMI handler at &9CCC (nmi_tx_data)", inline=True)
...
comment(0x9D8D, "Install NMI handler at &9D14 (nmi_tx_complete)", inline=True)
...
comment(0x9DAB, "Install nmi_reply_scout at &9D30", inline=True)
comment(0x9DB9, "Read first RX byte (destination station)", inline=True)
comment(0x9DC1, "Install nmi_reply_cont at &9D44", inline=True)
comment(0x9DD2, "Install nmi_reply_validate at &9D5B", inline=True)
comment(0x9DD9, "IRQ set -- fall through to &9D5B without RTI", inline=True)
...
comment(0x9E09, "Save handshake_await_ack (&9E50) in &0D4B/&0D4C", inline=True)
...
comment(0x9E24, "Install nmi_scout_ack_src at &9DA3", inline=True)
comment(0x9E26, "High byte &9D of handler address", inline=True)
comment(0x9E28, "Set NMI vector and return", inline=True)
comment(0x9E2B, "Read our station ID (also INTOFF)", inline=True)
...
comment(0x9E42, "Install nmi_data_tx at &9DC8", inline=True)
...
comment(0x9E49, "Install nmi_imm_data at &9E0F", inline=True)
...
comment(0x9E87, "Install discard_reset_listen at &99DB", inline=True)
comment(0x9E89, "High byte of &99DB handler", inline=True)
...
comment(0x9EE2, "Install nmi_final_ack at &9E5C", inline=True)
comment(0x9EE4, "High byte of handler address", inline=True)
comment(0x9EE6, "Install and return via set_nmi_vector", inline=True)
comment(0x9EF8, "Install nmi_final_ack_net at &9E70", inline=True)
comment(0x9F09, "Install nmi_final_ack_validate at &9E84", inline=True)
comment(0x9F10, "IRQ set -- fall through to &9E84 without RTI", inline=True)
...
comment(0x9F66, "Tube transfer in progress?", inline=True)
...
comment(0x9FBC, "Load RXCB[8] (start ptr lo)", inline=True)
...
# ============================================================
# Annotations back-propagated from NFS 3.35D
# ============================================================
...
comment(0x045B, "Discard return address (high byte)", inline=True)
comment(0x045C, "Discard return address (low byte)", inline=True)
...
comment(0x808F, "FSCV function >= 8?", inline=True)
...
comment(0x81BC, "Print ROM identification string", inline=True)
comment(0x822F, "A=&8F: issue service request", inline=True)
comment(0x8231, "X=&0F: 'vectors claimed' service", inline=True)
comment(0x8236, "X=&0A: service &0A", inline=True)
comment(0x823D, "Non-zero: skip auto-boot", inline=True)
comment(0x8241, "Y=&82: ROM page high byte", inline=True)
comment(0x8243, "Execute command string at (X, Y)", inline=True)
comment(0x8300, "Load FS state byte at offset Y", inline=True)
comment(0x8303, "Store to workspace backup area", inline=True)
comment(0x8305, "Next byte down", inline=True)
comment(0x8308, "Loop for offsets &1D..&15", inline=True)
comment(0x830A, "A=&7B: printer driver going dormant", inline=True)
comment(0x831C, "Return after port setup", inline=True)
comment(0x8342, "A=&2A: error ptr for retry", inline=True)
comment(0x834A, "A=&77: OSBYTE close spool/exec", inline=True)
comment(0x8358, "A=&2A: error ptr for retry", inline=True)
label(0x835A, "store_fs_hdr_clc")       # CLC entry: clear carry then store function code
comment(0x835A, "CLC: no byte-stream path", inline=True)
label(0x835B, "store_fs_hdr_fn")       # Store function code and CSD/LIB handles
comment(0x835B, "Store function code at &0F01", inline=True)
comment(0x835E, "Store error ptr for TX poll", inline=True)
comment(0x8385, "Load error ptr for TX retry", inline=True)
comment(0x83A3, "CLC for address addition", inline=True)
comment(0x8420, "Transfer A to Y for indexing", inline=True)
comment(0x8422, "Transfer to X for return", inline=True)
comment(0x847B, "A=&7E: OSBYTE acknowledge escape", inline=True)
subroutine(0x847D, "check_escape_handler", hook=None,
...
comment(0x847D, "Test escape flag (bit 7)", inline=True)
comment(0x847F, "Bit 7 clear: no escape, return", inline=True)
comment(0x8481, "Acknowledge escape via OSBYTE &7E", inline=True)
comment(0x8484, "Non-zero: report 'Not listening'", inline=True)
comment(0x84AF, "Return with handle mask in A", inline=True)
comment(0x8509, "A = function code / command", inline=True)
comment(0x850B, "X = control block ptr lo", inline=True)
comment(0x850D, "Y = control block ptr hi", inline=True)
comment(0x850F, "X dup for indexed access via (fs_crc)", inline=True)
comment(0x8511, "Y dup for indexed access", inline=True)
comment(0x8513, "Return", inline=True)
comment(0x853D, "Store return addr low as string ptr", inline=True)
comment(0x8540, "Store return addr high as string ptr", inline=True)
comment(0x8542, "Y=0: offset for indirect load", inline=True)
comment(0x8546, "No page wrap: skip high byte inc", inline=True)
comment(0x8548, "Handle page crossing in pointer", inline=True)
comment(0x8564, "Clear accumulator workspace", inline=True)
label(0x8566, "scan_decimal_digit")    # Read ASCII digits and accumulate decimal value
comment(0x8568, "Dot separator?", inline=True)
comment(0x8588, "Return with result in A", inline=True)
comment(0x85A3, "Transfer mask to X for return", inline=True)
comment(0x85A5, "Return with mask in X", inline=True)
comment(0x85D7, "Next byte", inline=True)
comment(0x85DA, "Return with Z flag result", inline=True)
comment(0x85DF, "Return (FSCV 7 read handles)", inline=True)
comment(0x8645, "X=&C0: TX control block at &00C0", inline=True)
comment(0x8647, "Set TX pointer lo", inline=True)
comment(0x8649, "X=0: page zero", inline=True)
comment(0x864B, "Set TX pointer hi", inline=True)
comment(0x864D, "A=&FF: full retry count", inline=True)
comment(0x8651, "Save retry count on stack", inline=True)
comment(0x8652, "Transfer timeout to A", inline=True)
comment(0x8653, "Save timeout on stack", inline=True)
comment(0x8654, "X=0 for (net_tx_ptr,X) indirect", inline=True)
comment(0x8656, "Load TXCB byte 0 (control/status)", inline=True)
label(0x8658, "tx_retry")             # Retry TX after error
comment(0x8658, "Write control byte to start TX", inline=True)
comment(0x865A, "Save control byte for retry", inline=True)
label(0x865B, "tx_semaphore_spin")    # TX semaphore spinlock
comment(0x865B, "Test TX semaphore (C=1 when free)", inline=True)
comment(0x865E, "Spin until semaphore released", inline=True)
comment(0x8660, "Copy TX ptr lo to NMI block", inline=True)
comment(0x8662, "Store for NMI handler access", inline=True)
comment(0x8664, "Copy TX ptr hi to NMI block", inline=True)
comment(0x8666, "Store for NMI handler access", inline=True)
comment(0x8668, "Initiate ADLC TX via trampoline", inline=True)
comment(0x866B, "Poll TXCB byte 0 for completion", inline=True)
comment(0x866D, "Bit 7 set: still busy, keep polling", inline=True)
comment(0x866F, "Shift bit 6 into bit 7 (error flag)", inline=True)
comment(0x8670, "Bit 6 clear: success, clean return", inline=True)
comment(0x8672, "Shift bit 5 into carry", inline=True)
comment(0x8673, "Zero: fatal error, no escape", inline=True)
comment(0x8675, "Check for user escape condition", inline=True)
comment(0x8678, "Discard saved control byte", inline=True)
comment(0x8679, "Save to X for retry delay", inline=True)
comment(0x867A, "Restore timeout parameter", inline=True)
comment(0x867B, "Back to Y", inline=True)
comment(0x867C, "Restore retry count", inline=True)
comment(0x867D, "No retries left: report error", inline=True)
comment(0x867F, "Decrement retry count", inline=True)
comment(0x8681, "Save updated retry count", inline=True)
comment(0x8682, "Timeout to A for delay", inline=True)
comment(0x8683, "Save timeout parameter", inline=True)
comment(0x8684, "Control byte for delay duration", inline=True)
comment(0x8685, "Inner delay loop", inline=True)
comment(0x8686, "Spin until X=0", inline=True)
comment(0x8688, "Outer delay loop", inline=True)
comment(0x8689, "Continue delay", inline=True)
label(0x868D, "tx_not_listening")     # Fatal TX error: "Not listening"
comment(0x868D, "Save error code in X", inline=True)
comment(0x868E, "Report 'Not listening' error", inline=True)
label(0x8691, "tx_success")           # TX success: clean return
comment(0x8691, "Discard saved control byte", inline=True)
comment(0x8692, "Discard timeout parameter", inline=True)
comment(0x8693, "Discard retry count", inline=True)
comment(0x8694, "Return (success)", inline=True)
subroutine(0x8698, "copy_filename_ptr", hook=None,
...
comment(0x86C7, "A=&FF: branch to load path", inline=True)
comment(0x86CC, "Copy parsed filename to cmd buffer", inline=True)
comment(0x86CF, "Y=2: FS function code offset", inline=True)
comment(0x86D6, "A=&2A: error ptr for retry", inline=True)
comment(0x8704, "Display file info after FS reply", inline=True)
comment(0x877F, "A=&14: FS function code for SAVE", inline=True)
comment(0x8781, "Build header and send FS save command", inline=True)
comment(0x8784, "Send file data blocks to server", inline=True)
label(0x8787, "save_csd_display")       # Save CSD from reply and display file info
comment(0x8787, "Save CSD from reply for catalogue display", inline=True)
comment(0x878A, "Print file length in hex", inline=True)
label(0x8791, "send_fs_reply")        # Sends FS reply command after transfer
comment(0x8791, "Send FS reply acknowledgement", inline=True)
comment(0x879F, "Z=1: first byte, use A directly", inline=True)
label(0x87A1, "copy_attr_loop")       # Copy 4 attribute bytes loop
comment(0x87A1, "Load attribute byte from FS reply", inline=True)
label(0x87A4, "direct_attr_copy")     # Skip attr decode, direct copy
comment(0x87A4, "Store decoded access in param block", inline=True)
comment(0x87A6, "Next attribute byte", inline=True)
comment(0x87C3, "Next byte (descending)", inline=True)
comment(0x87C6, "Loop until offset 2 reached", inline=True)
comment(0x87C8, "Return to caller", inline=True)
label(0x87CF, "next_block")           # Reinit for next transfer block
comment(0x87CF, "X=0: clear hi bytes of block size", inline=True)
comment(0x87D1, "Y=4: process 4 address bytes", inline=True)
comment(0x87D3, "Clear block size hi byte 1", inline=True)
comment(0x87D6, "Clear block size hi byte 2", inline=True)
comment(0x87D9, "CLC for ADC in loop", inline=True)
label(0x87DA, "block_addr_loop")      # Multi-byte address addition loop
comment(0x87DA, "Source = current position", inline=True)
comment(0x87DC, "Store source address byte", inline=True)
comment(0x87DE, "Add block size to current position", inline=True)
comment(0x87E9, "Carry: address overflowed, clamp", inline=True)
label(0x87F8, "clamp_dest_setup")     # Set up 4-byte dest clamping
comment(0x8819, "A=&2A: error ptr for retry", inline=True)
subroutine(0x8946, "return_a_zero", hook=None,
...
comment(0x89EA, "Return (unsupported function)", inline=True)
comment(0x8A6F, "Non-zero: branch past error ptr", inline=True)
label(0x8A71, "gbpb_read_path")      # BNE: non-zero read, skip write transfer
comment(0x8A71, "Read path: receive data blocks from FS", inline=True)
comment(0x8CA3, "Print two CRs (blank line)", inline=True)
label(0x8CBD, "cat_examine_loop")      # Send examine request and display entries
comment(0x8CCC, "Load entry count from reply", inline=True)
comment(0x8CCF, "Zero entries returned: catalogue done", inline=True)
comment(0x8CD1, "X=2: first entry offset in reply", inline=True)
comment(0x8CD3, "Print/format this directory entry", inline=True)
comment(0x8CD6, "CLC for addition", inline=True)
comment(0x8CD7, "Load current examine start offset", inline=True)
comment(0x8CD9, "Add entries returned this batch", inline=True)
comment(0x8CDC, "Update next examine start offset", inline=True)
comment(0x8CDF, "Save updated start offset", inline=True)
comment(0x8CE1, "Reload batch size for next request", inline=True)
comment(0x8CE3, "Store batch size in command buffer", inline=True)
comment(0x8CE6, "Loop for remaining characters", inline=True)
comment(0x8DC9, "Return after calculation", inline=True)
comment(0x8DE7, "Preserve carry via ROL", inline=True)
comment(0x8DEA, "A=&3F: close marker byte", inline=True)
comment(0x8DEA, "&3F = '?' marks slot as unused", inline=True)
comment(0x8DEA, "A=&3F: handle closed/unused marker", inline=True)
comment(0x8DEC, "Mark handle as closed in workspace", inline=True)
comment(0x8DEC, "Write close marker to workspace slot", inline=True)
comment(0x8DEC, "Write marker to handle slot", inline=True)
label(0x8DEE, "restore_rx_flags")
subroutine(0x8DEE, hook=None,
...
comment(0x8DEE, "Restore carry via ROR", inline=True)
comment(0x8DFC, "Outside our OSWORD range, exit", inline=True)
comment(0x8E02, "X = sub-function code for table lookup", inline=True)
label(0x8E2B, "load_workspace_byte")   # Load byte from workspace (C=0 skip-copy path)
comment(0x8E2D, "Store to param block (no-op if C=1)", inline=True)
comment(0x8E33, "Return after copy", inline=True)
comment(0x8E34, "ASL: set C if TX in progress", inline=True)
comment(0x8E37, "C=0: read path", inline=True)
comment(0x8EFC, "Load from ROM template (zero = use NMI workspace value)", inline=True)
comment(0x8F27, "Enable interrupts before transmit", inline=True)
comment(0x8F2D, "Dest station = &FFFF (accept reply from any station)", inline=True)
comment(0x8F3F, "Initiate receive with timeout", inline=True)
...
comment(0x8FC7, "ROR/ASL on stacked P: zeros carry to signal success", inline=True)
...
comment(0x8FE3, "Set up TX control block", inline=True)
comment(0x8FE6, "Enable interrupts for TX", inline=True)
comment(0x8FE7, "Enable IRQs and transmit", inline=True)
label(0x8FEA, "delay_between_tx")      # Spin-delay between consecutive TX packets
comment(0x8FEA, "Short delay loop between TX packets", inline=True)
comment(0x8FEB, "Spin until X reaches 0", inline=True)
comment(0x8FED, "Restore data byte for terminator check", inline=True)
comment(0x8FEE, "Z=1: not intercepted, pass through", inline=True)
subroutine(0x9001, "enable_irq_and_tx", hook=None,
...
comment(0x909B, "Write &7F to RXCB (wait for reply)", inline=True)
label(0x909D, "poll_rxcb_loop")       # Poll RXCB for OSBYTE reply
comment(0x909D, "Poll RXCB for completion (bit7)", inline=True)
comment(0x90F4, "Load template byte from ctrl_block_template[X]", inline=True)
comment(0x9177, "V=1: use (net_rx_ptr) page", inline=True)
comment(0x9179, "V=1: skip to net_rx_ptr page", inline=True)
comment(0x917B, "V=0: use (nfs_workspace) page", inline=True)
label(0x917D, "rxcb_matched")         # Scout frame matched RXCB
comment(0x9189, "Loop until all template bytes done", inline=True)
comment(0x918C, "Store final offset as net_tx_ptr", inline=True)
comment(0x91C4, "Store initial PFLAGS value", inline=True)
comment(0x9230, "Load current PFLAGS", inline=True)
comment(0x9233, "Save current PFLAGS", inline=True)
comment(0x9234, "Carry = current sequence (bit 7)", inline=True)
comment(0x9235, "Restore original PFLAGS", inline=True)
comment(0x9236, "Toggle sequence number (bit 7 of PFLAGS)", inline=True)
comment(0x9238, "Store toggled sequence number", inline=True)
comment(0x928A, "Transfer count to X", inline=True)
comment(0x928B, "Test for retry exhaustion", inline=True)
comment(0x928C, "X wrapped to 0: retries exhausted", inline=True)
comment(0x9304, "Return after storing result", inline=True)
...
comment(0x9699, "BIT INTON: re-enable NMIs", inline=True)
...
comment(0x96E1, "CR4=&1E: 8-bit word, abort ext, NRZ", inline=True)
...
comment(0x970B, "Clear TX flags for new reception", inline=True)
...
comment(0x9A4F, "Store to port buffer", inline=True)
comment(0x9A51, "Next field", inline=True)
comment(0x9A51, "Advance buffer pointer", inline=True)
comment(0x9A52, "All 8 fields copied?", inline=True)
comment(0x9A54, "No: continue copy loop", inline=True)
comment(0x9A54, "No page crossing", inline=True)
comment(0x9A56, "Skip buffer pointer update", inline=True)
...
comment(0x9A86, "Push handler address high", inline=True)
comment(0x9A87, "Load handler low byte from jump table", inline=True)
comment(0x9A87, "Look up handler address low byte", inline=True)
comment(0x9A8A, "Push handler low byte", inline=True)
comment(0x9A8A, "Push handler address low", inline=True)
comment(0x9A8B, "RTS dispatches to handler", inline=True)
comment(0x9A8B, "RTS dispatch to handler", inline=True)
...
comment(0x9C13, "(Y -= 4: reach start addr offset)", inline=True)
...
comment(0x9F7A, "Y += 4: advance to high ptr offset", inline=True)
comment(0x9F7B, "(continued)", inline=True)
comment(0x9F7C, "(continued)", inline=True)
comment(0x9F84, "Y -= 3: back to next low ptr byte", inline=True)
...
# ============================================================
# 3.34B-specific annotations (gap-filling after backfill)
# ============================================================

# --- Inline comments for under-commented subroutines ---

# tube_code_page4: Tube data transfer setup
...
# init_vectors_and_copy: install relocated vectors
...
# svc_1_abs_workspace: claim workspace pages
comment(0x8270, "Compare Y against workspace boundary", inline=True)
comment(0x8272, "Y>=&10: already allocated, return", inline=True)
comment(0x8274, "Claim workspace up to page &10", inline=True)
comment(0x8276, "Return to caller", inline=True)

# store_fs_error: scan for CR and build BRK error
comment(0x8410, "Advance to next reply buffer byte", inline=True)
comment(0x8411, "A=CR: terminator to search for", inline=True)
comment(0x8413, "XOR with buffer byte (0 when CR)", inline=True)
comment(0x8415, "Not CR: continue scanning", inline=True)
comment(0x8417, "Store 0 (from XOR) to replace CR", inline=True)
comment(0x8419, "Execute error via JMP indirect", inline=True)
comment(0x843E, "Advance output buffer position", inline=True)
comment(0x843F, "Advance source string pointer", inline=True)
comment(0x8440, "Continue copying message bytes", inline=True)
comment(0x8442, "Execute constructed BRK error", inline=True)

# print_inline
comment(0x8551, "Continue printing loop", inline=True)

# skip_spaces: scan past space characters
comment(0x8556, "Load character from parameter string", inline=True)
comment(0x8558, "Advance to next character", inline=True)
comment(0x8559, "Compare against space (ASCII &20)", inline=True)
comment(0x855B, "Space found: keep scanning", inline=True)
comment(0x855D, "Back up one (first non-space char)", inline=True)
comment(0x855E, "Compare against 'A' for case flag", inline=True)
comment(0x8560, "Return: A=char, C set if >= 'A'", inline=True)

# mask_to_handle
comment(0x85AF, "Return with A=handle number", inline=True)

# print_decimal_digit: repeated subtraction
comment(0x85BD, "Store divisor in temporary", inline=True)
comment(0x85BF, "Transfer dividend (Y) to A", inline=True)
comment(0x85C0, "X=&2F: ASCII '0'-1 (loop init)", inline=True)
comment(0x85C2, "Set carry for subtraction", inline=True)
comment(0x85C3, "Increment digit (ASCII '0'..'9')", inline=True)
comment(0x85C4, "Subtract divisor from remainder", inline=True)
comment(0x85C6, "Carry set: subtract again", inline=True)
comment(0x85C8, "Add back divisor (undo last SBC)", inline=True)
comment(0x85CA, "Remainder to Y for next digit", inline=True)
comment(0x85CB, "Quotient digit (X) to A for print", inline=True)

# clear_fs_flag: clear bits in EOF flags
comment(0x85E0, "Invert A (NOT mask)", inline=True)
comment(0x85E2, "AND inverted mask to clear bits", inline=True)

# set_fs_flag: set bits in EOF flags
comment(0x85E5, "OR mask into EOF flags", inline=True)
comment(0x85E8, "Store updated EOF flags", inline=True)
comment(0x85EB, "Return to caller", inline=True)

# print_hex: print A as two hex digits
comment(0x85EC, "Save full byte on stack", inline=True)
comment(0x85ED, "Shift high nibble to low position", inline=True)
comment(0x85EE, "Continue shift (4 LSRs total)", inline=True)
comment(0x85EF, "Continue shift", inline=True)
comment(0x85F0, "High nibble now in bits 0-3", inline=True)
comment(0x85F1, "Print high nibble as hex", inline=True)
comment(0x85F4, "Restore original byte", inline=True)
comment(0x85F5, "Mask to low nibble", inline=True)
comment(0x85F7, "Convert to ASCII digit ('0'-'9')", inline=True)
comment(0x85F9, "Compare against ':' (past '9'?)", inline=True)
comment(0x85FB, "Digit 0-9: skip A-F adjustment", inline=True)
comment(0x85FD, "Add 7 to get ASCII 'A'-'F'", inline=True)
comment(0x85FF, "ALWAYS branch to print character", inline=True)

# restore_args_return: dispatch FS number
comment(0x8934, "Transfer A to Y for test", inline=True)
comment(0x8935, "Non-zero: halve A", inline=True)
comment(0x8937, "A=5: default FS number", inline=True)

# fs_cmd_match_table
comment(0x8BD7, "XOR with '.' (abbreviation check)", inline=True)

# ex_handler: set up extended catalogue display
comment(0x8BF3, "Pre-decrement Y for parameter", inline=True)
comment(0x8BF4, "X=1: boot option display field", inline=True)
comment(0x8BF6, "Store to fs_work_7 (&B7)", inline=True)
comment(0x8BF8, "X=&50: 80-column display width", inline=True)
comment(0x8BFA, "Store column width at &B6", inline=True)

# fsreply_5_set_lib: store library handle
comment(0x8CF8, "Store Y (library handle) to &0E04", inline=True)
comment(0x8CFB, "Non-zero: continue to set handle", inline=True)

# option_name_offsets
comment(0x8D4C, "Offset 0 (BRK opcode as zero byte)", inline=True)

# print_reply_bytes: print counted bytes from reply
comment(0x8D50, "Y=10: default character count", inline=True)
comment(0x8D52, "Load character from reply buffer", inline=True)
comment(0x8D58, "Advance to next character", inline=True)
comment(0x8D59, "Decrement remaining count", inline=True)
comment(0x8D5A, "Loop until count exhausted", inline=True)
comment(0x8D5C, "Return to caller", inline=True)

# print_spaces: print X space characters
comment(0x8D5D, "Print one space character", inline=True)
comment(0x8D60, "Decrement space count", inline=True)
comment(0x8D61, "Loop until all spaces printed", inline=True)
comment(0x8D63, "Return to caller", inline=True)

# copy_filename: initialise and copy
comment(0x8D64, "X=0: start of output buffer", inline=True)

# copy_string_to_cmd
comment(0x8D73, "Return to caller", inline=True)

# fsreply_0_print_dir: print directory path
comment(0x8D74, "X=0: start of reply buffer", inline=True)
comment(0x8D76, "Load character from reply", inline=True)
comment(0x8D79, "Bit 7 set: end of string", inline=True)
comment(0x8D7B, "Non-zero: printable character", inline=True)
comment(0x8D7D, "Replace null with CR", inline=True)
comment(0x8D82, "Advance to next character", inline=True)
comment(0x8D83, "Continue printing directory path", inline=True)

# net_1_read_handle: read handle from RX buffer
comment(0x8DB0, "Y=&6F: handle offset in RX buffer", inline=True)
comment(0x8DB2, "Load handle byte from RX data", inline=True)
comment(0x8DB4, "Store handle to &F0", inline=True)
comment(0x8DB6, "Branch to cleanup path", inline=True)

# restore_rx_flags
comment(0x8DF1, "C=0: branch to cleanup exit", inline=True)

# fs_osword_dispatch
comment(0x8E18, "RTS dispatches to pushed handler", inline=True)

# net_write_char
comment(0x903E, "Get stack pointer for flag access", inline=True)

# lang_3_execute_at_0100: zero &0100-&0102 and JMP
comment(0x912B, "X=2: zero 3 bytes (offsets 2,1,0)", inline=True)
comment(0x912D, "A=0: zero / BRK opcode", inline=True)
comment(0x912F, "Store zero at &0100+X", inline=True)
comment(0x9132, "Decrement byte counter", inline=True)
comment(0x9133, "Loop until 3 bytes zeroed", inline=True)
comment(0x9135, "Release JSR protection mask", inline=True)
comment(0x9138, "Execute downloaded code at &0100", inline=True)

# lang_4_remote_validated: validate source station
comment(0x913B, "Y=4: validation byte offset", inline=True)
comment(0x913D, "Load validation byte from RX data", inline=True)
comment(0x913F, "Zero: validation passed, continue", inline=True)
comment(0x9141, "Y=&80: source station offset", inline=True)
comment(0x9143, "Load source station from RX buffer", inline=True)
comment(0x9145, "Y=&0E: controlling station offset", inline=True)
comment(0x9147, "Compare with controlling station", inline=True)
comment(0x9149, "Mismatch: reject remote command", inline=True)

# lang_0_insert_remote_key: insert keypress
comment(0x914B, "Y=&82: character offset in RX data", inline=True)
comment(0x914D, "Load remote keypress character", inline=True)
comment(0x914F, "Transfer character to Y", inline=True)
comment(0x9150, "X=0: keyboard input buffer", inline=True)
comment(0x9152, "Release JSR protection before call", inline=True)
comment(0x9155, "A=&99: OSBYTE insert into buffer", inline=True)

# save_econet_state / restore_econet_state
comment(0x969D, "Disable interrupts for save", inline=True)
comment(0x969F, "Save A on stack", inline=True)
comment(0x96A0, "A=0: prepare to test ADLC state", inline=True)
comment(0x96A2, "Load current ADLC CR1/SR1", inline=True)
comment(0x96A5, "Push current ADLC state", inline=True)
comment(0x96A6, "Read NMI handler low byte", inline=True)
comment(0x96A8, "Save NMI low on stack", inline=True)
comment(0x96A9, "Read NMI handler high byte", inline=True)
comment(0x96AB, "Save NMI high on stack", inline=True)
comment(0x96AC, "Write &00 to CR1 (disable ADLC)", inline=True)
comment(0x96AF, "Transfer saved state to processor", inline=True)

# install_nmi_shim: copy NMI shim from ROM to RAM
...
comment(0x96D1, "Load NMI shim byte from ROM", inline=True)
...
# print_decimal
comment(0x85B0, "Y=dividend, A=100: hundreds digit", inline=True)
...
comment(0x85BA, "Transfer remainder to A", inline=True)
comment(0x85BB, "Convert to ASCII and print", inline=True)

# print_file_info: print catalogue line
comment(0x8601, "X=0: start of filename buffer", inline=True)
comment(0x8603, "Load character from filename", inline=True)
comment(0x8606, "CR: end of filename, print access", inline=True)
comment(0x8608, "Print filename character", inline=True)
comment(0x860B, "Advance to next filename byte", inline=True)
comment(0x860C, "Loop printing filename chars", inline=True)
comment(0x8610, "Print access attribute string", inline=True)
comment(0x8613, "Print space separator", inline=True)
comment(0x8616, "Load sequence number from reply", inline=True)
comment(0x8619, "Print sequence as decimal", inline=True)
comment(0x861C, "Print space separator", inline=True)
comment(0x861F, "Load load address low byte", inline=True)
comment(0x8622, "Print load address as 2 hex bytes", inline=True)
comment(0x8627, "Print exec address as 2 hex bytes", inline=True)
comment(0x862C, "Print file length as 2 hex bytes", inline=True)
comment(0x862F, "Y=&0A: end marker offset", inline=True)
comment(0x8631, "Load end marker byte", inline=True)
comment(0x8634, "Zero: no creation date", inline=True)
comment(0x8636, "Print space separator", inline=True)
comment(0x8639, "Print date byte as decimal", inline=True)
comment(0x863C, "Print separator", inline=True)
comment(0x863F, "Print date byte as decimal", inline=True)
comment(0x8642, "Print separator", inline=True)
comment(0x8645, "Load year high byte", inline=True)
comment(0x8648, "Print year high byte as decimal", inline=True)
comment(0x864B, "Print CR to end catalogue line", inline=True)

# filev_handler
comment(0x8695, "Load FILEV function code from A", inline=True)

# fsreply_3_set_csd: set CSD handle
comment(0x8CFD, "Store Y (CSD handle) to &0E03", inline=True)
comment(0x8D00, "Jump to restore_args_return", inline=True)

# lang_1_remote_boot: handle remote boot
comment(0x90FD, "Y=&80: RX data buffer offset", inline=True)
comment(0x90FF, "Load first data byte from RX", inline=True)
comment(0x9101, "Zero: standard boot, skip code", inline=True)
comment(0x9103, "Load language ROM number", inline=True)
comment(0x9105, "Y=&7F: workspace offset for boot", inline=True)
comment(0x9107, "Store ROM number for boot handler", inline=True)
comment(0x9109, "Load boot option byte", inline=True)
comment(0x910B, "Store boot flag for later dispatch", inline=True)
comment(0x910D, "Decrement to get boot type 0-3", inline=True)
comment(0x910F, "Zero: standard boot sequence", inline=True)
comment(0x9111, "Non-zero: remote boot handler", inline=True)
comment(0x9113, "Y=&82: filename offset in RX data", inline=True)
comment(0x9115, "Release JSR protection mask", inline=True)
comment(0x9118, "A=2: OSBYTE function code", inline=True)
comment(0x911A, "Copy command to &0100 area", inline=True)
comment(0x911D, "Point to &0100 command string", inline=True)
comment(0x911F, "Y=0: command string high byte", inline=True)
comment(0x9121, "A=&04: issue *RUN command", inline=True)
comment(0x9123, "Process remote boot command", inline=True)

# store_output_byte
comment(0x91ED, "Check if Tube transfer active", inline=True)
comment(0x91EF, "Store byte to local buffer at &AE", inline=True)
comment(0x91F1, "Increment buffer pointer low byte", inline=True)
comment(0x91F3, "No carry: skip high byte increment", inline=True)
comment(0x91F5, "Increment buffer pointer high byte", inline=True)
comment(0x91F7, "Decrement transfer count low byte", inline=True)
comment(0x91F9, "Borrow: decrement high byte", inline=True)
comment(0x91FB, "Decrement transfer count high byte", inline=True)
comment(0x91FD, "Check if both bytes zero", inline=True)
comment(0x91FF, "Not done: C=0 for more", inline=True)
comment(0x9201, "Count exhausted: SEC for done", inline=True)
comment(0x9202, "Return with C=1 (done) or C=0", inline=True)
comment(0x9203, "Tube active: send via R3", inline=True)
comment(0x9206, "Decrement Tube count low", inline=True)
comment(0x9208, "No borrow: skip high decrement", inline=True)
comment(0x920A, "Decrement Tube count high", inline=True)

# print_station_info
comment(0x81F9, "Load local station number", inline=True)
comment(0x81FC, "Print station number as decimal", inline=True)
comment(0x81FF, "A=&20: test bit 5 of SR2 (clock)", inline=True)
comment(0x8201, "Test ADLC SR2 for network clock", inline=True)
comment(0x8204, "Clock present: skip warning msg", inline=True)

# restore_econet_state
comment(0x96B4, "Disable interrupts for restore", inline=True)
comment(0x96B6, "Restore NMI handler high byte", inline=True)
comment(0x96B8, "Write NMI high byte to &0D01", inline=True)
comment(0x96BB, "Restore NMI handler low byte", inline=True)
comment(0x96BD, "Write NMI low byte to &0D00", inline=True)
comment(0x96C0, "Restore saved ADLC CR1 state", inline=True)
comment(0x96C1, "Write CR1 to re-enable ADLC", inline=True)
comment(0x96C4, "Restore original A from stack", inline=True)

# check_escape_handler
comment(0x8497, "C=0: no escape, test for retry", inline=True)
comment(0x8499, "Test if retry needed", inline=True)
comment(0x849C, "Branch if flow control set", inline=True)
comment(0x849E, "Error code: TX failed", inline=True)
comment(0x84A0, "Jump to error handler", inline=True)
comment(0x84A1, "Shift error bits right", inline=True)
comment(0x84A3, "Non-zero: retry with escape check", inline=True)
comment(0x84A5, "Fatal TX error: cannot retry", inline=True)
comment(0x84A7, "Jump to error handler", inline=True)

# net_4_resume_remote
comment(0x8DF3, "Jump to clear_svc_restore_args", inline=True)
comment(0x8DF6, "Enable remote operations flag", inline=True)

# fsreply_4_notify_exec: handle FS exec notification
comment(0x8D85, "X=2: OSWORD parameter block size", inline=True)
comment(0x8D87, "Y=0: param block offset", inline=True)
comment(0x8D89, "A=&10: OSWORD &10 (open RXCB)", inline=True)
comment(0x8D8B, "Issue OSWORD &10 to open RXCB", inline=True)
comment(0x8D8E, "Load RXCB number from result", inline=True)
comment(0x8D90, "Zero: no free RXCB, skip", inline=True)
comment(0x8D92, "Store RXCB number in exec context", inline=True)
comment(0x8D94, "Y=&70: FS workspace offset", inline=True)
comment(0x8D96, "Load reply byte from workspace", inline=True)
comment(0x8D98, "Store to exec param area", inline=True)
comment(0x8D9A, "Advance to next param byte", inline=True)
comment(0x8D9B, "Compare against boundary", inline=True)
comment(0x8D9D, "Loop for all exec parameters", inline=True)
comment(0x8D9F, "Store final byte", inline=True)
comment(0x8DA1, "X=&16: OSBYTE param", inline=True)
comment(0x8DA3, "A=&C7: OSBYTE read/write flag", inline=True)
comment(0x8DA5, "Enable exec notification via OSBYTE", inline=True)
comment(0x8DA8, "Save original value on stack", inline=True)
comment(0x8DA9, "Decrement to update counter", inline=True)

# --- Subroutine promotions ---

label(0x8F49, "clear_svc_restore_args")
subroutine(0x8F49, hook=None,
...
# --- Simple subroutine label renames ---
label(0x9A15, "load_rxcb_buf_hi")
label(0x9B0D, "tx_nmi_dispatch_page")

# --- Shared tail label renames ---
label(0x81CA, "restore_y_return")
label(0x85CC, "print_via_osasci")
label(0x85FF, "print_hex_digit")
label(0x878D, "set_star_reply_port")
label(0x8934, "argsv_dispatch_a")
label(0x8C08, "cat_init_display")
label(0x8D00, "set_handle_return")
label(0x8DDB, "clear_svc_return")
label(0x8EAD, "set_carry_dispatch")
label(0x8EE2, "carry_exit_or_read")
label(0x9005, "jmp_clear_svc_restore")

# --- Internal loop label renames ---
label(0x85C3, "decimal_divide_loop")
label(0x8608, "print_filename_loop")
label(0x8C77, "print_option_char")
label(0x8CB0, "count_columns_loop")
label(0x8EB0, "read_local_station")
label(0x912F, "zero_0100_loop")
label(0x96D1, "poll_nmi_complete")

# --- Internal conditional label renames ---
label(0x0445, "tube_ctrl_write_2")
label(0x8410, "find_cr_terminator")
label(0x8442, "execute_brk_error")
label(0x849C, "tx_flow_control")
label(0x84A1, "tx_error_classify")
label(0x8553, "filev_attrib_code_check")
label(0x86B4, "tx_result_check")
label(0x88DF, "argsv_zero_length")
label(0x89AA, "gbpbv_func_dispatch")
label(0x8A74, "findv_eof_check")
label(0x8BCD, "cmd_match_retry")
label(0x8C4E, "cat_access_setup")
label(0x8CBB, "cat_examine_continue")
label(0x8D17, "fsreply_handle_copy")
label(0x8DAD, "net_handle_validate")
label(0x8E50, "osword_12_subfunc")
label(0x8EDC, "osword_12_error")
label(0x8EFF, "post_reply_check")
label(0x8FF4, "rx_first_packet")
label(0x920C, "rx_imm_discard")
label(0x920F, "rx_data_phase")
...
label(0x8445, "sp_dot_string")

# FS command match table
label(0x8BD8, "cmd_table_entry_1")

# OSWORD 12 workspace offset table
label(0x8E7A, "osword_12_ws_offsets")

# Mid-instruction operands (self-modifying code targets)
...
# Remaining auto-label rename
label(0x8EFE, "scan_or_read_rxcb")

# Additional inline comments for under-commented subs

# clear_svc_restore_args: shared OSWORD exit
comment(0x8F49, "Y=0: clear service claim", inline=True)
comment(0x8F4B, "Release ROM service number", inline=True)
comment(0x8F4F, "Load saved arg from (net_rx_ptr)+Y", inline=True)
comment(0x8F51, "Restore saved OSWORD argument byte", inline=True)
comment(0x8F54, "Decrement byte counter", inline=True)
comment(0x8F55, "Loop for bytes 2,1,0", inline=True)
comment(0x8F57, "Return to caller", inline=True)

# check_imm_op_ctrl: immediate operation control check
...
# Batch 2: subs below 70% (small subs first)

# svc_4_star_command: service 4 unrecognised * command
...
# svc_9_help: *HELP response
comment(0x81CA, "Restore Y from temporary", inline=True)

# enable_irq_and_tx
comment(0x9002, "Poll TX until complete", inline=True)

# i_am_handler: *I AM station parser
comment(0x8D07, "Skip spaces before station number", inline=True)
comment(0x8D0A, "C=1: alphabetic, forward to FS", inline=True)
comment(0x8D0C, "A=0: default network (local)", inline=True)
comment(0x8D11, "C=0: no dot, single number only", inline=True)

# adlc_init_workspace: set up NMI workspace
...
# net_2_read_handle_entry: read handle from workspace
comment(0x8DCA, "Load handle number from &F0", inline=True)
comment(0x8DDB, "A=0: clear service claim", inline=True)
comment(0x8DDD, "Release ROM service number", inline=True)
comment(0x8DDF, "Return to caller", inline=True)

# restore_econet_state: reload from RX control block
comment(0x96B9, "Load saved rx_status_flags", inline=True)
comment(0x96C5, "Load saved tx_in_progress", inline=True)
comment(0x96C7, "Restore TX state", inline=True)
comment(0x96CA, "Reinitialise NMI workspace", inline=True)

# net_3_close_handle
comment(0x8DE0, "Load handle number from &F0", inline=True)

# svc_2_private_workspace: workspace init
comment(0x8279, "Store RX buffer page pointer", inline=True)
comment(0x827B, "Advance to next page", inline=True)
comment(0x827C, "Store workspace page pointer", inline=True)
comment(0x82A1, "Y=2: printer server offset", inline=True)
comment(0x82A3, "A=&EB: default printer server", inline=True)
comment(0x82A5, "Store printer server at offset 2", inline=True)
comment(0x82A8, "A=0: clear remaining fields", inline=True)
comment(0x82AA, "Clear FS server network number", inline=True)
comment(0x82AD, "Clear workspace byte at offset 3", inline=True)
comment(0x82AF, "Clear protection status mask", inline=True)
comment(0x82B2, "Clear FS messages flag", inline=True)
comment(0x82C7, "Store station ID for TX scout", inline=True)
...
comment(0x91B6, "A=0: clear printer buffer state", inline=True)

# init_fs_vectors
comment(0x821A, "Load vector address from ROM table", inline=True)

# check_escape_handler: BGET EOF flag handling
comment(0x8493, "Bit 7 set: set EOF hint flag", inline=True)
comment(0x8495, "Load handle bitmask for flag op", inline=True)
comment(0x849A, "Flag cleared: load handle mask", inline=True)
comment(0x84A4, "Return with handle mask in A", inline=True)

# add_4_to_y / sub_4_from_y: Y adjustment utilities
comment(0x84A6, "Y += 4 (entry point)", inline=True)
comment(0x84A8, "Y += 2", inline=True)
comment(0x84A9, "Y += 1", inline=True)
comment(0x84AA, "Return with Y adjusted", inline=True)
comment(0x84AB, "Y -= 4 (entry point)", inline=True)
comment(0x84AC, "Y -= 3 (entry point)", inline=True)
comment(0x84AD, "Y -= 2", inline=True)
comment(0x84AE, "Y -= 1", inline=True)

# fscv_3_star_cmd: post-match pointer adjustment
comment(0x8BBE, "CLC for pointer calculation", inline=True)
comment(0x8BBF, "A = chars consumed from input", inline=True)
comment(0x8BC0, "X = command text pointer high", inline=True)
comment(0x8BC2, "Add chars consumed to pointer low", inline=True)
comment(0x8BC4, "Store adjusted text pointer low", inline=True)
comment(0x8BC7, "Duplicate to second pointer copy", inline=True)
comment(0x8BCA, "No page overflow: skip INX", inline=True)
comment(0x8BCC, "Adjust high byte for page crossing", inline=True)
comment(0x8BCD, "Store high byte to context ptr 1", inline=True)
comment(0x8BD0, "Store high byte to context ptr 2", inline=True)
comment(0x8BD3, "Store high byte to context ptr 3", inline=True)
comment(0x8BD6, "Dispatch via PHA/PHA/RTS", inline=True)

# rom_set_nmi_vector: ROM-resident NMI tail
...
# fscv_5_cat: *CAT handler (directory catalogue)
comment(0x8BFE, "X=&14 (20): column width for display", inline=True)
comment(0x8C00, "Store column width for batch calc", inline=True)
comment(0x8C02, "X=3: column count for examine request", inline=True)
comment(0x8C04, "Store column count", inline=True)
comment(0x8C06, "Y=0: initial entry start offset", inline=True)
comment(0x8C08, "A=6: examine format type in command", inline=True)
comment(0x8C0A, "Store format type at &0F05", inline=True)
comment(0x8C0D, "Skip spaces in dir name argument", inline=True)
comment(0x8C10, "Save parameter offset after spaces", inline=True)
comment(0x8C2F, "X=5: space padding count", inline=True)
comment(0x8C31, "Print 5 spaces for alignment", inline=True)
comment(0x8C34, "Access: 0=Owner, non-zero=Public", inline=True)
comment(0x8C42, "Always taken (high-bit term. str)", inline=True)
comment(0x8C59, "X=4: space padding count", inline=True)
comment(0x8C5B, "Print 4 spaces for alignment", inline=True)
comment(0x8C5E, "Print 'Option ' label", inline=True)
comment(0x8C68, "Load boot option from workspace", inline=True)
comment(0x8C74, "Load string offset for option name", inline=True)
comment(0x8C77, "Load char from option name string", inline=True)
comment(0x8C7A, "Zero terminator: name complete", inline=True)
comment(0x8C8C, "X=&11: CSD name offset in reply", inline=True)
comment(0x8C8E, "Print current directory name", inline=True)
comment(0x8C91, "X=5: space padding count", inline=True)
comment(0x8C93, "Print 5 spaces for alignment", inline=True)
comment(0x8C96, "Print 'Lib. ' label", inline=True)
comment(0x8C9E, "X=&1B: library name offset in reply", inline=True)
comment(0x8CA8, "Y=0: initial examine start position", inline=True)
comment(0x8CAD, "A = reply buffer bytes consumed", inline=True)
comment(0x8CAE, "Complement for divide-by-subtraction", inline=True)
comment(0x8CB0, "SEC for subtraction", inline=True)
comment(0x8CB1, "Subtract one column width (20)", inline=True)
comment(0x8CB3, "Count another entry that fits", inline=True)
comment(0x8CB4, "Loop while space remains", inline=True)
comment(0x8CB6, "Store entries per examine batch", inline=True)
comment(0x8CB9, "Save batch size for loop reset", inline=True)
comment(0x8CBB, "Reload dir name offset for examine", inline=True)
comment(0x8CE8, "Fallthrough (also boot string 'L.!')", inline=True)
...
# print_file_info: Print file catalogue line
comment(0x8604, "No info available: return", inline=True)
comment(0x860E, "Also end name on space character", inline=True)
comment(0x8615, "Advance to next filename byte", inline=True)
comment(0x861B, "Advance past filename position", inline=True)
comment(0x8620, "Y=5: high byte of load address", inline=True)
comment(0x8632, "X=3: print 3 bytes for file length", inline=True)
comment(0x863D, "Move to next lower address byte", inline=True)
comment(0x863E, "Decrement byte counter", inline=True)

# osword_12_handler: read/set state information
comment(0x8E7C, "Sub-function >= 6? (handle ops)", inline=True)
comment(0x8E7E, "Sub >= 6: handle/station/error", inline=True)
comment(0x8E80, "Sub-function >= 4? (protection)", inline=True)
comment(0x8E97, "Copy station bytes to/from workspace", inline=True)
comment(0x8E9A, "Always taken (Y=2 after copy)", inline=True)
comment(0x8EAD, "SEC: set carry for exit", inline=True)
comment(0x8EB0, "Load local station number", inline=True)
comment(0x8EB3, "Y=1: param block offset for result", inline=True)
comment(0x8EB4, "Return station number to caller", inline=True)
comment(0x8EB6, "Always taken (C set above)", inline=True)
comment(0x8EDC, "Y=1: param block offset for error", inline=True)
comment(0x8EDD, "Load last FS error number", inline=True)
comment(0x8EE0, "Return error code to caller", inline=True)
comment(0x8EE2, "Exit with carry set", inline=True)

# store_output_byte / toggle_print_flag
comment(0x9204, "Bit 0=0 (active): just flush", inline=True)
comment(0x920C, "Flush accumulated output to network", inline=True)
comment(0x920F, "Save PFLAGS bit 0 via carry", inline=True)
comment(0x9212, "Restore original reason code", inline=True)
comment(0x9213, "Old PFLAGS bit 0 to A bit 7", inline=True)
comment(0x9214, "Reason bit 0 into PFLAGS bit 0", inline=True)

# handle_bput_bget: BPUT/BGET handler
comment(0x83CF, "Load handle bitmask for BPUT/BGET", inline=True)
comment(0x83EE, "Convert SPOOL handle to bitmask", inline=True)
comment(0x83F1, "Compare SPOOL mask with file mask", inline=True)
comment(0x83F3, "Not SPOOL file: dispatch FS error", inline=True)
comment(0x83F5, "Load '*SP.' command string low", inline=True)

# send_to_fs: wait for FS reply
comment(0x845B, "Load TX flag byte from ctrl block", inline=True)
comment(0x845D, "Bit 7 set: TX complete, clean up", inline=True)
comment(0x845F, "Check for Escape during TX wait", inline=True)

# copy_filename_ptr: parse filename into command buffer
comment(0x86A2, "Y=1: offset past filename pointer", inline=True)
comment(0x86A3, "X=&FF: parse all characters", inline=True)
comment(0x86A5, "C=0: normal string parse entry", inline=True)
comment(0x86A6, "Init string parsing via GSINIT", inline=True)

# findv_handler: set up sequence tracking for new handle
comment(0x8979, "A = handle bitmask from set_fs_flag", inline=True)
comment(0x897A, "Merge handle into sequence tracking", inline=True)
comment(0x897D, "Store updated sequence tracking", inline=True)

# fscv_0_opt: *OPT handler
comment(0x89AA, "X=1? (*OPT 1: set messaging)", inline=True)
comment(0x89AC, "Not *OPT 1: bad option error", inline=True)
comment(0x89AE, "Y < 2? (valid: 0=off, 1=on)", inline=True)
comment(0x89B0, "Y >= 2: bad option value error", inline=True)

# fsreply_4_notify_exec: handle notify/exec reply
comment(0x8DA6, "Y=&0F: workspace offset for params", inline=True)
comment(0x8DAA, "Claim Tube for address transfer", inline=True)
comment(0x8DAD, "Execute at load address", inline=True)

# osword_0f_handler: initiate transmit
comment(0x8E4A, "Set up and start low-level transmit", inline=True)
comment(0x8E4D, "Exit: release service claim", inline=True)
comment(0x8E50, "SEC: alternate entry for OSWORD &11", inline=True)
comment(0x8E51, "A = param block high for branch", inline=True)

# osword_11_handler: read/poll RX control block
comment(0x8E66, "Reset JSR protection status", inline=True)
comment(0x8E69, "Branch to set carry and dispatch", inline=True)
comment(0x8E78, "Always taken (SEC set above)", inline=True)

# remote_cmd_data: handle remote command data
comment(0x90CE, "Y=&0E: 14-byte parameter block", inline=True)
comment(0x90D0, "OSWORD 7? (make sound)", inline=True)
comment(0x90F9, "Set up control block for reply", inline=True)
comment(0x90FC, "Return from remote command handler", inline=True)

# lang_1_remote_boot: handle remote boot
comment(0x9110, "Save boot type on stack", inline=True)
comment(0x9116, "Store filename offset at ws+&0F", inline=True)
comment(0x9119, "Restore boot type from stack", inline=True)
comment(0x9124, "Y=0: second argument for OSBYTE", inline=True)
comment(0x9126, "A=&C9: disable keyboard for boot", inline=True)

# tube_init_reloc: Tube initialisation
comment(0x04EF, "Restore saved Y register", inline=True)

# handle_bput_bget: error ptr setup
comment(0x8449, "A=&2A: error ptr for FS retry", inline=True)

# resume_after_remote: workspace reset
comment(0x8171, "Return from workspace reset", inline=True)

# filev_save: FS error ptr setup
comment(0x878D, "A=&2A: error ptr for FS retry", inline=True)
comment(0x878F, "Store error ptr for TX poll", inline=True)

# fscv_1_eof: save handle for check
comment(0x8821, "Save handle for EOF check", inline=True)

# filev_attrib_dispatch: attribute data handling
comment(0x88BA, "Load FS command code from reply", inline=True)
comment(0x88BD, "Zero: no attribute data returned", inline=True)
comment(0x88DC, "Reload FS command code", inline=True)
comment(0x88DF, "A = command code for exit test", inline=True)

# gbpbv_handler: FS error ptr setup
comment(0x8A74, "A=&2A: error ptr for FS retry", inline=True)
comment(0x8A76, "Store error ptr for TX poll", inline=True)

# econet_tx_rx: first packet handling
comment(0x8FF4, "First packet: exit handler", inline=True)

# remote_print_handler: post-flush loop
comment(0x91EB, "Flush done: continue loop", inline=True)
