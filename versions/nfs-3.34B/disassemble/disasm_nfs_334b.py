...
subroutine(0x853C, "print_inline", hook=None,
...
comment(0x003A, "BIT R4 status: check WRCH request", inline=True)
comment(0x003D, "R1 not ready: check R2 instead", inline=True)
comment(0x003F, "Read character from Tube R4 data", inline=True)
comment(0x0045, "BIT R2 status: check command byte", inline=True)
comment(0x0048, "R2 not ready: loop back to R1 check", inline=True)
comment(0x004A, "Re-check R4: WRCH has priority over R2", inline=True)
...
comment(0x83E8, "A=&C7: read *SPOOL file handle", inline=True)
...
comment(0x88A4, "X=1: filename only, no data extent", inline=True)
...
comment(0x8E56, "Store as copy source high byte in &BF", inline=True)
...
comment(0x8E82, "Sub-function 4 or 5: read/set protection", inline=True)
...
comment(0x8E8C, "Store workspace page in &BF (hi byte)", inline=True)
comment(0x8E8E, "Load offset: &FF (sub 0-1) or &01 (sub 2-3)", inline=True)
comment(0x8E91, "Store offset in &BE (lo byte)", inline=True)
...
comment(0x8F4D, "Y=2: copy 3 bytes (indices 2,1,0)", inline=True)
...
comment(0x8F77, "Y=&2F: start of template (descending)", inline=True)
...
comment(0x906C, "X=7: 8-entry NCTBPL table size", inline=True)
...
comment(0x90DC, "Load param byte from OSWORD param block", inline=True)
...
comment(0x90E6, "Load original OSWORD code", inline=True)
comment(0x90E8, "Store OSWORD code at ws+0", inline=True)
...
comment(0x9826, "Install data_rx_setup at &982D", inline=True)
...
comment(0x9877, "Install bulk read at &989A", inline=True)
comment(0x9879, "High byte of &989A handler", inline=True)
...
comment(0x9E49, "Install nmi_data_tx_tube at &9EA4", inline=True)
...
comment(0x85A3, "Restore X from stack", inline=True)
...
comment(0x8784, "Display filename being saved", inline=True)
label(0x8787, "save_csd_display")       # Save CSD from reply and display file info
comment(0x8787, "Load CSD from FS reply", inline=True)
comment(0x878A, "Transfer file data blocks to server", inline=True)
...
subroutine(0x8DEE, "restore_rx_flags", hook=None,
...
comment(0x8F58, "Y=&28: RXCB template offset", inline=True)
...
comment(0x90F4, "Set up TX and send RWORD packet", inline=True)
...
comment(0x984A, "High byte of nmi_data_rx_net handler", inline=True)
...
# save_econet_state / restore_econet_state: comments removed
# (were from wrong version layout — described NMI save/restore
# operations but actual code saves RXCB state fields)
...
comment(0x860C, "CR: pad rest of filename field", inline=True)
comment(0x8610, "Space: also ends filename", inline=True)
comment(0x8616, "Loop until all chars printed", inline=True)
comment(0x861C, "Pad to 12 chars wide", inline=True)
comment(0x8622, "Print load address as 2 hex bytes", inline=True)
comment(0x8634, "ALWAYS branch", inline=True)
comment(0x8636, "X=4: print 4 bytes for address", inline=True)
comment(0x863F, "Loop for remaining hex bytes", inline=True)
...
# lang_1_remote_boot: comments removed (were from wrong version layout)

# store_output_byte: comments removed (were from wrong version layout)
...
# restore_econet_state: comments removed (were from wrong version
# layout — described NMI restore operations but actual code restores
# RXCB state fields)

# check_escape_handler
comment(0x84A5, "Y += 5 (entry point)", inline=True)

# net_4_resume_remote
comment(0x8DF3, "Jump to clear_svc_restore_args", inline=True)
comment(0x8DF6, "V always clear: branch to cleanup exit", inline=True)

# fsreply_4_notify_exec: handle FS exec notification
comment(0x8D85, "X=&0E: OSWORD &10 parameter block size", inline=True)
...
comment(0x8DA8, "A=4: Tube claim type (multi-byte transfer)", inline=True)

# --- Subroutine promotions ---

label(0x8F49, "clear_svc_restore_args")
subroutine(0x8F49, "clear_svc_restore_args", hook=None,
...
label(0x96D1, "copy_nmi_shim_loop")
...
# save_econet_state: save to RX control block
...
comment(0x8497, "Clear EOF hint flag for this handle", inline=True)
comment(0x849A, "Flag cleared: load handle mask", inline=True)
comment(0x849C, "Load handle bitmask for flow control", inline=True)
comment(0x849E, "Set EOF hint flag for this handle", inline=True)
comment(0x84A1, "Load handle mask for return value", inline=True)
...
comment(0x84A7, "Y += 3", inline=True)
...
comment(0x8601, "Check if file info available", inline=True)
comment(0x8604, "No info available: return", inline=True)
comment(0x8606, "Y=0: start of filename string", inline=True)
comment(0x8608, "Load filename character", inline=True)
comment(0x860A, "CR: end of filename", inline=True)
comment(0x860E, "Also end name on space character", inline=True)
comment(0x8615, "Advance to next filename byte", inline=True)
comment(0x8618, "Print padding space", inline=True)
comment(0x861B, "Advance past filename position", inline=True)
comment(0x861E, "Continue padding if < 12 chars", inline=True)
comment(0x8620, "Y=5: high byte of load address", inline=True)
comment(0x8622, "Print load address as 2 hex bytes", inline=True)
comment(0x8625, "Print exec address and length", inline=True)
comment(0x862B, "Y=9: exec address offset", inline=True)
comment(0x862D, "Print exec address bytes", inline=True)
comment(0x8630, "Y=&0C: file length offset", inline=True)
comment(0x8632, "X=3: print 3 bytes for file length", inline=True)
comment(0x8638, "Load address/length byte", inline=True)
comment(0x863A, "Print as 2 hex digits", inline=True)
comment(0x863D, "Move to next lower address byte", inline=True)
comment(0x863E, "Decrement byte counter", inline=True)
comment(0x8641, "A=space: separator character", inline=True)
comment(0x864B, "Set TX pointer hi byte to page 0", inline=True)
...
comment(0x91FA, "XOR with transfer count flags", inline=True)
comment(0x91FD, "Shift bit 0 into carry", inline=True)
comment(0x91FE, "Data phase active: continue", inline=True)
comment(0x9200, "Load transfer count flags", inline=True)
comment(0x9203, "Shift bit 0 into carry", inline=True)
comment(0x9206, "Rotate carry back", inline=True)
comment(0x9207, "Mask off control bits", inline=True)
comment(0x9209, "Store updated flags", inline=True)
...
comment(0x90FD, "Y=4: RX control block byte 4", inline=True)
...
comment(0x9106, "OR with 9: set remote boot bits", inline=True)
comment(0x9108, "Store modified control byte", inline=True)
comment(0x910A, "X=&80: exec address offset lo", inline=True)
comment(0x910C, "Y=&80: exec address offset hi", inline=True)
comment(0x910E, "Load exec address low byte", inline=True)
comment(0x9112, "Load exec address high byte", inline=True)
comment(0x9114, "Y=&0F: workspace offset for hi", inline=True)

# lang_1_remote_boot: handle remote boot
comment(0x911A, "Copy command to &0100 area", inline=True)
comment(0x911C, "Initialize OSBYTE vectors", inline=True)
comment(0x911F, "Set up control block for boot", inline=True)
comment(0x9122, "X=1: enable parameter", inline=True)
