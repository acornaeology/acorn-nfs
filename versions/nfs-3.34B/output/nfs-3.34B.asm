...
;    R4 for WRCH requests (forwarded to OSWRITCH &FFCB) and R2 for command bytes
...
;
; Copied from ROM (tube_code_page4) during init. The first 28 bytes (&0400-&041B) overlap
...
; read data byte to A
    ...
    jmp tube_escape_check                                             ; 9350: 4c e2 06    L.. :005e[1]        ; JMP to tube_escape_check (&06A7)
...
;
; Copied from ROM (tube_dispatch_table) during init. Contains: &0500: tube_dispatch_table
; — 14-entry handler address table &051C: tube_wrch_handler — WRCHV target &051F:
; tube_send_and_poll — send byte via R2, poll for reply &0527: tube_poll_r4_wrch —
; service R4 WRCH while waiting for R2 &053D: tube_release_return — restore regs and RTS
...
;
; Copied from ROM (tube_code_page6) during init. &0600-&0601 is the tail of tube_osfile
...
lang_entry_hi = language_entry+2
    jmp lang_entry_dispatch                                           ; 8000: 4c 99 80    L..      ; JMP language_handler
; &8003 referenced 1 time by &93e7
.service_entry
; &8004 referenced 1 time by &93ea
svc_entry_lo = service_entry+1
    jmp check_svc_high                                                ; 8003: 4c af 80    L..      ; JMP service_handler
...
; Dispatch table: low bytes of (handler_address - 1) Each entry stores the low byte of a handler address minus 1, for use with the PHA/PHA/RTS dispatch trick at &809F. See dispatch_0_hi (&8045) for the corresponding high bytes.
;
...
; command match table at &8BD7, and from FSCV 2/3/4 indirectly. If CSD handle is zero
...
; Econet error message table (ERRTAB, 8 entries). Each entry: error number byte followed by NUL-terminated string. &A0: "Line Jammed"     &A1: "Net Error" &A2: "Not listening"   &A3: "No Clock" &A4: "Bad Txcb"        &11: "Escape" &CB: "Bad Option"      &A5: "No reply" Indexed by the low 3 bits of the TXCB flag byte (AND #&07), which encode the specific Econet failure reason. The NREPLY and NLISTN routines build a MOS BRK error block at &100 on the stack page: NREPLY fires when the fileserver does not respond within the timeout period; NLISTN fires when the destination station actively refused the connection. Indexed via the error dispatch at c8424/c842c.
; &84b0 referenced 1 time by &8436
.error_msg_table
    equb &a0                                                          ; 84b0: a0          .     
    equs "Line Jammed", &00                                           ; 84b1: 4c 69 6e... Lin...
    equb &a1                                                          ; 84bd: a1          .     
    equs "Net Error", &00                                             ; 84be: 4e 65 74... Net...
    equb &a2                                                          ; 84c8: a2          .     
    equs "Not listening", &00                                         ; 84c9: 4e 6f 74... Not...
    equb &a3                                                          ; 84d7: a3          .     
    equs "No Clock", &00                                              ; 84d8: 4e 6f 20... No ...
    equb &a4                                                          ; 84e1: a4          .     
    equs "Bad Txcb", &00                                              ; 84e2: 42 61 64... Bad...
    equb &11                                                          ; 84eb: 11          .     
    equs "Escape", &00                                                ; 84ec: 45 73 63... Esc...
    equb &cb                                                          ; 84f3: cb          .     
    equs "Bad Option", &00                                            ; 84f4: 42 61 64... Bad...
    equb &a5                                                          ; 84ff: a5          .     
    equs "No reply", &00                                              ; 8500: 4e 6f 20... No ...
; ***************************************************************************************
...
;
; Loads the RX control byte and compares against &82 (immediate HALT). If HALT, discards
; the frame via imm_op_discard. Otherwise falls through to imm_op_build_reply.
...
;
; Pushes a return address on the stack (pointing to tx_done_exit), then does JMP indirect
; to call the remote JSR target routine. When that routine returns via RTS, control
...
;     Number of data bytes     = 350 bytes
;     Number of data words     = 28 bytes
;     Number of string bytes   = 215 bytes
