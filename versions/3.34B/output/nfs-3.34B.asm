...
.initl
    ...
    beq init_fs_vectors                                               ; 8199: f0 7d       .}             ; ALWAYS branch
...
.svc_3_autoboot
    ...
    bmi print_station_info                                            ; 81db: 30 0a       0.
...
; the .BOOT file.
    equs "I .BOOT", &0d                                               ; 8246: 49 20 2e... I .
; ***************************************************************************************
...
.store_rom_ptr_pair
    lda run_fscv_cmd,y                                                ; 82e6: b9 41 82    .A.
    sta (osrdsc_ptr),y                                                ; 82e9: 91 f6       ..
    iny                                                               ; 82eb: c8          .
    lda run_fscv_cmd,y                                                ; 82ec: b9 41 82    .A.
...
.loop_c8566
    ...
    bcs c8585                                                         ; 856a: b0 19       ..             ; Yes: not a digit, done
    cmp #&2e ; '.'                                                    ; 856c: c9 2e       ..             ; Dot separator?
    beq c8586                                                         ; 856e: f0 16       ..             ; Yes: exit with C=1 (dot found)
    bmi c8585                                                         ; 8570: 30 13       0.             ; Control char or space: done
    and #&0f                                                          ; 8572: 29 0f       ).             ; Mask ASCII digit to 0-9
    sta l00b3                                                         ; 8574: 85 b3       ..             ; Save new digit
    ...
    adc l00b3                                                         ; 857e: 65 b3       e.             ; + digit = total*10 + digit
...
.handle_to_mask
    ...
    beq c85a2                                                         ; 8591: f0 0f       ..             ; C=1 and Y=0: skip (handle 0 = none)
...
.y2fsl2
    ...
    bne c85a2                                                         ; 859f: d0 01       ..             ; Non-zero: valid mask, skip to exit
...
.fs2al1
    ...
    txa                                                               ; 85ac: 8a          .              ; A = bit position (1-based)
...
.loop_c89d5
    ...
    bmi c89e1                                                         ; 89d9: 30 06       0.             ; Negative: subtract instead
...
.gbpbx
    ...
    bne loop_c89d5                                                    ; 89e8: d0 eb       ..             ; Loop 4 times (X=&FC,&FD,&FE,&FF,done)
    rts                                                               ; 89ea: 60          `
...
.copy_param_block
    ...
    lda (l00f0),y                                                     ; 8e25: b1 f0       ..             ; Load byte from param block
...
.c8e2b
    ...
    sta (l00f0),y                                                     ; 8e2d: 91 f0       ..             ; Store to param block
