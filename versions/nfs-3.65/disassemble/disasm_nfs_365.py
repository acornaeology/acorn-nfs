...
d.use_environment("acorn_sideways_rom", rom_title="NFS ROM 3.65 disassembly (Acorn Econet filing system)")
...
d.comment(0x0018, "Send `&FF` to Tube R4 (signal error)", align=Align.INLINE)
...
d.comment(0x0023, "`Y=0`: error block starts at (`brk_ptr`)", align=Align.INLINE)
d.comment(0x0024, "Load error number from (`brk_ptr`),0", align=Align.INLINE)
...
d.subroutine(
    ...
    description="""Copied from ROM at [`reloc_p4_src`](address:9365) during init. The first 28
...
`&8040`–`&8045`, immediately before the hi bytes. Their hi bytes are
at `&8065`–`&806A`, after [`dispatch_0_hi`](label:dispatch_0_hi).""",
)
...
in the [`fs_cmd_match_table`](label:fs_cmd_match_table) entries near `&8C4B`, and
from FSCV 2 / 3 / 4 indirectly. If the CSD handle is zero (not logged
...
On service 1 (workspace claim) probes ADLC status registers
`SR1` (`&FEA0`) and `SR2` (`&FEA1`) to detect whether Econet
hardware is present. Non-zero reads indicate bus noise from
absent hardware; sets bit 7 of the per-ROM workspace as a
disable flag. For services `<&80`, the flag causes an early
return (disabling this ROM). Services `>=&80` (`&FE`, `&FF`)
...
d.comment(0x813E, "Set `EVNTV` high byte (`EVNTV`+1)", align=Align.INLINE)
...
d.comment(0x8148, "Set `BRKV` high byte (`brkv`+1)", align=Align.INLINE)
...
d.expr(0x81A4, sym("cmd_roff_str") - sym("binary_version"))
...
d.expr(0x81D9, sym("cmd_net_str") - sym("binary_version"))
...
d.subroutine(
    ...
    description="""Copies 14 bytes from [`fs_vector_addrs`](label:fs_vector_addrs)+`2` (= `&8288`)
into `FILEV`–`FSCV` at `&0212`, setting all 7 filing-system vectors to
the extended-vector dispatch addresses . Calls
`setup_rom_ptrs_netv` to install the ROM pointer-table entries with
the actual NFS handler addresses.

Also reached directly from [`svc_13_select_nfs`](label:svc_13_select_nfs),
bypassing the station display. Falls through to
[`issue_vectors_claimed`](label:issue_vectors_claimed).""",
)
...
against [`fs_cmd_match_table`](label:fs_cmd_match_table)). The `"I."` prefix
triggers the catch-all entry which forwards the command to the
fileserver. Falls through to [`run_fscv_cmd`](label:run_fscv_cmd).""",
)
...
`FILEV`–`FSCV` at `&0212` by [`init_fs_vectors`](label:init_fs_vectors). Each
2-byte pair is a dispatch address that the MOS uses
...
d.index_base(0x8288, "fs_dispatch_addrs")
...
d.subroutine(
    ...
    description="""Advances Y past leading spaces in the text at (os_text_ptr),Y.
...
d.subroutine(
    ...
    description="""Clears the escapable flag via clear_escapable, then falls
...
d.subroutine(
    ...
    description="""BPUTV enters at &8413 (CLC; fall through) and BGETV enters
...
d.subroutine(
    ...
    description="""Tests bit 7 of the MOS escape flag (&FF) ANDed with the
...
d.subroutine(
    ...
    description="""Checks byte 4 of the RX control block (remote status flag).
...
d.subroutine(
    ...
    description="""Clears JSR protection, zeroes &0100-&0102 (creating a BRK
...
d.subroutine(
    ...
    description="""Validates that the source station in the received packet matches
...
d.subroutine(
    ...
    description="""Loads A with &2A ('*') as the FS command prefix byte, then
...
d.index_base(0x856E, "error_msg_table")
d.banner(
    0x856E,
...
d.index_base(0x856E, "msg_line_jammed")
d.label(0x857B, "msg_net_error")
d.label(0x8586, "msg_not_listening")
d.label(0x8595, "msg_no_clock")
d.label(0x859F, "msg_escape")
d.label(0x85A7, "msg_bad_option")
d.label(0x85B3, "msg_no_reply")
...
d.index_base(0x85DA, "access_bit_table")
...
d.subroutine(
    ...
    description="""Claims the TX semaphore (tx_clear_flag) via ASL -- a busy-wait
...
d.subroutine(
    ...
    description="""PHP/LSR escapable/PLP: clears bit 7 of the escapable flag
...
d.subroutine(
    ...
    description="""ORs A into fs_eof_flags then stores the result via
...
d.subroutine(
    ...
    description="""Inverts A (EOR #&FF), then ANDs the result into fs_eof_flags
...
d.subroutine(
    ...
    description="""Uses the MOS GSINIT/GSREAD API to parse a filename string from
...
d.subroutine(
    ...
    description="""Sub-entry of parse_filename_gs that accepts a non-zero Y offset
...
d.subroutine(
    ...
    description="""Calls [`save_fscv_args`](label:save_fscv_args) to preserve `A`/`X`/`Y`, then
`JSR` [`copy_filename_ptr`](label:copy_filename_ptr) to copy the 2-byte filename
pointer from the parameter block to `os_text_ptr` and fall through to
[`parse_filename_gs`](label:parse_filename_gs), which parses the filename into
`&0E30+`. Sets `fs_crc_lo`/`hi` to point at the parsed filename buffer.

Dispatches by function code:

| `A`     | Action |
|---------|--------|
| `&FF`   | Load file → [`send_fs_examine`](label:send_fs_examine) |
| `&00`   | Save file → [`filev_save`](label:filev_save) |
| `&01`–`&06` | Attribute ops → [`filev_attrib_dispatch`](label:filev_attrib_dispatch) |
| Other   | [`restore_args_return`](label:restore_args_return) (unsupported, no-op) |""",
    on_entry={
...
d.subroutine(
    ...
    description="""Sends an FS examine/load command to the fileserver. The function
...
| ≥7    | Fall through to [`restore_args_return`](label:restore_args_return) (no-op) | — |
...
The handle in `Y` is converted via
[`handle_to_mask_clc`](label:handle_to_mask_clc). For writes (`A=1`), the carry
flag from the mask conversion is used to branch to `save_args_handle`,
which records the handle for later use.""",
    on_entry={
...
unsupported or should return zero.""",
    on_exit={"a": "0", "x": "restored from fs_options (&BB)", "y": "restored from fs_block_offset (&BC)"},
)
...
d.subroutine(
    ...
    description="""Format: command letters (bit 7 clear), then dispatch address as two
big-endian bytes: `high|(bit 7 set)`, `low`. The bit-7-set on the high
byte marks the end of the command string. The `PHA`/`PHA`/`RTS` trick
adds 1 to the stored `(address − 1)`. Matching is case-insensitive
(`AND &DF`) and supports `.` abbreviation.

Entries:

| Pattern        | Handler | Note |
|----------------|---------|------|
| `"I."`         | [`forward_star_cmd`](label:forward_star_cmd) | placed first as a fudge to catch `*I.` abbreviation before matching `*I AM` |
| `"I AM"`       | [`i_am_handler`](label:i_am_handler) | parse `station.net`, logon |
| `"}EX"`        | [`ex_handler`](label:ex_handler) | `}` terminator rejects `*EXEC` |
| `"BYE"\\r`      | [`bye_handler`](label:bye_handler) | logoff |
| catch-all      | [`forward_star_cmd`](label:forward_star_cmd) | forward anything else to the FS |""",
)
...
d.subroutine(
    ...
    description="""Initialises &B5=&0B (examine arg count) and &B7=&03 (column
...
d.index_base(0x8D42, "option_name_offsets")
...
d.subroutine(
    ...
    description="""Handles column formatting for *CAT display. On a null byte
...
d.subroutine(
    ...
    description="""Prints the byte in A as two hex digits (e.g. &3F prints "3F").
...
d.subroutine(
    ...
    description="""Parses the filename via parse_filename_gs and calls infol2,
...
d.subroutine(
    ...
    description="""Initialises a GS reader to skip past the filename and
...
d.subroutine(
    ...
    description="""CLC entry (SDISC): copies handles only, then jumps to
...
d.subroutine(
    ...
    description="""Loads the file handle byte from &F0, then falls through to
...
d.subroutine(
    ...
    description="""Reached by fall-through from svc_8_osword with A = OSWORD
...
d.subroutine(
    ...
    description="""Saves the param block pointer (&AA-&AC) to (net_rx_ptr) and
...
d.index_base(0x8EBB, "osword_handler_lo")
...
d.index_base(0x8EC0, "fs_osword_tbl_hi")
...
d.index_base(0x8F02, "osword_12_offsets")
...
d.subroutine(
    ...
    description="""Range-checks the sub-function code from the param block and dispatches:
...
Sub-functions 0–3 select the appropriate workspace page (static `&0D`
or dynamic) and offset, then fall through to the bidirectional param-
block copy loop. Sub-functions `>=6` are re-dispatched via `rsl1`;
values `>=10` return the last FS error.

Note: there is no sub-function that *sets* the local station number —
on the Model B that is hardwired via the 8 station-ID links read from
`&FE18`.""",
)
...
d.subroutine(
    ...
    description="""If C=1, copies from OSWORD param block (&F0),Y to workspace
...
d.subroutine(
    ...
    description="""C=1: copy X+1 bytes from (&F0),Y to (&AB),Y (param to workspace)
C=0: copy X+1 bytes from (&AB),Y to (&F0),Y (workspace to param)""",
    on_entry={"c": "1=param to workspace, 0=workspace to param", "x": "byte count minus 1", "y": "starting offset"},
    on_exit={"a": "last byte copied", "x": "&FF", "y": "start + count + 1"},
)
...
d.comment(0x8FBB, "Dest station = broadcast (accept reply from any station)", align=Align.INLINE)
...
d.subroutine(
    ...
    description="""Dispatch by `A`:

- `A=0`: initialise TX control block from the ROM template at
  [`init_tx_ctrl_block`](label:init_tx_ctrl_block) (zero entries substituted from
  NMI workspace [`nmi_sub_table`](label:nmi_sub_table)), transmit it, set up the RX control block,
  and receive reply.
...
d.comment(0x902C, "Set RX end address to max (accept any length)", align=Align.INLINE)
...
d.subroutine(
    ...
    description="""Entered from the econet_tx_rx response handler at &903E after
...
d.index_base(0x90A7, "osword_tbl_lo")
...
d.index_base(0x90B0, "osword_tbl_hi")
...
d.subroutine(
    ...
    description="""Zeros the carry flag in the stacked processor status to
...
d.subroutine(
    ...
    description="""Stores A at workspace offset &D9 (command type), then sets byte
...
d.index_base(0x9148, "remote_osbyte_table")
...
d.subroutine(
    ...
    description="""Only accepts OSWORD 7 (make a sound) and OSWORD 8 (define an
...
d.subroutine(
    ...
    description="""Sets X=&0D, Y=&7C. Tests bit 6 of &83B3 to choose target:
...
d.subroutine(
    ...
    description="""Read by the loop at &918D, indexed by X from a starting value
...
d.subroutine(
    ...
    description="""Called when the printer selection changes. Compares X against
the network printer buffer number (&F0). If it matches,
initialises the printer buffer pointer (&0D61 = &1F) and
sets the initial flag byte (pflags = &41). Otherwise falls
through to return.""",
...
d.subroutine(
    ...
    description="""Transmits a data packet over econet with sequence number tracking.
...
d.comment(0x927F, "End address = max (unlimited data length)", align=Align.INLINE)
...
d.index_base(0x9321, "vdu_osbyte_table")
...
handler. imm_op_build_reply arms the system VIA shift
register as a one-shot delayed interrupt source: free-running
in φ2 shift-in mode it sets the SR-complete flag (IFR bit 2)
a fixed, short number of cycles after arming — by which time
the NMI receive handler has already returned — and that raises
an ordinary IRQ which reaches the ROM here as service call &05
(unrecognised interrupt). This is how a remote operation is
bounced out of NMI context into the normal IRQ path, where it
is safe to JSR into user code or call OS routines.
...
d.label(0x9666, "svc5_save_registers")
d.entry(0x9666)
d.comment(0x9666, "Save X on stack", align=Align.INLINE)
d.comment(0x9667, "Push saved X", align=Align.INLINE)
d.comment(0x9668, "Save Y on stack", align=Align.INLINE)
d.comment(0x9669, "Push saved Y", align=Align.INLINE)
d.comment(0x966A, "Read VIA auxiliary control reg", align=Align.INLINE)
d.comment(0x966D, "Mask shift register bits", align=Align.INLINE)
d.comment(0x966F, "OR in saved SR mode", align=Align.INLINE)
d.comment(0x9672, "Write back ACR with SR mode", align=Align.INLINE)
d.comment(0x9675, "Read SR to clear shift complete", align=Align.INLINE)
d.comment(0x9678, "A=4: SR interrupt bit", align=Align.INLINE)
d.comment(0x967A, "Clear SR interrupt flag", align=Align.INLINE)
d.comment(0x967D, "Disable SR interrupt", align=Align.INLINE)
d.comment(0x9680, "Load deferred operation type", align=Align.INLINE)
...
d.subroutine(
    ...
    description="""Issues OSBYTE &8F with X=&0C (NMI claim service request) before
copying the NMI shim. Sub-entry at &96B8 skips the service
request for quick re-init. Then copies 32 bytes of
NMI shim from ROM (&9FB2) to RAM (nmi_code_base), patches the current
...
d.subroutine(
    ...
    description="""Sub-entry of adlc_init_workspace that skips the OSBYTE &8F
service request. Copies 32 bytes of NMI shim from ROM to
nmi_code_base, patches the ROM bank number, sets init flags, reads
station ID, and re-enables NMIs.""",
)


d.comment(0x96B8, "Copy 32 bytes of NMI shim from ROM to nmi_code_base", align=Align.INLINE)
d.label(0x96BA, "copy_nmi_shim")

d.comment(0x96BA, "Read byte from NMI shim ROM source", align=Align.INLINE)
d.comment(0x96BD, "Write to NMI shim RAM (nmi_code_base,Y)", align=Align.INLINE)
...
d.subroutine(
    ...
    description="""Reached when the scout data loop sees no RDA (BPL at &9733) or
...
d.subroutine(
    ...
    description="""Selects either the normal bulk RX handler (&9865) or the Tube
...
d.subroutine(
    ...
    description="""Post-scout completion for data transfer frames (port != 0)
...
d.subroutine(
    ...
    description="""Conditionally releases the Tube co-processor before discarding.
...
d.subroutine(
    ...
    description="""Calls adlc_rx_listen to re-enter idle RX mode (CR1=&82, CR2=&67),
...
d.subroutine(
    ...
    description="""Installs nmi_rx_scout (&96DF) as the NMI handler via
...
d.subroutine(
    ...
    description="""Increments a 4-byte counter across port_buf_len / port_buf_len_hi
...
LSTAT can be read / set via [`osword_12_dispatch`](label:osword_12_dispatch)
sub-functions 4 / 5.

The execute-class operations (&83-&87) cannot run inside the NMI
receive handler — a JSR into user code or an OS call is unsafe there
— so they are not run inline. They are completed later from normal
IRQ context via the shift-register delayed interrupt armed in
imm_op_build_reply and serviced by svc5_irq_check (dispatch table
svc5_dispatch_lo). PEEK, POKE and machine-type (&81/&82/&88) only
touch memory and reply immediately, so they run here.""",
)
...
d.comment(0x9A73, "CONTINUE(&87)/mc-type(&88) skip protection", align=Align.INLINE)
...
d.subroutine(
    ...
    description="""Sets up a reply buffer (open_port_buf=&21, page &7F,
length 508 bytes) for the machine type query response, then
...
Then arms the deferred-dispatch interrupt: disables SR interrupts
...
d.subroutine(
    ...
    description="""Generates a network event (event 8) via OSEVEN with
...
d.subroutine(
    ...
    "tx_done_halt",
    title="TX done: HALT",
    description="""Sets bit 2 of rx_flags (&0D64), enables interrupts, and
...
d.subroutine(
    ...
    description="""Clears bit 2 of rx_flags (&0D64), releasing any station
...
d.label(0x9BE7, "tx_dcd_clock_check")

d.comment(0x9BE7, "A=&20: mask for SR2 DCD (clock/carrier detect)", align=Align.INLINE)
d.comment(0x9BE9, "BIT SR2: test DCD -- is there a clock?", align=Align.INLINE)
d.comment(0x9BEC, "DCD set: no clock on the line, abandon TX", align=Align.INLINE)
...
d.comment(0x9C10, "Read SR1 -- arms the CLR_RX_ST below", align=Align.INLINE)
...
d.subroutine(
    ...
    description="""Stores result code (A) into the TX control block at
...
(&A2-&A5). The fallback path (no Tube or buffer address unset)
...
d.subroutine(
    ...
    description="""Called via svc_12_nmi_release (&06D4). Checks if Econet has been
...
d.subroutine(
    ...
    description="""Disables NMIs via INTOFF (BIT &FE18), clears tx_clear_flag and
...
d.char_literal(0x806E)
d.char_literal(0x8081)
d.char_literal(0x8085)
d.char_literal(0x80A3)
d.char_literal(0x82FD)
d.char_literal(0x8355)
d.char_literal(0x836F)
d.char_literal(0x851F)
d.char_literal(0x866C)
d.char_literal(0x87E0)
d.char_literal(0x8C2C)
d.char_literal(0x8C3A)
d.char_literal(0x8DD6)
d.char_literal(0x8DD8)
d.char_literal(0x8E78)
d.char_literal(0x8E86)
...
d.banner(0x9EED, title="Immediate-op TX control-frame length table",
...
d.label(0x9EED, "tx_length_table")
d.byte(0x9EED, 8)
d.banner(0x9EF5, title="Immediate-op TX flags table",
