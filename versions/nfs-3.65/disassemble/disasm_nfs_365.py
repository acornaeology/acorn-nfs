...
d.comment(
    0x800D,
    """The 'ROFF' suffix of "(C)ROFF" at [`cmd_roff_str`](address:8010) is reused by the `*ROFF` command matcher [`svc_4_star_command`](address:81A2?hex) — a space-saving trick that shares ROM bytes between the copyright string and the star-command-name table.
...
d.banner(
    ...
    description="""Each byte is a Y offset into [`error_msg_table`](address:856E).
...
[`dispatch`](address:80E3).
See [`dispatch_0_hi`](address:8046) for the corresponding high bytes.
...
at `&8065`–`&806A`, after [`dispatch_0_hi`](address:8046).""",
)
...
[`dispatch`](address:80E3). Characters outside `'1'`–`'4'` fall through
to `return_1` (`RTS`).
...
|---------|---------|------|
| `*NET1` | [`net_1_read_handle`](address:8E6A) | read file handle from received packet |
| `*NET2` | [`net_2_read_handle_entry`](address:8E70) | read handle entry from workspace |
| `*NET3` | [`net_3_close_handle`](address:8E80) | close handle / mark as unused |
| `*NET4` | [`net_4_resume_remote`](address:81AA) | resume after remote operation |""",
)
...
Finally jumps to [`forward_star_cmd`](address:80BD).""",
)
...
in the [`fs_cmd_match_table`](address:8C39) entries near `&8C4B`, and
...
[`dispatch_0_lo`](address:8021) table with base offset `Y=&13` (table
indices 20–27).
...
code (0–4). Dispatches via [`dispatch_0_lo`](address:8021) at indices
...
[`dispatch_0_lo`](address:8021) for the low-byte table and the caller
groups.""",
)
...
[`nlistn`](address:84FE) / [`nlisne`](address:8500): the TXCB
...
by [`copy_error_message`](address:850B) until the trailing NUL
...
d.subroutine(
    ...
    description="""Calls [`save_fscv_args`](address:863B) to preserve `A`/`X`/`Y`, then
`JSR` [`copy_filename_ptr`](address:86CC) to copy the 2-byte filename
pointer from the parameter block to `os_text_ptr` and fall through to
[`parse_filename_gs`](address:86D6), which parses the filename into
...
| `A`     | Action |
|---------|--------|
| `&FF`   | Load file → [`send_fs_examine`](address:8710) |
| `&00`   | Save file → [`filev_save`](address:8783) |
| `&01`–`&06` | Attribute ops → [`filev_attrib_dispatch`](address:88BF) |
| Other   | [`restore_args_return`](address:89A1) (unsupported, no-op) |""",
    on_entry={
...
| ≥7    | Fall through to [`restore_args_return`](address:89A1) (no-op) | — |
...
[`handle_to_mask_clc`](address:8689). For writes (`A=1`), the carry
...
| `A`     | Action |
|---------|--------|
| `&00`   | Close file — delegates to [`close_handle`](address:89FE) |
| `&40`   | Open for **read** |
...
|----------------|---------|------|
| `"I."`         | [`forward_star_cmd`](address:80BD) | placed first as a fudge to catch `*I.` abbreviation before matching `*I AM` |
| `"I AM"`       | [`i_am_handler`](address:807E) | parse `station.net`, logon |
| `"}EX"`        | [`ex_handler`](address:8C4F) | `}` terminator rejects `*EXEC` |
| `"BYE"\\r`      | [`bye_handler`](address:83AE) | logoff |
| catch-all      | [`forward_star_cmd`](address:80BD) | forward anything else to the FS |""",
)
...
LSTAT can be read / set via [`osword_12_dispatch`](address:8F04)
sub-functions 4 / 5.
