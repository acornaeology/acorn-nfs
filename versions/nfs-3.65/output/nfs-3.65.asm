...
; tube_init_reloc — extract relocation address from ROM
    ...
    jmp tube_escape_check                                             ; 9368: 4c a7 06    L.. :005a[1]        ; JMP to tube_escape_check (&06A7)
...
language_handler_hi = language_entry+2
    jmp lang_entry_dispatch                                           ; 8000: 4c dd 80    L..      ; JMP language_handler
...
.binary_version
    equb &03                                                          ; 8008: 03          .        ; Binary version number
.title
...
error_offsets = copyright+8
    equb &00, "(C)ROFF", &00                                          ; 800c: 00 28 43... .(C...   ; Null terminator before copyright  Copyright string "(C)ROFF"  "Line Jammed"
...
; Y=&21, and dispatches via &80E3. Characters outside '1'-'4' fall through to return_1
; (RTS).
;
...
.net_4_resume_remote
    ...
    jsr osbyte                                                        ; 81b8: 20 f4 ff     ..      ; Re-enable keyboard (X=0, Y=0)  osbyte: read write econet keyboard disable
...
.clear_osbyte_ce_cf
    ...
    lda #osbyte_read_write_econet_os_call_interception                ; 81c2: a9 ce       ..       ; OSBYTE &CE: first system mask to reset
...
.clear_osbyte_masks
    ...
    jsr osbyte                                                        ; 81c8: 20 f4 ff     ..      ; Reset system mask byte  osbyte: read write econet os call interception
...
.error1
    ...
    jsr osbyte                                                        ; 844c: 20 f4 ff     ..      ; osbyte: read write exec file handle
...
.remot1
    ...
    jsr osbyte                                                        ; 84c8: 20 f4 ff     ..      ; osbyte: read write econet keyboard disable
; ***************************************************************************************
...
;
; Calls save_fscv_args (&863B) to preserve A/X/Y, then JSR &86CC to copy the 2-byte
; filename pointer from the parameter block to os_text_ptr and fall through to
; parse_filename_gs (&86D6) which parses the filename into &0E30+. Sets fs_crc_lo/hi to
; point at the parsed filename buffer. Dispatches by function code A: A=&FF: load file
; (send_fs_examine at &8710) A=&00: save file (filev_save at &8783) A=&01-&06: attribute
; operations (filev_attrib_dispatch at &88BF) Other: restore_args_return (unsupported,
; no-op)
;
; On Entry:
...
;
; A=0: close file -- delegates to close_handle (&89FE) A>0: open file -- modes &40=read,
