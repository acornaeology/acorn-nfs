...
_cmd_entries = [
    ...
    (0xA416, "cmd_type"),
    (0xA41F, "cmd_unprot"),
    (0xA429, "cmd_fs_operation"),
    (0xA42F, "cmd_bye"),
    (0xA436, "cmd_cdir"),
    (0xA43F, "cmd_fs_operation"),
    (0xA445, "cmd_dir"),
    (0xA44A, "cmd_ex"),
    (0xA451, "cmd_flip"),
    (0xA456, "cmd_fs"),
    (0xA45D, "cmd_fs_operation"),
    (0xA464, "cmd_iam"),
    (0xA46B, "cmd_lcat"),
    (0xA471, "cmd_lex"),
    (0xA477, "cmd_fs_operation"),
    (0xA47E, "cmd_pass"),
    (0xA487, "cmd_remove"),
    (0xA490, "cmd_rename"),
    (0xA497, "cmd_wipe"),
...
_attr_entries = [
    (0xA4AF, 0xA4B0, 0xA4B1),
    (0xA4B5, 0xA4B6, 0xA4B7),
    (0xA4BC, 0xA4BD, 0xA4BE),
    (0xA4C3, 0xA4C4, 0xA4C5),
    (0xA4CA, 0xA4CB, 0xA4CC),
    (0xA4D2, 0xA4D3, 0xA4D4),
]
handler_names = [
    ("FILEV", 0x9921),
    ("ARGSV", 0x9BAF),
...
d.use_environment("acorn_sideways_rom", rom_title="ANFS ROM 4.08.53 disassembly (Acorn Advanced Network Filing System)")
...
Called by the Tube OSCLI and OSFIND handlers.""",
    on_exit={"x": "0 (low byte of &0700)", "y": "7 (high byte of &0700)"},
)
...
d.comment(0x80CB, "Next NMI handler address (low)", align=Align.INLINE)
...
d.comment(0x80EB, "Install scout-data reader (low)", align=Align.INLINE)
d.comment(0x80ED, "Install scout-data reader (high)", align=Align.INLINE)
...
d.comment(0x81D7, "Next NMI handler address (low)", align=Align.INLINE)
...
d.comment(0x81EB, "Next NMI handler address (low)", align=Align.INLINE)
...
d.comment(0x8224, "Next NMI handler address (low)", align=Align.INLINE)
d.comment(0x8226, "Next NMI handler address (high)", align=Align.INLINE)
...
d.comment(0x8314, "Next NMI handler address (low)", align=Align.INLINE)
d.comment(0x8316, "Next NMI handler address (high)", align=Align.INLINE)
...
d.comment(0x83F1, "Next NMI handler address (low)", align=Align.INLINE)
d.comment(0x83F3, "Next NMI handler address (high)", align=Align.INLINE)
...
d.label(0x8405, "rts_discard_reset")
...
d.comment(0x8456, "CONTINUE(&87)/mc-type(&88) skip protection", align=Align.INLINE)
...
d.label(0x8533, "rts_advance_buf")
...
d.comment(0x8721, "Next NMI handler address (low)", align=Align.INLINE)
d.comment(0x8723, "Next NMI handler address (high)", align=Align.INLINE)
...
d.subroutine(
    ...
    description="""Called via NMI after the frame (including CRC and closing flag) has been fully transmitted.
...
| Flag                          | Next handler                                   |
|-------------------------------|------------------------------------------------|
| bit 6 set (broadcast)         | [`tx_result_ok`](address:88C6) – skip the ACK  |
| bit 0 set (handshake pending) | [`handshake_await_ack`](address:886E)          |
| both clear                    | install [`nmi_reply_scout`](address:8744) for scout-ACK reception |""",
)
...
d.comment(0x873F, "Next NMI handler address (low)", align=Align.INLINE)
...
d.subroutine(
    ...
    description="""Receives the **scout ACK** – the 4-byte acknowledgement frame the remote station sends back after our scout. This is step 2 of the four-way handshake (`CR1` previously written to &82 by [`nmi_tx_complete`](address:8728)). The frame layout is identical to a scout: `[dst_stn][dst_net][src_stn][src_net]`.
...
d.comment(0x8753, "Next NMI handler address (low)", align=Align.INLINE)
...
d.subroutine(
    ...
    description="""Second byte of the scout ACK. Reads the destination network and validates it is zero (local network). Installs [`nmi_reply_validate`](address:876F) for the remaining two bytes (source station and network).

Optimisation: checks `SR1` bit 7 (IRQ still asserted) via `BMI` at &8767. If IRQ is still set the next byte has already arrived, so the routine falls through directly to [`nmi_reply_validate`](address:876F) without an `RTI`, avoiding NMI re-entry overhead for short frames where all bytes arrive in quick succession.""",
)
...
d.subroutine(
    0x876F,
    "nmi_reply_validate",
    title="Scout-ACK validation, then begin data TX (step 3 of handshake)",
    description="""Validates the source station/network of the scout ACK against the original TX destination, then switches back to TX mode for the **data frame** (step 3 of the four-way handshake).

Validation sequence:

1. `SR2` bit 7 (`RDA`) at &876F – must see data available
2. Read source station at &8774, compare to `tx_dst_stn` (&0D20)
3. Read source network at &877C, compare to `tx_dst_net` (&0D21)
4. `SR2` bit 1 (`FV`) at &8786 – must see frame valid

On success, writes `CR2 = &A7` then `CR1 = &44` to switch the ADLC back to TX mode and begin transmitting the data frame. The `CR1 = &44` write re-asserts `RX_RESET` (bit 6), which means the *data ACK* (step 4) cannot be received until the next [`nmi_tx_complete`](address:8728) writes `CR1 = &82` again. The full `CR1` walk through the handshake:

| Step | `CR1` | Phase                  | Routine fires                                |
|------|-------|------------------------|----------------------------------------------|
| 1    | &44   | scout TX               | [`tx_prepare`](address:8643?hex)             |
| 2    | &82   | await scout ACK        | [`nmi_tx_complete`](address:8728)            |
| 3    | &44   | data TX                | [`nmi_reply_validate`](address:876F) (here) |
| 4    | &82   | await data ACK         | [`handshake_await_ack`](address:886E)        |""",
)
...
d.comment(0x87B0, "Next NMI handler address (low)", align=Align.INLINE)
d.comment(0x87B2, "Next NMI handler address (high)", align=Align.INLINE)
...
d.comment(0x87CE, "Install nmi_data_tx alt-entry (low)", align=Align.INLINE)
d.comment(0x87D0, "Install nmi_data_tx alt-entry (high)", align=Align.INLINE)
d.comment(0x87D2, "Install and return via set_nmi_vector", align=Align.INLINE)
d.label(0x87D5, "install_imm_data_nmi")

d.comment(0x87D5, "Next NMI handler address (low)", align=Align.INLINE)
d.comment(0x87D7, "Next NMI handler address (high)", align=Align.INLINE)
...
d.comment(0x881D, "Next NMI handler address (low)", align=Align.INLINE)
d.comment(0x881F, "Next NMI handler address (high)", align=Align.INLINE)
...
d.comment(0x8873, "Next NMI handler address (low)", align=Align.INLINE)
d.comment(0x8875, "Next NMI handler address (high)", align=Align.INLINE)
...
d.comment(0x8889, "Next NMI handler address (low)", align=Align.INLINE)
...
d.label(0x8B04, "rts_save_text_ptr")
...
for caller convenience.""",
    on_exit={"a": "workspace page number", "y": "workspace page number (same as A)"},
)
...
d.label(0x8CBE, "rts_setup_ws_ptr")
...
d.label(0x8E09, "rts_station_match")
...
d.word(0x8E74)
d.expr(0x8E74, sym("netv_handler"))
...
d.label(0x8E95, "rts_svc_1_workspace")
...
d.label(0x8FB1, "rts_fs_shutdown")
...
for i in range(13):
    d.byte(0x910E + i)
d.expr(0x910E, sym("syn_iam") - sym("cmd_syntax_strings") - 2)
d.comment(
    0x910E,
...
d.expr(0x9110, sym("syn_iam") - sym("cmd_syntax_strings") - 1)
d.comment(0x9110, 'Idx 2: "(<stn.id.>) <user id.>..."', align=Align.INLINE)
d.expr(0x9111, sym("syn_object") - sym("cmd_syntax_strings") - 1)
d.comment(0x9111, 'Idx 3: "<object>"', align=Align.INLINE)
d.expr(0x9112, sym("syn_file_offset") - sym("cmd_syntax_strings") - 1)
d.comment(0x9112, 'Idx 4: "<filename> (<offset>...)"', align=Align.INLINE)
d.expr(0x9113, sym("syn_dir") - sym("cmd_syntax_strings") - 1)
d.comment(0x9113, 'Idx 5: "<dir>"', align=Align.INLINE)
d.expr(0x9114, sym("syn_dir_num") - sym("cmd_syntax_strings") - 1)
d.comment(0x9114, 'Idx 6: "<dir> (<number>)"', align=Align.INLINE)
d.expr(0x9115, sym("syn_password") - sym("cmd_syntax_strings") - 1)
d.comment(0x9115, 'Idx 7: "(:<CR>) <password>..."', align=Align.INLINE)
d.expr(0x9116, sym("syn_ps_type") - sym("cmd_syntax_strings") - 1)
d.comment(0x9116, 'Idx 8: "(<stn.id.>|<ps type>)"', align=Align.INLINE)
d.expr(0x9117, sym("syn_access") - sym("cmd_syntax_strings") - 1)
d.comment(0x9117, 'Idx 9: "<object> (L)(W)(R)..."', align=Align.INLINE)
d.expr(0x9118, sym("syn_rename") - sym("cmd_syntax_strings") - 1)
d.comment(0x9118, 'Idx 10: "<filename> <new filename>"', align=Align.INLINE)
d.expr(0x9119, sym("syn_opt_stn") - sym("cmd_syntax_strings") - 1)
d.comment(0x9119, 'Idx 11: "(<stn. id.>)"', align=Align.INLINE)
d.expr(0x911A, sym("syn_filename") - sym("cmd_syntax_strings") - 1)
...
cmd_fs_operation and cmd_rename.""",
    on_exit={"x": "TX buffer offset past name+space", "y": "command line offset (restored)"},
)
...
d.label(0x9334, "rts_copy_cmd_name")
...
d.label(0x9603, "rts_cond_save_err")
...
for i in range(12):
    d.byte(0x9798 + i)
d.expr(0x9798, sym("error_msg_table") - sym("error_msg_table"))
d.comment(
    0x9798,
...
loads the FCB flag byte from &1030+X.""",
    ...
    on_exit={"a": "FCB flag byte", "x": "channel slot index"},
)
...
d.comment(0xA402, "Syn 14: (protection-type keywords)", align=Align.INLINE)
...
d.comment(0xA41E, "Syn 14: (protection-type keywords)", align=Align.INLINE)
...
d.byte(0xA4D5)
d.comment(0xA4D5, "End of protection-type keyword table", align=Align.INLINE)
...
d.label(0xA507, "rts_osword_setup")
...
Fire-and-forget from the caller's perspective: the routine sets up `nmi_tx_block` and the OSWORD parameter block, then `JMP`s to [`tx_begin`](address:8582) and the four-way handshake runs entirely under NMI ([`nmi_tx_data`](address:86E0) → `tx_last_data` → [`nmi_tx_complete`](address:8728) → [`nmi_reply_scout`](address:8744)). This bypasses [`send_net_packet`](address:982A) and [`poll_adlc_tx_status`](address:98B4), which poll synchronously for completion – there is no retry loop here.
...
3. Copy the 15 parameter bytes from the OSWORD PB into the TX workspace via [`copy_pb_byte_to_ws`](address:A6FB).
4. `JMP tx_begin`.
...
d.entry(0xA58B)
d.comment(0xA58B, "Test tx_complete_flag (bit 7 = TX in progress)", align=Align.INLINE)
...
d.label(0xA72A, "rts_write_ws_pair")
...
d.label(0xA8CF, "rts_bridge_poll")
...
d.label(0xAA23, "rts_claim_release")
...
d.label(0xAADA, "rts_spool_reset")
...
d.label(0xAEC8, "rts_strip_prefix")
...
d.label(0xAF94, "rts_print_digit")
...
d.comment(0xB2F4, "Not CR: protection-type keywords follow", align=Align.INLINE)
...
d.comment(0xB2FE, "X=&D3: protection-type keyword table offset", align=Align.INLINE)
...
d.comment(0xB304, "Match protection-type keyword in table", align=Align.INLINE)
...
d.comment(0xB316, "No: invalid protection-type keyword", align=Align.INLINE)
...
d.comment(0xB32B, "X=&D3: protection-type keyword table offset", align=Align.INLINE)
...
d.comment(0xB331, "Match protection-type keyword in table", align=Align.INLINE)
...
d.label(0xB79E, "rts_inc_fcb_count")
...
d.expr(0x83F2, lo(sym("nmi_rx_scout")))
d.expr(0x83F4, hi(sym("nmi_rx_scout")))

d.expr(0x80CC, lo(sym("nmi_rx_scout_net")))
d.expr(0x81D8, lo(sym("nmi_data_rx")))
d.expr(0x81EC, lo(sym("nmi_data_rx_net")))
d.expr(0x8225, lo(sym("nmi_data_rx_tube")))
d.expr(0x8227, hi(sym("nmi_data_rx_tube")))
d.expr(0x8315, lo(sym("nmi_ack_tx_src")))
d.expr(0x8317, hi(sym("nmi_ack_tx_src")))
d.expr(0x8722, lo(sym("nmi_tx_complete")))
d.expr(0x8724, hi(sym("nmi_tx_complete")))
d.expr(0x8740, lo(sym("nmi_reply_scout")))
d.expr(0x8754, lo(sym("nmi_reply_cont")))
d.expr(0x87B1, lo(sym("nmi_scout_ack_src")))
d.expr(0x87B3, hi(sym("nmi_scout_ack_src")))
d.expr(0x87D6, lo(sym("nmi_data_tx_tube")))
d.expr(0x87D8, hi(sym("nmi_data_tx_tube")))
d.expr(0x881E, lo(sym("discard_reset_rx")))
d.expr(0x8820, hi(sym("discard_reset_rx")))
d.expr(0x8874, lo(sym("nmi_final_ack")))
d.expr(0x8876, hi(sym("nmi_final_ack")))
d.expr(0x888A, lo(sym("nmi_final_ack_net")))

d.label(0x8102, "nmi_scout_data")
d.label(0x87E4, "nmi_data_tx_alt")
d.expr(0x80EC, lo(sym("nmi_scout_data")))
d.expr(0x80EE, hi(sym("nmi_scout_data")))
d.expr(0x87CF, lo(sym("nmi_data_tx_alt")))
d.expr(0x87D1, hi(sym("nmi_data_tx_alt")))

# --- character-literal immediate operands ---
d.char_literal(0x8A74)
d.char_literal(0x8A7B)
d.char_literal(0x8A82)
d.char_literal(0x8BC0)
d.char_literal(0x8C39)
d.char_literal(0x8C6C)
d.char_literal(0x8DBF)
d.char_literal(0x8FF7)
d.char_literal(0x912D)
d.char_literal(0x9161)
d.char_literal(0x916D)
d.char_literal(0x9171)
d.char_literal(0x9175)
d.char_literal(0x9179)
d.char_literal(0x919D)
d.char_literal(0x91A1)
d.char_literal(0x91E2)
d.char_literal(0x9245)
d.char_literal(0x9249)
d.char_literal(0x924D)
d.char_literal(0x9251)
d.char_literal(0x92F9)
d.char_literal(0x932B)
d.char_literal(0x933E)
d.char_literal(0x9345)
d.char_literal(0x9352)
d.char_literal(0x935C)
d.char_literal(0x939F)
d.char_literal(0x93AF)
d.char_literal(0x93CC)
d.char_literal(0x93D7)
d.char_literal(0x93E3)
d.char_literal(0x93F2)
d.char_literal(0x962E)
d.char_literal(0x9632)
d.char_literal(0x96B6)
d.char_literal(0x9739)
d.char_literal(0x974C)
d.char_literal(0x975E)
d.char_literal(0x978A)
d.char_literal(0x9A5C)
d.char_literal(0x9B90)
d.char_literal(0x9B9A)
d.char_literal(0x9BC3)
d.char_literal(0x9BCA)
d.char_literal(0xA0DA)
d.char_literal(0xA0F5)
d.char_literal(0xA14A)
d.char_literal(0xA172)
d.char_literal(0xA198)
d.char_literal(0xA19C)
d.char_literal(0xA1A3)
d.char_literal(0xA1F5)
d.char_literal(0xA294)
d.char_literal(0xA5C6)
d.char_literal(0xA5EF)
d.char_literal(0xAE89)
d.char_literal(0xAE9B)
d.char_literal(0xAEBB)
d.char_literal(0xAECA)
d.char_literal(0xAED8)
d.char_literal(0xAEFD)
d.char_literal(0xAEFF)
d.char_literal(0xAF06)
d.char_literal(0xAF8A)
d.char_literal(0xAFA4)
d.char_literal(0xAFB1)
d.char_literal(0xB011)
d.char_literal(0xB070)
d.char_literal(0xB0F1)
d.char_literal(0xB17D)
d.char_literal(0xB1EB)
d.char_literal(0xB232)
d.char_literal(0xB2BE)
d.char_literal(0xB2E0)
d.char_literal(0xB2E4)
d.char_literal(0xB384)
d.char_literal(0xB38D)
d.char_literal(0xB3B8)
d.char_literal(0xB3DE)
d.char_literal(0xB3F5)
d.char_literal(0xB3F7)
d.char_literal(0xB41A)
d.char_literal(0xB448)
d.char_literal(0xB46B)
d.char_literal(0xB46F)
d.char_literal(0xBA81)
d.char_literal(0xBAAC)
d.char_literal(0xBAB0)
d.char_literal(0xBAE9)
d.char_literal(0xBB49)
d.char_literal(0xBB50)
d.char_literal(0xBB6B)
