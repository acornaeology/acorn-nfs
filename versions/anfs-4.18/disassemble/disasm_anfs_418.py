...
_osword_handler_descs = [
    "OSWORD &0E: read clock",
    "OSWORD &0F: (unimplemented)",
    "OSWORD &10: transmit",
    "OSWORD &11: receive",
    "OSWORD &12: read station info",
    "OSWORD &13: misc operations",
    "OSWORD &14: bridge/net config",
]
_tube_r2_entries = [
...
_cmd_entries = [
    ...
    (0xA469, "cmd_flip"),
    (0xA46E, "cmd_fs"),
    (0xA475, "cmd_fs_operation"),
    (0xA47C, "cmd_iam"),
    (0xA483, "cmd_lcat"),
    (0xA489, "cmd_lex"),
    (0xA48F, "cmd_fs_operation"),
    (0xA496, "cmd_pass"),
    (0xA49F, "cmd_remove"),
    (0xA4A8, "cmd_rename"),
    (0xA4AF, "cmd_wipe"),
...
_attr_entries = [
    (0xA4C7, 0xA4C8, 0xA4C9),
    (0xA4CD, 0xA4CE, 0xA4CF),
    (0xA4D4, 0xA4D5, 0xA4D6),
    (0xA4DB, 0xA4DC, 0xA4DD),
    (0xA4E2, 0xA4E3, 0xA4E4),
    (0xA4EA, 0xA4EB, 0xA4EC),
]
handler_names = [
    ("FILEV", 0x9935),
    ("ARGSV", 0x9BBE),
...
d.comment(0x80D3, "Store broadcast flag in net_frame_flags", align=Align.INLINE)
d.label(0x80D6, "accept_frame")

d.comment(0x80D6, "Next NMI handler address (low)", align=Align.INLINE)
...
d.comment(0x817D, "&7F = RXCB still open (&FF once completed)", align=Align.INLINE)
...
d.comment(0x81E2, "Next NMI handler address (low)", align=Align.INLINE)
...
d.comment(0x81F6, "Next NMI handler address (low)", align=Align.INLINE)
...
d.comment(0x822F, "Next NMI handler address (low)", align=Align.INLINE)
d.comment(0x8231, "Next NMI handler address (high)", align=Align.INLINE)
...
**Setting bit 7 also closes the block.** The slot
scanner at [`scout_ctrl_check`](label:scout_ctrl_check)
only accepts a slot whose control byte is exactly
&7F; the `ORA #&80` at &83D8 turns it into &FF, so
from this instruction onwards the slot matches
nothing. An RXCB is one-shot.

The consequence is that the ROM has no duplicate
detection on inbound frames. A retransmission of a
reply the ROM has already consumed walks the port
list, matches no open slot, and is discarded
silently at [`discard_no_match`](label:discard_no_match)
— no ACK, no NAK, no error. On a real Econet wire
this cannot arise: the four-way handshake is
synchronous and retransmission is handled beneath
the ROM. The ROM assumes the layer below it
de-duplicates, which is an assumption a datagram
transport such as AUN over UDP does not satisfy.""",
)
...
d.label(0x840F, "rts_discard_reset")
...
d.label(0x853D, "rts_advance_buf")

d.comment(0x853D, "Return", align=Align.INLINE)
d.subroutine(
    0x853E,
    "tx_done_dispatch_lo",
    title="TX-done dispatch table (lo bytes, 5 entries)",
    description="""Low bytes of `PHA`/`PHA`/`RTS` dispatch targets for TX-done
...
d.label(0x85E3, "tx_dcd_clock_check")

d.comment(0x85E3, "A=&20: mask for SR2 DCD (clock/carrier detect)", align=Align.INLINE)
d.comment(0x85E5, "BIT SR2: test DCD -- is there a clock?", align=Align.INLINE)
d.comment(0x85E8, "DCD set: no clock on the line, abandon TX", align=Align.INLINE)
...
**And why the SR1 read at &860A precedes it.** CLR Rx ST
...
has in fact read both registers by this point — SR2 via the BIT
at &8605 and SR1 explicitly at &860A — so it is correct whether
the datasheet's "last read status operation" is tracked per
register or globally. The SR1 read is therefore a prerequisite
of the clear, not merely an interrupt acknowledgement.""",
)
...
d.comment(0x860A, "Read SR1 -- arms the CLR_RX_ST below", align=Align.INLINE)
...
d.subroutine(
    0x8681,
    "tx_ctrl_dispatch_lo",
    title="TX-ctrl dispatch table (lo bytes, 8 entries)",
...
d.comment(0x872B, "Next NMI handler address (low)", align=Align.INLINE)
d.comment(0x872D, "Next NMI handler address (high)", align=Align.INLINE)
...
d.comment(0x87BA, "Next NMI handler address (low)", align=Align.INLINE)
d.comment(0x87BC, "Next NMI handler address (high)", align=Align.INLINE)
...
d.comment(0x87DF, "Next NMI handler address (low)", align=Align.INLINE)
d.comment(0x87E1, "Next NMI handler address (high)", align=Align.INLINE)
...
d.comment(0x8827, "Next NMI handler address (low)", align=Align.INLINE)
d.comment(0x8829, "Next NMI handler address (high)", align=Align.INLINE)
...
d.comment(0x887D, "Next NMI handler address (low)", align=Align.INLINE)
d.comment(0x887F, "Next NMI handler address (high)", align=Align.INLINE)
...
d.comment(0x8893, "Next NMI handler address (low)", align=Align.INLINE)
...
d.index_base(0x89CA, "svc_dispatch_lo")
...
d.index_base(0x89EF, "svc_dispatch_hi")
d.subroutine(
    0x8A15,
    "service_handler",
...
d.label(0x8B0C, "rts_save_text_ptr")
...
d.label(0x8CC9, "rts_setup_ws_ptr")
...
d.label(0x8D37, "rts_credits_check")

d.comment(0x8D37, "Return", align=Align.INLINE)
d.index_base(0x8D38, "credits_keyword_start")
...
d.index_base(0x8D61, "ps_template_base")
...
d.label(0x8E14, "rts_station_match")
...
d.label(0x8EAB, "rts_svc_1_workspace")
...
d.label(0x8FCA, "rts_fs_shutdown")
...
d.subroutine(
    0x9022,
    "cmd_syntax_strings",
    title="*HELP command syntax strings (13 entries)",
    description="""13 null-terminated syntax help strings displayed by `*HELP`
...
d.label(0x9348, "rts_copy_cmd_name")
...
d.label(0x9619, "rts_cond_save_err")
...
d.subroutine(
    ...
    title="Raise a network error from a status byte",
    description="""Three entry points into one classifier. The status
byte is masked to an error class (`AND #7`) which indexes
[`net_error_lookup_data`](label:net_error_lookup_data); class 2
("Station") additionally appends the station address and one of
two suffixes, chosen by the **V flag**:
...
So the two halves of the compound message come from different
sources, not from different failures: "not listening" is what a
*remote reply* or the general
[`send_net_packet`](label:send_net_packet) path produces, while
"not present" is reserved for the probe that reaches the
classifier through here.
...
station reports `Station n.n not present` rather than
"not listening", even though the underlying TX result code is
the not-listening one.""",
)


d.comment(0x9641, "Load status byte from TXCB byte 0", align=Align.INLINE)
d.comment(0x9643, "Is it 'A' (&41, TX 'not listening')?", align=Align.INLINE)
d.comment(0x9645, "No: keep original", align=Align.INLINE)
d.comment(0x9647, "Yes: 'B' (&42) -- reclassify as class 2", align=Align.INLINE)
d.label(0x9649, "skip_if_not_a")

d.comment(0x9649, "CLV: V=0 selects the ' not present' suffix", align=Align.INLINE)
...
d.comment(0x964E, "BIT &FF: V=1 selects ' not listening'", align=Align.INLINE)
...
d.comment(0x9678, "V set: append ' not listening' (see &9641)", align=Align.INLINE)
...
d.comment(0x9682, "V clear: Y=&0B for ' not present' suffix", align=Align.INLINE)
...
d.label(0x97AC, "rts_store_digit")
...
At the default 255 retries the inter-attempt
delays alone come to roughly 255 x 61 ms, about
15.5 seconds, on top of however long each attempt
spends inside
[`poll_adlc_tx_status`](label:poll_adlc_tx_status)
— which is itself unbounded, so this count places
no ceiling on the total wait.""",
)
...
d.comment(0x987E, "A is always 0 here: X=0 for the ZP index", align=Align.INLINE)
...
d.index_base(0x9888, "pass_txbuf_init_table")
for i in range(12):
    d.byte(0x9888 + i)
...
d.comment(0x98C4, "X=0: ZP index for the (net_tx_ptr,x) read", align=Align.INLINE)
...
`ASL tx_complete_flag` / `BCC` pair at &98C9-&98CC
is an unbounded spin, and tx_complete_flag is set
only by the NMI completion and error paths
([`tx_store_result`](label:tx_store_result),
[`store_tx_error`](label:store_tx_error)). The
retry loop in
[`send_net_packet`](label:send_net_packet) is
therefore *not* an independent watchdog — every
one of its attempts blocks here until an NMI
arrives. If the ADLC never raises the interrupt
described at
[`nmi_error_dispatch`](label:nmi_error_dispatch),
the ROM waits forever.

The ROM's software timeouts sit either side of
this window, never inside it: the pre-transmit
INACTIVE poll times out to 'Line Jammed', and the
post-transmit reply wait in
[`wait_net_tx_ack`](label:wait_net_tx_ack) times
out (~22 s by default) to 'No reply'. The
handshake itself relies wholly on the ADLC, which
is sound on a real wire — the line always falls
idle after a frame — but leaves no backstop for
an ADLC implementation that fails to signal
Inactive Idle.""",
)
...
d.comment(0x9A47, "Loop while Y wraps to 0 — i.e. print all 12 chars", align=Align.INLINE)
...
d.index_base(0xA17C, "sep_table_data")
for i in range(9):
    d.byte(0xA17C + i)
...
d.index_base(0xA291, "library_dir_prefix")
...
d.index_base(0xA3DF, "boot_oscli_lo_table")
for i in range(4):
    d.byte(0xA3DF + i)
...
d.index_base(0xA477, "cmd_table_nfs_iam")
...
d.comment(0xA516, "Load handler address high byte (X = OSWORD index 0-6)", align=Align.INLINE)
...
d.comment(0xA520, "Y=0 from copy loop above — clear svc_state", align=Align.INLINE)
d.label(0xA522, "rts_osword_setup")
...
for i, desc in enumerate(_osword_handler_descs):
    d.comment(0xA523 + i, f"lo-&{0x0E + i:02X}: {desc.split(': ', 1)[1]}", align=Align.INLINE)
    d.comment(0xA52A + i, f"hi-&{0x0E + i:02X}: {desc.split(': ', 1)[1]}", align=Align.INLINE)

d.index_base(0xA52A, "osword_dispatch_hi_table")
...
d.label(0xA63B, "rts_osword_13")

d.comment(0xA63B, "RTS dispatches to handler", align=Align.INLINE)
d.index_base(0xA63C, "osword_13_lo_table")

for i in range(18):
    d.rts_code_ptr(0xA63C + i, 0xA64E + i)
d.index_base(0xA64E, "osword_13_hi_table")
...
d.label(0xA727, "rts_write_ws_pair")
...
d.label(0xA84C, "rts_bridge_query")

d.comment(0xA84C, "Return", align=Align.INLINE)
d.index_base(0xA84D, "bridge_txcb_init_table")
...
d.label(0xA8C4, "rts_bridge_poll")
...
d.comment(0xA911, "Re-enable IRQ and poll the TX completion flag", align=Align.INLINE)
...
d.index_base(0xA99E, "osword_handler_lo_table")
...
d.index_base(0xA9A7, "osword_handler_hi_table")
...
d.label(0xAA35, "rts_claim_release")
...
d.label(0xAA3E, "rts_match_rx_code")

d.comment(0xAA3E, "Return (Z clear = not found)", align=Align.INLINE)
d.index_base(0xAA3F, "osword_claim_codes")
for i in range(18):
    d.byte(0xAA3F + i)
...
d.index_base(0xAAB1, "ws_txcb_template_data")
for i in range(39):
    d.byte(0xAAB1 + i)
...
d.label(0xAAEC, "rts_spool_reset")
...
d.index_base(0xAC80, "tx_econet_txcb_template")
for i in range(12):
    d.byte(0xAC80 + i)
...
d.index_base(0xAC8C, "rx_palette_txcb_template")
for i in range(12):
    d.byte(0xAC8C + i)
...
d.index_base(0xAD0D, "osbyte_mode_read_codes")
for i in range(3):
    d.byte(0xAD0D + i)
...
d.label(0xAEDA, "rts_strip_prefix")
...
d.index_base(0xAEFF, "roff_off_string")
...
d.label(0xAFB4, "rts_print_digit")
...
d.index_base(0xB194, "ps_tx_header_template")
for i in range(4):
    d.byte(0xB194 + i)
...
d.comment(0xB55A, "Set V flag via BIT bit_test_ff (alternate-mode entry)", align=Align.INLINE)
...
d.comment(0xB6B0, "Store station low in FCB slot", align=Align.INLINE)
d.comment(0xB6B3, "Load working station high", align=Align.INLINE)
d.comment(0xB6B6, "Store station high in FCB slot", align=Align.INLINE)
...
d.label(0xB798, "rts_inc_fcb_count")
...
d.comment(0xBA95, "Advance Y to next column", align=Align.INLINE)
...
d.char_literal(0x8A7D)
d.char_literal(0x8A84)
d.char_literal(0x8A8B)
d.char_literal(0x8BCB)
d.char_literal(0x8C44)
d.char_literal(0x8C77)
d.char_literal(0x8DCA)
d.char_literal(0x8E1A)
d.char_literal(0x900B)
d.char_literal(0x9141)
d.char_literal(0x9175)
d.char_literal(0x9181)
d.char_literal(0x9185)
d.char_literal(0x9189)
d.char_literal(0x918D)
d.char_literal(0x91B1)
d.char_literal(0x91B5)
d.char_literal(0x91F6)
d.char_literal(0x9259)
d.char_literal(0x925D)
d.char_literal(0x9261)
d.char_literal(0x9265)
d.char_literal(0x930D)
d.char_literal(0x933F)
d.char_literal(0x9352)
d.char_literal(0x9359)
d.char_literal(0x9366)
d.char_literal(0x9370)
d.char_literal(0x93B3)
d.char_literal(0x93C3)
d.char_literal(0x93E0)
d.char_literal(0x93EB)
d.char_literal(0x93F7)
d.char_literal(0x9406)
d.char_literal(0x9644)
d.char_literal(0x9648)
d.char_literal(0x96CC)
d.char_literal(0x974E)
d.char_literal(0x9761)
d.char_literal(0x9773)
d.char_literal(0x979F)
d.char_literal(0x9A6E)
d.char_literal(0x9B9F)
d.char_literal(0x9BA9)
d.char_literal(0x9BD2)
d.char_literal(0x9BD9)
d.char_literal(0xA0F2)
d.char_literal(0xA10D)
d.char_literal(0xA162)
d.char_literal(0xA18A)
d.char_literal(0xA1B0)
d.char_literal(0xA1B4)
d.char_literal(0xA1BB)
d.char_literal(0xA20D)
d.char_literal(0xA2AC)
d.char_literal(0xA5DE)
d.char_literal(0xA607)
d.char_literal(0xAE9B)
d.char_literal(0xAEAD)
d.char_literal(0xAECD)
d.char_literal(0xAEDC)
d.char_literal(0xAEEA)
d.char_literal(0xAF0F)
d.char_literal(0xAF11)
d.char_literal(0xAF15)
d.char_literal(0xAF20)
d.char_literal(0xAFAA)
d.char_literal(0xAFC4)
d.char_literal(0xAFD1)
d.char_literal(0xB031)
d.char_literal(0xB094)
d.char_literal(0xB115)
d.char_literal(0xB1A1)
d.char_literal(0xB20F)
d.char_literal(0xB256)
d.char_literal(0xB2DA)
d.char_literal(0xB2FC)
d.char_literal(0xB300)
d.char_literal(0xB3A0)
d.char_literal(0xB3A9)
d.char_literal(0xB3CF)
d.char_literal(0xB3F0)
d.char_literal(0xB407)
d.char_literal(0xB409)
d.char_literal(0xB42C)
d.char_literal(0xB458)
d.char_literal(0xB47B)
d.char_literal(0xB47F)
d.char_literal(0xBABB)
d.char_literal(0xBABF)
d.char_literal(0xBB08)
d.char_literal(0xBB24)
d.char_literal(0xBB28)
d.char_literal(0xBB2C)
d.char_literal(0xBB72)
