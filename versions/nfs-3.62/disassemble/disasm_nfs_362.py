...
# ============================================================
# NFS 3.62 — derived from NFS 3.60
# ============================================================
# This driver was copied from the NFS 3.60 disassembly driver.
# The two ROMs are 99.7% identical at the opcode level (only
# 16 bytes differ). All code addresses are unchanged.
#
# Changes from 3.60:
# - Version byte $8023: $60 → $62
# - Version string at $820B: "NFS 3.60" → "NFS 3.62"
# - Command table data at $8C57-$8C58: 2 bytes changed
# - New code at $9FB5 replaces $FF padding (12 bytes)
...
_rom_filepath = os.environ.get(
    "ACORN_NFS_ROM",
    str(_version_dirpath / "rom" / "nfs-3.62.rom"),
)
...
label(0x8181, "tube_chars_done")       # BEQ: zero byte received, transfer complete
label(0x8183, "check_svc_12")          # Convergence before CMP #&12 test
label(0x818F, "not_svc_12_nfs")        # BNE: not service &12 or not NFS (Y!=5)
label(0x8193, "do_svc_dispatch")        # BNE: entry to TAX/workspace save/dispatch
...
label(0x81E3, "skip_kbd_reenable")      # BEQ: keyboard not disabled, skip re-enable
...
label(0x825F, "skip_no_clock_msg")     # BEQ: clock present, skip "No Clock" message
label(0x8316, "read_station_id")       # BEQ/BCS: soft break or RXCBs done, read &FE18
...
label(0x8377, "check_rom_end")        # BEQ/BNE: mismatch or space, test ROM string end
...
label(0x8405, "check_fs_error")        # BVC: standard path, BNE to store_fs_error
...
label(0x846D, "close_spool_exec")       # BEQ: SPOOL handle matched, OSCLI close
label(0x8473, "dispatch_fs_error")     # BNE: no handle match, reset ptr for FSERR
label(0x8487, "error_code_clamped")    # BCS: error >= &A8, skip clamp, start scan
...
comment(0x852C, "CR + E. + CR: *EXEC boot command", inline=True)
...
label(0x85E9, "skip_set_attrib_bit")   # BCC: source bit was 0, don't OR destination
label(0x860D, "poll_tx_semaphore")     # Spin on tx_clear_flag until TX semaphore is free
label(0x860A, "rearm_tx_attempt")     # BEQ: after retry delay, write back control byte
label(0x8637, "tx_retry_delay")       # Two-level X*Y countdown delay between TX retries
label(0x863F, "tx_abort")             # Retries exhausted or escape: TAX/JMP nlistn
label(0x8643, "tx_success_exit")      # BPL: success, PLA*3 then JMP clear_escapable
label(0x8664, "print_inline_char")     # Print inline string bytes via OSASCI until bit-7 terminator
label(0x866A, "print_next_char")      # INC lo-byte of pointer, load and print next char
label(0x8674, "jump_via_addr")        # BMI: bit 7 terminator, JMP (fs_load_addr)
label(0x867B, "scan_decimal_digit")    # Read ASCII digits and accumulate decimal value
label(0x8696, "no_dot_exit")          # BCC: char < '.', CLC then RTS (no dot found)
label(0x8697, "parse_decimal_rts")    # BEQ: char = '.', LDA result/RTS
label(0x86B3, "handle_mask_exit")     # BEQ/BNE: PLA/TAX/PLA/RTS exit restoring X and A
...
label(0x8540, "skip_rx_flag_set")       # BNE: non-zero TX hi byte, skip bit-7 set
...
label(0x86FD, "terminate_filename")   # BEQ/BCS: empty or end-of-GSREAD, append CR
...
label(0x8740, "skip_lodfil")           # BCC: skip lodfil block, proceed to compute end addr
...
label(0x87EF, "print_filename_char")  # BEQ/BNE: load and print each filename char
label(0x87FB, "pad_filename_space")   # BCC: end of name, pad with spaces to col 12
label(0x8803, "print_addresses")      # BMI: skip filename, start printing hex load/exec
label(0x8817, "skip_catalogue_msg")   # BEQ: messages flag=0, skip display
label(0x8822, "copy_attribs_reply")    # Copy 4 decoded attribute bytes from FS reply
label(0x8859, "transfer_loop_top")     # BNE: blocks remain, top of block transfer loop
...
label(0x888E, "send_block")           # BCC: no overshoot, skip clamp, send block
...
label(0x89C5, "halve_args_a")         # BEQ: A=2, LSR A then dispatch

label(0x891C, "send_fs_cmd_v1")       # BNE: after copy_filename, BIT tx_ctrl_upper
label(0x8922, "check_attrib_result")  # BCS: A>=7 unsupported, dispatch to error/return
label(0x8966, "attrib_error_exit")    # BCS: not-found/error, BPL restore_xy_return
...
label(0x8A36, "check_opt1")           # BNE: X!=4, DEX/BNE to check for *OPT 1
label(0x8A39, "set_messages_flag")     # Set *OPT 1 local messages flag from Y
label(0x8A50, "opt_return")           # BCC: *OPT 1 done, to close_opt_return
...
label(0x8A68, "subtract_adjust")      # BMI: fs_load_addr_2 negative, SBC instead of ADC
label(0x8A7D, "gbpb_invalid_exit")    # BEQ/fall: A=0 or X>=8 out of range, JMP restore_args
...
label(0x8AB9, "gbpb_write_path")      # BEQ: A=0 write, keeps X=&91/Y=&0B
label(0x8AF8, "gbpb_read_path")      # BNE: non-zero read, skip write transfer
label(0x8AFB, "wait_fs_reply")        # BCS/fall: post-transfer, JSR send_fs_reply_cmd
label(0x8B08, "skip_clear_flag")      # BMI: reply bit 7 set (not EOF), skip clear_fs_flag
label(0x8B3F, "store_tube_flag")      # BEQ/BNE: store Tube-transfer flag to l00a9
label(0x8B53, "gbpb6_read_name")      # BEQ: A=0 OSGBPB 6, load CSD/LIB/URD handle
label(0x8B7F, "copy_reply_to_caller") # Copy FS reply data to caller buffer (direct or via Tube)
label(0x8B94, "tube_transfer")        # BNE: l00a9 non-zero, claim Tube and send via R3
label(0x8BA1, "no_page_wrap")         # BNE: no page boundary crossed in copy
label(0x8BBB, "gbpb_done")            # BEQ: copy/adjust done, JMP return_a_zero
label(0x8BBE, "gbpb8_read_dir")       # BCS: C=1 OSGBPB 8, read dir entries
label(0x8BFA, "skip_copy_reply")      # BEQ: zero byte count, skip copy_reply_to_caller
...
label(0x8C45, "dispatch_cmd")         # BMI: end-of-name marker, push handler addr/dispatch

# --- *EX and *CAT handlers ---
label(0x8C72, "init_cat_params")       # Store examine arg count, init catalogue display

# --- Boot command strings and option tables ---
label(0x8CB0, "print_public")          # BNE: access non-zero (public), print "Public"+CR
label(0x8CBA, "print_user_env")       # Skip after Owner/Public, print user environment
label(0x8CE2, "print_option_name")     # Print boot option name until bit-7 terminator
label(0x8CED, "done_option_name")      # BMI: bit 7 set, option name printing complete
label(0x8D11, "fetch_dir_batch")       # Loop: store entry offset, issue next examine batch
label(0x8D33, "process_entries")      # BNE: non-zero entry count, save and process batch
...
label(0x8DB2, "print_cr")               # Column counter wraps/negative, load &0D
label(0x8DB4, "print_newline")            # Print CR via OSASCI
label(0x8DB7, "next_dir_entry")        # Always-branch: advance X, loop for next entry
...
label(0x8E29, "exec_local")            # BNE: no Tube, execute via indirect JMP locally
label(0x8E43, "copy_handles_loop")    # SDISC entry: skip boot option byte, X=3 descending
...
label(0x8F11, "set_workspace_page")   # Sub-fn 0-1: use static page &0D, store to &AC
label(0x8F22, "skip_param_write")     # C=0: skip writing param to workspace
label(0x8F69, "return_last_error")    # Sub-fn >= 10, bit 7 clear: return last FS error
label(0x8FA6, "read_rxcb")            # BNE: non-zero RXCB param, skip free-slot scan
label(0x8FD1, "reenable_rx")          # Common exit: ROL rx_flags, RTS
label(0x8FFE, "store_txcb_byte")      # BNE: non-zero template byte, skip substitution
...
label(0x903E, "handle_tx_result")     # BCS: A>=1, save flags and load RX result
label(0x9101, "dispatch_remote_osbyte") # Common target for OSBYTE dispatch setup
...
label(0x922A, "skip_flush")            # Sequence unchanged, skip flushing output block
...
label(0x9672, "dispatch_svc5")         # BCS: Y>=&86, push dispatch address via RTS
...
label(0x96D7, "accept_frame")         # Station ID matched, install next NMI handler
label(0x96F2, "accept_local_net")     # Network=0: clear broadcast marker
label(0x96F5, "accept_scout_net")     # Common accept for local/broadcast frames
label(0x976B, "scan_port_list")        # Non-broadcast: skip CR2 setup, begin port scan
label(0x9774, "scan_nfs_port_list")   # NFS workspace port list scan entry
label(0x9778, "check_port_slot")      # Loop: read port control byte, zero=end
label(0x978C, "check_station_filter") # Port matched: advance to station filter check
label(0x979E, "next_port_slot")        # Mismatch: advance pointer by 12 for next slot
label(0x97AB, "discard_no_match")     # No match found: JMP nmi_error_dispatch
label(0x97AE, "try_nfs_port_list")    # Paged list exhausted: check NFS workspace RX
label(0x97B9, "port_match_found")      # Station/network passed: record scout_status=3
label(0x97CB, "send_data_rx_ack")     # Non-broadcast: set up CR1/CR2 for TX ACK
...
label(0x982E, "install_tube_rx")      # BNE: Tube active, install Tube RX handler
...
label(0x9848, "data_rx_loop")          # Loop: check SR2, if RDA continue reading byte pairs
label(0x9858, "read_sr2_between_pairs") # After page boundary, read SR2 before second byte
label(0x985F, "read_second_rx_byte")  # BMI: SR2 bit7 set, read second byte from FIFO
label(0x986F, "check_sr2_loop_again") # After page boundary on second byte, check SR2
label(0x988E, "read_last_rx_byte")    # Multi-source: check buffer then read trailing byte
label(0x989D, "send_ack")             # No more data: unconditional JMP ack_tx
...
label(0x9992, "skip_tube_update")     # BEQ: Tube flag clear, skip Tube pointer update
label(0x99B2, "store_buf_ptr_lo")     # BCC: no carry, store updated buffer low byte
label(0x99BB, "skip_buf_ptr_update")  # BNE: transfer not done, skip buffer ptr write
...
label(0x9A19, "copy_scout_via_tube")  # Tube path: read byte and write to R3
label(0x9A34, "clear_release_flag")   # BNE: Tube already released, LSR clears flag
...
label(0x9A63, "dispatch_imm_op")      # BNE: ctrl &87/&88 bypass protection, dispatch
label(0x9A76, "imm_op_out_of_range")  # BCC/BCS: ctrl < &81 or > &88, JMP error dispatch
...
label(0x9B86, "tx_imm_op_setup")       # BMI: TXCB bit7 set, store ctrl byte for imm op
label(0x9BB1, "tx_ctrl_range_check")  # BCS: ctrl >= &83, validate &81-&88 range
label(0x9BC5, "tx_line_idle_check")   # BNE: port != 0, test SR2 INACTIVE before polling
...
label(0x9BFB, "inactive_retry")       # BEQ: INACTIVE not set, re-enable NMIs/retry
label(0x9C11, "tx_active_start")       # Begin TX (CR1=&44)
label(0x9CB0, "setup_unicast_xfer")   # BNE: not broadcast, clear flags/scout_status=2
...
label(0x9CF6, "tx_fifo_not_ready")    # BVC: TDRA clear, write CR2=&67, error &41
label(0x9CFD, "tx_store_error")       # BNE: error &42 path, merge with &9CF6
label(0x9D21, "check_handshake_bit")  # BVC: tx_flags bit6 clear, check bit0
label(0x9D2B, "install_reply_scout")  # BEQ: handshake bit0 clear, install nmi_reply_scout
...
label(0x9DC1, "install_imm_data_nmi")  # BNE: tx_flags bit1 set, install nmi_imm_data
label(0x9DCD, "data_tx_check_fifo")   # Test TDRA and write data byte pair to FIFO
label(0x9DDD, "write_second_tx_byte") # BNE: skip page check, write second FIFO byte
label(0x9DED, "check_irq_loop")       # BNE: skip page check, test SR1 IRQ for loop
...
label(0x9E12, "tube_tx_fifo_write")    # BMI: IRQ set, Tube TX FIFO write entry point
label(0x9E2A, "write_second_tube_byte") # BNE: skip carry, read/write second Tube byte
label(0x9E40, "check_tube_irq_loop")  # BNE: skip carry, test SR1 IRQ for loop
label(0x9E48, "tx_tdra_error")        # BVC: TDRA check failed, inspect tx_flags
...
label(0x9EA1, "check_fv_final_ack")    # BPL: tx_flags bit7 clear, test FV for frame end
label(0x9EE6, "calc_transfer_size")    # 4-byte subtraction to calculate actual bytes received
label(0x9F16, "restore_x_and_return") # BCC: Tube not claimed, restore X/RTS
label(0x9F19, "fallback_calc_transfer") # BEQ: no buffer/Tube, simple 2-byte subtract
label(0x9EAC, "tx_result_fail")        # Store result=&41 (not listening) (9 refs)

label(0x9F5C, "poll_nmi_idle")          # BNE: spin reading nmi_jmp until idle (&96BF)
...
label(0x9FAE, "add_ascii_base")        # BCC: nibble 0-9, ADC #&30 converts to ASCII
...
entry(0x06DA)   # IRQ service: check SR, dispatch TX done handlers
...
comment(0x85EC, "Attribute bit mapping table (11 entries)", inline=True)
...
comment(0x9FB5, "Load next char from command line", inline=True)
comment(0x9FB7, "Printable non-space character?", inline=True)
comment(0x9FB9, "Yes: not *EX, forward to FS", inline=True)
comment(0x9FBB, "No: handle *EX locally", inline=True)
label(0x9FBE, "forward_ex_to_fs")
comment(0x9FBE, "Forward *EXEC etc. to fileserver", inline=True)
comment(0x9FC1, "&FF padding (end of ROM)", inline=True)
...
comment(0x8235, 'Inline string "Econet Station "', inline=True)
...
comment(0x8255, 'Inline string " No Clock"', inline=True)
...
byte(0x8C4D)                            # I. handler hi
byte(0x8C4E)                            # I. handler lo
string(0x8C4F, 4)                       # "I AM" command string
byte(0x8C53)                            # I AM handler hi
byte(0x8C54)                            # I AM handler lo
string(0x8C55, 2)                       # "EX" command string
byte(0x8C57)                            # EX handler hi
byte(0x8C58)                            # EX handler lo
string(0x8C59, 3)                       # "BYE" command string
byte(0x8C5C)                            # CR terminator
byte(0x8C5D)                            # BYE handler hi
byte(0x8C5E)                            # BYE handler lo
byte(0x8C5F)                            # Catch-all handler hi
byte(0x8C60)                            # Catch-all handler lo
comment(0x8C4D, "I. handler hi → &80C1 (forward_star_cmd)", inline=True)
comment(0x8C4E, "I. handler lo", inline=True)
comment(0x8C4F, '"I AM" command string', inline=True)
comment(0x8C53, "I AM handler hi → &8082 (i_am_handler)", inline=True)
comment(0x8C54, "I AM handler lo", inline=True)
comment(0x8C55, '"EX" command string', inline=True)
comment(0x8C57, "EX handler hi → &9FB5 (ex_trampoline)", inline=True)
comment(0x8C58, "EX handler lo", inline=True)
comment(0x8C59, '"BYE" command string', inline=True)
comment(0x8C5C, "CR terminator for BYE", inline=True)
comment(0x8C5D, "BYE handler hi → &83C0 (bye_handler)", inline=True)
comment(0x8C5E, "BYE handler lo", inline=True)
comment(0x8C5F, "Catch-all hi → &80C1 (forward_star_cmd)", inline=True)
comment(0x8C60, "Catch-all lo", inline=True)
...
comment(0x8C90, 'Inline string "("', inline=True)
...
comment(0x8CDD, 'Inline string " ("', inline=True)
...
comment(0x8CF0, 'Inline string ")" + CR + "Dir. "', inline=True)
...
comment(0x8D6C, '"Exec" option name string', inline=True)
...
comment(0x8EB9, "lo(osword_10_handler-1): OSWORD &10", inline=True)
...
comment(0x90A5, "Push low byte of handler address", inline=True)
...
subroutine(0x913C, "match_osbyte_code", hook=None,
...
for addr in range(0x9145, 0x9154):
    byte(addr)
comment(0x9145, "OSBYTE &04: cursor key status", inline=True)
comment(0x9146, "OSBYTE &09: flash duration (1st colour)", inline=True)
comment(0x9147, "OSBYTE &0A: flash duration (2nd colour)", inline=True)
comment(0x9148, "OSBYTE &15: flush specific buffer", inline=True)
comment(0x9149, "OSBYTE &9A: video ULA control register", inline=True)
comment(0x914A, "OSBYTE &9B: video ULA palette", inline=True)
comment(0x914B, "OSBYTE &E1: function key &C0-&CF", inline=True)
comment(0x914C, "OSBYTE &E2: function key &D0-&DF", inline=True)
comment(0x914D, "OSBYTE &E3: function key &E0-&EF", inline=True)
comment(0x914E, "OSBYTE &E4: function key &F0-&FF", inline=True)
comment(0x914F, "OSBYTE &0B: auto-repeat delay", inline=True)
comment(0x9150, "OSBYTE &0C: auto-repeat rate", inline=True)
comment(0x9151, "OSBYTE &0F: flush buffer class", inline=True)
comment(0x9152, "OSBYTE &79: keyboard scan from X", inline=True)
comment(0x9153, "OSBYTE &7A: keyboard scan from 16", inline=True)
...
comment(0x9B03, "IER bit 2: disable SR interrupt", inline=True)
comment(0x9B05, "Write IER to disable SR", inline=True)
...
comment(0x9B20, "Immediate op port number table", inline=True)
...
comment(0x9879, "Write CR1", inline=True)
comment(0x987E, "Write CR2", inline=True)
...
comment(0x9C63, "TX timeout/retry parameter table", inline=True)
...
comment(0x9D79, "Write CR2: enable RTS for TX handshake", inline=True)
comment(0x9D7C, "CR1=&44: RX_RESET | TIE (TX active for scout ACK)", inline=True)
comment(0x9D7E, "Write CR1: reset RX, enable TX interrupt", inline=True)
...
comment(0x9D83, "High byte &9E of next handler address", inline=True)
comment(0x9D85, "Store low byte to nmi_next_lo", inline=True)
comment(0x9D88, "Store high byte to nmi_next_hi", inline=True)
...
comment(0x9D96, "Load dest network for scout ACK TX", inline=True)
comment(0x9D99, "Write dest network to TX FIFO", inline=True)
comment(0x9D9C, "Install nmi_scout_ack_src at &9DA3", inline=True)
comment(0x9D9E, "High byte &9D of handler address", inline=True)
comment(0x9DA0, "Set NMI vector and return", inline=True)
...
comment(0x81AF, "Return to MOS service handler", inline=True)
comment(0x81B1, "NOP padding for command table", inline=True)
comment(0x81B2, "NOP padding", inline=True)
comment(0x81B3, "NOP padding", inline=True)
comment(0x81B4, "NOP padding", inline=True)
comment(0x827B, "A=&8F: issue service request", inline=True)
comment(0x827D, "X=&0F: 'vectors claimed' service", inline=True)
comment(0x8282, "X=&0A: service &0A", inline=True)
comment(0x8289, "Non-zero: skip auto-boot", inline=True)
comment(0x828D, "Y=&82: ROM page high byte", inline=True)
comment(0x828F, "Execute command string at (X, Y)", inline=True)
comment(0x829C, "ARGSV dispatch lo", inline=True)
comment(0x829F, "BGETV dispatch hi", inline=True)
comment(0x82A0, "BPUTV dispatch lo", inline=True)
comment(0x82A2, "GBPBV dispatch lo", inline=True)
comment(0x82A3, "GBPBV dispatch hi", inline=True)
comment(0x82A4, "FINDV dispatch lo", inline=True)
comment(0x82A5, "FINDV dispatch hi", inline=True)
comment(0x82A6, "FSCV dispatch lo", inline=True)
comment(0x82BB, "FSCV handler hi", inline=True)
comment(0x82C2, "Return (workspace claim done)", inline=True)
comment(0x82C3, "FS page hi:lo for workspace pointer", inline=True)
comment(0x8353, "Load FS state byte at offset Y", inline=True)
comment(0x8356, "Store to workspace backup area", inline=True)
comment(0x8358, "Next byte down", inline=True)
comment(0x835B, "Loop for offsets &1D..&15", inline=True)
comment(0x835D, "A=&7B: printer driver going dormant", inline=True)
comment(0x8386, "Mark TX semaphore as available", inline=True)
comment(0x8394, "Return after port setup", inline=True)
comment(0x83AD, "Control flag", inline=True)
comment(0x83AE, "Port (FS command = &99)", inline=True)
comment(0x83B1, "Buffer start low", inline=True)
comment(0x83B2, "Buffer start high (page &0F)", inline=True)
comment(0x83B3, "Buffer start pad (4-byte Econet addr)", inline=True)
comment(0x83B4, "Buffer start pad", inline=True)
comment(0x83B5, "Buffer end low", inline=True)
comment(0x83B6, "Buffer end high (page &0F)", inline=True)
comment(0x83B7, "Buffer end pad", inline=True)
comment(0x83B8, "Buffer end pad", inline=True)
subroutine(0x83B9, "prepare_cmd_with_flag", hook=None,
...
comment(0x849E, "Transfer to X for return", inline=True)
comment(0x84E2, "A=0: zero execution header bytes", inline=True)
comment(0x84E7, "Next byte", inline=True)
comment(0x84E8, "Loop until all zeroed", inline=True)
comment(0x8579, "Set EOF flag for this handle", inline=True)
label(0x857C, "load_handle_mask")      # Load handle bitmask for return
comment(0x857C, "Load handle bitmask for caller", inline=True)
comment(0x857F, "Return with handle mask in A", inline=True)
comment(0x858D, "Error &A1: Net Error", inline=True)
comment(0x858E, 'Error string "Net Error"', inline=True)
comment(0x8598, "Error &A2: Not listening", inline=True)
comment(0x8599, 'Error string "Not listening"', inline=True)
comment(0x85A7, "Error &CB: Bad Option", inline=True)
comment(0x85A8, 'Error string "No Clock"', inline=True)
comment(0x85B1, "Error &11: Escape", inline=True)
comment(0x85B2, 'Error string "Escape"', inline=True)
comment(0x85B9, "Error &CB: Bad Option", inline=True)
comment(0x85BA, 'Error string "Bad Option"', inline=True)
comment(0x85C5, "ALWAYS branch to shared bitmask builder", inline=True)
comment(0x85C6, 'Error string "No reply"', inline=True)
comment(0x865D, "Store return addr low as string ptr", inline=True)
comment(0x8660, "Store return addr high as string ptr", inline=True)
comment(0x8662, "Y=0: offset for indirect load", inline=True)
comment(0x8666, "No page wrap: skip high byte inc", inline=True)
comment(0x8668, "Handle page crossing in pointer", inline=True)
comment(0x8671, "Continue printing next character", inline=True)
comment(0x8679, "Initialise accumulator to zero", inline=True)
comment(0x8699, "Return with result in A", inline=True)
comment(0x86B4, "Transfer mask to X for return", inline=True)
comment(0x86B6, "Return with mask in X", inline=True)
comment(0x86BE, "Return (identity: no conversion)", inline=True)
comment(0x86C7, "Next byte", inline=True)
comment(0x86CA, "Return with Z flag result", inline=True)
comment(0x86CF, "Return (FSCV 7 read handles)", inline=True)
comment(0x870C, "Save A/X/Y in FS workspace", inline=True)
comment(0x8718, "A=&FF: branch to load path", inline=True)
comment(0x871D, "Copy parsed filename to cmd buffer", inline=True)
comment(0x8720, "Y=2: FS function code offset", inline=True)
label(0x8814, "send_fs_reply")        # Sends FS reply command after transfer
comment(0x8814, "Send FS reply acknowledgement", inline=True)
comment(0x883D, "Add 1 (of 5) to Y", inline=True)
comment(0x883E, "Add 2 (of 5) to Y", inline=True)
comment(0x883F, "Add 3 (of 5) to Y", inline=True)
comment(0x8849, "Next byte (descending)", inline=True)
comment(0x884C, "Loop until offset 2 reached", inline=True)
comment(0x884F, "Subtract 1 (of 3) from Y", inline=True)
comment(0x8850, "Subtract 2 (of 3) from Y", inline=True)
comment(0x8851, "Subtract 3 (of 3) from Y", inline=True)
comment(0x8852, "Return to caller", inline=True)
label(0x8885, "clamp_dest_setup")     # Set up 4-byte dest clamping
comment(0x8A0A, "A=handle bitmask for new file", inline=True)
comment(0x8A71, "Return (unsupported function)", inline=True)
comment(0x8D0E, "Print two CRs (blank line)", inline=True)
label(0x8D1B, "cat_examine_loop")      # Send examine request and display entries
comment(0x8D53, "Return from column separator", inline=True)
comment(0x8D54, "Option name offsets (4 entries)", inline=True)
comment(0x8D57, "Opt 1 (Load): L.!BOOT at &8D46", inline=True)
comment(0x8D58, "Opt 2 (Run): !BOOT at &8D48", inline=True)
comment(0x8D5B, '"BOOT" string for boot option 2', inline=True)
comment(0x8D5F, "CR terminator for BOOT string", inline=True)
comment(0x8D60, "Load byte from parameter block", inline=True)
comment(0x8D67, "Loop until 4 bytes printed", inline=True)
comment(0x8D84, "Start copying from offset 0", inline=True)
comment(0x8D86, "Load next byte from source string", inline=True)
comment(0x8D88, "Store to command buffer", inline=True)
comment(0x8D8B, "Advance write position", inline=True)
comment(0x8D8C, "Advance read position", inline=True)
label(0x8E2A, "exec_at_load_addr")    # JMP (fs_load_vector) — execute locally
comment(0x8E59, "X = stack pointer", inline=True)
comment(0x8E63, "Invalid: Y = 0", inline=True)
comment(0x8E65, "A = 0, C set (error)", inline=True)
comment(0x8E66, "Return after calculation", inline=True)
comment(0x8E67, "Y=&6F: RX buffer handle offset", inline=True)
comment(0x8E69, "Read handle from RX packet", inline=True)
comment(0x8E6B, "Valid handle: store and return", inline=True)
comment(0x8E8B, "Outside our OSWORD range, exit", inline=True)
comment(0x8F1E, "Load byte from param block", inline=True)
comment(0x8F20, "Store to workspace", inline=True)
comment(0x8F22, "Load byte from workspace", inline=True)
comment(0x8F27, "Decrement byte counter", inline=True)
comment(0x8F28, "Loop while X >= 0", inline=True)
comment(0x8F87, "Load from ROM template (zero = use NMI workspace value)", inline=True)
comment(0x8FB2, "Enable interrupts before transmit", inline=True)
comment(0x8FB8, "Dest station = &FFFF (accept reply from any station)", inline=True)
comment(0x8FCA, "Initiate receive with timeout", inline=True)
comment(0x8FDB, "Receive data blocks until command byte = &00 or &0D", inline=True)
comment(0x9014, "Retrieve original A (function code) from stack", inline=True)
comment(0x9044, "ROR/ASL on stacked P: zeros carry to signal success", inline=True)
comment(0x917A, "Load template byte from ctrl_block_template[X]", inline=True)
comment(0x919C, "V=1: use (net_rx_ptr) page", inline=True)
...
label(0x91A2, "rxcb_matched")         # Scout frame matched RXCB
comment(0x91B9, "PAGE byte → Y=&02 / Y=&74", inline=True)
comment(0x91B9, "→ Y=&0D (main only)", inline=True)
comment(0x91BA, "→ Y=&03 / Y=&75", inline=True)
comment(0x91BB, "→ Y=&04 / Y=&76", inline=True)
comment(0x91BB, "SKIP (main only)", inline=True)
comment(0x91BC, "→ Y=&05 / Y=&77", inline=True)
comment(0x91BC, "→ Y=&10 (main only)", inline=True)
comment(0x91BF, "→ Y=&08 / Y=&7A", inline=True)
comment(0x91C0, "→ Y=&09 / Y=&7B", inline=True)
comment(0x91C1, "→ Y=&0A / Y=&7C", inline=True)
comment(0x91C1, "PAGE byte → Y=&15 (main only)", inline=True)
comment(0x91C2, "STOP — main-path boundary", inline=True)
comment(0x91C2, "→ Y=&16 (main only)", inline=True)
comment(0x91C5, "SKIP (main only)", inline=True)
comment(0x91C8, "PAGE byte → Y=&11 (main only)", inline=True)
comment(0x91C9, "→ Y=&12 (main only)", inline=True)
comment(0x91CA, "→ Y=&13 (main only)", inline=True)
comment(0x91CB, "→ Y=&14 (main only)", inline=True)
comment(0x91CE, "→ Y=&17 (main only)", inline=True)
comment(0x931D, "Return after storing result", inline=True)
comment(0x931E, "OSBYTE &85: read cursor position", inline=True)
comment(0x9320, "OSBYTE &C3: read screen start address", inline=True)
comment(0x9662, "CR terminator for VDU state data", inline=True)
comment(0x96D4, "Clear TX flags for new reception", inline=True)
comment(0x96D7, "Install next NMI handler at &9715 (RX scout second byte)", inline=True)
comment(0x96D9, "Install next handler and RTI", inline=True)
comment(0x96EC, "Write CR1 to discontinue RX", inline=True)
comment(0x96EF, "Return to idle scout listening", inline=True)
comment(0x96F2, "Network = &FF broadcast: clear &0D4A", inline=True)
comment(0x96F7, "Install scout data reading loop at &9747", inline=True)
comment(0x96F9, "High byte of scout data handler", inline=True)
comment(0x96FB, "Install scout data loop and RTI", inline=True)
comment(0x9703, "Neither set -- clean end, discard via &9A40", inline=True)
comment(0x970B, "Gentle discard: RX_DISCONTINUE", inline=True)
comment(0x9713, "No RDA -- error handler &9737", inline=True)
comment(0x972A, "Copied all 12 scout bytes?", inline=True)
comment(0x972E, "Save final buffer offset", inline=True)
comment(0x9738, "Save Y for next iteration", inline=True)
label(0x977A, "scout_ctrl_check")     # Check scout control byte
comment(0x9782, "Y=1: advance to port byte in slot", inline=True)
label(0x9796, "scout_port_match")     # Check scout port number
comment(0x9877, "CR1=&00: disable all interrupts", inline=True)
comment(0x987C, "CR2=&84: disable PSE for individual bit testing", inline=True)
comment(0x9903, "Install saved next handler (&99BB for scout ACK)", inline=True)
comment(0x991E, "Install handler at &9992 (write src addr)", inline=True)
comment(0x9932, "Write network=0 (local) to TX FIFO", inline=True)
comment(0x9935, "Check tx_flags for data phase", inline=True)
comment(0x9938, "bit7 set: start data TX phase", inline=True)
comment(0x993A, "CR2=&3F: TX_LAST_DATA | CLR_RX_ST | FLAG_IDLE | FC_TDRA | 2_1_BYTE | PSE", inline=True)
label(0x99A9, "add_buf_to_base")       # Add buffer length to base address (no Tube)
label(0x99B0, "inc_rxcb_buf_hi")      # Increments RXCB buffer high byte
label(0x99B4, "store_rxcb_buf_ptr")  # Store updated buffer pointer pair to RXCB
...
comment(0x9A89, "Buffer length hi = 1", inline=True)
comment(0x9A8B, "Set buffer length hi", inline=True)
...
comment(0x9A9A, "Loop until all 4 bytes copied", inline=True)
comment(0x9A9C, "Enter common data-receive path", inline=True)
comment(0x9A9D, "Immediate op dispatch lo bytes", inline=True)
comment(0x9A9F, "Port workspace offset = &3D", inline=True)
comment(0x9AA1, "Store workspace offset lo", inline=True)
comment(0x9AA3, "RX buffer page = &0D", inline=True)
comment(0x9AA5, "Store workspace offset hi", inline=True)
comment(0x9AA7, "Enter POKE data-receive path", inline=True)
comment(0x9AAA, "Buffer length hi = 1", inline=True)
comment(0x9AAC, "Set buffer length hi", inline=True)
comment(0x9AAE, "Buffer length lo = &FC", inline=True)
comment(0x9AB0, "Set buffer length lo", inline=True)
comment(0x9AB2, "Buffer start lo = &25", inline=True)
comment(0x9AB4, "Set port buffer lo", inline=True)
comment(0x9AB6, "Buffer hi = &7F (below screen)", inline=True)
comment(0x9AB8, "Set port buffer hi", inline=True)
comment(0x9ABC, "Port workspace offset = &3D", inline=True)
comment(0x9ABE, "Store workspace offset lo", inline=True)
comment(0x9AC0, "RX buffer page = &0D", inline=True)
comment(0x9AC2, "Store workspace offset hi", inline=True)
comment(0x9AC4, "Scout status = 2 (PEEK response)", inline=True)
comment(0x9AC6, "Store scout status", inline=True)
comment(0x9AC9, "Calculate transfer size for response", inline=True)
comment(0x9ACC, "C=0: transfer not set up, discard", inline=True)
label(0x9ACE, "set_tx_reply_flag")    # Sets TX reply-pending flag (bit7)
comment(0x9ACE, "Mark TX flags bit 7 (reply pending)", inline=True)
comment(0x9AD1, "Set reply pending flag", inline=True)
comment(0x9AD3, "Store updated TX flags", inline=True)
label(0x9AD6, "rx_imm_halt_cont")     # Handler for HALT/CONTINUE immediate ops
comment(0x9AD6, "CR1=&44: TIE | TX_LAST_DATA", inline=True)
comment(0x9AD8, "Write CR1: enable TX interrupts", inline=True)
label(0x9ADB, "tx_cr2_setup")         # Self-modifying CR2 configuration
comment(0x9ADB, "NMI handler hi byte (self-modifying)", inline=True)
comment(0x9ADD, "Write CR2 for TX setup", inline=True)
label(0x9AE0, "tx_nmi_setup")         # Self-modifying NMI handler lo byte
comment(0x9AE0, "NMI handler lo byte (self-modifying)", inline=True)
comment(0x9AE2, "Y=&9B: dispatch table page", inline=True)
...
comment(0x9B2B, "Call remote JSR; RTS to tx_done_exit", inline=True)
comment(0x9B2D, "Isolate shift register mode bits (2-4)", inline=True)
comment(0x9B2E, "Y=8: network event type", inline=True)
comment(0x9B30, "X = remote address lo", inline=True)
comment(0x9B33, "A = remote address hi", inline=True)
comment(0x9B39, "Exit TX done handler", inline=True)
comment(0x9B3C, "X = remote address lo", inline=True)
comment(0x9B3F, "Y = remote address hi", inline=True)
comment(0x9B42, "Call ROM entry point at &8000", inline=True)
comment(0x9B45, "Exit TX done handler", inline=True)
comment(0x9B48, "A=&04: bit 2 mask for rx_flags", inline=True)
comment(0x9B4A, "Test if already halted", inline=True)
comment(0x9B4D, "Already halted: skip to exit", inline=True)
comment(0x9B4F, "Set bit 2 in rx_flags", inline=True)
comment(0x9B52, "Store halt flag", inline=True)
comment(0x9B55, "A=4: re-load halt bit mask", inline=True)
comment(0x9B57, "Enable interrupts during halt wait", inline=True)
label(0x9B58, "halt_spin_loop")       # Spin-wait during system halt state
comment(0x9B58, "Test halt flag", inline=True)
comment(0x9B5B, "Still halted: keep spinning", inline=True)
comment(0x9B5F, "Load current RX flags", inline=True)
comment(0x9B62, "Clear bit 2: release halted station", inline=True)
comment(0x9B64, "Store updated flags", inline=True)
comment(0x9B67, "Restore Y from stack", inline=True)
comment(0x9B68, "Transfer to Y register", inline=True)
comment(0x9B69, "Restore X from stack", inline=True)
comment(0x9B6A, "Transfer to X register", inline=True)
comment(0x9B6B, "A=0: success status", inline=True)
comment(0x9B6D, "Return with A=0 (success)", inline=True)
comment(0x9B9D, "(Y -= 4: reach start addr offset)", inline=True)
comment(0x9B9E, "(continued)", inline=True)
comment(0x9B9F, "(continued)", inline=True)
comment(0x9BA6, "(Y += 5: advance to next end byte)", inline=True)
comment(0x9BA7, "(continued)", inline=True)
comment(0x9BA8, "(continued)", inline=True)
comment(0x9BA9, "(continued)", inline=True)
comment(0x9BAA, "(continued)", inline=True)
label(0x9BB5, "check_imm_range")       # Check if ctrl byte is in immediate op range
comment(0x9BD6, "Save TX index", inline=True)
comment(0x9BD9, "Push timeout byte 1 on stack", inline=True)
comment(0x9BDA, "Push timeout byte 2 on stack", inline=True)
comment(0x9BDF, "Save interrupt state", inline=True)
comment(0x9BE0, "Disable interrupts for ADLC access", inline=True)
label(0x9BE7, "test_line_idle")       # Tests SR2 INACTIVE bit for line idle
comment(0x9BF1, "Write CR2: clear status, prepare TX", inline=True)
comment(0x9BFE, "Restore interrupt state", inline=True)
comment(0x9C00, "Increment timeout counter byte 1", inline=True)
comment(0x9C03, "Not overflowed: retry INACTIVE test", inline=True)
comment(0x9C05, "Increment timeout counter byte 2", inline=True)
comment(0x9C08, "Not overflowed: retry INACTIVE test", inline=True)
comment(0x9C0A, "Increment timeout counter byte 3", inline=True)
comment(0x9C0D, "Not overflowed: retry INACTIVE test", inline=True)
comment(0x9C11, "CR1=&44: TIE | TX_LAST_DATA", inline=True)
comment(0x9C1B, "Pop saved register", inline=True)
comment(0x9C1C, "Pop saved register", inline=True)
comment(0x9C37, "Install NMI handler at &9D4C (TX data handler)", inline=True)
label(0x9C6B, "imm_op_status3")       # Loads scout_status=3 for immediate ops
comment(0x9C6B, "A=3: scout_status for POKE", inline=True)
comment(0x9C6F, "A=3: scout_status for PEEK op", inline=True)
comment(0x9C73, "Scout status = 2 (POKE transfer)", inline=True)
label(0x9C75, "store_status_add4")    # Stores scout status + 4-byte addition
comment(0x9C75, "Store scout status", inline=True)
comment(0x9C78, "Clear carry for 4-byte addition", inline=True)
comment(0x9C79, "Save carry on stack", inline=True)
comment(0x9C7A, "Y=&0C: start at offset 12", inline=True)
label(0x9C7C, "add_bytes_loop")       # 4-byte address addition loop
comment(0x9C7C, "Load workspace address byte", inline=True)
comment(0x9C7F, "Restore carry from previous byte", inline=True)
comment(0x9C80, "Add TXCB address byte", inline=True)
comment(0x9C82, "Store updated address byte", inline=True)
comment(0x9C85, "Next byte", inline=True)
comment(0x9C86, "Save carry for next addition", inline=True)
...
comment(0x9CDA, "Next TX buffer byte", inline=True)
comment(0x9CDB, "Load second byte from TX buffer", inline=True)
comment(0x9CDE, "Advance TX index past second byte", inline=True)
comment(0x9CDF, "Save updated TX buffer index", inline=True)
comment(0x9CF8, "Write CR2: clear status, idle listen", inline=True)
comment(0x9D01, "PHA/PLA delay (~7 cycles each)", inline=True)
comment(0x9D02, "Increment delay counter", inline=True)
comment(0x9D03, "Loop 256 times for NMI disable", inline=True)
comment(0x9D05, "Store error and return to idle", inline=True)
comment(0x9D0D, "Install NMI handler at &9D94 (TX completion)", inline=True)
comment(0x9D14, "Jump to error handler", inline=True)
comment(0x9D16, "Write CR1 to switch from TX to RX", inline=True)
comment(0x9D21, "A=1: mask for bit0 test", inline=True)
comment(0x9D23, "Test tx_flags bit0 (handshake)", inline=True)
comment(0x9D26, "bit0 clear: install reply handler", inline=True)
comment(0x9D2B, "Install RX reply handler at &9DB2", inline=True)
comment(0x9D2D, "Install handler and RTI", inline=True)
comment(0x9D3F, "Install next handler at &9DC8 (reply continuation)", inline=True)
comment(0x9D41, "Install continuation handler", inline=True)
comment(0x9D44, "Read RX byte (destination station)", inline=True)
comment(0x9D4E, "Install next handler at &9DE3 (reply validation)", inline=True)
comment(0x9D53, "IRQ set -- fall through to &9DE3 without RTI", inline=True)
comment(0x9D58, "Store error and return to idle", inline=True)
comment(0x9D81, "Install next handler at &9EDD (four-way data phase) into &0D4B/&0D4C", inline=True)
comment(0x9D96, "Write dest network to TX FIFO", inline=True)
comment(0x9D9C, "Install handler at &9E2B (write src addr for scout ACK)", inline=True)
label(0x9E34, "tube_tx_inc_byte2")    # Increment byte 2 of 4-byte counter
label(0x9E3C, "tube_tx_inc_byte4")    # Increment byte 4 of 4-byte counter
comment(0x9E55, "Install handler at &9EE9 (RX final ACK)", inline=True)
comment(0x9E6B, "Install handler at &9EFF (final ACK continuation)", inline=True)
comment(0x9E6D, "Install continuation handler", inline=True)
comment(0x9E7A, "Install handler at &9F15 (final ACK validation)", inline=True)
comment(0x9E7F, "IRQ set -- fall through to &9F15 without RTI", inline=True)
comment(0x9E81, "Install handler and RTI", inline=True)
comment(0x9E99, "Load TX flags for next action", inline=True)
comment(0x9E9C, "bit7 clear: no data phase", inline=True)
comment(0x9E9E, "Install data RX handler", inline=True)
comment(0x9ED5, "Transmit in progress?", inline=True)
comment(0x9EDA, "Load TX flags for transfer setup", inline=True)
comment(0x9EDF, "Store with bit 1 set (Tube xfer)", inline=True)
comment(0x9EE9, "Y += 4: advance to high ptr offset", inline=True)
comment(0x9EEA, "(continued)", inline=True)
comment(0x9EEB, "(continued)", inline=True)
comment(0x9EF3, "Y -= 3: back to next low ptr byte", inline=True)
comment(0x9EF4, "(continued)", inline=True)
comment(0x9EFB, "Save X", inline=True)
comment(0x9EFF, "CLC for base pointer addition", inline=True)
comment(0x9F00, "Add RXCB base to get RXCB+4 addr", inline=True)
comment(0x9F07, "Claim Tube transfer address", inline=True)
comment(0x9F0F, "Reclaim with scout status type", inline=True)
comment(0x9F17, "Restore X from stack", inline=True)
comment(0x9F18, "Return with C = transfer status", inline=True)
comment(0x9F28, "Propagate borrow only", inline=True)
comment(0x9F2A, "Temp store of adjusted hi byte", inline=True)
comment(0x9F2E, "Copy RXCB[8] to open port buffer lo", inline=True)
comment(0x9F34, "Load RXCB[9]", inline=True)
comment(0x9F37, "Subtract adjusted hi byte", inline=True)
comment(0x9F3C, "Return with C=1 (success)", inline=True)
comment(0x9F97, "Transfer ROM bank to Y", inline=True)
comment(0x9F9F, "Shift high nibble to low", inline=True)
comment(0x9FA0, "Shift high nibble to low", inline=True)
comment(0x9FA1, "Shift high nibble to low", inline=True)
...
# tx_poll_ff (&85FF)
comment(0x85FF, "A=&FF: full retry count", inline=True)

# bgetv_entry (&8414)
comment(0x8414, "Clear escapable flag before BGET", inline=True)

# tx_calc_transfer (&9ECA)
comment(0x9ECC, "Load workspace byte at offset Y", inline=True)
comment(0x9F12, "Release Tube claim after reclaim", inline=True)

# adlc_full_reset (&9F3D)
comment(0x9F3F, "Write CR1 to ADLC register 0", inline=True)
comment(0x9F44, "Write CR4 to ADLC register 3", inline=True)
comment(0x9F49, "Write CR3 to ADLC register 1", inline=True)

# save_vdu_state / svc_5_unknown_irq dispatch (&92F7)
comment(0x9663, "Y >= &86: above dispatch range", inline=True)
comment(0x9665, "Out of range: skip protection", inline=True)
comment(0x9667, "Save current JSR protection mask", inline=True)
comment(0x966A, "Backup to saved_jsr_mask", inline=True)
comment(0x966D, "Set protection bits 2-4", inline=True)
comment(0x966F, "Apply protection during dispatch", inline=True)
comment(0x9672, "Push return addr high (&9B)", inline=True)
comment(0x9674, "High byte on stack for RTS", inline=True)
comment(0x9675, "Load dispatch target low byte", inline=True)
comment(0x9678, "Low byte on stack for RTS", inline=True)
comment(0x9679, "RTS = dispatch to PHA'd address", inline=True)
...
comment(0x06CE, "Trampoline: begin TX operation", inline=True)
comment(0x06D1, "Trampoline: full ADLC init", inline=True)
comment(0x06D4, "Trampoline: wait idle and reset", inline=True)
comment(0x06D7, "Trampoline: init NMI workspace", inline=True)
comment(0x06DA, "A=4: SR interrupt bit mask", inline=True)
comment(0x06DC, "Test SR flag in VIA IFR", inline=True)
comment(0x06DF, "SR active: handle interrupt", inline=True)
comment(0x06E1, "A=5: NMI not for us", inline=True)
...
comment(0x8010, "Null terminator before copyright", inline=True)
comment(0x8011, 'Copyright string "(C)ROFF"', inline=True)
