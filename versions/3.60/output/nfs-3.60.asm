...
.tube_brk_handler
    ...
    lda (l00fd),y                                                     ; 932f: b1 fd       ..  :0024[1]   ; Load error number from (&FD),0
...
.sub_c0414
    ...
    lda l0015                                                         ; 937d: a5 15       ..  :041b[2]   ; Load our currently-claimed address
...
.tube_post_init
    ...
    sta l0014                                                         ; 9387: 85 14       ..  :0425[2]   ; Store to claim-in-progress flag
...
.c0435
    ...
    stx l0012                                                         ; 939b: 86 12       ..  :0439[2]   ; Store address pointer low byte
...
.loop_c0446
    lda (l0012),y                                                     ; 93a8: b1 12       ..  :0446[2]   ; Load transfer address byte from (X,Y)
    ...
    bpl loop_c0446                                                    ; 93ae: 10 f8       ..  :044c[2]   ; Loop for all 4 address bytes
    ...
    bcc c0463                                                         ; 93bd: 90 06       ..  :045b[2]   ; C=0: no flush needed, skip R3 reads
...
.loop_c0466
    ...
    bvc loop_c0466                                                    ; 93cb: 50 fb       P.  :0469[2]   ; Bit 6 clear: not ready, keep polling
...
.c047a
    ...
    bcc c0482                                                         ; 93dd: 90 05       ..  :047b[2]   ; C=0: NMI not used, skip NMI release
...
.c0498
    ...
    bcc c0498                                                         ; 93ff: 90 f9       ..  :049d[2]   ; C=0: claim failed, retry
    jsr sub_c04d2                                                     ; 9401: 20 d2 04     .. :049f[2]   ; Init reloc pointers from ROM header
; &9404 referenced 1 time by &04c4[2]
.c04a2
    lda #7                                                            ; 9404: a9 07       ..  :04a2[2]   ; R4 cmd 7: SENDW to send ROM to parasite
    jsr sub_c04cb                                                     ; 9406: 20 cb 04     .. :04a4[2]   ; Set up Tube for SENDW transfer
    ldy #0                                                            ; 9409: a0 00       ..  :04a7[2]   ; Y=0: start at beginning of page
    sty l0000                                                         ; 940b: 84 00       ..  :04a9[2]   ; Store to zero page pointer low byte
...
.loop_c04ab
    ...
    bne loop_c04ab                                                    ; 9416: d0 f5       ..  :04b4[2]   ; Loop for all 256 bytes
    inc l0054                                                         ; 9418: e6 54       .T  :04b6[2]   ; Increment 24-bit destination addr
    bne c04c0                                                         ; 941a: d0 06       ..  :04b8[2]   ; No carry: skip higher bytes
    inc l0055                                                         ; 941c: e6 55       .U  :04ba[2]   ; Carry into second byte
    bne c04c0                                                         ; 941e: d0 02       ..  :04bc[2]   ; No carry: skip third byte
    inc l0056                                                         ; 9420: e6 56       .V  :04be[2]   ; Carry into third byte
; &9422 referenced 2 times by &04b8[2], &04bc[2]
.c04c0
    inc l0001                                                         ; 9422: e6 01       ..  :04c0[2]   ; Increment page counter
    bit l0001                                                         ; 9424: 24 01       $.  :04c2[2]   ; Bit 6 set = all pages transferred
    bvc c04a2                                                         ; 9426: 50 dc       P.  :04c4[2]   ; More pages: loop back to SENDW
    jsr sub_c04d2                                                     ; 9428: 20 d2 04     .. :04c6[2]   ; Re-init reloc pointers for final claim
...
.sub_c04d2
    ...
    sta l0054                                                         ; 9436: 85 54       .T  :04d4[2]   ; Store &80 as source page high byte
    sta l0001                                                         ; 9438: 85 01       ..  :04d6[2]   ; Store &80 as page counter initial value
    ...
    beq c04fb                                                         ; 9442: f0 19       ..  :04e0[2]   ; No reloc addr: use defaults
...
.loop_c04e5
    ...
    bne loop_c04e5                                                    ; 944b: d0 fa       ..  :04e9[2]   ; Loop until null terminator found
    ...
    lda l8002,x                                                       ; 9452: bd 02 80    ... :04f0[2]   ; Load reloc addr byte 2
    sta l0054                                                         ; 9455: 85 54       .T  :04f3[2]   ; Store as source page start
    ldy service_entry,x                                               ; 9457: bc 03 80    ... :04f5[2]   ; Load reloc addr byte 3
    lda l8004,x                                                       ; 945a: bd 04 80    ... :04f8[2]   ; Load reloc addr byte 4 (highest)
; &945d referenced 1 time by &04e0[2]
.c04fb
    sta l0056                                                         ; 945d: 85 56       .V  :04fb[2]   ; Store high byte of end address
    sty l0055                                                         ; 945f: 84 55       .U  :04fd[2]   ; Store byte 3 of end address
...
.tube_osbput
    jsr c06c5                                                         ; 9482: 20 c5 06     .. :0520[3]   ; Read channel handle from R2 for BPUT
    tay                                                               ; 9485: a8          .   :0523[3]   ; Y=channel handle from R2
    jsr c06c5                                                         ; 9486: 20 c5 06     .. :0524[3]   ; Read data byte from R2 for BPUT
.tube_poll_r1_wrch
...
.tube_osbget
    jsr c06c5                                                         ; 948f: 20 c5 06     .. :052d[3]   ; Read channel handle from R2 for BGET
...
.tube_osfind
    jsr c06c5                                                         ; 94a4: 20 c5 06     .. :0542[3]   ; Read open mode from R2 for OSFIND
...
.tube_osfind_close
    jsr c06c5                                                         ; 94b4: 20 c5 06     .. :0552[3]   ; OSFIND close: read handle from R2
...
.tube_osargs
    jsr c06c5                                                         ; 94c0: 20 c5 06     .. :055e[3]   ; Read file handle from R2 for OSARGS
...
.loop_c0564
    jsr c06c5                                                         ; 94c6: 20 c5 06     .. :0564[3]   ; Read next param byte from R2
    sta l00ff,x                                                       ; 94c9: 95 ff       ..  :0567[3]   ; Params stored at &00-&03 (little-endian)
    dex                                                               ; 94cb: ca          .   :0569[3]   ; Decrement byte counter
    bne loop_c0564                                                    ; 94cc: d0 f8       ..  :056a[3]   ; Loop for 4 bytes
    jsr c06c5                                                         ; 94ce: 20 c5 06     .. :056c[3]   ; Read OSARGS reason code from R2
...
.loop_c0577
    lda l0000,x                                                       ; 94d9: b5 00       ..  :0577[3]   ; Load result byte from zero page
    jsr tube_send_r2                                                  ; 94db: 20 95 06     .. :0579[3]   ; Send byte to co-processor via R2
    dex                                                               ; 94de: ca          .   :057c[3]   ; Previous byte (count down)
    bpl loop_c0577                                                    ; 94df: 10 f8       ..  :057d[3]   ; Loop for all 4 bytes
...
.strnh
    jsr c06c5                                                         ; 94e8: 20 c5 06     .. :0586[3]   ; Read next string byte from R2
    sta l0700,y                                                       ; 94eb: 99 00 07    ... :0589[3]   ; Store byte in string buffer at &0700+Y
...
.argsw
    jsr c06c5                                                         ; 950d: 20 c5 06     .. :05ab[3]   ; Read next control block byte from R2
    sta l0001,x                                                       ; 9510: 95 01       ..  :05ae[3]   ; Store at &01+X (descending)
    ...
    sty l0001                                                         ; 951a: 84 01       ..  :05b8[3]   ; Store Y=7 as pointer high byte
    ldy #0                                                            ; 951c: a0 00       ..  :05ba[3]   ; Y=0 for OSFILE control block offset
    jsr c06c5                                                         ; 951e: 20 c5 06     .. :05bc[3]   ; Read OSFILE reason code from R2
...
.loop_c05d3
    jsr c06c5                                                         ; 9535: 20 c5 06     .. :05d3[3]   ; Read next control block byte from R2
    sta l00ff,x                                                       ; 9538: 95 ff       ..  :05d6[3]   ; Store at &FF+X (descending into &00-&0C)
    dex                                                               ; 953a: ca          .   :05d8[3]   ; Decrement byte counter
    bne loop_c05d3                                                    ; 953b: d0 f8       ..  :05d9[3]   ; Loop for all 13 bytes
    jsr c06c5                                                         ; 953d: 20 c5 06     .. :05db[3]   ; Read OSGBPB reason code from R2
...
.loop_c05e6
    lda l0000,x                                                       ; 9548: b5 00       ..  :05e6[3]   ; Load result byte from zero page
    jsr tube_send_r2                                                  ; 954a: 20 95 06     .. :05e8[3]   ; Send byte to co-processor via R2
    dex                                                               ; 954d: ca          .   :05eb[3]   ; Decrement byte counter
    bpl loop_c05e6                                                    ; 954e: 10 f8       ..  :05ec[3]   ; Loop for 13 bytes (X=12..0)
    ...
    jsr c06c5                                                         ; 9554: 20 c5 06     .. :05f2[3]   ; Read X param from R2 for 2-param OSBYTE
    tax                                                               ; 9557: aa          .   :05f5[3]   ; X = first parameter
    jsr c06c5                                                         ; 9558: 20 c5 06     .. :05f6[3]   ; Read A (OSBYTE number) from R2
...
.svc_star_command
    ...
    sta l00a9                                                         ; 81aa: 85 a9       ..             ; Save to workspace &A9
...
.svc_13_select_nfs
    ...
    ror l00a8                                                         ; 81f5: 66 a8       f.             ; Set bit 7 of l00a8 (inhibit auto-boot)
...
.dofsl1
    beq c825f                                                         ; 8250: f0 0d       ..             ; Clock present: skip warning
...
.loop_c8266
    lda l829a,y                                                       ; 8266: b9 9a 82    ...            ; Load extended vector dispatch address
    sta filev,y                                                       ; 8269: 99 12 02    ...            ; Write to FILEV-FSCV vector table
    dey                                                               ; 826c: 88          .              ; Next byte (descending)
    bpl loop_c8266                                                    ; 826d: 10 f7       ..             ; Loop until all 14 bytes copied
...
.setup_rom_ptrs_netv
    ...
    sty l00f7                                                         ; 8330: 84 f7       ..             ; Store table base address high byte; Y=value of address of ROM pointer table (high byte)
...
.fstxl1
    ...
    sta l00c0,y                                                       ; 839b: 99 c0 00    ...            ; Store to TX control block at &00C0
    ...
    sta l00c2,y                                                       ; 83a5: 99 c2 00    ...            ; Store to dest station/network at &00C2
...
.loop_c83d4
    ...
    bpl loop_c83d4                                                    ; 83db: 10 f7       ..             ; Loop for both handles
; ***************************************************************************************
...
.handle_bput_bget
    ...
    sta l0fdf                                                         ; 8418: 8d df 0f    ...            ; Also save byte at &0FDF for BSXMIT
    ...
    sty l0fde                                                         ; 8425: 8c de 0f    ...            ; Store handle bitmask at &0FDE
    sty l00cf                                                         ; 8428: 84 cf       ..             ; Store handle bitmask for sequence tracking
    ...
    sta l00c4                                                         ; 8434: 85 c4       ..             ; Store reply buffer ptr low in TXCB
    lda #&e0                                                          ; 8436: a9 e0       ..             ; Error buffer at &0FE0
    sta l00c8                                                         ; 8438: 85 c8       ..             ; Store error buffer ptr low in TXCB
...
.store_retry_count
    ...
    lda l00cf                                                         ; 8444: a5 cf       ..             ; Load handle bitmask for BSXMIT
...
.error1
    ...
    sta l0fe0,y                                                       ; 8455: 99 e0 0f    ...            ; Store to error buffer at &0FE0+Y
    ...
    beq c846d                                                         ; 8465: f0 06       ..             ; Handle matches SPOOL -- close it
    lda #&2d ; '-'                                                    ; 8467: a9 2d       .-             ; '-': offset into "E." string at &852D
    cpx l00ba                                                         ; 8469: e4 ba       ..             ; X=value of *EXEC file handle
    bne c8473                                                         ; 846b: d0 06       ..             ; No EXEC match -- skip close
...
.c8473
    ...
    sta l00c4                                                         ; 8475: 85 c4       ..             ; Reset reply ptr to error buffer
...
.store_fs_error
    ...
    bcs c8487                                                         ; 8481: b0 04       ..             ; Error >= &A8: keep original value
    lda #&a8                                                          ; 8483: a9 a8       ..             ; Error < &A8: override with standard &A8
    sta (l00c4),y                                                     ; 8485: 91 c4       ..             ; Write clamped error number to reply buffer
...
.loop_c8489
    ...
    sta l0100,y                                                       ; 848c: 99 00 01    ...            ; Build BRK error block at &0100
...
.sub_c84a1
    lda l00ff                                                         ; 84a1: a5 ff       ..             ; Read escape flag from MOS workspace
...
.loop_c84e4
    sta l0100,x                                                       ; 84e4: 9d 00 01    ...            ; BRK at &0100 as safe default
    dex                                                               ; 84e7: ca          .
...
.set_listen_offset
    ...
    stx l0100                                                         ; 851a: 8e 00 01    ...            ; Store BRK opcode at &0100
...
.send_to_fs_star
    ...
    bne c8540                                                         ; 8539: d0 05       ..             ; High byte != 0: skip flag set
...
.loop_c85e1
    ...
    bcc c85e9                                                         ; 85e4: 90 03       ..             ; Bit was 0: skip this destination bit
    ora access_bit_table,x                                            ; 85e6: 1d ec 85    ...            ; OR in destination bit from lookup table
; &85e9 referenced 1 time by &85e4
.c85e9
    bne loop_c85e1                                                    ; 85e9: d0 f6       ..             ; Loop while source bits remain (A != 0)
...
.save_fscv_args_with_ptrs
    ...
    sty l00f3                                                         ; 864b: 84 f3       ..             ; Set os_text_ptr high = Y
; ***************************************************************************************
...
.quote1
    ...
    sta l0e30,x                                                       ; 86f8: 9d 30 0e    .0.            ; Store parsed character to &0E30+X
...
.c86fd
    ...
    sta l0e30,x                                                       ; 8700: 9d 30 0e    .0.            ; Store CR terminator at end of string
...
.loop_c8742
    ...
    sta l00c8,x                                                       ; 8744: 95 c8       ..             ; Store as current transfer position
    adc l0f0d,x                                                       ; 8746: 7d 0d 0f    }..            ; Add file length byte
    sta l00b4,x                                                       ; 8749: 95 b4       ..             ; Store as end position
    inx                                                               ; 874b: e8          .              ; Next address byte
    dey                                                               ; 874c: 88          .              ; Decrement byte counter
    bne loop_c8742                                                    ; 874d: d0 f3       ..             ; Loop for all 4 address bytes
    sec                                                               ; 874f: 38          8              ; Adjust high byte for 3-byte length overflow
    sbc l0f10                                                         ; 8750: ed 10 0f    ...            ; Subtract 4th length byte from end addr
    sta l00b7                                                         ; 8753: 85 b7       ..             ; Store adjusted end address high byte
...
.floop
    lda l0f10,x                                                       ; 875a: bd 10 0f    ...            ; Load file length byte
...
.loop_c87b7
    ...
    bne loop_c87b7                                                    ; 87bd: d0 f8       ..             ; Loop for bytes 9..1
...
.c87d8
    ...
    beq c87ef                                                         ; 87e8: f0 05       ..             ; CSD=0: no directory prefix
    jsr print_dir_from_offset                                         ; 87ea: 20 98 8d     ..            ; Print directory prefix from reply
    bmi c8803                                                         ; 87ed: 30 14       0.             ; Dir printed: skip to address display
...
.c87ef
    ...
    bcc c87fb                                                         ; 87f3: 90 06       ..             ; Below &21: pad with spaces to column 12
    jsr osasci                                                        ; 87f5: 20 e3 ff     ..            ; Write character
    iny                                                               ; 87f8: c8          .              ; Next character in filename
    bne c87ef                                                         ; 87f9: d0 f4       ..             ; Loop for more filename characters
...
.c87fb
    ...
    bcc c87fb                                                         ; 8801: 90 f8       ..             ; No: keep padding with spaces
...
.c8817
    stx l0f08                                                         ; 8817: 8e 08 0f    ...            ; Store reply command for attr decode
...
.loop_c8822
    ...
    lda l0ef7,y                                                       ; 8825: b9 f7 0e    ...            ; Load remaining reply data for param block
    cpy #&12                                                          ; 8828: c0 12       ..             ; Copied all 4 bytes? (Y=&0E..&11)
    bne loop_c8822                                                    ; 882a: d0 f6       ..             ; Loop for 4 attribute bytes
...
.lodrl1
    ...
    sta l00ae,y                                                       ; 8833: 99 ae 00    ...            ; Store to local workspace
...
.c89ba
    ...
    bcs c89d4                                                         ; 89be: b0 14       ..             ; A>=3: FS command (ARGSV write)
...
.osarg1
    ...
    sty l0002,x                                                       ; 89d0: 94 02       ..             ; Y=&FF after loop; fill high bytes
    sty l0003,x                                                       ; 89d2: 94 03       ..             ; Set 32-bit result bytes 2-3 to &FF
...
.findv_handler
    ...
    bne c89d4                                                         ; 89e4: d0 ee       ..             ; Invalid mode bits: return
...
.fscv_0_opt
    ...
    bne c8a36                                                         ; 8a30: d0 04       ..             ; No: check for *OPT 1
...
.gbpbv_handler
    ...
    beq c8a7d                                                         ; 8a76: f0 05       ..             ; A=0: invalid, restore and return
...
.gbpbf1
    ...
    sta l0f06,x                                                       ; 8a99: 9d 06 0f    ...            ; Store to FS command buffer at &0F06+X
...
.gbpbl1
    ...
    beq c8ab9                                                         ; 8ab4: f0 03       ..             ; Even (write): keep &91 and Y=&0B
...
.gbpbl3
    ...
    sta l00af,y                                                       ; 8acf: 99 af 00    ...            ; Store as source address
    sta l00c7,y                                                       ; 8ad2: 99 c7 00    ...            ; Store as current transfer position
    jsr add_4_to_y                                                    ; 8ad5: 20 3c 88     <.            ; Skip 4 bytes to reach transfer length
    adc (fs_options),y                                                ; 8ad8: 71 bb       q.             ; Dest = source + length
    sta l00af,y                                                       ; 8ada: 99 af 00    ...            ; Store as end address
...
.gbpbf3
    ...
    bcs c8afb                                                         ; 8af6: b0 03       ..             ; Carry set: transfer error
...
.c8afb
    ...
    bmi c8b08                                                         ; 8b03: 30 03       0.             ; Bit 7 set: not EOF, skip clear
...
.c8b08
    ...
    jmp c89d4                                                         ; 8b19: 4c d4 89    L..            ; Return via restore_args path
...
.c8b1c
    ...
    sta l0f16                                                         ; 8b24: 8d 16 0f    ...            ; Store boot option in reply area
...
.osgbpb_info
    ...
    beq c8b3f                                                         ; 8b36: f0 07       ..             ; No Tube: skip Tube address check
    cmp (fs_options),y                                                ; 8b38: d1 bb       ..             ; Compare Tube flag with addr byte 4
    bne c8b3f                                                         ; 8b3a: d0 03       ..             ; Mismatch: not Tube space
    dey                                                               ; 8b3c: 88          .              ; Y=&03
...
.c8b53
    ...
    sta l0f06                                                         ; 8b6d: 8d 06 0f    ...            ; Store reply length in command buffer
...
.loop_c8b87
    ...
    bne loop_c8b87                                                    ; 8b90: d0 f5       ..             ; Loop until all bytes copied
    beq c8bbb                                                         ; 8b92: f0 27       .'             ; ALWAYS branch to exit; ALWAYS branch
...
.c8b94
    ...
    bne c8ba1                                                         ; 8b9e: d0 01       ..             ; No page wrap: keep Y
...
.loop_c8baf
    ...
    bne loop_c8baf                                                    ; 8bb0: d0 fd       ..             ; Loop until delay complete
...
.c8bbb
    jmp c89d4                                                         ; 8bbb: 4c d4 89    L..            ; Return via restore_args path
...
.c8bbe
    ...
    lda l0f06                                                         ; 8bde: ad 06 0f    ...            ; Load reply entry count
    ...
    lda l00c8                                                         ; 8bec: a5 c8       ..             ; Load total reply length
    sbc #7                                                            ; 8bee: e9 07       ..             ; Subtract header (7 bytes) from reply len
    sta l0f06                                                         ; 8bf0: 8d 06 0f    ...            ; Store adjusted length in command buffer
    sta fs_load_addr_2                                                ; 8bf3: 85 b2       ..             ; Store as byte count for copy loop
    beq c8bfa                                                         ; 8bf5: f0 03       ..             ; Zero bytes: skip copy
...
.loop_c8bfc
    sta l0f07,x                                                       ; 8bfc: 9d 07 0f    ...            ; Zero out &0F07+X area
    dex                                                               ; 8bff: ca          .              ; Next byte
    bpl loop_c8bfc                                                    ; 8c00: 10 fa       ..             ; Loop for X=2,1,0
    ...
    sta l0f06                                                         ; 8c0b: 8d 06 0f    ...            ; Store in command buffer
    jsr adjust_addrs                                                  ; 8c0e: 20 5a 8a     Z.            ; Adjust param block addresses
    beq c8bbb                                                         ; 8c11: f0 a8       ..             ; Z=1: all done, exit
...
.fscv_3_star_cmd
    ...
    stx l00b9                                                         ; 8c20: 86 b9       ..             ; Disable column formatting
...
.decmor
    ...
    bmi c8c45                                                         ; 8c2b: 30 18       0.             ; Bit 7: end of name, dispatch
...
.decmin
    ...
    bne loop_c8c24                                                    ; 8c3f: d0 e3       ..             ; No: try next table entry
...
.c8c45
    ...
    lda l8c4c,x                                                       ; 8c46: bd 4c 8c    .L.            ; Load handler address low byte
...
.c8c72
    ...
    jsr sub_c86ea                                                     ; 8c7b: 20 ea 86     ..            ; Set up command parameter pointers
    ...
    lda l0f13                                                         ; 8c91: ad 13 0f    ...            ; Load station number from FS reply
    ...
    bne c8cb0                                                         ; 8ca3: d0 0b       ..             ; Non-zero: Public access
    jsr print_inline                                                  ; 8ca5: 20 5c 86     \.            ; Print 'Owner' + CR
    equs "Owner", &0d                                                 ; 8ca8: 4f 77 6e... Own

    bne c8cba                                                         ; 8cae: d0 0a       ..             ; Always branches (print_inline sets N=1)
...
.c8cba
    ...
    ldy l8d54,x                                                       ; 8cdf: bc 54 8d    .T.            ; Load string offset for option name
; &8ce2 referenced 1 time by &8ceb
.loop_c8ce2
    lda l8d54,y                                                       ; 8ce2: b9 54 8d    .T.            ; Load character from option name string
    bmi c8ced                                                         ; 8ce5: 30 06       0.             ; Bit7 set: string terminated, done
    jsr osasci                                                        ; 8ce7: 20 e3 ff     ..            ; Write character
    iny                                                               ; 8cea: c8          .              ; Next character
    bne loop_c8ce2                                                    ; 8ceb: d0 f5       ..             ; Continue printing option name
...
.c8d11
    sty l0f06                                                         ; 8d11: 8c 06 0f    ...            ; Store entry start offset for request
    sty l00b4                                                         ; 8d14: 84 b4       ..             ; Save start offset in zero page for loop
    ldx l00b5                                                         ; 8d16: a6 b5       ..             ; Load examine arg count for batch size
    stx l0f07                                                         ; 8d18: 8e 07 0f    ...            ; Store as request count at &0F07
    ldx l00b7                                                         ; 8d1b: a6 b7       ..             ; Load column count for display format
...
.loop_c8d34
    ...
    bpl loop_c8d34                                                    ; 8d38: 10 fa       ..             ; Loop until high-bit terminator found
    sta fs_cmd_lib,y                                                  ; 8d3a: 99 04 0f    ...            ; Store terminator as print boundary
    jsr sub_c8d9f                                                     ; 8d3d: 20 9f 8d     ..            ; Print/format this directory entry
    pla                                                               ; 8d40: 68          h              ; Restore entry count from stack
    clc                                                               ; 8d41: 18          .              ; CLC for addition
    adc l00b4                                                         ; 8d42: 65 b4       e.             ; Advance start offset by entry count
    tay                                                               ; 8d44: a8          .              ; Y = new entry start offset
    bne c8d11                                                         ; 8d45: d0 ca       ..             ; More entries: fetch next batch
...
.copy_string_to_cmd
    ldy #0                                                            ; 8d84: a0 00       ..             ; Start source offset at 0
; &8d86 referenced 1 time by &8d8f
.copy_string_from_offset
    lda (fs_crc_lo),y                                                 ; 8d86: b1 be       ..             ; Load byte from source string
    sta fs_cmd_data,x                                                 ; 8d88: 9d 05 0f    ...            ; Store to FS command buffer (&0F05+X)
    inx                                                               ; 8d8b: e8          .              ; Advance dest pointer
...
.sub_c8d9f
    ldy l00b9                                                         ; 8d9f: a4 b9       ..             ; Null byte: check column counter
    bmi c8db2                                                         ; 8da1: 30 0f       0.             ; Negative: print CR (no columns)
    iny                                                               ; 8da3: c8          .              ; Advance column counter
    tya                                                               ; 8da4: 98          .              ; Transfer to A for modulo
    and #3                                                            ; 8da5: 29 03       ).             ; Modulo 4 columns
    sta l00b9                                                         ; 8da7: 85 b9       ..             ; Update column counter
    beq c8db2                                                         ; 8da9: f0 07       ..             ; Column 0: start new line
    jsr print_inline                                                  ; 8dab: 20 5c 86     \.            ; Print 2-space column separator
    equs "  "                                                         ; 8dae: 20 20

    bne c8db7                                                         ; 8db0: d0 05       ..             ; ALWAYS branch to next byte
...
.loop_c8dd0
    ...
    bcs loop_c8dd0                                                    ; 8dd3: b0 fb       ..             ; Loop while A >= 0 (borrow clear)
...
.loop_c8de8
    ...
    bcc loop_c8de8                                                    ; 8deb: 90 fb       ..             ; C=0: more characters, keep reading
    jsr c837e                                                         ; 8ded: 20 7e 83     ~.            ; Skip spaces after filename
    ...
    lda l00f3                                                         ; 8df7: a5 f3       ..             ; Load text pointer high byte
    adc #0                                                            ; 8df9: 69 00       i.             ; Add carry from low byte addition
    sta l0e0b                                                         ; 8dfb: 8d 0b 0e    ...            ; Store context address high byte
    ...
    sta l0e16                                                         ; 8e06: 8d 16 0e    ...            ; Store to FS workspace
    ...
    beq c8e29                                                         ; 8e13: f0 14       ..             ; No Tube: execute locally
    adc l0f0b                                                         ; 8e15: 6d 0b 0f    m..            ; Check load address upper bytes
    adc l0f0c                                                         ; 8e18: 6d 0c 0f    m..            ; Continue address range check
    bcs c8e29                                                         ; 8e1b: b0 0c       ..             ; Carry set: not Tube space, exec locally
...
.c8e29
    ...
    jmp (l0f09)                                                       ; 8e2a: 6c 09 0f    l..            ; Execute at load address via indirect JMP
...
.fsreply_2_copy_handles
    ...
    bcc c8e43                                                         ; 8e3b: 90 06       ..             ; SDISC: skip boot option, copy handles only
...
.boot_cmd_execute
    ...
    ldx l8d68,y                                                       ; 8e4b: be 68 8d    .h.            ; X = command string offset from table
...
.sub_c8e53
    lda l00f0                                                         ; 8e53: a5 f0       ..             ; Load handle from &F0
; ***************************************************************************************
...
; ***************************************************************************************
.net_2_read_handle_entry
    jsr sub_c8e53                                                     ; 8e6d: 20 53 8e     S.            ; Look up handle &F0 in workspace
...
; ***************************************************************************************
.net_3_close_handle
    jsr sub_c8e53                                                     ; 8e7d: 20 53 8e     S.            ; Look up handle &F0 in workspace
...
.loop_c8e96
    ...
    sta l00aa,y                                                       ; 8e98: 99 aa 00    ...            ; Restore to &AA-&AC
    dey                                                               ; 8e9b: 88          .              ; Next byte (descending)
    bpl loop_c8e96                                                    ; 8e9c: 10 f8       ..             ; Loop for all 3 bytes
...
.fs_osword_dispatch
    ...
    lda l8eb8,x                                                       ; 8ea4: bd b8 8e    ...            ; Load handler address low byte from table
.fs_osword_tbl_lo
...
.save1
    lda l00aa,y                                                       ; 8eaa: b9 aa 00    ...            ; Load param block pointer byte
    ...
    lda (l00f0),y                                                     ; 8eb3: b1 f0       ..             ; Read sub-function code from (&F0)+1
    sty l00a9                                                         ; 8eb5: 84 a9       ..             ; Store Y=1 to &A9
...
.osword_0f_handler
    ...
    sta l00ac                                                         ; 8eca: 85 ac       ..             ; Set param block high byte
    ...
    sta l00ab                                                         ; 8ed0: 85 ab       ..             ; Set param block low byte
    sta nmi_tx_block                                                  ; 8ed2: 85 a0       ..             ; Set LTXCBP low byte for low-level TX
    ldx #&0f                                                          ; 8ed4: a2 0f       ..             ; X=15: copy 16 bytes (OSWORD param block)
    jsr c8f1c                                                         ; 8ed6: 20 1c 8f     ..            ; Copy param block to user TX control block
    jmp c9630                                                         ; 8ed9: 4c 30 96    L0.            ; Start user transmit via BRIANX
...
.osword_11_handler
    ...
    sta l00ac                                                         ; 8ede: 85 ac       ..             ; Store as copy source high byte in &AC
    ...
    sty l00ab                                                         ; 8ee5: 84 ab       ..             ; Store &80 as copy source low byte
    ...
    jsr c8f1c                                                         ; 8eeb: 20 1c 8f     ..            ; Copy X+1 bytes from workspace to param
...
.read_args_size
    ...
    sta (l00f0),y                                                     ; 8ef7: 91 f0       ..             ; Store buffer size to (&F0)+1
...
.readry
    sta (l00f0),y                                                     ; 8efc: 91 f0       ..             ; Store args start offset to (&F0)+2
    rts                                                               ; 8efe: 60          `              ; Return
...
.sub_c8f01
    ...
    beq c8f11                                                         ; 8f0d: f0 02       ..             ; Y=0 (sub 0-1): use page &0D
    ldx nfs_workspace_hi                                              ; 8f0f: a6 9f       ..             ; Y=1 (sub 2-3): use dynamic workspace
; &8f11 referenced 1 time by &8f0d
.c8f11
    stx l00ac                                                         ; 8f11: 86 ac       ..             ; Store workspace page in &AC (hi byte)
    lda l8eff,y                                                       ; 8f13: b9 ff 8e    ...            ; Load offset: &FF (sub 0-1) or &01 (sub 2-3)
    sta l00ab                                                         ; 8f16: 85 ab       ..             ; Store offset in &AB (lo byte)
...
.c8f1c
    ...
    lda (l00f0),y                                                     ; 8f1e: b1 f0       ..             ; C=1: copy from param to workspace
    sta (l00ab),y                                                     ; 8f20: 91 ab       ..             ; Store param byte to workspace
; &8f22 referenced 1 time by &8f1c
.c8f22
    lda (l00ab),y                                                     ; 8f22: b1 ab       ..             ; Load workspace byte for return
; ***************************************************************************************
...
.copy_param_block
    ...
    bpl c8f1c                                                         ; 8f28: 10 f2       ..             ; Loop while bytes remain
.logon3
.return_copy_param
    rts                                                               ; 8f2a: 60          `              ; Return
...
.rssl1
    ...
    lda (l00f0),y                                                     ; 8f2d: b1 f0       ..             ; Load protection byte from param block
    bcs rssl2                                                         ; 8f2f: b0 05       ..             ; C=1 (odd sub): set protection
    lda prot_status                                                   ; 8f31: ad 63 0d    .c.            ; C=0 (even sub): read current status
    sta (l00f0),y                                                     ; 8f34: 91 f0       ..             ; Return current value to param block
...
.rsl1
    ...
    sty l00a8                                                         ; 8f55: 84 a8       ..             ; Init loop counter at Y=3
; &8f57 referenced 1 time by &8f66
.loop_c8f57
    ldy l00a8                                                         ; 8f57: a4 a8       ..             ; Reload loop counter
    lda (l00f0),y                                                     ; 8f59: b1 f0       ..             ; Read handle from caller's param block
    jsr handle_to_mask_a                                              ; 8f5b: 20 9a 86     ..            ; Convert handle number to bitmask
    tya                                                               ; 8f5e: 98          .              ; TYA: get bitmask result
    ldy l00a8                                                         ; 8f5f: a4 a8       ..             ; Reload loop counter
    sta fs_server_net,y                                               ; 8f61: 99 01 0e    ...            ; Store bitmask to FS server table
    dec l00a8                                                         ; 8f64: c6 a8       ..             ; Next handle (descending)
    bne loop_c8f57                                                    ; 8f66: d0 ef       ..             ; Loop for handles 3,2,1
    rts                                                               ; 8f68: 60          `              ; Return
...
.c8f69
    ...
    sta (l00f0),y                                                     ; 8f6d: 91 f0       ..             ; Return error to caller's param block
    rts                                                               ; 8f6f: 60          `              ; Return
...
.osword_10_handler
    ...
    stx l00ac                                                         ; 8f7e: 86 ac       ..             ; Set up pointer high byte in &AC
    sty l00ab                                                         ; 8f80: 84 ab       ..             ; Save param block high byte in &AB
    ror rx_flags                                                      ; 8f82: 6e 64 0d    nd.            ; Disable user RX during CB operation
    lda (l00f0),y                                                     ; 8f85: b1 f0       ..             ; Read first byte of param block
    sta l00aa                                                         ; 8f87: 85 aa       ..             ; Save: 0=open new, non-zero=read RXCB
    bne c8fa6                                                         ; 8f89: d0 1b       ..             ; Non-zero: read specified RXCB
...
.scan0
    ...
    lda (l00ab),y                                                     ; 8f95: b1 ab       ..             ; Read flag byte from RXCB workspace
...
.scan1
    ...
    sta (l00f0,x)                                                     ; 8fa4: 81 f0       ..             ; Return RXCB number to caller's byte 0
...
.c8fa6
    ...
    sty l00ab                                                         ; 8fac: 84 ab       ..             ; Set &AB = workspace ptr low byte
    lda #&c0                                                          ; 8fae: a9 c0       ..             ; &C0: test mask for flag byte
    ldy #1                                                            ; 8fb0: a0 01       ..             ; Y=1: flag byte offset in RXCB
    ldx #&0b                                                          ; 8fb2: a2 0b       ..             ; X=11: copy 12 bytes from RXCB
    cpy l00aa                                                         ; 8fb4: c4 aa       ..             ; Compare Y(1) with saved byte (open/read)
    adc (l00ab),y                                                     ; 8fb6: 71 ab       q.             ; ADC flag: test if slot is in use
    beq openl6                                                        ; 8fb8: f0 03       ..             ; Zero: slot open, do copy
...
.openl6
    jsr c8f1c                                                         ; 8fbd: 20 1c 8f     ..            ; Copy RXCB data to param block
    bcs c8fd1                                                         ; 8fc0: b0 0f       ..             ; Done: skip deletion on error
    lda #&3f ; '?'                                                    ; 8fc2: a9 3f       .?             ; Mark CB as consumed (consume-once)
    ldy #1                                                            ; 8fc4: a0 01       ..             ; Y=1: flag byte offset
    sta (l00ab),y                                                     ; 8fc6: 91 ab       ..             ; Write &3F to mark slot deleted
    bne c8fd1                                                         ; 8fc8: d0 07       ..             ; Branch to exit (always taken); ALWAYS branch

; &8fca referenced 1 time by &8fba
.openl7
    adc #1                                                            ; 8fca: 69 01       i.             ; Advance through multi-byte field
    bne loop_c8fbc                                                    ; 8fcc: d0 ee       ..             ; Loop until all bytes processed
    dey                                                               ; 8fce: 88          .              ; Y=-1 → Y=0 after STA below
; &8fcf referenced 3 times by &8f90, &8f97, &8fa9
.openl4
    sta (l00f0),y                                                     ; 8fcf: 91 f0       ..             ; Return zero (no free RXCB found)
...
.setup_rx_buffer_ptrs
    ...
    lda l00f0                                                         ; 8fd7: a5 f0       ..             ; A = base address low byte
    adc #1                                                            ; 8fd9: 69 01       i.             ; A = base + 1 (skip length byte)
    jsr store_16bit_at_y                                              ; 8fdb: 20 e6 8f     ..            ; Store 16-bit start addr at ws+&1C/&1D
    ldy #1                                                            ; 8fde: a0 01       ..             ; Read data length from (&F0)+1
    lda (l00f0),y                                                     ; 8fe0: b1 f0       ..             ; A = data length byte
    ldy #&20 ; ' '                                                    ; 8fe2: a0 20       .              ; Workspace offset &20 = RX data end
    adc l00f0                                                         ; 8fe4: 65 f0       e.             ; A = base + length = end address low
...
.store_16bit_at_y
    ...
    lda l00f1                                                         ; 8fe9: a5 f1       ..             ; A = high byte of base address
...
.econet_tx_rx
    ...
    bcs c903e                                                         ; 8ff2: b0 4a       .J             ; A >= 1: handle TX result
...
.c8ffe
    ...
    sta (l00f0),y                                                     ; 9011: 91 f0       ..             ; Store port &90 at (&F0)+2
    iny                                                               ; 9013: c8          .              ; Y=&03
    iny                                                               ; 9014: c8          .              ; Y=&04
; &9015 referenced 1 time by &901d
.loop_c9015
    lda l0dfe,y                                                       ; 9015: b9 fe 0d    ...            ; Copy FS station addr from workspace
    sta (l00f0),y                                                     ; 9018: 91 f0       ..             ; Store to RX param block
    iny                                                               ; 901a: c8          .              ; Next byte
    cpy #7                                                            ; 901b: c0 07       ..             ; Done 3 bytes (Y=4,5,6)?
    bne loop_c9015                                                    ; 901d: d0 f6       ..             ; No: continue copying
    ...
    jmp c8530                                                         ; 903b: 4c 30 85    L0.            ; Jump to RX poll (BRIANX)

; &903e referenced 1 time by &8ff2
.c903e
    php                                                               ; 903e: 08          .              ; Save processor flags
    ldy #1                                                            ; 903f: a0 01       ..             ; Y=1: first data byte offset
    lda (l00f0),y                                                     ; 9041: b1 f0       ..             ; Load first data byte from RX buffer
; ***************************************************************************************
...
.net_write_char
    ...
    lda (l00f0),y                                                     ; 9045: b1 f0       ..             ; Load station address high byte
    iny                                                               ; 9047: c8          .              ; Advance past station addr
    sty l00ab                                                         ; 9048: 84 ab       ..             ; Save Y as data index
...
.loop_c9055
    ldy l00ab                                                         ; 9055: a4 ab       ..             ; Reload data index
    inc l00ab                                                         ; 9057: e6 ab       ..             ; Advance data index for next iteration
    lda (l00f0),y                                                     ; 9059: b1 f0       ..             ; Load next data byte
    beq return_8                                                      ; 905b: f0 13       ..             ; Zero byte: end of data, return
    ldy #&7d ; '}'                                                    ; 905d: a0 7d       .}             ; Y=&7D: store byte for TX at offset &7D
    sta (net_rx_ptr),y                                                ; 905f: 91 9c       ..             ; Store data byte at (net_rx_ptr)+&7D for TX; Store data byte at (net_rx_ptr)+&7D for TX
    pha                                                               ; 9061: 48          H              ; Save data byte for &0D check after TX
    jsr ctrl_block_setup_alt                                          ; 9062: 20 7f 91     ..            ; Set up TX control block
    jsr sub_c907c                                                     ; 9065: 20 7c 90     |.            ; Enable IRQs and transmit
; &9068 referenced 1 time by &9069
.loop_c9068
    dex                                                               ; 9068: ca          .              ; Short delay loop between TX packets
    bne loop_c9068                                                    ; 9069: d0 fd       ..             ; Spin until X reaches 0
    pla                                                               ; 906b: 68          h              ; Restore data byte for terminator check
    eor #&0d                                                          ; 906c: 49 0d       I.             ; Test for end-of-data marker (&0D)
    bne loop_c9055                                                    ; 906e: d0 e5       ..             ; Not &0D: continue with next byte
...
.osword_trampoline
    ...
    lda l00ef                                                         ; 90a1: a5 ef       ..             ; Load workspace byte &EF for handler
...
.sub_c90b6
    ...
    asl l0106,x                                                       ; 90ba: 1e 06 01    ...            ; ASL: restore P after ROR zeroed carry
...
.remote_cmd_dispatch
    ...
    beq c9101                                                         ; 90ec: f0 13       ..             ; Forward &81 to terminal for keyboard read
    ...
    beq c9101                                                         ; 90f5: f0 0a       ..             ; Match found: dispatch with Y=1 (both)
    ...
    beq c9101                                                         ; 90fe: f0 01       ..             ; Match found: dispatch with Y=&FF (terminal)
...
.loop_c9123
    ...
    bpl loop_c9123                                                    ; 9125: 10 fc       ..             ; Bit7 clear: still waiting, poll again
...
.match_osbyte_code
    cmp l9145,x                                                       ; 913c: dd 45 91    .E.            ; Compare OSBYTE code with table entry
...
.loop_c9162
    ...
    bpl loop_c9162                                                    ; 9167: 10 f9       ..             ; Loop for all parameter bytes
    ...
    jsr setup_tx_and_send                                             ; 917a: 20 c4 90     ..            ; Transmit via workspace TXCB
...
.ctrl_block_setup_alt
    ...
    bit l83b3                                                         ; 9183: 2c b3 83    ,..            ; BIT test: V flag = bit 6 of &83B3
...
.cbset2
    ...
    beq l91b0                                                         ; 9192: f0 1c       ..             ; End of template: jump to exit
    cmp #&fd                                                          ; 9194: c9 fd       ..             ; &FD = skip sentinel
    beq l91ac                                                         ; 9196: f0 14       ..             ; Skip: don't store, just decrement Y
...
.printer_select_handler
    ...
    cpx l00f0                                                         ; 91dc: e4 f0       ..             ; Is this the network printer buffer?
...
.setup1
    sta l0099                                                         ; 91e7: 85 99       ..             ; Store A to printer status byte
...
.c922a
    ...
    sta l0099                                                         ; 9234: 85 99       ..             ; Update PFLAGS
    rts                                                               ; 9236: 60          `              ; Return
...
.flush_output_block
    ...
    lda l0099                                                         ; 924f: a5 99       ..             ; Load protocol flags (PFLAGS)
    ...
    sta l0099                                                         ; 9256: 85 99       ..             ; Save toggled PFLAGS
...
.bsxl0
    ...
    jsr c8530                                                         ; 929b: 20 30 85     0.            ; Transmit and wait for reply (BRIANX)
...
.loop_c969a
    lda l9f7c,y                                                       ; 969a: b9 7c 9f    .|.            ; Read byte from NMI shim ROM source
    sta l0cff,y                                                       ; 969d: 99 ff 0c    ...            ; Write to NMI shim RAM at &0D00
    dey                                                               ; 96a0: 88          .              ; Next byte (descending)
    bne loop_c969a                                                    ; 96a1: d0 f7       ..             ; Loop until all 32 bytes copied
...
.scout_match_port
    ...
    bvc c976b                                                         ; 9764: 50 05       P.             ; Not broadcast -- skip CR2 setup
...
.c9778
    ...
    bne c979e                                                         ; 9780: d0 1c       ..             ; Not wildcard -- check specific port match
    iny                                                               ; 9782: c8          .              ; Y=&01
    lda (port_ws_offset),y                                            ; 9783: b1 a6       ..             ; Read port number from slot (offset 1)
    beq c978c                                                         ; 9785: f0 05       ..             ; Zero port in slot = match any port
    cmp rx_port                                                       ; 9787: cd 40 0d    .@.            ; Check if port matches this slot
    bne c979e                                                         ; 978a: d0 12       ..             ; Port mismatch -- try next slot
...
.c978c
    ...
    beq c97b9                                                         ; 978f: f0 28       .(             ; Zero station = match any station, accept
    cmp rx_src_stn                                                    ; 9791: cd 3d 0d    .=.            ; Check if source station matches
    bne c979e                                                         ; 9794: d0 08       ..             ; Station mismatch -- try next slot
...
.c979e
    ...
    bcc c9778                                                         ; 97a9: 90 cd       ..             ; Always branches (page &C0 won't overflow)
; &97ab referenced 2 times by &977c, &97b1
.c97ab
    jmp c9835                                                         ; 97ab: 4c 35 98    L5.            ; No match found -- discard frame

; &97ae referenced 2 times by &976e, &97a0
.c97ae
    bit rx_flags                                                      ; 97ae: 2c 64 0d    ,d.            ; Try NFS workspace if paged list exhausted
    bvc c97ab                                                         ; 97b1: 50 f8       P.             ; No NFS workspace RX (bit6 clear) -- discard
    ...
    bne c9774                                                         ; 97b7: d0 bb       ..             ; Scan NFS workspace port list
...
.c97b9
    ...
    bcc c9835                                                         ; 97c1: 90 72       .r             ; C=0: no Tube claimed -- discard
    bit tx_flags                                                      ; 97c3: 2c 4a 0d    ,J.            ; Check broadcast flag for ACK path
    bvc c97cb                                                         ; 97c6: 50 03       P.             ; Not broadcast -- normal ACK path
    jmp c99f2                                                         ; 97c8: 4c f2 99    L..            ; Broadcast: different completion path
...
.data_rx_setup
    ...
    jmp l0d11                                                         ; 97e3: 4c 11 0d    L..            ; Install nmi_data_rx and return from NMI
...
.nmi_data_rx
    ...
    beq c9835                                                         ; 97eb: f0 48       .H             ; No AP: wrong frame or error
    ...
    bne c9835                                                         ; 97f3: d0 40       .@             ; Not for us: error path
    lda #&fa                                                          ; 97f5: a9 fa       ..             ; Install net check handler at &97FA
    jmp l0d11                                                         ; 97f7: 4c 11 0d    L..            ; Set NMI vector via RAM shim

.nmi_data_rx_net
    bit econet_control23_or_status2                                   ; 97fa: 2c a1 fe    ,..            ; Validate source network = 0
    bpl c9835                                                         ; 97fd: 10 36       .6             ; SR2 bit7 clear: no data ready -- error
    lda econet_data_continue_frame                                    ; 97ff: ad a2 fe    ...            ; Read dest network byte
    bne c9835                                                         ; 9802: d0 31       .1             ; Network != 0: wrong network -- error
...
.nmi_data_rx_skip
    ...
    bpl c9835                                                         ; 9813: 10 20       .              ; SR2 bit7 clear: error
...
.c981b
    ...
    bne c982e                                                         ; 9820: d0 0c       ..             ; Tube active: use Tube RX path
...
.c9848
    ...
    bne c9858                                                         ; 9850: d0 06       ..             ; Y != 0: no page boundary crossing
    ...
    beq c9835                                                         ; 9856: f0 dd       ..             ; No pages left: handle as complete
; &9858 referenced 1 time by &9850
.c9858
    lda econet_control23_or_status2                                   ; 9858: ad a1 fe    ...            ; Read SR2 between byte pairs
    bmi c985f                                                         ; 985b: 30 02       0.             ; SR2 bit7 set: more data available
...
.c985f
    ...
    bne c986f                                                         ; 9867: d0 06       ..             ; Y != 0: no page boundary crossing
...
.c986f
    ...
    bne c9848                                                         ; 9872: d0 d4       ..             ; SR2 non-zero: more data, loop back
...
.data_rx_complete
    ...
    sta econet_control23_or_status2                                   ; 9879: 8d a1 fe    ...            ; Write CR2: disable PSE for bit testing
    lda #0                                                            ; 987c: a9 00       ..             ; CR1=&00: disable all interrupts
    sta econet_control1_or_status1                                    ; 987e: 8d a0 fe    ...            ; Write CR1: disable all interrupts
...
.c988e
    beq c9835                                                         ; 988e: f0 a5       ..             ; No buffer space: error/discard frame
    ...
    bne c989d                                                         ; 9899: d0 02       ..             ; No page wrap: proceed to send ACK
...
.loop_c98a3
    ...
    jsr sub_c9a37                                                     ; 98a8: 20 37 9a     7.            ; Check buffer limits and transfer size
    beq c988e                                                         ; 98ab: f0 e1       ..             ; Zero: buffer full, handle as error
    ...
    jsr sub_c9a37                                                     ; 98b6: 20 37 9a     7.            ; Check limits after byte pair
    ...
    bne loop_c98a3                                                    ; 98be: d0 e3       ..             ; More data available: continue loop
.data_rx_tube_error
...
.data_rx_tube_complete
    ...
    beq c988e                                                         ; 98d2: f0 ba       ..             ; No FV: incomplete frame, error
    ...
    beq c988e                                                         ; 98de: f0 ae       ..             ; All zero (null buffer): error
...
.ack_tx
    ...
    jsr sub_c994e                                                     ; 98f3: 20 4e 99     N.            ; Final ACK: call completion handler
...
.c994b
    jmp c9835                                                         ; 994b: 4c 35 98    L5.            ; Jump to error handler
...
.loop_c9959
    ...
    bcc loop_c9959                                                    ; 9965: 90 f2       ..             ; No: continue adding
    ...
    beq c9992                                                         ; 996d: f0 23       .#             ; No Tube: skip Tube update
...
.loop_c9987
    ...
    bcs loop_c9987                                                    ; 998e: b0 f7       ..             ; Keep going while carry propagates
...
.return_10
    ...
    bne c99a4                                                         ; 9998: d0 0a       ..             ; Port != 0: data transfer frame
    ldy rx_ctrl                                                       ; 999a: ac 3f 0d    .?.            ; Port=0: load control byte
    cpy #&82                                                          ; 999d: c0 82       ..             ; Ctrl = &82 (POKE)?
    beq c99a4                                                         ; 999f: f0 03       ..             ; Yes: POKE also needs data transfer
    jmp c9ae7                                                         ; 99a1: 4c e7 9a    L..            ; Other port-0 ops: immediate dispatch

; &99a4 referenced 3 times by &9998, &999f, &9a16
.c99a4
    jsr sub_c994e                                                     ; 99a4: 20 4e 99     N.            ; Update buffer pointer and check for Tube
    bne c99bb                                                         ; 99a7: d0 12       ..             ; Transfer not done: skip buffer update
    ...
    bcc c99b2                                                         ; 99ae: 90 02       ..             ; No carry: skip high byte increment
...
.c99b2
    ...
    iny                                                               ; 99b6: c8          .              ; Y=9: buffer high byte offset; Y=&09
...
.c9a14
    ...
    jmp c99a4                                                         ; 9a16: 4c a4 99    L..            ; Jump to completion handler
...
.c9a19
    ...
    jsr sub_c9a37                                                     ; 9a1f: 20 37 9a     7.            ; Increment buffer position counters
...
.sub_c9a2b
    ...
    bmi c9a34                                                         ; 9a2d: 30 05       0.             ; Bit7 set: already released
...
.discard_after_reset
    ...
    bcc c9a76                                                         ; 9a4b: 90 29       .)             ; Out of range low: jump to discard
    cpy #&89                                                          ; 9a4d: c0 89       ..             ; Above &88: not an immediate op
    bcs c9a76                                                         ; 9a4f: b0 25       .%             ; Out of range high: jump to discard
    cpy #&87                                                          ; 9a51: c0 87       ..             ; HALT(&87)/CONTINUE(&88) skip protection
    bcs c9a63                                                         ; 9a53: b0 0e       ..             ; Ctrl >= &87: dispatch without mask check
...
.loop_c9a5d
    ...
    bpl loop_c9a5d                                                    ; 9a5f: 10 fc       ..             ; Loop until bit aligned
...
.c9a76
    jmp c9835                                                         ; 9a76: 4c 35 98    L5.            ; Jump to discard handler
...
.c9ae7
    ...
    lda #&84                                                          ; 9b03: a9 84       ..             ; IER bit 2: disable CB1 interrupt
    sta system_via_ier                                                ; 9b05: 8d 4e fe    .N.            ; Write IER to disable CB1
    ...
    ora #8                                                            ; 9b15: 09 08       ..             ; SR mode 4: shift out under CB1 control
...
.c9b6e
    ...
    bmi c9b86                                                         ; 9b81: 30 03       0.             ; Bit7 set: immediate operation ctrl byte
...
.c9b86
    ...
    bne c9bc5                                                         ; 9b90: d0 33       .3             ; Port != 0: skip immediate op setup
    cpx #&83                                                          ; 9b92: e0 83       ..             ; Ctrl < &83: PEEK/POKE need address calc
    bcs c9bb1                                                         ; 9b94: b0 1b       ..             ; Ctrl >= &83: skip to range check
...
.loop_c9b9a
    ...
    bcc loop_c9b9a                                                    ; 9bae: 90 ea       ..             ; No: next byte pair
...
.loop_c9bbb
    ...
    bcc loop_c9bbb                                                    ; 9bc3: 90 f6       ..             ; No: continue copying
...
.tx_prepare
    ...
    lda l9e39,y                                                       ; 9c55: b9 39 9e    .9.            ; Look up tx_length from table
    ...
    lda l9be2,y                                                       ; 9c5e: b9 e2 9b    ...            ; Look up handler address low from table
...
.c9c8e
    ...
    bne c9cb0                                                         ; 9c96: d0 18       ..             ; Not broadcast: unicast path
...
.loop_c9ca4
    ...
    bcc loop_c9ca4                                                    ; 9cac: 90 f6       ..             ; No: continue copying
...
.data_tx_begin
    ...
    bne c9dc1                                                         ; 9db8: d0 07       ..             ; Bit 1 set: immediate op, use alt handler
...
.c9dcd
    ...
    bne c9ddd                                                         ; 9dd5: d0 06       ..             ; No page crossing
...
.c9ddd
    ...
    bne c9ded                                                         ; 9de5: d0 06       ..             ; No page crossing
...
.c9e12
    bvc c9e48                                                         ; 9e12: 50 34       P4             ; TDRA not ready -- error
    ...
    bne c9e2a                                                         ; 9e1c: d0 0c       ..             ; Low byte didn't wrap
    inc port_buf_len_hi                                               ; 9e1e: e6 a3       ..             ; Carry into second byte
    bne c9e2a                                                         ; 9e20: d0 08       ..             ; No further carry
    inc open_port_buf                                                 ; 9e22: e6 a4       ..             ; Carry into third byte
    bne c9e2a                                                         ; 9e24: d0 04       ..             ; No further carry
...
; &9e41 referenced 1 time by &9c4f
    bmi c9e12                                                         ; 9e43: 30 cd       0.             ; IRQ still set: write 2 more bytes
...
.print_hex_nibble
    ...
    bcc c9fae                                                         ; 9faa: 90 02       ..             ; No: skip letter offset
