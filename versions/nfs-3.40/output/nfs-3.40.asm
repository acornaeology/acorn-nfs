...
;    (dispatched via the 12-entry table at &0500). The R2 command byte is stored at &55
...
;
; Copied from ROM at &9456 during init. Contains: &0500: tube_dispatch_table — 12-entry
; handler address table &0518: R2 command byte table — 8 even command bytes (&00-&0E)
; &0520: tube_osbput — write byte to file &052D: tube_osbget — read byte from file &0537:
; tube_osrdch — read character &053A: tube_rdch_reply — ROR carry into byte, send via R2
; &053D: tube_release_return — dead code (unreferenced) &0542: tube_osfind — open file
; &0552: tube_osfind_close — close file (A=0) &055E: tube_osargs — file argument
; read/write &0582: tube_read_string — read CR-terminated string into &0700 &0596:
; tube_oscli — execute * command &059C: tube_reply_ack — send &7F acknowledge &059E:
; tube_reply_byte — send byte and return to main loop &05A9: tube_osfile — whole file
; operation
...
.skip_stn_parse
    jsr copy_filename                                                 ; 809d: 20 75 8d     u.      ; Copy command text to FS buffer
...
; command match table at &8C05, and from FSCV 2/3/4 indirectly. If CSD handle is zero
; (not logged in), returns without sending.
.forward_star_cmd
    jsr copy_filename                                                 ; 80c1: 20 75 8d     u.      ; Copy command text to FS buffer
    tay                                                               ; 80c4: a8          .     
...
.net_4_resume_remote
    ...
    jsr osbyte                                                        ; 81c6: 20 f4 ff     ..      ; Re-enable keyboard (X=0, Y=0)  osbyte: read write econet keyboard disable
...
.clear_osbyte_ce_cf
    ...
    lda #osbyte_read_write_econet_os_call_interception                ; 81d0: a9 ce       ..       ; OSBYTE &CE: first system mask to reset
...
.clear_osbyte_masks
    ...
    jsr osbyte                                                        ; 81d6: 20 f4 ff     ..      ; Reset system mask byte  osbyte: read write econet os call interception
...
.error1
    ...
    jsr osbyte                                                        ; 8457: 20 f4 ff     ..      ; osbyte: read write exec file handle
...
.remot1
    ...
    jsr osbyte                                                        ; 84c5: 20 f4 ff     ..      ; osbyte: read write econet keyboard disable
; ***************************************************************************************
...
.error_msg_table
    equb &a0                                                          ; 8579: a0          .     
    equs "Line Jammed", &00                                           ; 857a: 4c 69 6e... Lin...
    equb &a1                                                          ; 8586: a1          .     
    equs "Net Error", &00                                             ; 8587: 4e 65 74... Net...
    equb &a2                                                          ; 8591: a2          .     
    equs "Not listening", &00                                         ; 8592: 4e 6f 74... Not...
    equb &a3                                                          ; 85a0: a3          .     
    equs "No Clock", &00                                              ; 85a1: 4e 6f 20... No ...
    equb &11                                                          ; 85aa: 11          .     
    equs "Escape", &00                                                ; 85ab: 45 73 63... Esc...
    equb &cb                                                          ; 85b2: cb          .     
    equs "Bad Option", &00                                            ; 85b3: 42 61 64... Bad...
    equb &a5                                                          ; 85be: a5          .     
    equs "No reply", &00                                              ; 85bf: 4e 6f 20... No ...
; ***************************************************************************************
...
.loadop
    jsr copy_filename                                                 ; 8716: 20 75 8d     u.      ; Copy parsed filename to cmd buffer
...
.cha6
    jsr copy_filename                                                 ; 88d3: 20 75 8d     u.      ; A=6: copy filename (delete)
...
.gbpbf1
    ...
    bne gbpbf2                                                        ; 8a5b: d0 01       ..       ; Not at handle offset: continue
...
.fscv_2_star_run
    ...
    jsr copy_filename                                                 ; 8dd2: 20 75 8d     u.      ; Copy filename to FS command buffer
; ***************************************************************************************
...
;
; Pushes address &9BEB on the stack (so RTS returns to tx_done_exit), then does JMP
