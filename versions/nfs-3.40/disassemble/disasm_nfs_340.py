...
at [`tube_jmp_target`](label:tube_jmp_target) (self-modifying the
...
d.subroutine(
    ...
    description="""Copied from ROM at [`reloc_p4_src`](address:935D) during init. The first 28
...
| `&0414` | [`tube_send_release`](label:tube_send_release) — send R4 release-request to co-processor |
| `&041E` | [`tube_post_init`](label:tube_post_init) — reset claimed-address state to `&80` |
| `&0430` | [`tube_transfer_setup`](label:tube_transfer_setup) — data transfer setup (SENDW), `&0430`–`&047C` |
| `&047D` | [`tube_begin`](label:tube_begin) — startup entry, sends ROM contents to Tube |
| `&04C4` | [`tube_claim_default`](label:tube_claim_default) — claim default transfer address |
| `&04CB` | [`tube_init_reloc`](label:tube_init_reloc) — extract relocation address from ROM |""",
)
...
d.subroutine(
    ...
    description="""Copied from ROM at [`reloc_p5_src`](address:9456) during init.
...
| `&0518`–`&051F`  | [`tube_ctrl_values`](label:tube_ctrl_values) — R2 command byte table (8 even bytes `&00`–`&0E`) |
...
| `&053A`          | [`tube_rdch_reply`](label:tube_rdch_reply) — `ROR` carry into byte, send via R2 |
| `&053D`          | `tube_release_return` — dead code (unreferenced) |
| `&0542`          | [`tube_osfind`](label:tube_osfind) — open file |
| `&0552`          | [`tube_osfind_close`](label:tube_osfind_close) — close file (`A=0`) |
| `&055E`          | [`tube_osargs`](label:tube_osargs) — file argument read / write |
| `&0582`          | [`tube_read_string`](label:tube_read_string) — read CR-terminated string into `&0700` |
| `&0596`          | [`tube_oscli`](label:tube_oscli) — execute `*` command |
| `&059C`          | [`tube_reply_ack`](label:tube_reply_ack) — send `&7F` acknowledge |
| `&059E`          | [`tube_reply_byte`](label:tube_reply_byte) — send byte and return to main loop |
...
primarily during [`tube_send_release`](label:tube_send_release) and
...
d.index_base(0x8018, "error_offsets")
d.banner(
    0x8018,
...
d.comment(0x8147, "Set EVNTV high byte (EVNTV+1)", align=Align.INLINE)
...
d.comment(0x8151, "Set BRKV high byte (brkv+1)", align=Align.INLINE)
...
d.subroutine(
    ...
    description="""The first 7 bytes (&81A5-&81AB) are the service handler epilogue:
...
- `X=&0C`: matches `"ROFF"` at [`cmd_roff_str`](label:cmd_roff_str) — the
  suffix of the copyright string `"(C)ROFF"` → `*ROFF` (Remote Off,
  end remote session). Falls through to
  [`net_4_resume_remote`](label:net_4_resume_remote).
- `X=5`: matches `"NET"` at [`cmd_net_str`](label:cmd_net_str) — the ROM
  title suffix → `*NET` (select NFS). Falls through to
  [`svc_13_select_nfs`](label:svc_13_select_nfs).
...
d.expr(0x81B2, sym("cmd_roff_str") - sym("binary_version"))
...
d.expr(0x81E7, sym("cmd_net_str") - sym("binary_version"))
...
d.subroutine(
    ...
    description="""Copies 14 bytes from fs_vector_addrs (&8296) into FILEV-FSCV (&0212),
...
d.index_base(0x8579, "error_msg_table")
d.banner(
    0x8579,
...
d.index_base(0x8579, "msg_line_jammed")
...
d.index_base(0x85FA, "access_bit_table")
...
d.subroutine(
    ...
    description="""Calls save_fscv_args (&85D2) to preserve A/X/Y, then JSR &86D7
...
d.subroutine(
    ...
    description="""  A=0: close file -- delegates to close_handle (&89CC)
...
d.index_base(0x8D08, "boot_option_text")
...
`&8D`. The offset table at [`boot_oscli_offset`](label:boot_oscli_offset) is
...
for i in range(5):
    d.byte(0x8D1B + i)
d.index_base(0x8D1C, "boot_oscli_offset")
...
d.index_base(0x8EB0, "osword_handler_lo")

d.label(0x8EB3, "copyl3")

d.index_base(0x8EB5, "fs_osword_tbl_hi")
...
d.index_base(0x8EF7, "osword_12_offsets")
d.label(0x8EF9, "rs")
...
d.comment(0x8FB0, "Dest station = broadcast (accept reply from any station)", align=Align.INLINE)
...
d.comment(0x901F, "Set RX end address to max (accept any length)", align=Align.INLINE)
...
d.index_base(0x909A, "osword_tbl_lo")

for i in range(9):
    d.rts_code_ptr(0x909A + i, 0x90A3 + i)
d.label(0x90A1, "osword_tbl_hi")

d.index_base(0x90A3, "netvec_handler_hi")
...
d.index_base(0x9139, "remote_osbyte_table")
...
d.subroutine(
    ...
    description="""Sets X=&0D, Y=&7C. Tests bit 6 of &83AF to choose target:
...
d.subroutine(
    ...
    description="""Read by the loop at &9181, indexed by X from a starting value
...
d.comment(0x9277, "End address = max (unlimited data length)", align=Align.INLINE)
...
d.index_base(0x9319, "vdu_osbyte_table")
...
NMI shim from ROM (&9FA8) to RAM (nmi_code_base), patches the current
...
d.comment(0x968D, "Copy 32 bytes of NMI shim from ROM to nmi_code_base", align=Align.INLINE)
...
d.comment(0x9692, "Write to NMI shim RAM (nmi_code_base,Y)", align=Align.INLINE)
...
d.subroutine(
    ...
    description="""Reached when the scout data loop sees no RDA (BPL at &9748) or
...
d.subroutine(
    ...
    description="""Selects either the normal bulk RX handler (&9880) or the Tube
...
d.subroutine(
    ...
    description="""Installs nmi_rx_scout (&96F2) as the NMI handler via
...
(deferred dispatch table at `&9B74`). PEEK, POKE and machine-type
...
d.comment(0x9A61, "CONTINUE(&87)/machine-type(&88) skip mask", align=Align.INLINE)
...
d.subroutine(
    ...
    description="""Sets up a response buffer (start `&25`, page `&7F`, length 508 bytes)
...
d.comment(0x9B66, "Set bits 2-4: block re-entrant exec ops", align=Align.INLINE)
...
&9BEB on the stack (so RTS returns to tx_done_exit), then does
JMP (l0d58) to call the remote JSR target routine. When that
...
d.label(0x9C1E, "tx_dcd_clock_check")
d.comment(0x9C1E, "A=&20: mask for SR2 DCD (clock/carrier detect)", align=Align.INLINE)
d.comment(0x9C20, "BIT SR2: test DCD -- is there a clock?", align=Align.INLINE)
d.comment(0x9C23, "DCD set: no clock on the line, abandon TX", align=Align.INLINE)
...
d.comment(0x9C45, "Read SR1 -- arms the CLR_RX_ST below", align=Align.INLINE)
...
d.subroutine(
    ...
    description="""Prints the high nibble first (via 4x LSR), then the low
...
d.subroutine(
    ...
    description="""Converts the low nibble of A to ASCII hex ('0'-'9' or 'A'-'F')
...
d.char_literal(0x830B)
d.char_literal(0x8363)
d.char_literal(0x837D)
d.char_literal(0x8627)
d.char_literal(0x8BF8)
d.char_literal(0x8C06)
d.char_literal(0x8D3C)
d.char_literal(0x8D6F)
d.char_literal(0x8E67)
d.char_literal(0x8E78)
d.char_literal(0x8F92)
d.char_literal(0x8FBB)
...
d.label(0x9A85, "imm_op_dispatch_lo")
d.comment(0x9A85, "Ctrl &81: PEEK", align=Align.INLINE)
d.comment(0x9A86, "Ctrl &82: POKE", align=Align.INLINE)
d.comment(0x9A87, "Ctrl &83: JSR", align=Align.INLINE)
d.comment(0x9A88, "Ctrl &84: UserProc", align=Align.INLINE)
d.comment(0x9A89, "Ctrl &85: OSProc", align=Align.INLINE)
d.comment(0x9A8A, "Ctrl &86: HALT", align=Align.INLINE)
d.comment(0x9A8B, "Ctrl &87: CONTINUE", align=Align.INLINE)
d.comment(0x9A8C, "Ctrl &88: machine-type", align=Align.INLINE)
d.label(0x9A8D, "imm_op_dispatch_hi")
d.comment(0x9A8D, "Ctrl &81: PEEK", align=Align.INLINE)
d.comment(0x9A8E, "Ctrl &82: POKE", align=Align.INLINE)
d.comment(0x9A8F, "Ctrl &83: JSR", align=Align.INLINE)
d.comment(0x9A90, "Ctrl &84: UserProc", align=Align.INLINE)
d.comment(0x9A91, "Ctrl &85: OSProc", align=Align.INLINE)
d.comment(0x9A92, "Ctrl &86: HALT", align=Align.INLINE)
d.comment(0x9A93, "Ctrl &87: CONTINUE", align=Align.INLINE)
d.comment(0x9A94, "Ctrl &88: machine-type", align=Align.INLINE)

d.label(0x9B74, "tx_done_dispatch_lo")
d.comment(0x9B74, "Ctrl &83: JSR", align=Align.INLINE)
d.comment(0x9B75, "Ctrl &84: UserProc", align=Align.INLINE)
d.comment(0x9B76, "Ctrl &85: OSProc", align=Align.INLINE)
d.comment(0x9B77, "Ctrl &86: HALT", align=Align.INLINE)
d.comment(0x9B78, "Ctrl &87: CONTINUE", align=Align.INLINE)
d.label(0x9B79, "tx_done_dispatch_hi")
d.comment(0x9B79, "Ctrl &83: JSR", align=Align.INLINE)
d.comment(0x9B7A, "Ctrl &84: UserProc", align=Align.INLINE)
d.comment(0x9B7B, "Ctrl &85: OSProc", align=Align.INLINE)
d.comment(0x9B7C, "Ctrl &86: HALT", align=Align.INLINE)
d.comment(0x9B7D, "Ctrl &87: CONTINUE", align=Align.INLINE)

d.label(0x9CBB, "tx_ctrl_dispatch_lo")
d.comment(0x9CBB, "Ctrl &81: PEEK", align=Align.INLINE)
d.comment(0x9CBC, "Ctrl &82: POKE", align=Align.INLINE)
d.comment(0x9CBD, "Ctrl &83: JSR", align=Align.INLINE)
d.comment(0x9CBE, "Ctrl &84: UserProc", align=Align.INLINE)
d.comment(0x9CBF, "Ctrl &85: OSProc", align=Align.INLINE)
d.comment(0x9CC0, "Ctrl &86: HALT", align=Align.INLINE)
d.comment(0x9CC1, "Ctrl &87: CONTINUE", align=Align.INLINE)
d.comment(0x9CC2, "Ctrl &88: machine-type", align=Align.INLINE)
d.label(0x9CC3, "tx_ctrl_dispatch_hi")
d.comment(0x9CC3, "Ctrl &81: PEEK", align=Align.INLINE)
d.comment(0x9CC4, "Ctrl &82: POKE", align=Align.INLINE)
d.comment(0x9CC5, "Ctrl &83: JSR", align=Align.INLINE)
d.comment(0x9CC6, "Ctrl &84: UserProc", align=Align.INLINE)
d.comment(0x9CC7, "Ctrl &85: OSProc", align=Align.INLINE)
d.comment(0x9CC8, "Ctrl &86: HALT", align=Align.INLINE)
d.comment(0x9CC9, "Ctrl &87: CONTINUE", align=Align.INLINE)
d.comment(0x9CCA, "Ctrl &88: machine-type", align=Align.INLINE)

d.comment(0x06D1, "ROM padding (&FF fill).")
d.label(0x06D1, "rom_padding_06d1")
