...
comment(0x823E, "X = lo byte of auto-boot string at &8292", inline=True)
...
comment(0x82F9, "Y = workspace high byte + 1 = next free page", inline=True)
...
comment(0x83E7, "Returns X=EXEC handle, Y=SPOOL handle", inline=True)
...
comment(0x85A5, "X = &1F (handle base - 1)", inline=True)
comment(0x85AB, "A = X = &1F + bit position = handle", inline=True)
...
comment(0x88A3, "A=5: X=1 (filename only, no data)", inline=True)
...
comment(0x893A, "Shared: A=0->&05, A=2->&01", inline=True)
comment(0x893B, "Return with A = FS number or 1", inline=True)
comment(0x893D, "Read FS command context byte", inline=True)
...
comment(0x8DC0, "Clean up stack (discard handle*4)", inline=True)
comment(0x8DC1, "Offset >= &48? (6 handles max)", inline=True)
comment(0x8DC3, "Valid: return with C clear", inline=True)
...
comment(0x8E55, "Store as copy source high byte in &AC", inline=True)
...
comment(0x8E81, "Sub-function 4 or 5: read/set station", inline=True)
...
comment(0x8E8B, "Store workspace page in &AC (hi byte)", inline=True)
comment(0x8E8D, "Load offset: &FF (sub 0-1) or &01 (sub 2-3)", inline=True)
comment(0x8E90, "Store offset in &AB (lo byte)", inline=True)
...
comment(0x8EB7, "Sub-function 8: read FS handle", inline=True)
comment(0x8EB9, "Match: read handle from RX buffer", inline=True)
...
comment(0x8F4C, "Workspace offset &1C = RX data start", inline=True)
...
comment(0x8F76, "Y=&23: start of template (descending)", inline=True)
comment(0x8F78, "Load ROM template byte", inline=True)
...
comment(0x8FA7, "Y=&20: RX end address offset", inline=True)
...
comment(0x8FB0, "Y=&19: port byte in workspace RXCB", inline=True)
...
comment(0x8FFC, "Add 3 for header bytes", inline=True)
...
comment(0x906B, "X=9: 10-entry NCTBPL table size", inline=True)
...
comment(0x90DB, "Copy parameter bytes from RX buffer", inline=True)
...
comment(0x90E5, "Store original OSBYTE code at workspace+0", inline=True)
comment(0x90E7, "Store OSBYTE code at ws+0", inline=True)
...
comment(0x915D, "BIT test: V flag = bit 6 of &83B3", inline=True)
...
comment(0x8567, "Dot separator?", inline=True)
comment(0x8587, "Return with result in A", inline=True)
comment(0x85A2, "Transfer mask to X for return", inline=True)
...
comment(0x8644, "Load year high byte", inline=True)
...
comment(0x864A, "Print CR to end catalogue line", inline=True)
...
comment(0x8D84, "X=2: OSWORD parameter block size", inline=True)
...
comment(0x8DC5, "Invalid: Y = 0", inline=True)
comment(0x8DC7, "A = 0, C set (error)", inline=True)
...
comment(0x8DE9, "A=&3F: close marker byte", inline=True)
comment(0x8DE9, "&3F = '?' marks slot as unused", inline=True)
comment(0x8DE9, "A=&3F: handle closed/unused marker", inline=True)
comment(0x8DEB, "Mark handle as closed in workspace", inline=True)
comment(0x8DEB, "Write close marker to workspace slot", inline=True)
comment(0x8DEB, "Write marker to handle slot", inline=True)
...
comment(0x8EFB, "Load from ROM template (zero = use NMI workspace value)", inline=True)
label(0x8EFD, "scan_or_read_rxcb")
comment(0x8F26, "Enable interrupts before transmit", inline=True)
comment(0x8F2C, "Dest station = &FFFF (accept reply from any station)", inline=True)
comment(0x8F3E, "Initiate receive with timeout", inline=True)
...
comment(0x8F57, "Y=&1C: RXCB template offset", inline=True)
comment(0x8F5D, "Receive data blocks until command byte = &00 or &0D", inline=True)
comment(0x8F94, "Retrieve original A (function code) from stack", inline=True)
comment(0x8FC6, "ROR/ASL on stacked P: zeros carry to signal success", inline=True)
...
comment(0x90F3, "Load template byte from ctrl_block_template[X]", inline=True)
comment(0x90FC, "Y=&80: RX data buffer offset", inline=True)
...
comment(0x91EC, "Check if Tube transfer active", inline=True)
comment(0x91F4, "Increment buffer pointer high byte", inline=True)
...
comment(0x96CD, "Y=nmi_shim_end-nmi_shim: byte count", inline=True)
comment(0x96CF, "Load NMI shim byte from ROM", inline=True)
label(0x96D1, "poll_nmi_complete")
comment(0x96D8, "NMI shim: JMP to relocated handler", inline=True)
comment(0x96DB, "Low byte of NMI handler address", inline=True)
comment(0x96DC, "High byte of NMI handler address", inline=True)
...
# print_file_info
comment(0x8603, "No data: exit early", inline=True)
comment(0x8609, "Compare with CR (end of filename)", inline=True)
comment(0x860D, "Compare with space", inline=True)
...
comment(0x861F, "Y=5: load address offset", inline=True)
...
comment(0x9123, "Y=0: query all", inline=True)
...
comment(0x920B, "Send accumulated data block", inline=True)
comment(0x920E, "Rotate flags for bit test", inline=True)
comment(0x9211, "Restore original byte", inline=True)
comment(0x9212, "Rotate for bit test", inline=True)
comment(0x9213, "Update control flags", inline=True)
...
comment(0x8DA9, "Transfer data via Tube", inline=True)
...
comment(0x84A5, "Y += 4 (entry point)", inline=True)
comment(0x84A7, "Y += 2", inline=True)
comment(0x84A8, "Y += 1", inline=True)
comment(0x84A9, "Return with Y adjusted", inline=True)
comment(0x84AA, "Y -= 4 (entry point)", inline=True)
comment(0x84AB, "Y -= 3 (entry point)", inline=True)
comment(0x84AC, "Y -= 2", inline=True)
comment(0x84AD, "Y -= 1", inline=True)
comment(0x8603, "No info available: return", inline=True)
comment(0x860D, "Also end name on space character", inline=True)
comment(0x8614, "Advance to next filename byte", inline=True)
comment(0x861A, "Advance past filename position", inline=True)
...
comment(0x8DA5, "Y=&0F: workspace offset for params", inline=True)
comment(0x8DA9, "Claim Tube for address transfer", inline=True)
comment(0x8DAC, "Execute at load address", inline=True)
...
comment(0x910F, "Save boot type on stack", inline=True)
comment(0x9115, "Store filename offset at ws+&0F", inline=True)
comment(0x9118, "Restore boot type from stack", inline=True)
comment(0x9123, "Y=0: second argument for OSBYTE", inline=True)
comment(0x9125, "A=&C9: disable keyboard for boot", inline=True)
comment(0x91B5, "A=0: clear printer buffer state", inline=True)
comment(0x91EA, "Flush done: continue loop", inline=True)
comment(0x9203, "Bit 0=0 (active): just flush", inline=True)
