...
.initl
    ...
    beq init_fs_vectors                                               ; 81d4: f0 7e       .~             ; ALWAYS branch
...
.svc_3_autoboot
    ...
    bmi print_station_info                                            ; 8216: 30 0a       0.
; ***************************************************************************************
; Check boot key
; 
...
; the .BOOT file.
    equs "I .BOOT", &0d                                               ; 8282: 49 20 2e... I .
; ***************************************************************************************
...
.store_rom_ptr_pair
    lda run_fscv_cmd,y                                                ; 8329: b9 7d 82    .}.
    sta (osrdsc_ptr),y                                                ; 832c: 91 f6       ..
    iny                                                               ; 832e: c8          .
    lda run_fscv_cmd,y                                                ; 832f: b9 7d 82    .}.
...
.loop_c8602
    ...
    bcs c8621                                                         ; 8606: b0 19       ..             ; Yes: not a digit, done
    cmp #&2e ; '.'                                                    ; 8608: c9 2e       ..             ; Dot separator?
    beq c8622                                                         ; 860a: f0 16       ..             ; Yes: exit with C=1 (dot found)
    bmi c8621                                                         ; 860c: 30 13       0.             ; Control char or space: done
    and #&0f                                                          ; 860e: 29 0f       ).             ; Mask ASCII digit to 0-9
    sta l00b3                                                         ; 8610: 85 b3       ..             ; Save new digit
    ...
    adc l00b3                                                         ; 861a: 65 b3       e.             ; + digit = total*10 + digit
...
.handle_to_mask
    ...
    beq c863e                                                         ; 862d: f0 0f       ..             ; C=1 and Y=0: skip (handle 0 = none)
...
.y2fsl2
    ...
    bne c863e                                                         ; 863b: d0 01       ..             ; Non-zero: valid mask, skip to exit
...
.loop_c89fa
    ...
    bmi c8a06                                                         ; 89fe: 30 06       0.             ; Negative: subtract instead
...
.gbpbx
    ...
    bne loop_c89fa                                                    ; 8a0d: d0 eb       ..             ; Loop 4 times (X=&FC,&FD,&FE,&FF,done)
    rts                                                               ; 8a0f: 60          `
...
.copy_string_from_offset
    ...
    sta fs_cmd_data,x                                                 ; 8d51: 9d 05 0f    ...            ; Store to command buffer
    inx                                                               ; 8d54: e8          .              ; Advance write position
    iny                                                               ; 8d55: c8          .              ; Advance read position
