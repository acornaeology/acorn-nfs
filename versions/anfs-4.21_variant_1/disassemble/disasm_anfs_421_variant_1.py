...
_svc_dispatch_entries = [
    ...
    (0x10, 0x8EE9, "svc_21_claim_abs_ws", "svc &21: claim absolute workspace (Hazel)"),
    (0x11, 0x8EFE, "svc_22_claim_private_ws", "svc &22: claim private workspace (Hazel)"),
    (0x12, 0x8EF0, "svc_23_record_abs_top", "svc &23: record top of absolute workspace"),
    (0x13, 0x8E71, "svc_24_claim_private_page", "svc &24: reserve one private-workspace page"),
    (0x14, 0x8E73, "svc_25_return_fs_info", "svc &25: FS name + info reply"),
...
for idx, name, role in _netv_dispatch_entries:
    d.expr(0xAD20 + idx, lo(sym(name) - 1))
    d.expr(0xAD29 + idx, hi(sym(name) - 1))
...
for idx, name, role in _osword_13_entries:
    d.expr(0xA9A8 + idx, lo(sym(name) - 1))
    d.expr(0xA9BA + idx, hi(sym(name) - 1))
...
for idx, target, name, desc in _svc_dispatch_entries:
    if name is not None:
        d.expr(0x89ED + idx, lo(sym(name) - 1))
        d.expr(0x8A20 + idx, hi(sym(name) - 1))
...
[`tx_done_dispatch_lo`](label:tx_done_dispatch_lo) low-byte table and a hard-
...
[`tx_done_dispatch_lo`](label:tx_done_dispatch_lo)+`Y` page-`&85` handler.""",
...
d.comment(0x80D3, "Install scout-data reader (low)", align=Align.INLINE)
...
d.comment(0x822A, "Save current ACCCON on stack", align=Align.INLINE)
...
Single caller: [`port_match_found`](label:port_match_found) at `&81A4`.""",
)
...
d.comment(0x840B, "Save current ACCCON on stack", align=Align.INLINE)
...
[`imm_op_dispatch_lo`](label:imm_op_dispatch_lo) (PHA/PHA/RTS).
...
[`tx_done_dispatch_lo`](label:tx_done_dispatch_lo) table. PEEK, POKE and
...
d.subroutine(
    ...
    description="""Eight low-byte entries at `&848B`-`&8492` indexed by the
...
[`tx_calc_transfer`](label:tx_calc_transfer) to send the PEEK response
...
d.subroutine(
    ...
    description="""Writes the reply-frame header for a port-0 immediate operation
into the RX buffer at offsets `&7F..&81`:

| RX offset | Source | Meaning |
|---|---|---|
| `&7F` | `port_buf_len + &80` | reply data length (raw count + header offset) |
| `&80` | [`scout_buf`](label:scout_buf) | requesting station |
| `&81` | [`scout_src_net`](label:scout_src_net) | requesting network |

Then loads the control byte from
[`scout_ctrl`](label:scout_ctrl) into `A` and falls through into
[`setup_sr_tx`](label:setup_sr_tx), which records `A` as
[`tx_op_type`](label:tx_op_type) and (for execute-class ops) arms the
...
[`dispatch_svc5`](label:dispatch_svc5) via
`LDA tx_done_dispatch_lo-&83,Y` (the operand lands mid-instruction
inside [`set_rx_buf_len_hi`](label:set_rx_buf_len_hi)). The dispatch
...
[`tx_done_dispatch_lo`](label:tx_done_dispatch_lo) / hi. The dispatcher
...
Reached only via `PHA`/`PHA`/`RTS` dispatch from
[`tx_done_dispatch_lo`](label:tx_done_dispatch_lo) / hi.""",
    on_exit={"a": "0 (success status)", "x, y": "restored from stack via tx_done_exit"},
)
...
Reached only via `PHA`/`PHA`/`RTS` dispatch from
[`tx_done_dispatch_lo`](label:tx_done_dispatch_lo) / hi. Falls through to
...
[`tx_done_dispatch_lo`](label:tx_done_dispatch_lo) table. Pulls the saved
...
d.subroutine(
    ...
    description="""Main TX initiation entry point (called via the NETV trampoline).
...
3. Calculates transfer sizes via
   [`tx_calc_transfer`](label:tx_calc_transfer); copies extra
   parameters into the workspace.
4. Checks DCD (SR2 bit 5): no clock on the line aborts with
   'No Clock'.
5. Enters the INACTIVE polling loop at
   [`inactive_poll`](label:inactive_poll).""",
)
...
d.label(0x85E0, "tx_dcd_clock_check")

d.comment(0x85E0, "A=&20: mask for SR2 DCD (clock/carrier detect)", align=Align.INLINE)
d.comment(0x85E2, "Test SR2 DCD -- is there a clock?", align=Align.INLINE)
d.comment(0x85E5, "DCD set: no clock on the line, abandon TX", align=Align.INLINE)
...
5. Falls through to [`intoff_test_inactive`](label:intoff_test_inactive)
...
[`disable_net_nmis`](label:disable_net_nmis) (the Master 128 INTOFF register),
then polls `SR2` for the INACTIVE bit (bit 2):

| `SR2` INACTIVE | Action |
|---|---|
| set   | read `SR1`, write `CR2=&67` to clear status, then test `CTS` (`SR1` bit 4); if `CTS` present, branch to [`tx_prepare`](label:tx_prepare) |
| clear | re-enable NMIs via [`enable_net_nmis`](label:enable_net_nmis) (INTON) and decrement the 3-byte timeout counter on the stack |

On timeout, falls through to
[`tx_line_jammed`](label:tx_line_jammed).""",
    on_entry={"a": "&04 (INACTIVE bit mask)", "y": "&E7 (CR2 value for tx_prepare)"},
)
...
d.comment(0x8607, "Read SR1 -- arms the CLR_RX_ST below", align=Align.INLINE)
...
[`tx_begin`](label:tx_begin) (`&859E`, `&85CE`, `&85D2`) when the
...
[`intoff_test_inactive`](label:intoff_test_inactive) loop times out without
...
[`tx_ctrl_machine_type`](label:tx_ctrl_machine_type), etc.), not here.""",
...
d.subroutine(
    ...
    description="""Handler for control byte `&88`. Sets `scout_status = 3` and
...
Reached only via `PHA`/`PHA`/`RTS` dispatch from
[`tx_ctrl_dispatch_lo`](label:tx_ctrl_dispatch_lo) entry `&88`.""",
...
d.comment(0x87D5, "Install nmi_data_tx alt-entry (low)", align=Align.INLINE)
d.comment(0x87D7, "Install nmi_data_tx alt-entry (high)", align=Align.INLINE)
...
d.comment(0x87F4, "Save current ACCCON on stack", align=Align.INLINE)
...
d.subroutine(
    ...
    description="""Inspects `RXCB[6..7]` (buffer end address byte 2 and high) to
...
Three callers: [`scout_complete`](label:scout_complete) (`&819A`),
[`rx_imm_peek`](label:rx_imm_peek) (`&84DB`),
[`tx_ctrl_proc`](label:tx_ctrl_proc) (`&86DD`).""",
    on_entry={"y": "0 -- caller convention"},
    on_exit={"a": "transfer status", "c": "set if Tube address claimed, clear otherwise"},
)


d.comment(0x8900, "Read ACCCON (access-control register)", align=Align.INLINE)
...
to [`save_econet_state`](label:save_econet_state) to clear the initialised
...
[`tx_complete_flag`](label:tx_complete_flag) and
[`econet_init_flag`](label:econet_init_flag) by storing the current `A`
value. Sets `Y=5` (service-call workspace page) and jumps to
[`adlc_rx_listen`](label:adlc_rx_listen) to configure the ADLC for
passive listening.

Used during the wait-idle-and-reset path (svc &0D) to safely
tear down the Econet state before another ROM can claim the
NMI workspace.""",
    on_entry={"a": "value to store into tx_complete_flag / econet_init_flag (typically 0 to clear)"},
...
[`svc_dispatch`](label:svc_dispatch) as `LDA &89ED,X`. Paired with
the high-byte half at [`svc_dispatch_hi`](label:svc_dispatch_hi).
...
d.banner(
    ...
    description="""High-byte half of the `PHA`/`PHA`/`RTS` dispatch table read by
[`svc_dispatch`](label:svc_dispatch) as `LDA &8A20,X`. The
...
via the PHA/PHA/RTS table at [`svc_dispatch_lo`](label:svc_dispatch_lo) / [`svc_dispatch_hi`](label:svc_dispatch_hi).
...
|  41 |  &29 | 24 | [`svc_29_status`](label:svc_29_status)           | `*STATUS` handler           |

Every other value (&0D–&11, &13–&17, &19–&20, &2A and up) falls
through to index 1 ([`dispatch_rts`](label:dispatch_rts), a no-op) and returns the call
...
d.comment(
    0x8ABA, "C clear: service number was below the prior CMP threshold, take dispatch fall-through", align=Align.INLINE
)
...
d.comment(0x8BA6, "Store to the HAZEL &C2 FCB shadow copy", align=Align.INLINE)
...
[`print_cmd_table_loop`](label:print_cmd_table_loop) to enumerate
commands.""",
...
d.subroutine(
    ...
    description="""Walks the ANFS command table from offset `X`, printing each
...
[`help_wrap_if_serial`](label:help_wrap_if_serial) to handle line
...
[`print_inline`](label:print_inline): `CR + "Advanced NFS 4.21" +
...
[`svc_dispatch`](label:svc_dispatch) table at index 2. Pushes `Y`
...
[`check_urd_prefix`](label:check_urd_prefix) (`&8E1F` via
...
d.label(0x8EE9, "svc_21_claim_abs_ws")
...
d.label(0x8EEF, "rts_svc_21_claim_abs_ws")

d.comment(0x8EEF, "Return", align=Align.INLINE)
d.label(0x8EF0, "svc_23_record_abs_top")
...
d.entry(0x8EFE)
d.label(0x8EFE, "svc_22_claim_private_ws")
...
d.comment(0x8F35, "Yes: jump to svc_22_claim_private_ws (error path)", align=Align.INLINE)
...
| 41    | `&29`      | [`svc_29_status`](label:svc_29_status)           | *STATUS option         |
...
d.comment(0x9083, "Add byte from the HAZEL &C2 FCB shadow", align=Align.INLINE)
...
d.comment(0x908D, "Load byte from the HAZEL &C2 FCB shadow", align=Align.INLINE)
...
for i in range(12):
    d.byte(0x91ED + i)
d.expr(0x91ED, sym("syn_iam") - sym("syn_opt_dir") - 2)
d.comment(
    0x91ED,
...
d.expr(0x91EF, sym("syn_iam") - sym("syn_opt_dir") - 1)
d.comment(0x91EF, 'Idx 2: \\"(<stn.id.>) <user id.>...\\"', align=Align.INLINE)
d.expr(0x91F0, sym("syn_object") - sym("syn_opt_dir") - 1)
...
d.expr(0x91F6, sym("syn_access") - sym("syn_opt_dir") - 1)
...
d.expr(0x91F8, sym("syn_opt_stn") - sym("syn_opt_dir") - 1)
...
[`print_5_hex_bytes`](label:print_5_hex_bytes) at `&9D53`.""",
    on_entry={"a": "byte to print"},
    on_exit={"a": "preserved"},
)
...
([`cmd_ex`](label:cmd_ex)), `&B2F0` (`ex_print_col_sep`),
`&B75E` ([`cmd_wipe`](label:cmd_wipe)), `&B7CB`
(`prompt_yn`).""",
...
d.comment(
    0x92AF, "Resume execution at the terminator byte's address (JMP indirect via fs_error_ptr)", align=Align.INLINE
)
d.subroutine(
    0x92B2,
    "parse_addr_arg",
...
[`&962E`](address:962E) inside [`cmd_nospace`](label:cmd_nospace), and
...
d.subroutine(
    ...
    description="""Reads the printer-server's saved network number from CMOS byte
&04, prints it as decimal (no leading zeros), prints a `.`
separator, then sets `X=3` and falls into
[`print_cmos_decimal_nl`](label:print_cmos_decimal_nl) to read CMOS &03 and
print the printer-server station with a trailing newline.
Returns via the [`print_cmos_done`](label:print_cmos_done) trampoline.""",
)
...
d.subroutine(
    ...
    description="""Reads the file-server's saved network number from CMOS byte
&02, prints it as decimal (no leading zeros), prints a `.`
separator, then sets `X=1` and falls into
[`print_cmos_decimal_nl`](label:print_cmos_decimal_nl) to read CMOS &01 and
print the file-server station with a trailing newline. Returns
via the [`print_cmos_done`](label:print_cmos_done) trampoline.""",
)
...
d.comment(
    0x9697,
    "'ON ' -- 3-char pattern read by match_on_suffix at &969A via EOR &9697,X with X=0..2 to detect '... ON ' help-line suffix",
    align=Align.INLINE,
)
...
2. Copies the TX control-block pointer from `net_tx_ptr` to
   `nmi_tx_block`.
3. Calls [`tx_begin`](label:tx_begin), which performs a
...
4. After [`tx_begin`](label:tx_begin) returns, polls the TXCB
...
d.subroutine(
    0x9FEE, "send_open_file_request", description="Send file open request with V flag set for directory check."
)
...
d.label(
    0xA0FF,
    "cmos_attr_table",
    description="""Indexing-base alias of [`cmos_opt_mask_table`](label:cmos_opt_mask_table) - 4.
...
[`cmos_attr_table`](label:cmos_attr_table) (this label - 4): for write
...
d.comment(
    0xA3DA, "Store network number into fs_work_6 (the canonical form: 0=local, non-zero=remote)", align=Align.INLINE
)
...
d.comment(
    0xA3E7, "Read PB[0] (the OSWORD sub-function code in most calls); fall into byte_to_2bit_index", align=Align.INLINE
)
...
d.comment(
    0xA3F4, "ASL doubles the halved value (effectively undoes the ROR's divide while reusing C)", align=Align.INLINE
)
...
d.comment(
    0xA5FB, "Store low byte of (os_text_ptr + Y) -> fs_crc_lo (repurposed as a generic pointer)", align=Align.INLINE
)
...
[`boot_persist_fs_maybe`](label:boot_persist_fs_maybe) at `&A71C`, which
...
d.expr(0xA716, lo(sym("findlib_oscli_cmd")))
d.expr(0xA718, hi(sym("findlib_oscli_cmd")))
...
d.comment(0xA734, "Auto-boot flag was set: skip CTRL check, dispatch boot via boot_select_cmd", align=Align.INLINE)
d.comment(0xA736, "OSBYTE &79: scan keyboard", align=Align.INLINE)
d.comment(0xA738, "X = CTRL key scan code (negative-X INKEY form for OSBYTE &79)", align=Align.INLINE)
...
[`boot_cmd_oscli`](label:boot_cmd_oscli). Combined with `Y=&A7` (high
...
because [`boot_select_cmd`](label:boot_select_cmd) `BEQ`s out when
`hazel_fs_flags` is zero.

Index 2 (`&48`) lands inside
[`boot_cmd_load_str`](label:boot_cmd_load_str) — at offset 7, on its `!`
...
d.comment(
    0xA874,
    "BIT $abs -- 3-byte skip-trick that jumps over the extract_osword_subcode prologue when called via &A874",
    align=Align.INLINE,
)
d.entry(0xA874)
...
d.subroutine(
    ...
    description="""Right-shifts `ws_page` (PB[0]) into `A`, transfers it to `Y` for
...
[`tx_begin`](label:tx_begin) to launch the transmission.""",
...
d.banner(
    ...
    description="""Read by [`osword_13_dispatch`](label:osword_13_dispatch) as `LDA &A9A8,X`. Paired
with the high-byte half at [`osword_13_dispatch_hi`](label:osword_13_dispatch_hi).
...
[`cmd_prot`](label:cmd_prot) at `&B6D9` (the *Prot path).
...
`&ACCC`. Calls [`init_ws_copy_wide`](label:init_ws_copy_wide) to copy the
...
[`netv_dispatch_hi`](label:netv_dispatch_hi). The wrapper at
[`netv_handler`](label:netv_handler) reads the original A from the MOS
stack frame (`&0103,X` after TSX) and gates 9..&FF away to
[`return_6`](label:return_6) before dispatching reasons 0..8.""",
)
...
d.comment(
    0xAD54, "TX high = workspace page (so net_tx_ptr now points at the abort packet in workspace)", align=Align.INLINE
)
...
d.comment(
    0xADAA, "Always taken (after ORA result is non-zero); store into stack[&106+X] then walk down", align=Align.INLINE
)
...
d.comment(
    0xB29F, "X=0: place the argument at the start of the TX buffer; fall into copy_arg_to_buf", align=Align.INLINE
)
...
d.comment(0xB2CF, "Read fs_lib_flags (&C271)", align=Align.INLINE)
...
d.comment(
    0xB30E, "Divisor for units digit (always print at least the units to avoid the empty 0 case)", align=Align.INLINE
)
...
d.comment(
    0xB31F, "Save divisor in X across the print (print_char_no_spool preserves X is not guaranteed)", align=Align.INLINE
)
...
d.comment(0xB4E1, "A=&9E: control byte (PS-init pattern)", align=Align.INLINE)
...
d.comment(
    0xB53F, "Copy net_rx_ptr_hi as the TX page (TX shares the same page as RX for this packet)", align=Align.INLINE
)
...
d.comment(
    0xB571, "Restore caller's V (so print_decimal_3dig honours its own leading-zero suppression)", align=Align.INLINE
)
...
d.comment(0xB997, "Y=&C2: HAZEL page &C2", align=Align.INLINE)
d.comment(0xB999, "Return; X/Y point to &C2CA", align=Align.INLINE)
...
d.comment(0xB9EA, "Y=&C2: HAZEL page &C2", align=Align.INLINE)
...
d.comment(0xBA71, "Y=&C2: HAZEL page &C2", align=Align.INLINE)
...
d.comment(
    0xBF19, "Y=2: shift 3 bytes down 2 positions to drop the first 2 bytes (action code + a flag)", align=Align.INLINE
)
...
d.comment(
    0xBF64, "Y=&14: target offset = workspace+&13 (top of end-addr field, stored hi-byte-first)", align=Align.INLINE
)
...
d.comment(
    0xBFC4, "Return — total X advance depends on the entry: 4 (inx4), 8 (inx8), or 16 (inx16)", align=Align.INLINE
)
...
d.banner(
    ...
    description="""Three labels positioned `&1A`, `2`, and `1` bytes before `&C000`
...
d.comment(
    0xBFFF,
    "Base for `hazel_minus_1,Y` reads/writes -- `&BFFF + Y` reaches into HAZEL for Y >= 1 (used by loop_copy_station, osword_13_set_station)",
    align=Align.INLINE,
)
d.label(
    0xC000,
...
d.expr(0x81FF, lo(sym("nmi_data_rx_bulk")))
d.expr(0x8201, hi(sym("nmi_data_rx_bulk")))
d.expr(0x820B, lo(sym("nmi_data_rx_tube")))
d.expr(0x820D, hi(sym("nmi_data_rx_tube")))
d.expr(0x8310, lo(sym("nmi_ack_tx_src")))
d.expr(0x83EC, lo(sym("nmi_rx_scout")))
d.expr(0x83EE, hi(sym("nmi_rx_scout")))
d.expr(0x8541, hi(sym("tx_done_exit") - 1))
d.expr(0x8544, lo(sym("tx_done_exit") - 1))
d.expr(0x8653, lo(sym("nmi_tx_data")))
d.expr(0x876A, lo(sym("nmi_reply_validate")))
d.expr(0x88A2, lo(sym("nmi_final_ack_net")))
d.expr(0x9C1A, lo(sym("hazel_parse_buf")))
d.expr(0x9C1E, hi(sym("hazel_parse_buf")))

d.expr(0x8747, lo(sym("nmi_reply_scout")))

d.expr(0x80B4, lo(sym("nmi_rx_scout_net")))
d.expr(0x81BE, lo(sym("nmi_data_rx")))
d.expr(0x81D2, lo(sym("nmi_data_rx_net")))
d.expr(0x8312, hi(sym("nmi_ack_tx_src")))
d.expr(0x8729, lo(sym("nmi_tx_complete")))
d.expr(0x872B, hi(sym("nmi_tx_complete")))
d.expr(0x87B8, lo(sym("nmi_scout_ack_src")))
d.expr(0x87BA, hi(sym("nmi_scout_ack_src")))
d.expr(0x87DD, lo(sym("nmi_data_tx_tube")))
d.expr(0x87DF, hi(sym("nmi_data_tx_tube")))
d.expr(0x8836, lo(sym("discard_reset_rx")))
d.expr(0x8838, hi(sym("discard_reset_rx")))
d.expr(0x888C, lo(sym("nmi_final_ack")))
d.expr(0x888E, hi(sym("nmi_final_ack")))

d.label(0x80E8, "nmi_scout_data")
d.label(0x87EB, "nmi_data_tx_alt")
d.expr(0x80D4, lo(sym("nmi_scout_data")))
d.expr(0x87D6, lo(sym("nmi_data_tx_alt")))
d.expr(0x87D8, hi(sym("nmi_data_tx_alt")))

# --- character-literal immediate operands ---
d.char_literal(0x8BF5)
d.char_literal(0x8C39)
d.char_literal(0x8C6B)
d.char_literal(0x8DE3)
d.char_literal(0x8E32)
d.char_literal(0x90E1)
d.char_literal(0x9211)
d.char_literal(0x9215)
d.char_literal(0x9248)
d.char_literal(0x925E)
d.char_literal(0x92B7)
d.char_literal(0x92C3)
d.char_literal(0x92C7)
d.char_literal(0x92CB)
d.char_literal(0x92CF)
d.char_literal(0x92F3)
d.char_literal(0x92F7)
d.char_literal(0x9338)
d.char_literal(0x939B)
d.char_literal(0x939F)
d.char_literal(0x93A3)
d.char_literal(0x93A7)
d.char_literal(0x944A)
d.char_literal(0x947A)
d.char_literal(0x948C)
d.char_literal(0x9493)
d.char_literal(0x94A0)
d.char_literal(0x94AA)
d.char_literal(0x94EB)
d.char_literal(0x94FA)
d.char_literal(0x9515)
d.char_literal(0x9520)
d.char_literal(0x952C)
d.char_literal(0x953B)
d.char_literal(0x96C5)
d.char_literal(0x96CC)
d.char_literal(0x96E3)
d.char_literal(0x96F3)
...
d.char_literal(0x9D5B)
d.char_literal(0x9E8C)
d.char_literal(0x9E96)
d.char_literal(0x9EBF)
d.char_literal(0x9EC6)
d.char_literal(0xA40D)
d.char_literal(0xA428)
d.char_literal(0xA47D)
d.char_literal(0xA4A5)
d.char_literal(0xA4CB)
d.char_literal(0xA4CF)
d.char_literal(0xA4D6)
d.char_literal(0xA5F2)
d.char_literal(0xA94A)
d.char_literal(0xA973)
d.char_literal(0xB233)
d.char_literal(0xB246)
d.char_literal(0xB267)
d.char_literal(0xB276)
d.char_literal(0xB284)
d.char_literal(0xB288)
d.char_literal(0xB2AC)
d.char_literal(0xB2AE)
d.char_literal(0xB2B2)
d.char_literal(0xB2BD)
d.char_literal(0xB34C)
d.char_literal(0xB35B)
d.char_literal(0xB360)
d.char_literal(0xB382)
d.char_literal(0xB38F)
d.char_literal(0xB3EF)
d.char_literal(0xB452)
d.char_literal(0xB4D3)
d.char_literal(0xB55F)
d.char_literal(0xB5CD)
d.char_literal(0xB614)
d.char_literal(0xB6A0)
d.char_literal(0xB6C2)
d.char_literal(0xB6C6)
d.char_literal(0xB73A)
d.char_literal(0xB743)
d.char_literal(0xB769)
d.char_literal(0xB78A)
d.char_literal(0xB7A1)
d.char_literal(0xB7A3)
d.char_literal(0xB7C6)
d.char_literal(0xB815)
d.char_literal(0xB819)
d.char_literal(0xBDDF)
d.char_literal(0xBDE3)
d.char_literal(0xBE3C)
d.char_literal(0xBE58)
d.char_literal(0xBE5C)
d.char_literal(0xBE60)
