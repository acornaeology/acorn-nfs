...
subroutine(0x9AB5, "rx_imm_exec", hook=None,
...
subroutine(0x9AD3, "rx_imm_poke", hook=None,
...
subroutine(0x9BB3, "tx_done_user_proc", hook=None,
...
subroutine(0x9BC1, "tx_done_os_proc", hook=None,
...
subroutine(0x9BCD, "tx_done_halt", hook=None,
    title="TX done: HALT",
    description="""\
...
subroutine(0x9BE4, "tx_done_continue", hook=None,
...
subroutine(0x9CF7, "tx_ctrl_peek", hook=None,
...
subroutine(0x9CFB, "tx_ctrl_poke", hook=None,
...
subroutine(0x9D1A, "tx_ctrl_proc", hook=None,
...
# ============================================================
subroutine(0x8DA5, "print_hex", hook=None,
...
subroutine(0x85E2, "print_inline", hook=None,
...
# ============================================================
subroutine(0x80D4, "lang_entry_dispatch", hook=None,
...
# ============================================================
subroutine(0x80EA, "service_handler_entry", hook=None,
...
comment(0x900F, "Y=&04: advance to station address", inline=True)
...
comment(0x9F6F, "AND with TX block[7] (byte 3)", inline=True)
...
subroutine(0x9751, "scout_data_loop", hook=None,
...
#   Read byte 0 at &9DC8 -> refills byte 1 (RDA visible at &9DD7)
#   Read byte 1 at &9DDC -> refills byte 2 (RDA visible at &9DF2)
#   Read byte 2 at &9DF7 -> refills byte 3/LAST (FV set)
#   Read byte 3 at &9E00 -> FIFO empty
#   Check FV at &9E09 -> FV is set
...
comment(0x0646, "Test for OSBYTE &9D (fast Tube BPUT)", inline=True)
...
comment(0x06C6, "Check for CR terminator", inline=True)
...
subroutine(0x80F4, "check_svc_high", hook=None,
...
comment(0x81B8, "X=1: ROM offset for \"NET\" match", inline=True)
...
comment(0x8432, "A=&C6: read *EXEC file handle", inline=True)
comment(0x8437, "A=&F1: OSCLI \"SP.\" string at &84F1", inline=True)
...
comment(0x86C5, "X=&FF: next INX wraps to first char index", inline=True)
...
comment(0x87E2, "Y += 5", inline=True)
comment(0x87E3, "Y += 4", inline=True)
...
comment(0x87F5, "Y -= 4", inline=True)
...
comment(0x88D4, "X=1: filename only, no data extent", inline=True)
...
comment(0x8F05, "Sub-function 4 or 5: read/set protection", inline=True)
...
label(0x8F2F, "read_local_station_id") # Read cached local station number from RX buffer offset &14
comment(0x8F2F, "Y=&14: RX buf offset of cached station ID", inline=True)
comment(0x8F31, "Read cached local station number", inline=True)
comment(0x8F33, "Y=1: param block byte 1", inline=True)
comment(0x8F35, "Return station number to caller's param block", inline=True)
comment(0x8F37, "Return", inline=True)
comment(0x8F38, "Sub-function 8: read local station number", inline=True)
comment(0x8F3A, "Match: read cached station ID from RX buffer", inline=True)
...
comment(0x8FC7, "Y=2: copy 3 bytes (indices 2,1,0)", inline=True)
...
comment(0x9150, "Load param byte from OSWORD param block", inline=True)
...
comment(0x915A, "Load original OSWORD code", inline=True)
comment(0x915C, "Store OSWORD code at ws+0", inline=True)
...
comment(0x917B, "Set up TX and send RWORD packet", inline=True)
...
comment(0x9718, "Install next NMI handler at &971F (RX scout net byte)", inline=True)
...
comment(0x983C, "Install nmi_data_rx at &9843", inline=True)
...
comment(0x9863, "Install skip handler at &986F", inline=True)
comment(0x9865, "High byte of &986F handler", inline=True)
...
comment(0x98DA, "Write CR1", inline=True)
comment(0x98DF, "Write CR2", inline=True)
...
subroutine(0x9AF7, "rx_imm_peek_setup", hook=None,
    title="RX immediate: PEEK setup",
    description="""\
...
comment(0x9B44, "IER bit 2: disable SR interrupt", inline=True)
comment(0x9B46, "Write IER to disable SR", inline=True)
...
subroutine(0x9D0F, "tx_ctrl_add_done", hook=None,
    title="TX ctrl: JSR/UserProc/OSProc setup",
    description="""\
Sets scout_status=2 and calls tx_calc_transfer directly
(no 4-byte address addition needed for procedure calls).
...
comment(0x9D9C, "Install NMI handler at &9DA3 (nmi_tx_complete)", inline=True)
...
comment(0x9E96, "Install discard_reset_listen at &9A4A", inline=True)
comment(0x9E98, "High byte of &9A4A handler", inline=True)
...
comment(0x0453, "Discard return address (low byte)", inline=True)
comment(0x0454, "Discard return address (high byte)", inline=True)
...
comment(0x06E8, "Send zero prefix via R1", inline=True)
...
comment(0x8125, "A=&8E: Tube control register init value", inline=True)
comment(0x8127, "Write to Tube control register", inline=True)
...
comment(0x863F, "Restore X from stack", inline=True)
comment(0x8641, "Return with mask in X", inline=True)
comment(0x8649, "Return with handle in A", inline=True)
...
comment(0x87B0, "Display filename being saved", inline=True)
label(0x87B3, "save_csd_display")       # Save CSD from reply and display file info
comment(0x87B3, "Load CSD from FS reply", inline=True)
comment(0x87B6, "Transfer file data blocks to server", inline=True)
...
comment(0x87E4, "(continued)", inline=True)
comment(0x87E5, "(continued)", inline=True)
comment(0x87E6, "(continued)", inline=True)
comment(0x87F0, "Next byte (descending)", inline=True)
comment(0x87F3, "Loop until offset 2 reached", inline=True)
comment(0x87F6, "Y -= 3", inline=True)
comment(0x87F7, "(continued)", inline=True)
comment(0x87F8, "(continued)", inline=True)
...
comment(0x8C04, "Y=0: initialise column counter", inline=True)
...
comment(0x8EC3, "Save param block high byte to A", inline=True)
...
subroutine(0x96B1, "wait_nmi_ready", hook=None,
...
subroutine(0x96DC, "restore_econet_state", hook=None,
...
label(0x9B61, "check_sr_irq")         # Tests SR interrupt pending via IFR
comment(0x9B61, "A=&04: IFR bit 2 (SR) mask", inline=True)
comment(0x9B63, "Test SR interrupt pending", inline=True)
comment(0x9B66, "SR fired: handle TX completion", inline=True)
comment(0x9B68, "A=5: no SR, return status 5", inline=True)
comment(0x9B6A, "Return (no SR interrupt)", inline=True)
...
comment(0x9B7D, "A=&04: SR bit mask", inline=True)
comment(0x9B7F, "Clear SR in IFR", inline=True)
comment(0x9B82, "Disable SR in IER", inline=True)
...
comment(0x9C22, "(continued)", inline=True)
...
comment(0x9F89, "(continued)", inline=True)
comment(0x9F8A, "(continued)", inline=True)
comment(0x9F8B, "(continued)", inline=True)
comment(0x9F93, "(continued)", inline=True)
...
subroutine(0x8E7A, "restore_rx_flags", hook=None,
...
subroutine(0x8EFF, "osword_12_dispatch", hook=None,
...
subroutine(0x90B2, "nwrch_handler", hook=None,
...
comment(0x866F, "Set TX pointer high = 0 (page zero)", inline=True)
...
comment(0x91FE, "Y = current printer buffer offset", inline=True)
...
subroutine(0x9A2F, "store_rxcb_completion", hook=None,
