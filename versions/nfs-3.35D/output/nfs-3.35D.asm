...
;
; Copied from ROM during init (reloc_p4_src-18). The first 28 bytes (&0400-&041B) overlap
; with the end of the ZP block (the same ROM bytes serve both the ZP copy at &005B-&0076
; and this page at &0400-&041B). Contains: &0400: JMP &0473 (BEGIN — CLI parser / startup
; entry) &0403: JMP &06E2 (tube_escape_check) &0406: tube_addr_claim — Tube address claim
; protocol (ADRR) &0414: tube_post_init — called after ROM→RAM copy &0473: BEGIN —
; startup/CLI entry, break type check &04E7: tube_rdch_handler — RDCHV target &04EF:
; tube_restore_regs — restore X,Y, dispatch entry 6 &04F7: tube_read_r2 — poll R2 status,
; read data byte to A
    jmp tube_begin                                                    ; 935f: 4c 73 04    Ls. :005b[1]        ; JMP to BEGIN startup entry
    jmp tube_escape_check                                             ; 9362: 4c df 06    L.. :005e[1]        ; JMP to tube_escape_check (&06A7)
...
;
; Copied from ROM during init (reloc_p5_src-18). Contains: &0500: tube_dispatch_table —
...
;
; Copied from ROM during init (reloc_p6_src-18). &0600-&0601 is the tail of tube_osfile
; (BEQ to tube_reply_byte when done). Contains: &0602: tube_osgbpb — multi-byte file I/O
; &0626: tube_osbyte_short — 2-param OSBYTE (returns X) &063B: tube_osbyte_long — 3-param
; OSBYTE (returns carry+Y+X) &065D: tube_osword — variable-length OSWORD (buffer at
; &0130) &06A3: tube_osword_rdln — OSWORD 0 (read line, 5-byte params) &06BB:
; tube_rdln_send_line — send input line from &0700 &06D0: tube_send_r2 — poll R2 status,
; write A to R2 data &06D9: tube_send_r4 — poll R4 status, write A to R4 data &06E2:
; tube_escape_check — check &FF, forward escape to R1 &06E8: tube_event_handler — EVNTV:
; forward event (A,X,Y) via R1 &06F7: tube_send_r1 — poll R1 status, write A to R1 data
...
; Dispatch table: low bytes of (handler_address - 1) Each entry stores the low byte of a handler address minus 1, for use with the PHA/PHA/RTS dispatch trick at &80DA. See dispatch_0_hi (&8045) for the corresponding high bytes.
;
; Five callers share this table via different Y base offsets: Y=&00  Service calls 0-12       (indices 1-13) Y=&0D  Language entry reasons    (indices 14-18) Y=&12  FSCV codes 0-7           (indices 19-26) Y=&16  FS reply handlers        (indices 27-32) Y=&20  *NET1-4 sub-commands     (indices 33-36)
.dispatch_0_lo
...
; Dispatch table: high bytes of (handler_address - 1) Paired with dispatch_0_lo (&8021). Together they form a table of 37 handler addresses, used via the PHA/PHA/RTS trick at &80DA.
.dispatch_0_hi
...
; Y=&20, and dispatches via &80DA. Characters outside '1'-'4' fall through to return_1
; (RTS).
;
...
; command match table at &8BE2, and from FSCV 2/3/4 indirectly. If CSD handle is zero
...
;
; Checks per-ROM disable flag at &0DF0+X (new in 3.35D). If bit 7 is set, returns
; immediately; service calls &FE/&FF bypass this check. Intercepts three service calls:
; &FE: Tube init -- explode character definitions &FF: Full init -- vector setup, copy
; code to RAM, select NFS &12 (Y=5): Select NFS as active filing system All other service
; calls < &0D dispatch via c8150.
...
.net_4_resume_remote
    ...
    jsr osbyte                                                        ; 8198: 20 f4 ff     ..      ; Re-enable keyboard (X=0, Y=0)  osbyte: read write econet keyboard disable
...
.clear_osbyte_ce_cf
    ...
    lda #osbyte_read_write_econet_os_call_interception                ; 81a2: a9 ce       ..       ; OSBYTE &CE: first system mask to reset
...
.clear_osbyte_masks
    ...
    jsr osbyte                                                        ; 81a8: 20 f4 ff     ..      ; Reset system mask byte  osbyte: read write econet os call interception
...
.error1
    ...
    jsr osbyte                                                        ; 8434: 20 f4 ff     ..      ; osbyte: read write exec file handle
...
.remot1
    ...
    jsr osbyte                                                        ; 84a2: 20 f4 ff     ..      ; osbyte: read write econet keyboard disable
; ***************************************************************************************
...
; Econet error message table (ERRTAB, 7 entries). Each entry: error number byte followed by NUL-terminated string. &A0: "Line Jammed"     &A1: "Net Error" &A2: "Not listening"   &A3: "No Clock" &11: "Escape"           &CB: "Bad Option" &A5: "No reply" Indexed by the low 3 bits of the TXCB flag byte (AND #&07), which encode the specific Econet failure reason. The NREPLY and NLISTN routines build a MOS BRK error block at &100 on the stack page: NREPLY fires when the fileserver does not respond within the timeout period; NLISTN fires when the destination station actively refused the connection. Indexed via the error dispatch at c8424/c842c.
; &8556 referenced 1 time by &84e5
.error_msg_table
    equb &a0                                                          ; 8556: a0          .     
    equs "Line Jammed", &00                                           ; 8557: 4c 69 6e... Lin...
    equb &a1                                                          ; 8563: a1          .     
    equs "Net Error", &00                                             ; 8564: 4e 65 74... Net...
    equb &a2                                                          ; 856e: a2          .     
    equs "Not listening", &00                                         ; 856f: 4e 6f 74... Not...
    equb &a3                                                          ; 857d: a3          .     
    equs "No Clock", &00                                              ; 857e: 4e 6f 20... No ...
    equb &11                                                          ; 8587: 11          .     
    equs "Escape", &00                                                ; 8588: 45 73 63... Esc...
    equb &cb                                                          ; 858f: cb          .     
    equs "Bad Option", &00                                            ; 8590: 42 61 64... Bad...
    equb &a5                                                          ; 859b: a5          .     
    equs "No reply", &00                                              ; 859c: 4e 6f 20... No ...
; ***************************************************************************************
...
; from NMI workspace &0DE6), transmit it, set up RX control block, and receive reply.
