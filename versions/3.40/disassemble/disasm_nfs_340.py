...
comment(0x8D7B, "Store to command buffer", inline=True)
comment(0x8D7E, "Advance write position", inline=True)
comment(0x8D7F, "Advance read position", inline=True)
...
comment(0x8F16, "Load byte from param block", inline=True)
comment(0x8F18, "Store to workspace", inline=True)
...
comment(0x8F1F, "Decrement byte counter", inline=True)
comment(0x8F20, "Loop while X >= 0", inline=True)
...
comment(0x9F3D, "AND with RXCB[7] (byte 3)", inline=True)
...
comment(0x9F93, "Propagate borrow only", inline=True)
comment(0x9F95, "Temp store of adjusted hi byte", inline=True)
comment(0x9F99, "Copy RXCB[8] to open port buffer lo", inline=True)
comment(0x9F9F, "Load RXCB[9]", inline=True)
comment(0x9FA2, "Subtract adjusted hi byte", inline=True)
...
label(0x04CB, "tube_init_reloc")     # Init relocation address for ROM→Tube transfer
...
comment(0x811D, "C=1 (ADLC active): duplicate ROM, skip", inline=True)
...
comment(0x81AC, "Padding: dispatch targets &81B5", inline=True)
...
comment(0x8287, "X = lo byte of auto-boot string at &8292", inline=True)
...
comment(0x8349, "Y = next workspace page for MOS", inline=True)
...
label(0x8383, "init_tx_reply_port")     # Init TX control block for FS reply on port &90
...
comment(0x8455, "Returns X=EXEC handle, Y=SPOOL handle", inline=True)
...
label(0x86D7, "copy_filename_ptr")       # Copy filename pointer to os_text_ptr and parse
...
label(0x86E3, "parse_filename_gs_y")     # Parse filename via GS from offset Y
...
comment(0x8976, "A=0: *ARGS Y,0; A=1: *ARGS Y,1; A>=2: FS", inline=True)
...
comment(0x897F, "&0A >> 1 = 5 = NFS filing system number", inline=True)
...
comment(0x8981, "Shared: A=0->&05, A=2->&01", inline=True)
comment(0x8982, "Return with A = FS number or 1", inline=True)
comment(0x8984, "Read FS command context byte", inline=True)
...
label(0x8990, "return_a_zero")          # Return with A=0 via register restore
...
comment(0x8C25, "Y=&FF: mark as escapable", inline=True)
...
comment(0x8D77, "Start source offset at 0", inline=True)
comment(0x8D79, "Load byte from source string", inline=True)
comment(0x8D7B, "Store to FS command buffer (&0F05+X)", inline=True)
comment(0x8D7E, "Advance dest pointer", inline=True)
...
label(0x8D92, "cat_column_separator")    # Print catalogue column separator or newline
...
label(0x8E45, "load_handle_calc_offset") # Load handle from &F0 and calc workspace offset
comment(0x8E45, "Load handle from &F0", inline=True)
comment(0x8E57, "A=0, C set = error indicator", inline=True)
...
comment(0x8EFF, "Sub-function 4 or 5: read/set station", inline=True)
...
comment(0x8F1A, "Load workspace byte for return", inline=True)
...
comment(0x8FAA, "X=11: copy 12 bytes from RXCB", inline=True)
...
comment(0x8FB0, "Zero: slot open, do copy", inline=True)
...
comment(0x8FCD, "Workspace offset &1C = RX data start", inline=True)
comment(0x8FCF, "A = base address low byte", inline=True)
comment(0x8FD1, "A = base + 1 (skip length byte)", inline=True)
comment(0x8FD3, "Store 16-bit start addr at ws+&1C/&1D", inline=True)
...
comment(0x90E6, "X=9: 10-entry NCTBPL table size", inline=True)
...
comment(0x9156, "Copy parameter bytes from RX buffer", inline=True)
...
comment(0x9160, "Store original OSBYTE code at workspace+0", inline=True)
comment(0x9162, "Store OSBYTE code at ws+0", inline=True)
...
comment(0x916E, "Transmit via workspace TXCB", inline=True)
...
comment(0x9177, "BIT test: V flag = bit 6 of &83B3", inline=True)
...
label(0x968D, "init_nmi_workspace")     # Init NMI workspace (skip service request)
...
comment(0x970A, "Install next NMI handler at &96DC (RX scout net byte)", inline=True)
...
comment(0x9738, "Neither set -- clean end, discard via &99E8", inline=True)
comment(0x9748, "No RDA -- error handler &96FE", inline=True)
...
comment(0x981A, "Install nmi_data_rx at &97E6", inline=True)
...
comment(0x9830, "Install net check handler at &97FA", inline=True)
...
label(0x9858, "install_data_rx_handler") # Select bulk or Tube RX handler
...
comment(0x986B, "Tube: install Tube RX at &98A0", inline=True)
comment(0x986D, "High byte of &98A0 handler", inline=True)
comment(0x986F, "Install Tube handler and RTI", inline=True)
label(0x9872, "nmi_error_dispatch")    # NMI error handler dispatch (12 refs)
...
comment(0x98B4, "CR2=&84: disable PSE for individual bit testing", inline=True)
comment(0x98B6, "Write CR2: disable PSE for bit testing", inline=True)
comment(0x98B9, "CR1=&00: disable all interrupts", inline=True)
comment(0x98BB, "Write CR1: disable all interrupts", inline=True)
...
comment(0x9959, "Save &9995 (post-ACK port check) in &0D4B/&0D4C", inline=True)
...
comment(0x9990, "CR2=&3F: TX_LAST_DATA | CLR_RX_ST | PSE", inline=True)
comment(0x9992, "Write CR2 to clear status after ACK TX", inline=True)
comment(0x9995, "Load saved next handler low byte", inline=True)
...
label(0x99A4, "advance_rx_buffer_ptr") # Advance RX buffer pointer after transfer
...
label(0x9A40, "install_rx_scout_handler") # Install RX scout NMI handler
comment(0x9A40, "Install nmi_rx_scout (&96BF) as NMI handler", inline=True)
...
label(0x9AFC, "imm_op_build_reply")    # Build immediate operation reply header
...
label(0x9BC7, "tx_begin")              # Begin TX operation
...
label(0x9C3A, "intoff_test_inactive")  # Disable NMIs and test INACTIVE
...
comment(0x9C91, "Install NMI handler at &9CCC (nmi_tx_data)", inline=True)
...
comment(0x9D6E, "Install NMI handler at &9D14 (nmi_tx_complete)", inline=True)
...
comment(0x9DA2, "Install nmi_reply_cont at &9D44", inline=True)
comment(0x9DB3, "Install nmi_reply_validate at &9D5B", inline=True)
comment(0x9DBA, "IRQ set -- fall through to &9D5B without RTI", inline=True)
...
comment(0x9DE8, "Save handshake_await_ack (&9E50) in &0D4B/&0D4C", inline=True)
...
comment(0x9E03, "Install nmi_scout_ack_src at &9DA3", inline=True)
...
comment(0x9E21, "Install nmi_data_tx at &9DC8", inline=True)
...
comment(0x9E28, "Install nmi_imm_data at &9E0F", inline=True)
...
comment(0x9F99, "Load RXCB[8] (start ptr lo)", inline=True)
