#!/usr/bin/env python3
"""Generate the initial disasm_nfs_340.py by mapping addresses from 3.35K.

Uses opcode-level sequence matching to build an address map between
the 3.35K and 3.40 ROMs, then transforms the 3.35K disassembly
...
def transform_script(script_text, addr_map):
    """Transform the 3.35K script to 3.40 by mapping addresses."""
    ...
    for start_idx, end_idx, group_lines in groups:
        ...
        if stripped.startswith('#') or stripped == '':
            for gl in group_lines:
                ...
                gl = gl.replace('nfs-3.35K', 'nfs-3.40')
                if 'Address map: NFS 3.35D' in gl:
                    gl = gl.replace('Address map: NFS 3.35D \u2192 NFS 3.35K',
                                    'Address map: NFS 3.35K \u2192 NFS 3.40')
                if 'derived from the NFS 3.35D disassembly driver' in gl:
                    gl = gl.replace('derived from the NFS 3.35D disassembly driver',
                                    'derived from the NFS 3.35K disassembly driver')
                if 'two ROMs are 97.8% identical' in gl:
                    gl = '# The two ROMs are 89.4% identical at the opcode level.'
                output_lines.append(gl)
            continue

        # --- File path replacements ---
        group_lines = [gl.replace('nfs-3.35K', 'nfs-3.40') for gl in group_lines]
        first_line = group_lines[0]
        stripped = first_line.strip()
        ...
        if stripped.startswith('move('):
            for gl in group_lines:
                if '0x9315' in gl:
                    gl = gl.replace('0x9315', '0x931C')
                    gl = re.sub(r'#.*', '# &931C (+7 from 3.35K)', gl)
                elif '0x935A' in gl:
                    gl = gl.replace('0x935A', '0x935D')
                    gl = re.sub(r'#.*', '# &935D (+3 from 3.35K)', gl)
                elif '0x945A' in gl:
                    gl = gl.replace('0x945A', '0x9456')
                    gl = re.sub(r'#.*', '# &9456 (-4 from 3.35K)', gl)
                elif '0x955A' in gl:
                    gl = gl.replace('0x955A', '0x9556')
                    gl = re.sub(r'#.*', '# &9556 (-4 from 3.35K)', gl)
                output_lines.append(gl)
            continue
...
def main():
    ...
    # 3.35K sources → 3.40 sources
    reloc_blocks = [
        (0x9315, 0x931C, 0x0016, 0x61),    # BRK handler + workspace
        (0x935A, 0x935D, 0x0400, 0x100),    # Page 4
        (0x945A, 0x9456, 0x0500, 0x100),    # Page 5
        (0x955A, 0x9556, 0x0600, 0x100),    # Page 6
    ]
    ...
    # In 3.40 the header is 4 bytes longer (title "    NET" vs "NET"),
    # so the dispatch tables shift by +4.
    ...
    key_addrs = [0x80D4, 0x80EA, 0x8103, 0x81F0, 0x8146, 0x9315]
